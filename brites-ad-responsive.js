...
(function(root){
  ...
  function document(plan,image,board,device="mobile"){
    ...
    if(banner){
      const bw=H<90?76:Math.min(148,Math.max(108,W*.16)),bh=Math.min(H-12,38),pw=Math.min(H*1.12,W*.26),tw=W-pw-bw-margin*4;
      photo={left:margin,top:4,width:pw-margin,height:H-8};
      const roomy=H>=120&&tw>=240,headline=roomy?copy.headline:copy.shortHeadline,hs=H<90?14:Math.min(30,H*.22),hh=lines(headline,tw,hs)*hs*1.25;
      const benefit=roomy?description:'',dh=benefit?lines(benefit,tw,13)*17:0,total=hh+15+(benefit?dh+6:0),y=Math.max(3,(H-total)/2);
      text('brand','BRITES JEWELRY',pw+margin,y,tw,13,10,'brand');
      text('headline',headline,pw+margin,y+15,tw,hh,hs,'headline',style.headlineFont);
      if(benefit)text('description',benefit,pw+margin,y+15+hh+6,tw,dh,13,'description');
      ...
      const compact=W<280,hs=(narrow?Math.min(40,Math.max(24,W*.14)):horizontal?22:compact?20:28)*typeScale,ds=13*typeScale,brandSize=10*typeScale;
      let headline=horizontal?copy.shortHeadline:copy.headline;
      if(compact&&!narrow&&H<280)headline=copy.shortHeadline;
      ...
      const showDescription=minimal+gap+descHeight<=(horizontal?H-margin*2:H*.68);
      ...
      if(horizontal){const pw=W*.48,right=layout.photoSide==='right';photo={left:right?W-pw:0,top:margin,width:pw,height:H-margin*2};area={left:right?margin:pw+margin,top:Math.max(margin,(H-contentHeight)/2),width:textWidth};}
        ...
        photo={left:0,top:margin,width:W,height:ph};
        area={left:margin,top:ph+margin*2,width:textWidth};
      }
      let y=area.top;
      ...
      // Tall formats use a deliberate bottom action anchor, with copy near the product.
      if(!horizontal&&H/W>1.8)y=Math.max(y,H-margin-bh);
