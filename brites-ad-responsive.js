...
(function(root){
  ...
  function document(plan,image,board,device="mobile"){
    ...
    if(banner){
      ...
      text('headline',copy.shortHeadline,area.left,H<90?H*.12:H*.3,tw,H<90?H*.38:H*.45,Math.max(10,Math.min(H*.28,tw/Math.max(9,copy.shortHeadline.length*.52))),'headline',style.headlineFont);
