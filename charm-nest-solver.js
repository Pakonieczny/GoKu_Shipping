...
(function (root, factory) {
  ...
  async function solve(job, cb) {
    ...
    // the inset band and cut stock, which the thickness of free space looks past (see THICK): a gap is thin between charms,
    ...
    // one charm at a time, graded spot by spot: on any sheet, empty or not, Rose Gold past its green line included
    ...
    async function carefulAppend() {
      ...
      /* Thickness (see THICK): a free cell a disc of TH.mm[k] covers is worth TH.worth[k + 1], one only the sliver disc
         covers TH.worth[0], a sliver nothing, capped by the share of its region as before. Measured on a grid thF times
         coarser than the fine one, where a coarse cell is taken when all its fine cells are.                          */
      const TH = job.thick || THICK, thF = TH.f || 2, thW = TH.worth;
      const thR = TH.mm.map(d => Math.max(1, Math.round(3 * d * MM_PX / 2 / thF)));
      const Dmax = Math.max(...TH.mm), bigMargin = Math.max(margin, Math.ceil(Dmax * MM_PX)), thReach = Math.ceil(Dmax * MM_PX) + 2 * thF;
      const thick2 = (occ2, W2, H2, A2, B2, M2, T2) => {
        const n = W2 * H2;
        chamfer(occ2, W2, H2, A2);
        for (let i = 0; i < n; i++) T2[i] = occ2[i] ? 0 : thW[0];
        for (let k = 0; k < thR.length; k++) {
          const r = thR[k]; let any = 0;
          for (let i = 0; i < n; i++) { const c = A2[i] >= r ? 1 : 0; M2[i] = c; any |= c; }
          if (!any) break;
          chamfer(M2, W2, H2, B2);
          const w = thW[k + 1];
          for (let i = 0; i < n; i++) if (B2[i] <= r && !occ2[i]) T2[i] = w;
        }
      };
      const FW2 = Math.ceil(baseFine.W / thF), FH2 = Math.ceil(baseFine.H / thF);
      const all2 = { occ: new Uint8Array(FW2 * FH2), A: new Int32Array(FW2 * FH2), B: new Int32Array(FW2 * FH2), M: new Uint8Array(FW2 * FH2), T: new Float32Array(FW2 * FH2) };
      let big = null;
      ...
      let exactN = 0, lastBonus = 0;
      const valueInL = (x0, y0, x1, y1) => { x0 = Math.max(0, x0); y0 = Math.max(0, y0); x1 = Math.min(FW, x1); y1 = Math.min(FH, y1); if (x1 <= x0 || y1 <= y0) return 0; const W1 = FW + 1; return valSATL[y1 * W1 + x1] - valSATL[y0 * W1 + x1] - valSATL[y1 * W1 + x0] + valSATL[y0 * W1 + x0]; };
      ...
      const sheetValue = () => {
        ...
        for (let y2 = 0; y2 < FH2; y2++) for (let x2 = 0; x2 < FW2; x2++) {
          let o = 1;
          for (let dy = 0; dy < thF && o; dy++) for (let dx = 0; dx < thF; dx++) { const gx = x2 * thF + dx, gy = y2 * thF + dy, gi = gy * FW + gx; if (gx < FW && gy < FH && (!all.occ[gi] || edgeFine[gi])) { o = 0; break; } }
          all2.occ[y2 * FW2 + x2] = o;
        }
        thick2(all2.occ, FW2, FH2, all2.A, all2.B, all2.M, all2.T);
        for (let y = 0; y < FH; y++) for (let x = 0; x < FW; x++) {
          const i = y * FW + x, sh = all.label[i] >= 0 ? all.regions[all.label[i]].share : 0, t = all2.T[((y / thF) | 0) * FW2 + ((x / thF) | 0)];
          all.sh[i] = sh; all.val[i] = sh < t ? sh : t;
        }
      ...
      const lossLive = (v, x, y) => {
        ...
        const regions = regionsOf(occ, local.out, local.A, local.label, local.stack, W, ax0, ay0, ax1, ay1, (r, i) => {
          ...
          r.kept += all.sh[gi]; r.same = r.same === -2 ? gl : r.same === gl ? gl : -1;
        });
        let after = 0;
        ...
        for (let id = 0; id < regions.length; id++) {
          ...
          else if (r.same >= 0 && all.regions[r.same].size === r.size) { after += r.size * all.regions[r.same].share; r.keep = true; }
          else after += r.size * (r.maxA >= openU ? 1 : regionShare(D, id, r.bb, probes, padCells));
        }
        // the bound: share less thickness worth over the cells whose worth the charm can take (covered, sliver, changed pocket)
        let bonus = 0;
        for (let yy = ay0; yy < ay1; yy++) for (let xx = ax0, i = yy * W + ax0, gi = (Y0 + yy) * FW + X0 + ax0; xx < ax1; xx++, i++, gi++) {
          const d = all.sh[gi] - all.val[gi]; if (d <= 0) continue;
          if (occ[i] || local.out[i]) { bonus += d; continue; }
          const id = local.label[i]; if (id < 0) { bonus += d; continue; }
          const r = regions[id]; if (!r.edge && !r.keep) bonus += d;
        }
        lastBonus = bonus;
        return valueInL(X0 + ax0, Y0 + ay0, X0 + ax1, Y0 + ay1) - after - v.cells;
      };
      const lossAt = (v, x, y) => {
        const cx0 = x - margin, cy0 = y - margin, cx1 = x + v.fine.w + margin, cy1 = y + v.fine.h + margin;
        const X0 = Math.max(0, cx0 - reach), Y0 = Math.max(0, cy0 - reach), X1 = Math.min(FW, cx1 + reach), Y1 = Math.min(FH, cy1 + reach);
        ...
        const D = { W, H, occ, label: local.label };
        const cap = new Float32Array(regions.length);
        for (let id = 0; id < regions.length; id++) {
          const r = regions[id];
          cap[id] = r.edge ? -1 : r.same >= 0 && all.regions[r.same].size === r.size ? all.regions[r.same].share : r.maxA >= openU ? 1 : regionShare(D, id, r.bb, probes, padCells);
        }
        // the bigger counting window (thickness reaches further), aligned to the coarse grid, and its coarse domain
        const bx0 = Math.max(0, x - bigMargin), by0 = Math.max(0, y - bigMargin), bx1 = Math.min(FW, x + v.fine.w + bigMargin), by1 = Math.min(FH, y + v.fine.h + bigMargin);
        const qx0 = Math.floor(Math.max(0, bx0 - thReach) / thF), qy0 = Math.floor(Math.max(0, by0 - thReach) / thF), qx1 = Math.ceil(Math.min(FW, bx1 + thReach) / thF), qy1 = Math.ceil(Math.min(FH, by1 + thReach) / thF);
        const W2 = qx1 - qx0, H2 = qy1 - qy0, n2 = W2 * H2;
        if (!big || big.n < n2) big = { n: n2, occ: new Uint8Array(n2), A: new Int32Array(n2), B: new Int32Array(n2), M: new Uint8Array(n2), T: new Float32Array(n2) };
        const occ2 = big.occ;
        for (let y2 = 0; y2 < H2; y2++) for (let x2 = 0; x2 < W2; x2++) {
          let o = 1;
          for (let dy = 0; dy < thF && o; dy++) for (let dx = 0; dx < thF; dx++) {
            const gx = (qx0 + x2) * thF + dx, gy = (qy0 + y2) * thF + dy;
            if (gx >= FW || gy >= FH) continue;
            if (((g[gy * words + (gx >> 5)] >>> (gx & 31)) & 1) && !edgeFine[gy * FW + gx]) continue;
            const mx = gx - x, my = gy - y;
            if (mx >= 0 && my >= 0 && mx < mw && my < v.fine.h && bits[my * mw + mx]) continue;
            o = 0; break;
          }
          occ2[y2 * W2 + x2] = o;
        }
        thick2(occ2, W2, H2, big.A, big.B, big.M, big.T);
        const T2 = big.T, lab = local.label, sx0 = X0 + ax0, sy0 = Y0 + ay0, sx1 = X0 + ax1, sy1 = Y0 + ay1;
        let after = 0;
        for (let gy = by0; gy < by1; gy++) {
          const trow = (((gy / thF) | 0) - qy0) * W2 - qx0, inY = gy >= sy0 && gy < sy1;
          for (let gx = bx0; gx < bx1; gx++) {
            if ((g[gy * words + (gx >> 5)] >>> (gx & 31)) & 1) continue;
            const t = T2[trow + ((gx / thF) | 0)];
            let c;
            if (inY && gx >= sx0 && gx < sx1) {
              const i = (gy - Y0) * W + gx - X0; if (occ[i]) continue;
              const id = lab[i]; if (id < 0) continue;
              c = cap[id] < 0 ? all.sh[gy * FW + gx] : cap[id];
            } else { const gi = gy * FW + gx; if (all.out[gi]) continue; c = all.sh[gi]; }
            after += c < t ? c : t;
          }
        }
        return valueIn(bx0, by0, bx1, by1) - after - v.cells;
      };
      ...
      /** What any of 16×16 positions covers (the hull of blockMasks, four blocks wide): on an empty sheet nearly every
          position is legal, and at 2° the 4×4 blocks alone took a third of a charm's time (24 Sep). */
      const wideHull = v => {
        const w = v.fine.w, h = v.fine.h, b = v.fine.bits, S = ratio * 4, W2 = w + S - 1, H2 = h + S - 1;
        const rAny = new Uint8Array(W2 * h), out = new Uint8Array(W2 * H2);
        for (let y = 0; y < h; y++) for (let x = 0; x < W2; x++) { for (let o = 0; o < S; o++) { const mx = x - o; if (mx >= 0 && mx < w && b[y * w + mx]) { rAny[y * W2 + x] = 1; break; } } }
        for (let y = 0; y < H2; y++) for (let x = 0; x < W2; x++) { for (let o = 0; o < S; o++) { const my = y - o; if (my >= 0 && my < h && rAny[my * W2 + x]) { out[y * W2 + x] = 1; break; } } }
        return cropped(out, W2, H2);
      };
      /** Whether every position of a block clears the Rose Gold stock already cut away (see clearOfRemnant): the block
          masks carry no outline, so the charm's own is tested at the block's positions nearest the cut. */
      const blockClear = (v, bx, by, ex, ey) => {
        const rem = fine.remnant, u = v.fine.pm.outline;
        if (!rem || !u) return true;
        if (rem.axis === "x") { for (let y = by; y < ey; y++) if (!clearOfRemnant(rem, u, bx, y)) return false; }
        else for (let x = bx; x < ex; x++) if (!clearOfRemnant(rem, u, x, by)) return false;
        return true;
      };
      let legal = null;
      ...
      const legalMap = v => {
        ...
        for (let sy = 0; sy < PH; sy += SB) for (let sx = 0; sx < PW; sx += SB) {
          ...
          for (let by = sy; by < sy1; by += B) for (let bx = sx; bx < sx1; bx += B) {
            if (core && !fine.fits(core.pm, bx + core.x, by + core.y)) continue;
            const ex = Math.min(PW, bx + B), ey = Math.min(PH, by + B);
            if (hull && fine.fits(hull.pm, bx + hull.x, by + hull.y) && blockClear(v, bx, by, ex, ey)) { for (let y = by; y < ey; y++) L.fill(1, y * PW + bx, y * PW + ex); continue; }
      ...
      async function gradeAll(p, front) {
        ...
        // exact thickness waste from the best bound down, until no bound left can reach the LEADERS best exact fits
        spots.sort((a, b) => b.fit - a.fit);
        const tops = [];
        for (let i = 0; i < spots.length; i++) {
          const s = spots[i];
      ...
      const WINDOW = 12;   // the oldest charms weighed together; the next join as these go in, so each step stays quick
      const userStop = () => !!(cb.shouldStop && cb.shouldStop());
      ...
      const lossPad = bigMargin + thReach + 1, aroundPad = Math.ceil(aroundU / 3) + 2;
      ...
      const legalIn = (v, x0, y0, x1, y1) => {
        ...
        for (let by = y0; by < y1; by += B) for (let bx = x0; bx < x1; bx += B) {
          ...
          if (hull && fine.fits(hull.pm, bx + hull.x, by + hull.y) && blockClear(v, bx, by, ex, ey)) { for (let y = by; y < ey; y++) L.fill(1, (y - y0) * w + bx - x0, (y - y0) * w + ex - x0); continue; }
      ...
      /** One charm's graded spots brought up to date with the placements made since ({box, chg} each): a new list, the
          cached spots are not changed. Spots a placement covers are dropped, and the snuggest few spots resting
          against it, over all angles, are added and graded. A spot whose grade a placement changes is only marked:
      ...
      const refresh = (p, spots, events, front) => {
        ...
        for (let changed = true; changed;) {
          ...
          for (let i = 0; i < Math.min(LEADERS, out.length); i++) {
            ...
            t.fit = fitOf(t); graded++; changed = true;
          }
        }
        return out;
      };
      ...
      async function pass(first) {
        ...
        const setAside = (p, list) => {
          ...
          liftedOrders.add(order);
          for (const l of [noRoom, over]) for (let i = l.length - 1; i >= 0; i--) if (!liftedOrders.has(l[i].order)) left.push(...l.splice(i, 1));
          if (fifo) left.sort(byAge);
          return true;
        };
        ...
        // a charm whose known spots were all taken is searched afresh, so "no spot" always means none on the sheet
        const current = async (p, front) => {
          let c = cache.get(p);
          if (c && c.at < events.length) { const spots = refresh(p, c.spots, events.slice(c.at), front); c = spots.length ? { spots, at: events.length } : null; }
        ...
        /* A sheet never holds part of an order, nor, with dated orders, an order younger than one it leaves out: the page
           lifts those off (oldest first, 23 Sep). Charms are placed best fit first whatever their dates, so a younger
           order can take the only spot an older one had. Such orders are lifted here, and the older charm, which had
           room, is seated first in the second pass; the pass that keeps more charms wins. */
        /* Out of time, a charm still waiting was never tried, so no order was left out for it: the charms placed stay,
           except the rest of an order it belongs to (24 Sep: a 60 s ceiling at 2° lifted all 17 charms of a sheet,
        ...
        if (waiting.length) {
          ...
          if (off.length) {
            ...
            ({ fine, coarse } = rebuildGrids(rec));
            // they wait for the reason the oldest order left out waits (out of time, only part of an order was placed)
            const to = [noRoom, over, left].find(l => l.some(q => rank.get(q.order) === cutoff)) || left;
      ...
      if (out && out.stranded.length && !out.timedOut) {
        ...
        // stopped during the second pass, the finished first one stands
        if (again && (again.rec.length > out.rec.length || (again.rec.length === out.rec.length && again.fit > out.fit))) out = again;
        else { for (const r of again ? again.rec : rec) probe(r.p, r.v, r.x, r.y, "lift", true); for (const r of out.rec) probe(r.p, r.v, r.x, r.y, "place", true); }
      }
      if (!out) return null;
      ...
      const smallRoom = !job.roomCheck ? undefined : (() => {
        for (const v of smallProbeVariants(fineRes, erodeFine, halfGapFine)) {
          const pm = v.fine.pm, PW = FW - pm.w + 1, PH = FH - pm.h + 1;
          ...
          for (let by = 0; by < PH; by += B) for (let bx = 0; bx < PW; bx += B) {
            if (core && !fine.fits(core.pm, bx + core.x, by + core.y)) continue;
            const ex = Math.min(PW, bx + B), ey = Math.min(PH, by + B);
            if (hull && fine.fits(hull.pm, bx + hull.x, by + hull.y) && blockClear(v, bx, by, ex, ey)) return true;
            for (let y = by; y < ey; y++) for (let x = bx; x < ex; x++) if (fine.fits(pm, x, y)) return true;
          }
        }
        return false;
      })();
    ...
    if (carefulOn && !best) {
      const careful = await carefulAppend(), overran = now() - t0 > budget;
      // every charm is placed, held back by the fill ceiling, or shown to have no spot at any angle: nothing is left to search
      // (and past the usual ceiling, no time is left for a search either)
      if (careful && (careful.timedOut || overran || careful.rejects.every(id => careful.capped.includes(id) || careful.noRoom.includes(id)))) {
        best = careful; carefulDone = true; endedBy = cb.shouldStop && cb.shouldStop() ? "stopped" : careful.timedOut ? "budget" : !careful.rejects.length ? "complete" : careful.noRoom.length ? "no-room" : "cap";
    ...
    ceiling = budget;   // a search after it keeps the usual ceiling
  ...
  // for one a disc of mm[k] covers. The shop's charms are 5.4 to 11.6 mm at their narrowest (8.3 median) and need a
  // circle of 9 to 16 mm (12.4 median): a gap a 7 mm disc fills takes few of them, one a 13 mm disc fills nearly all.
  const THICK = { mm: [7, 9, 11, 13], worth: [0.05, 0.2, 0.45, 0.75, 1], f: 2 };
  ...
  const probeCache = new Map(), smallCache = new Map();
  /** One PROBE on a grid of `fineRes` px/pt: each 1 px/pt pixel fills its fineRes × fineRes cells (resample only thins
      a bitmap: going up it left 3 cells in 4 empty). */
  function probeBits([w, h, b], fineRes) {
    const src = bitsFromBase64(b, w * h), f = { w: Math.round(w * fineRes), h: Math.round(h * fineRes) };
    f.bits = new Uint8Array(f.w * f.h);
    for (let y = 0; y < f.h; y++) for (let x = 0; x < f.w; x++) f.bits[y * f.w + x] = src[Math.min(h - 1, (y / fineRes) | 0) * w + Math.min(w - 1, (x / fineRes) | 0)];
    return f;
  }
  // the first SMALL_PROBES of PROBES lie in the smallest 7% of the library by area (the middles of its first two 24ths)
  const SMALL_PROBES = 2;
  /* Placing charms one at a time always ends: each step seats a charm or sets one aside. Its time ceiling is only a
  ...
  function smallProbeVariants(fineRes, erodeFine, halfGapFine) {
    ...
    if (!smallCache.has(key)) smallCache.set(key, PROBES.slice(0, SMALL_PROBES).flatMap(probe => {
      const f = probeBits(probe, fineRes), out = [];
      for (let a = 0; a < 360; a += 15) {
        ...
        if (m.w && areaOf(m.bits)) out.push({ angle: a, fine: { bits: m.bits, w: m.w, h: m.h, pm: packShifted(m.bits, m.w, m.h) } });
      }
      return out;
    }));
  ...
  function probeVariants(fineRes, erodeFine, halfGapFine) {
    const key = fineRes + ":" + erodeFine + ":" + halfGapFine;
    if (!probeCache.has(key)) probeCache.set(key, PROBES.map(probe => {
      const f = probeBits(probe, fineRes), out = [];
      for (let a = 0; a < 360; a += 45) {
        const rot = rotateBitmap(f.bits, f.w, f.h, a); if (!rot.w) continue;
        const m = erodeFine ? erode(rot.bits, rot.w, rot.h, erodeFine) : dilate(rot.bits, rot.w, rot.h, halfGapFine);
        if (m.w && areaOf(m.bits)) {
          const W2 = m.w + 2, H2 = m.h + 2, o = new Uint8Array(W2 * H2).fill(1);
          for (let y = 0; y < m.h; y++) for (let x = 0; x < m.w; x++) if (m.bits[y * m.w + x]) o[(y + 1) * W2 + x + 1] = 0;
          const D = chamfer(o, W2, H2, new Int32Array(W2 * H2)); let r = -1, bi = 0;
          for (let i = 0; i < W2 * H2; i++) if (D[i] > r) { r = D[i]; bi = i; }
          out.push({ pm: packShifted(m.bits, m.w, m.h), w: m.w, h: m.h, cells: areaOf(m.bits), ax: (bi % W2) - 1, ay: ((bi / W2) | 0) - 1, r });
        }
      }
      return out;
    }));
  ...
  function regionShare(D, id, bb, probes, pad) {
    ...
    const grid = new Grid(w, h), oc = new Uint8Array(n);
    for (let i = 0; i < n; i++) if (dist[i] < 0) { grid.set(i % w, (i / w) | 0); oc[i] = 1; }
    grid.buildSAT();
    const Dp = chamfer(oc, w, h, new Int32Array(n)); let deep = 0;
    for (let i = 0; i < n; i++) if (!oc[i] && Dp[i] > deep) deep = Dp[i];
    let fit = 0;
    for (const vs of probes) {
      let ok = false;
      for (const v of vs) {
        if (v.w > w || v.h > h || v.cells > open || v.r > deep) continue;
        for (let y = 0; y + v.h <= h && !ok; y++) for (let x = 0, a = (y + v.ay) * w + v.ax; x + v.w <= w; x++, a++) {
          if (Dp[a] < v.r) continue;
