...
function buildPortfolioActivationEnvelopes({ portfolioPlanProposal, proposals = [], activationSnapshot, riskMandate = POLICY.RISK_MANDATE, marks = {}, advBySymbol = {}, sectorOf = null, clusterOf = null, validation = null } = {}) {
  ...
  for (const p of proposals) {
    ...
    if (p.decision === "BUY") {
      ...
      const candidate = { symbol: p.symbol, sector: sector(p.symbol), proposedQuantityUnits: p.allocation.proposedQuantityUnits, limitPriceMicros: entry.limitPriceMicros,
        lossBoundaryPriceMicros: prot.lossBoundaryPriceMicros, costPerShareMicros: p.forecast && p.forecast.basis ? p.forecast.basis.estimatedRoundTripCostMicrosPerShare : riskMandate.stress.stressCostPerShareMicros,
        advMinor: (marks[p.symbol] && marks[p.symbol].advMinor) || advBySymbol[p.symbol] || null, spreadBps: (marks[p.symbol] && marks[p.symbol].spreadBps) || null };
...
async function stagePortfolioPlan({ planClass, portfolioPlanProposal = {}, proposals = [], verifiedProposalClaims = null, activationSnapshot, accountId, cutoff = null, policy = null,
  managerRunId = null, admin = null, eligibleSymbols = null, marks = {}, advBySymbol = {}, sectorOf = null, clusterOf = null, lineage = {}, nowMs = Date.now() } = {}) {
  const D = db(admin);
  ...
  const validation = {};
  for (const p of proposals) validation[p.symbol] = validateProposal(p, { eligibleSymbols, heldSymbols, pendingSymbols, nowMs, policy });
    ...
    const out = await D.runTransaction(async (tx) => {
      ...
      for (const p of proposals) {
        ...
        const binding = { schemaVersion: "mandate-binding.v1", mandateVersionId, mandateSeriesId: seriesId, version, previousVersionId: pointer.desiredVersionId || null,
          ...
          model: lineage.model || null, reasoningEffort: lineage.reasoningEffort || null, promptHash: lineage.promptHash || null, schemaHash: POLICY.schemaHash("mandate-proposal.v1"),
        ...
        const pointerNext = { accountId, symbol: p.symbol, mandateSeriesId: seriesId, desiredVersion: version, desiredVersionId: mandateVersionId, desiredIdempotencyKey: idempotencyKey,
