...
const STUDIO_GOLD_STYLE_REFERENCE_SHA256 = "bcf95123375f6c8e3de86f53065374297dbe09cd2002a69f2ebf7a35ea6f4161";
const STUDIO_GOLD_STYLE_REFERENCE_CACHE_MS = 10 * 60 * 1000;
...
function studioSha256(value) {
  return require("crypto").createHash("sha256").update(value).digest("hex");
}

async function ensureStudioGoldStyleReference() {
  if (__studioGoldStyleReferenceCache?.buffer?.length &&
      Date.now() - Number(__studioGoldStyleReferenceCache.cachedAt || 0) < STUDIO_GOLD_STYLE_REFERENCE_CACHE_MS) {
    return __studioGoldStyleReferenceCache;
  }
  let storageMissing = false;
  try {
    const got = await storagePathToBuffer(STUDIO_GOLD_STYLE_REFERENCE_STORAGE_PATH);
    const buffer = Buffer.from(got.buffer || Buffer.alloc(0));
    const sha256 = buffer.length ? studioSha256(buffer) : "";
    if (sha256 === STUDIO_GOLD_STYLE_REFERENCE_SHA256) {
      const hit = {
        buffer, sha256, mime: String(got.mime || STUDIO_GOLD_STYLE_REFERENCE_MIME),
        filename: STUDIO_GOLD_STYLE_REFERENCE_FILENAME,
        storagePath: STUDIO_GOLD_STYLE_REFERENCE_STORAGE_PATH,
        seeded: false, persisted: true, cachedAt: Date.now(),
      };
      __studioGoldStyleReferenceCache = hit;
      return hit;
    }
    console.error("[studio] gold style reference hash mismatch; using verified embedded production sample");
  } catch (e) {
    storageMissing = /not found|404/i.test(String(e?.message || e));
    console.warn("[studio] gold style reference read unavailable; using verified embedded production sample:", e?.message || e);
  }

  /* Never overwrite the canonical reference after a transient read error.
     The embedded copy is byte-locked to the production sample and can be sent
     directly. If the object is genuinely absent, create it only if absent. */
  const buffer = Buffer.from(STUDIO_GOLD_STYLE_REFERENCE_SEED_B64, "base64");
  const sha256 = buffer.length ? studioSha256(buffer) : "";
  if (!buffer.length || sha256 !== STUDIO_GOLD_STYLE_REFERENCE_SHA256) {
    throw new Error("gold_style_reference_integrity_failed");
  }
  let persisted = false;
  if (storageMissing) {
    try {
      ...
      await bucket.file(STUDIO_GOLD_STYLE_REFERENCE_STORAGE_PATH).save(buffer, {
        ...
        metadata: { metadata: { studioSystemAsset: "1", seedSource: "embedded_reference", purpose: "custom_studio_render_style_reference", sha256 } },
      });
      persisted = true;
    } catch (e) {
      if (!/409|412|condition|exists/i.test(String(e?.message || e))) {
        console.warn("[studio] gold style reference create-if-absent skipped:", e?.message || e);
      }
    }
  }
  const embedded = {
    buffer, sha256, mime: STUDIO_GOLD_STYLE_REFERENCE_MIME,
    filename: STUDIO_GOLD_STYLE_REFERENCE_FILENAME,
    storagePath: STUDIO_GOLD_STYLE_REFERENCE_STORAGE_PATH,
    seeded: true, persisted, cachedAt: Date.now(),
  };
  __studioGoldStyleReferenceCache = embedded;
  return embedded;
}
...
async function fetchWithTimeout(url, options, timeoutMs) {
  ...
  if (!Number.isFinite(ms) || ms < 1000) return fetch(url, options);
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), Math.round(ms));
  try {
    ...
    /* Keep the deadline alive through body consumption. fetch() resolves at
       headers; clearing here lets a stalled response body outlive the render
       lease indefinitely. All timed callers consume JSON text. */
    ...
    if (err?.name === "AbortError") {
      const timeout = new Error(`upstream_timeout_after_${Math.round(ms)}ms`);
      timeout.code = "UPSTREAM_TIMEOUT";
      /* A provider may have completed even when the connection timed out.
         Callers must not automatically retry this ambiguous outcome. */
      timeout.ambiguous = true;
      throw timeout;
    }
    if (err && (err.name === "TypeError" || /ECONNRESET|EPIPE|ETIMEDOUT|socket|network/i.test(String(err.code || err.message || "")))) {
      err.ambiguous = true;
    }
    throw err;
  } finally {
...
async function fetchBufferWithTimeout(url, options, timeoutMs) {
  ...
  try {
    ...
    return { ok: resp.ok, status: resp.status, headers: resp.headers, buffer };
  } catch (err) {
    if (err?.name === "AbortError") {
      const timeout = new Error(`upstream_timeout_after_${ms}ms`);
      timeout.code = "UPSTREAM_TIMEOUT"; timeout.ambiguous = true;
      throw timeout;
    }
    if (err && (err.name === "TypeError" || /ECONNRESET|EPIPE|ETIMEDOUT|socket|network/i.test(String(err.code || err.message || "")))) {
      err.ambiguous = true;
...
async function callOpenAIImagesEdits({
  ...
  if (imageRoles === "style_reference") {
    promptText = `${promptText.trim()}\n\n${IMAGE_ROLE_LABELS.style_reference.single}\n\n${IMAGE_ROLE_LABELS.style_reference.lock}`;
             ...
             imageRoles === "color_only_charm_repair") {
  ...
  const finalText = String(finalPromptSuffix || "").trim().slice(0, 20000);
  if (finalText) promptText = `${promptText}\n\n${finalText}`;
...
const IMAGE_ROLE_LABELS = {
  ...
  material_spec_to_charm: {
    ...
    second:
      "IMAGE 2 — PHYSICAL CRAFT AND PHOTOGRAPHY REFERENCE. Read from it the believable polish, reflection behaviour, shallow-engraving response, crisp edge character, lighting, white-ground product photography and contact shadow. It is NOT the authority on literal hue; the final 14K calibration below owns colour. Its SUBJECT is off limits — the object it depicts, its outline, its engraved artwork, numerals, lettering and attached jump ring must NOT appear in the output in whole, in part or blended.",
    extra: (n) => `IMAGE ${n} — UNUSED. Ignore.`,
    single:
      "IMAGE 1 — GREYSCALE GEOMETRY MAP (STRUCTURAL TRUTH, NO COLOUR). It marks three things and only three: MID GREY = plain polished metal, DARK GREY = shallow engraved work, WHITE = cut clean through. Its silhouette, openings and worked regions are already resolved exactly; materialise this same object without changing any geometry.",
    lock:
      "FINAL IMAGE-ROLE LOCK: IMAGE 1 owns WHERE — every edge, opening, engraved region and outline, exactly as mapped, nothing added, removed, moved, resized or reinterpreted. IMAGE 2 teaches physical realism — polish, reflections, shallow engraving, edge quality, lighting and shadow — but NOT literal hue. The final metal-calibration block owns colour. HARD FAIL: taking colour from IMAGE 1's greys. HARD FAIL: any part of IMAGE 2's subject, artwork, lettering or jump ring appearing in the output.",
  },
  color_only_charm_repair: {
    first:
      "IMAGE 1 — LOCKED FINISHED CHARM. This is the edit target. Preserve its canvas, exact silhouette, hoop, openings, engraving boundaries, proportions, camera and edge thickness. Change only metal hue/finish, reflection tonality, engraving texture, and add or correct only the required neutral contact shadow when the correction calls for it.",
    second:
      "IMAGE 2 — PHYSICAL CRAFT REFERENCE ONLY. It may guide polish, realistic gold reflections and shallow engraving response. Its literal hue, subject, silhouette, artwork, lettering, numerals and jump ring are off limits.",
    extra: (n) => `IMAGE ${n} — UNUSED. Ignore.`,
    single:
      "IMAGE 1 — LOCKED FINISHED CHARM. Apply a finish correction only. Preserve all geometry, openings, engraving boundaries and camera; only metal/reflection/texture tones and the required neutral contact shadow may be added or corrected.",
    lock:
      "FINAL FINISH-ONLY EDIT LOCK: preserve IMAGE 1's geometry, topology and camera exactly. Do not redraw, move, resize, close, open, add or remove any geometry, object or hardware. IMAGE 2 cannot contribute subject matter or literal colour. Only calibrated metal hue, reflections, engraving finish/texture, and adding or correcting the one required neutral contact shadow may change.",
  },
  line_art_style: {
    first:
...
async function callGeminiGenerateContentImage({
  ...
  /* Studio finish calibration belongs literally last. Role labels may define
     what a reference can teach, but they cannot overrule the approved 14K
     colour correction or the render-only camera contract. */
  const finalText = String(finalPromptSuffix || "").trim().slice(0, 20000);
  ...
  try {
    ...
    if (sharp) {
      ...
      if (needResize) {
        const preserveWholeCharm = imageRoles === "material_spec_to_charm" ||
          imageRoles === "color_only_charm_repair";
        outBuf = await img.resize(wantW, wantH, preserveWholeCharm
          ? { fit: "contain", background: { r: 255, g: 255, b: 255, alpha: 1 } }
          : { fit: "cover" }).png().toBuffer();
      } else {
...
const STUDIO_KINDS = new Set([
  ...
  "custom_proof_claim",
  "custom_session_delete",
  "custom_version_delete",
]);
...
const STUDIO_MAX_PROOF_CLAIMS_HOUR_UID = 20;
const STUDIO_MAX_PROOF_CLAIMS_HOUR_IP  = 40;
...
const STUDIO_DEFAULT_CONFIG = {
  ...
  /* Drawing and render routing are deliberately separate. Changing the final
     renderer must never move production-line-art generation to another model.
     Keep the cost-controlled Gemini default here; config/customStudio may set
     studioRenderModel:"gpt-image-2" after production comparison without a
     deploy. The finished render is 2K so the approved drawing and the output
     carry the same working resolution. */
  ...
  /* Production rendering fails closed. A paid 14K result is never allowed to
     fall back to the ambiguous colour drawing, an unverified style sample or
     an unnamed geometry map. Set an individual flag false only as an explicit
     emergency rollback in config/customStudio. */
  renderRequireDeterministicSpec: true,
  renderRequireStyleReference: true,
  renderSemanticPreflight: true,
  renderRequireSemanticPreflight: true,
  renderSemanticConfidenceMin: 0.55,
  renderSemanticTimeoutMs: 25000,
  renderProviderTimeoutMs: 150000,
  renderMaxRuntimeMs: 330000,
  /* "observe" stores metrics only; "repair" allows one conditional provider
     correction; "enforce" also rejects a still-failing corrected result.
     Production yellow-gold is fail-closed after its single repair attempt. */
  renderQaMode: "enforce",
  renderMaxQaRetries: 1,
...
const STUDIO_RENDER_MODEL_FALLBACK = "gemini-3-pro-image";
const STUDIO_DRAWING_TIER_FALLBACK = "2K";   // the input side
const STUDIO_RENDER_TIER_FALLBACK = "2K";    // the output side
...
function studioRenderModelId(cfg) {
  return String(
    (cfg && cfg.studioRenderModel) ||
    process.env.STUDIO_RENDER_IMAGE_MODEL ||
    process.env.GEMINI_STUDIO_RENDER_MODEL ||
    STUDIO_RENDER_MODEL_FALLBACK
  ).trim();
}
...
function studioMaskMinPx(cfg) {
  ...
  /* The signed-distance refiner keeps several full-frame masks/fields. This
     pipeline renders at 2K, so a larger floor buys no provider detail and can
     exceed a background function's memory budget. */
  return Number.isFinite(n) && n >= 256 && n <= 2048 ? Math.round(n) : STUDIO_MASK_MIN_PX_FALLBACK;
}
...
function studioCreditCost(value, fallback = 1) {
  const n = Number(value);
  const f = Number(fallback);
  return Number.isFinite(n) && n > 0 && n <= 1000
    ? n : (Number.isFinite(f) && f > 0 ? f : 1);
}
...
async function studioBudgetOk(key, id, max) {
  ...
  try {
    ...
    return false;  // budget controls fail closed; paid providers do not
  }
}
...
function studioWalletOperationRef(userRef, operationId) {
  const raw = String(operationId || "").trim();
  return raw
    ? userRef.collection("walletLedger").doc("op_" + studioSha256(raw).slice(0, 48))
    : null;
}

async function studioCanAfford(uid, cost) {
  const snap = await getDb().doc(`users/${uid}`).get();
  const w = (snap.exists && snap.data()?.wallet) || {};
  return walletIsUnlimited(w) || (Number(w.credits) || 0) >= Math.max(0, Number(cost) || 0);
}

async function studioDebit(uid, cost, sessionId, operationId) {
  const db = getDb();
  const ref = db.doc(`users/${uid}`);
  return db.runTransaction(async (tx) => {
    const opRef = studioWalletOperationRef(ref, operationId);
    const snap = await tx.get(ref);
    const opSnap = opRef ? await tx.get(opRef) : null;
    if (opSnap?.exists) {
      const prior = opSnap.data() || {};
      return { debited: prior.status === "debited", alreadyApplied: true, unlimited: !!prior.unlimited };
    }
    const w = (snap.exists && snap.data().wallet) || {};
    ...
    if (walletIsUnlimited(w)) {
      tx.create(opRef || ref.collection("walletLedger").doc(), {
        delta: 0, reason: "generate_unlimited", ref: sessionId || null,
        status: "debited", unlimited: true,
        at: admin.firestore.FieldValue.serverTimestamp(),
      });
      return { debited: true, alreadyApplied: false, unlimited: true };
    }
    ...
    tx.create(opRef || ref.collection("walletLedger").doc(), {
      ...
      status: "debited", unlimited: false,
      at: admin.firestore.FieldValue.serverTimestamp(),
    });
    return { debited: true, alreadyApplied: false, unlimited: false };
  });
}
/* A boolean cannot distinguish "already refunded / never debited" from a
   transient Firestore failure. Render recovery needs that distinction or it
   can erase the only evidence that a credit is still owed. */
async function studioRefundResult(uid, cost, sessionId, operationId) {
  try {
    const db = getDb();
    const ref = db.doc(`users/${uid}`);
    return await db.runTransaction(async (tx) => {
      ...
      const w = (snap.exists && snap.data().wallet) || {};
      if (opRef) {
        /* No deterministic debit record means this operation never took
           money. A repeated refund sees status=refunded and becomes a no-op. */
        if (!opSnap?.exists) return { settled: true, refunded: false, reason: "not_debited" };
        if (opSnap.data()?.status === "refunded") {
          return { settled: true, refunded: true, alreadyApplied: true, reason: "already_refunded" };
        }
        if (opSnap.data()?.status !== "debited") {
          return { settled: false, refunded: false, reason: "unknown_ledger_state" };
        }
      }
      ...
      const debitWasUnlimited = opRef
        ? opSnap?.data()?.unlimited === true
        : walletIsUnlimited(w);
      if (debitWasUnlimited) {
        if (opRef) tx.set(opRef, {
          status: "refunded", refundDelta: 0,
          refundedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
        return { settled: true, refunded: true, reason: "unlimited_refund" };
      }
      tx.set(ref, {
        wallet: Object.assign({}, w, {
      ...
      if (opRef) {
        tx.set(opRef, {
          status: "refunded", refundDelta: cost,
          refundedAt: admin.firestore.FieldValue.serverTimestamp(),
        }, { merge: true });
      } else {
      ...
      return { settled: true, refunded: true, reason: "refunded" };
    });
  } catch (e) {
    console.error("[studio] refund failed:", e?.message || e);
    return { settled: false, refunded: false, reason: "refund_error" };
  }
}
async function studioRefund(uid, cost, sessionId, operationId) {
  const result = await studioRefundResult(uid, cost, sessionId, operationId);
...
function renderConstraintBlocks() {
  ...
  for (const line of src.split("\n")) {
    if (isHeading(line)) {
      skipping = /BACKGROUND|ABSOLUTE FLATNESS|FRONT-FACING LOCK/i.test(line);
    }
    ...
    if (/^\s*•\s*FRONT VIEW:/i.test(line)) continue;
    if (/^\s*[45]\.\s*(?:Confirm|Repeat).*\b(?:perspective|3D)\b/i.test(line)) continue;
    if (/^\s*•\s*NO TEXT.*NO 3D, no perspective, no visible thickness/i.test(line)) {
      out.push("• NO TEXT of any kind. NO ATTACHED HARDWARE.");
      continue;
    }
    out.push(line);
  }
...
function buildLineArtToCharmPrompt({ metal, zones }) {
  ...
  /* Presentation, camera and metal calibration are appended once as the
     provider-level final lock after every image-role label. */
  const header =
...
• One uniformly flat, thin sheet. Preserve the exact front-face design while allowing only the hairline edge view defined in the final camera lock.
...
function buildMaterialSpecToCharmPrompt({ metal, hasReference }) {
  ...
  const material = hasReference
    ? "the physically realistic metal craft shown in IMAGE 2, with literal hue set only by the final calibration"
    : label;
...
/* This suffix is deliberately appended AFTER every provider-specific image
   role and policy block. It is the last word the image model receives. */
const STUDIO_RENDER_PIPELINE_VERSION = "14k-material-v2";
const STUDIO_RENDER_FINISH_VERSION = "warm-yellow-champagne-v2";
const STUDIO_SEMANTIC_VERSION = "material-map-semantic-v1";

function studioMetalKey(value) {
  const key = String(value || "gold").trim().toLowerCase();
  return ["silver", "gold", "rose", "solid10", "solid14"].includes(key) ? key : "gold";
}

function studioIsYellowGold(value) {
  return ["gold", "solid10", "solid14"].includes(studioMetalKey(value));
}

function studioUsesApproved14kCalibration(value) {
  return ["gold", "solid14"].includes(studioMetalKey(value));
}

function studioSemanticPromptBlock(plan) {
  if (!plan || !Array.isArray(plan.regions)) return "";
  const fixedRoleName = {
    outer_body: "outer body",
    eyelet_band: "integrated eyelet band",
    eyelet_hole: "eyelet opening",
    field: "surface field",
    recognition_feature: "recognition feature",
    through_opening: "through-opening",
    other: "other mapped region",
  };
  const groups = { polished: [], engraved: [], cutout: [] };
  for (const region of plan.regions) {
    if (!groups[region.class]) continue;
    /* The preflight's useful contribution is the factual name — e.g. fork or
       knife — that lets the renderer understand the same class map a person
       just verified. The label has already passed studioSafeSemanticLabel's
       strict character/instruction/colour deny-list; role and class remain
       server-owned enums and pixels remain the geometry authority. */
    const role = fixedRoleName[region.role] || fixedRoleName.other;
    const factualName = studioSafeSemanticLabel(region.semanticLabel);
    groups[region.class].push(factualName ? `${role} (${factualName})` : role);
  }
  const line = (label, values) => values.length
    ? `• ${label}: ${values.slice(0, 12).join(", ")}.`
    : `• ${label}: none identified.`;
  return [
    "SEMANTIC READING — NAMES ONLY, NEVER GEOMETRY:",
    "The names below corroborate what the three pixel tones mean. IMAGE 1 remains the only authority on every boundary and class.",
    line("POLISHED", groups.polished),
    line("ENGRAVED", groups.engraved),
    line("THROUGH-OPENINGS", groups.cutout),
  ].join("\n");
}

function buildStudioRenderFinalLock({ metal, semanticPlan, retryReason } = {}) {
  const key = studioMetalKey(metal);
  const colorLock = studioUsesApproved14kCalibration(key)
    ? `14K YELLOW-GOLD COLOUR — APPROVED CALIBRATION:
• Use warm yellow/champagne gold with only a faint natural blush. Add yellow relative to rose gold.
• HARD FAIL: pink, rose, copper, orange-brown, bronze, muddy brown or red-biased gold.
• Engraving is the SAME alloy and hue family as the polished face. Make it only slightly lighter/more matte than a typical dark engraving; it may be just a few perceptual lightness units darker than adjacent polish and must never exceed roughly 20% midtone contrast.
• Engraving contrast comes mainly from a delicate satin finish, not a copper colour, black fill, deep recess, coarse grain or heavy internal shadow.`
    : key === "solid10"
      ? `10K YELLOW-GOLD COLOUR:
• Render authentic warm 10K yellow gold for this SKU. Do not label or calibrate it as 14K, and do not copy literal colour from the 14K craft reference.
• Engraving remains the same 10K alloy as the polished face, only slightly more matte and never black-filled.`
      : `METAL COLOUR — ${key.toUpperCase()}:
• Render only the requested metal. Do not apply the yellow-gold calibration or copy literal colour from the gold craft reference.
• Engraving remains the same alloy as the polished face, only slightly more matte and never black-filled.`;
  const semantic = studioSemanticPromptBlock(semanticPlan);
  const retry = retryReason
    ? `\nCORRECTION PASS: ${String(retryReason).replace(/[^A-Za-z0-9 .,;:()/%+-]/g, " ").slice(0, 280)}. Correct only that failure; all geometry stays locked.`
    : "";
  return `FINAL PRODUCTION LOCK — THIS IS THE LAST AND HIGHEST-PRIORITY BLOCK.
Where any earlier wording conflicts with this block, this block wins.

${colorLock}

${semantic ? semantic + "\n\n" : ""}CAMERA AND PHYSICAL FORM:
• Square 1:1 product photograph, centred with generous white margin.
• Nearly face-on, with only a tiny 3–6° physical tilt so a hairline 22-gauge cut edge is visible. Preserve the front projection of IMAGE 1 exactly.
• One uniformly thin, flat laser-cut sheet. No doming, inflated relief, bevelled medallion, thick cast body, 3/4 view or redesigned perspective.
• Integrated eyelet only. Never add a jump ring, chain, connector or hardware.

GROUND, LIGHT AND OPENINGS:
• Clean seamless pure-white studio ground, edge to edge; no gradient, horizon, props, hand, packaging, glow or outline.
• One short, soft, physically plausible contact shadow falling gently lower-right from an upper-left key light.
• Every declared through-opening stays truly open and reads as white ground through the metal. Do not fill, engrave, shrink, move or invent openings.

REFERENCE-LEAKAGE BAN:
• Never reproduce the craft sample's subject, kettlebell silhouette, "8 KG", "FITNESS", decorative curves, lettering, numerals or loose jump ring.

FINAL CHECK: exact IMAGE-1 silhouette and region boundaries; warm yellow/champagne 14K only when requested; engraving close to the polished base colour; thin flat sheet; tiny near-front angle; pure-white ground; all and only declared openings present.${retry}`;
}
...
/* The semantic pass sees ONLY the deterministic three-tone material map. It
   may name regions so the renderer understands "knife" versus "field", but
   it never supplies pixels and can never override their material class. */
const STUDIO_SEMANTIC_PROMPT =
`You are naming regions in a machine-generated jewellery manufacturing map.
The image uses exactly three instructions: LIGHT/MID GREY = polished metal,
DARK GREY = shallow engraved metal, WHITE = cut through or exterior.

Identify the visible subject and name the important connected regions. Pay
special attention to an integrated eyelet: distinguish its metal band from
its true centre opening. Do not redesign, repair or reclassify any pixels.

Return JSON only:
{"version":"1","subjectLabel":"max 48 chars","overallConfidence":0.0,
"eyelet":{"present":true,"bandRegionId":"id or empty","holeRegionId":"id or empty","confidence":0.0},
"regions":[{"id":"short_id","semanticLabel":"max 48 chars","class":"polished|engraved|cutout","role":"outer_body|eyelet_band|eyelet_hole|field|recognition_feature|through_opening|other","bbox":{"x0":0.0,"y0":0.0,"x1":1.0,"y1":1.0},"confidence":0.0}],
"relations":[{"type":"inside|part_of|touches","parentId":"id","childId":"id"}],
"ambiguities":[{"regionId":"id","code":"white_background_vs_cutout|outline_vs_fill|class_boundary|low_resolution"}]}

Rules: at most 16 regions and 24 relations. Coordinates are normalized 0..1.
Use plain factual labels, no instructions or prose. If a white area is
ambiguous between background and opening, report the ambiguity instead of
guessing. The three-tone pixels remain authoritative.`;

const STUDIO_SEMANTIC_TEXT_MODEL = "gemini-2.5-flash";
const STUDIO_SEMANTIC_CACHE_TTL_MS = 10 * 60 * 1000;
const STUDIO_SEMANTIC_CACHE_MAX = 64;
const _studioSemanticCache = new Map();
function studioSemanticText(value, max = 48) {
  return String(value == null ? "" : value)
    .replace(/[^A-Za-z0-9 &'()/.+-]/g, " ")
    .replace(/\s+/g, " ").trim().slice(0, max);
}

function studioSafeSemanticLabel(value) {
  const text = studioSemanticText(value, 48);
  return /\b(?:ignore|disregard|system|prompt|instruction|assistant|model|render|generate|make|change|use|add|remove|must|never|override|execute|script|gold|silver|rose|copper|bronze|pink|yellow|orange|brown|black|white|colour|color|metal)\b/i.test(text)
    ? "" : text;
}

function studioValidateRenderZones(value) {
  if (value == null) return [];
  if (!Array.isArray(value) || value.length > 40) throw new Error("invalid_render_zones");
  return value.map((z) => {
    const intent = String(z?.intent || "");
    const x = Number(z?.x), y = Number(z?.y), w = Number(z?.w), h = Number(z?.h);
    if (!["engrave", "cutout", "none"].includes(intent) ||
        ![x, y, w, h].every(Number.isFinite) || x < 0 || y < 0 || w < 0 || h < 0 ||
        x > 1 || y > 1 || w > 1 || h > 1 || x + w > 1.02 || y + h > 1.02) {
      throw new Error("invalid_render_zones");
    }
    return { intent, x, y, w: Math.min(w, 1 - x), h: Math.min(h, 1 - y),
      ring: !!z?.ring, where: studioSemanticText(z?.where, 40) };
  });
}

function normalizeStudioSemanticPlan(raw) {
  const classSet = new Set(["polished", "engraved", "cutout"]);
  const roleSet = new Set(["outer_body", "eyelet_band", "eyelet_hole", "field", "recognition_feature", "through_opening", "other"]);
  const relationSet = new Set(["inside", "part_of", "touches"]);
  const ambiguitySet = new Set(["white_background_vs_cutout", "outline_vs_fill", "class_boundary", "low_resolution"]);
  const clamp01 = (value) => Math.max(0, Math.min(1, Number(value) || 0));
  const ids = new Set();
  const regions = [];
  for (const source of (Array.isArray(raw?.regions) ? raw.regions : []).slice(0, 16)) {
    const id = String(source?.id || "").trim();
    const cls = String(source?.class || "").trim();
    const role = String(source?.role || "other").trim();
    const b = source?.bbox || {};
    const bbox = { x0: clamp01(b.x0), y0: clamp01(b.y0), x1: clamp01(b.x1), y1: clamp01(b.y1) };
    if (!/^[A-Za-z][A-Za-z0-9_-]{0,31}$/.test(id) || ids.has(id) || !classSet.has(cls) || !roleSet.has(role)) continue;
    if (!(bbox.x1 > bbox.x0 && bbox.y1 > bbox.y0)) continue;
    const semanticLabel = studioSafeSemanticLabel(source?.semanticLabel);
    if (!semanticLabel) continue;
    ids.add(id);
    regions.push({ id, semanticLabel, class: cls, role, bbox, confidence: clamp01(source?.confidence) });
  }
  if (!regions.length) throw new Error("semantic_plan_empty");
  const relations = [];
  for (const source of (Array.isArray(raw?.relations) ? raw.relations : []).slice(0, 24)) {
    const type = String(source?.type || "");
    const parentId = String(source?.parentId || "");
    const childId = String(source?.childId || "");
    if (relationSet.has(type) && ids.has(parentId) && ids.has(childId) && parentId !== childId) {
      relations.push({ type, parentId, childId });
    }
  }
  const ambiguities = [];
  for (const source of (Array.isArray(raw?.ambiguities) ? raw.ambiguities : []).slice(0, 16)) {
    const regionId = String(source?.regionId || "");
    const code = String(source?.code || "");
    if (ids.has(regionId) && ambiguitySet.has(code)) ambiguities.push({ regionId, code });
  }
  const eyeletRaw = raw?.eyelet || {};
  const eyelet = {
    present: !!eyeletRaw.present,
    bandRegionId: ids.has(String(eyeletRaw.bandRegionId || "")) ? String(eyeletRaw.bandRegionId) : "",
    holeRegionId: ids.has(String(eyeletRaw.holeRegionId || "")) ? String(eyeletRaw.holeRegionId) : "",
    confidence: clamp01(eyeletRaw.confidence),
  };
  return {
    version: "1",
    subjectLabel: studioSafeSemanticLabel(raw?.subjectLabel) || "custom charm",
    overallConfidence: clamp01(raw?.overallConfidence),
    eyelet, regions, relations, ambiguities,
  };
}

async function studioSemanticPreflight(materialSpecBuffer, cfg) {
  const specHash = studioSha256(materialSpecBuffer);
  const requestedModel = String(process.env.GEMINI_STUDIO_SEMANTIC_MODEL ||
    process.env.GEMINI_STUDIO_PRECHECK_MODEL ||
    process.env.GEMINI_CHARM_PREFLIGHT_MODEL || STUDIO_SEMANTIC_TEXT_MODEL).trim();
  /* Semantic analysis is a text/vision job, never an image-generation job.
     A stale shared precheck setting must not make every render fail before
     debit or silently buy an unnecessary generated image. */
  const model = /(?:^|[-_.])image(?:[-_.]|$)/i.test(requestedModel)
    ? STUDIO_SEMANTIC_TEXT_MODEL : requestedModel;
  const cacheKey = `${STUDIO_SEMANTIC_VERSION}:${model}:${studioSha256(Buffer.from(STUDIO_SEMANTIC_PROMPT))}:${specHash}`;
  const cacheNow = Date.now();
  for (const [key, entry] of _studioSemanticCache) {
    if (!entry || cacheNow - Number(entry.at || 0) >= STUDIO_SEMANTIC_CACHE_TTL_MS) {
      _studioSemanticCache.delete(key);
    }
  }
  const cached = _studioSemanticCache.get(cacheKey);
  if (cached) {
    /* Refresh insertion order so the cap below is a small true LRU. */
    _studioSemanticCache.delete(cacheKey);
    _studioSemanticCache.set(cacheKey, cached);
    return { ...cached.plan, cached: true, specHash, model: cached.model };
  }
  const apiKey = process.env.GEMINI_API_KEY;
  if (!apiKey) throw new Error("semantic_preflight_unavailable");
  ...
  const resp = await fetchWithTimeout(url, {
    ...
    body: JSON.stringify({
      contents: [{ role: "user", parts: [
        { text: STUDIO_SEMANTIC_PROMPT },
        { inline_data: { mime_type: "image/png", data: Buffer.from(materialSpecBuffer).toString("base64") } },
      ] }],
      generationConfig: { temperature: 0, responseMimeType: "application/json",
        responseModalities: ["TEXT"] },
    }),
  }, Math.max(5000, Number(cfg?.renderSemanticTimeoutMs) || 25000));
  const data = await readUpstreamJson(resp, "studio semantic preflight");
  const text = (data?.candidates?.[0]?.content?.parts || []).map((p) => p?.text || "").join("").trim();
  const plan = normalizeStudioSemanticPlan(extractJsonObject(text));
  _studioSemanticCache.set(cacheKey, { at: Date.now(), plan, model });
  while (_studioSemanticCache.size > STUDIO_SEMANTIC_CACHE_MAX) {
    _studioSemanticCache.delete(_studioSemanticCache.keys().next().value);
  }
  return { ...plan, cached: false, specHash, model };
}

async function studioValidateSemanticAgainstSpec(plan, materialSpecBuffer, sharp, minConfidence = 0.55) {
  if (!plan || !sharp) throw new Error("semantic_plan_unverifiable");
  const { data, info } = await sharp(materialSpecBuffer).removeAlpha().raw().toBuffer({ resolveWithObject: true });
  const w = info.width, h = info.height, ch = info.channels;
  const confidenceFloor = Math.max(0.35, Math.min(0.95, Number(minConfidence) || 0.55));
  if (Number(plan.overallConfidence) < confidenceFloor) throw new Error("semantic_preflight_low_confidence");
  if (plan.ambiguities?.length) throw new Error("semantic_plan_ambiguous");
  const toneAt = (i) => {
    const v = data[i * ch];
    if (Math.abs(v - STUDIO_MASK_POLISHED[0]) <= 24) return "polished";
    if (Math.abs(v - STUDIO_MASK_ENGRAVED[0]) <= 24) return "engraved";
    return "cutout";
  };
  const masks = { polished: new Uint8Array(w * h), engraved: new Uint8Array(w * h), cutout: new Uint8Array(w * h) };
  for (let i = 0; i < w * h; i++) masks[toneAt(i)][i] = 1;
  const classes = {};
  for (const cls of Object.keys(masks)) {
    const labeled = studioLabel(masks[cls], w, h);
    const sizes = new Uint32Array(labeled.count + 1), edge = new Uint8Array(labeled.count + 1);
    for (let i = 0; i < w * h; i++) {
      const L = labeled.labels[i]; if (!L) continue;
      sizes[L]++;
      const x = i % w, y = (i / w) | 0;
      if (!x || !y || x === w - 1 || y === h - 1) edge[L] = 1;
    }
    classes[cls] = { ...labeled, sizes, edge };
  }
  const seenClasses = new Set();
  const coveredComponents = {
    polished: new Set(), engraved: new Set(), cutout: new Set(),
  };
  const componentRegions = {
    polished: new Map(), engraved: new Map(), cutout: new Map(),
  };
  for (const region of plan.regions) {
    if (Number(region.confidence) < confidenceFloor) throw new Error("semantic_region_low_confidence");
    const b = region.bbox;
    const x0 = Math.floor(b.x0 * w), y0 = Math.floor(b.y0 * h);
    const x1 = Math.max(x0 + 1, Math.ceil(b.x1 * w)), y1 = Math.max(y0 + 1, Math.ceil(b.y1 * h));
    let total = 0, matching = 0;
    const componentHits = new Map(), classInfo = classes[region.class];
    for (let y = y0; y < Math.min(h, y1); y++) for (let x = x0; x < Math.min(w, x1); x++) {
      total++;
      const i = y * w + x;
      if (toneAt(i) !== region.class) continue;
      const L = classInfo.labels[i];
      /* Exterior paper is not a through-opening. */
      if (region.class === "cutout" && classInfo.edge[L]) continue;
      matching++;
      componentHits.set(L, (componentHits.get(L) || 0) + 1);
    }
    let bestLabel = 0, bestHit = 0;
    for (const [L, hit] of componentHits) if (hit > bestHit) { bestLabel = L; bestHit = hit; }
    const densityMin = region.class === "engraved" ? 0.35 : 0.25;
    const coverage = bestLabel && classInfo.sizes[bestLabel] ? bestHit / classInfo.sizes[bestLabel] : 0;
    /* Eyelet bands and recognition features are legitimate semantic
       SUBREGIONS of a larger connected tone component. Requiring their tight
       bbox to cover 55% of the entire outer-body component rejects the exact
       production knife/fork map. Component coverage remains mandatory for
       whole structural regions and every true opening; subregions are proved
       by class density inside their own bbox instead. */
    const wholeComponentRole = region.class === "cutout" ||
      region.role === "outer_body" || region.role === "field" || region.role === "through_opening";
    if (!total || matching / total < densityMin || (wholeComponentRole && coverage < 0.55)) {
      throw new Error("semantic_plan_class_conflict");
    }
    seenClasses.add(region.class);
    /* A region identifies one connected manufacturing feature. Mark only its
       dominant component: a broad body bbox must not incidentally count a
       separate polished fork, nor an eyelet bbox count the knife opening. */
    if (bestLabel) {
      coveredComponents[region.class].add(bestLabel);
      if (!componentRegions[region.class].has(bestLabel)) componentRegions[region.class].set(bestLabel, []);
      componentRegions[region.class].get(bestLabel).push(region);
    }
  }
  for (const cls of ["polished", "engraved", "cutout"]) {
    let meaningful = 0;
    const classTotal = classes[cls].sizes.reduce((sum, size, L) =>
      sum + ((cls === "cutout" && classes[cls].edge[L]) ? 0 : size), 0);
    const componentFloor = Math.max(20, Math.round(classTotal * 0.012), Math.round(w * h * 0.0001));
    for (let L = 1; L <= classes[cls].count; L++) {
      if (cls === "cutout" && classes[cls].edge[L]) continue;
      meaningful += classes[cls].sizes[L];
      if (classes[cls].sizes[L] >= componentFloor && !coveredComponents[cls].has(L)) {
        /* Presence of a class is not enough. The target fixture has a body
           AND fork in polished metal, plus eyelet AND knife openings. Every
           meaningful connected feature must be named before the renderer is
           allowed to infer semantics. */
        throw new Error("semantic_plan_missing_component");
      }
    }
    if (meaningful >= Math.max(20, w * h * 0.0001) && !seenClasses.has(cls)) {
      throw new Error("semantic_plan_missing_class");
    }
  }
  /* Component coverage also needs a structurally honest role. Otherwise the
     correct fork/knife bboxes can be returned as vague `other` regions and
     the final renderer receives misleading names. Pixel classes cannot prove
     natural-language nouns, but they can prove these role constraints. */
  let largestPolishedLabel = 0;
  const polishedTotal = classes.polished.sizes.reduce((sum, size) => sum + size, 0);
  const polishedFloor = Math.max(20, Math.round(polishedTotal * 0.012), Math.round(w * h * 0.0001));
  for (let L = 1; L <= classes.polished.count; L++) {
    if (!largestPolishedLabel || classes.polished.sizes[L] > classes.polished.sizes[largestPolishedLabel]) {
      largestPolishedLabel = L;
    }
  }
  for (const [L, regions] of componentRegions.polished) {
    const meaningful = classes.polished.sizes[L] >= polishedFloor;
    if (!meaningful) continue;
    if (L === largestPolishedLabel) {
      if (!regions.some((region) => region.role === "outer_body")) throw new Error("semantic_plan_role_conflict");
    } else if (!regions.some((region) => region.role === "recognition_feature" ||
        (region.role === "eyelet_band" && region.id === plan.eyelet?.bandRegionId))) {
      throw new Error("semantic_plan_role_conflict");
    }
  }
  const cutoutTotal = classes.cutout.sizes.reduce((sum, size, L) =>
    sum + (classes.cutout.edge[L] ? 0 : size), 0);
  const cutoutFloor = Math.max(20, Math.round(cutoutTotal * 0.012), Math.round(w * h * 0.0001));
  for (const [L, regions] of componentRegions.cutout) {
    if (classes.cutout.edge[L] || classes.cutout.sizes[L] < cutoutFloor) continue;
    if (!regions.some((region) => region.role === "through_opening" ||
        (region.role === "eyelet_hole" && region.id === plan.eyelet?.holeRegionId))) {
      throw new Error("semantic_plan_role_conflict");
    }
  }
  /* Do not let the semantic model opt out of an eyelet that the deterministic
     pixels plainly contain. A compact enclosed opening near the top, with
     polished material on all four sides, is an integrated eyelet centre. */
  let deterministicEyelet = false;
  const cutInfo = classes.cutout;
  const cutBoxes = Array.from({ length: cutInfo.count + 1 }, () => ({ x0: w, y0: h, x1: -1, y1: -1 }));
  for (let i = 0; i < w * h; i++) {
    const L = cutInfo.labels[i]; if (!L || cutInfo.edge[L]) continue;
    const x = i % w, y = (i / w) | 0, b = cutBoxes[L];
    b.x0 = Math.min(b.x0, x); b.x1 = Math.max(b.x1, x);
    b.y0 = Math.min(b.y0, y); b.y1 = Math.max(b.y1, y);
  }
  for (let L = 1; L <= cutInfo.count && !deterministicEyelet; L++) {
    if (cutInfo.edge[L] || cutInfo.sizes[L] < Math.max(20, w * h * 0.0001)) continue;
    const b = cutBoxes[L], bw = b.x1 - b.x0 + 1, bh = b.y1 - b.y0 + 1;
    const cx = (b.x0 + b.x1) / 2, cy = (b.y0 + b.y1) / 2;
    if (b.x1 < b.x0 || cy > h * 0.24 || cx < w * 0.15 || cx > w * 0.85 ||
        bw / Math.max(1, bh) < 0.60 || bw / Math.max(1, bh) > 1.65) continue;
    const pad = Math.max(3, Math.round(Math.max(bw, bh) * 0.65));
    const side = [0, 0, 0, 0];
    for (let y = Math.max(0, b.y0 - pad); y <= Math.min(h - 1, b.y1 + pad); y++) {
      for (let x = Math.max(0, b.x0 - pad); x <= Math.min(w - 1, b.x1 + pad); x++) {
        if (!masks.polished[y * w + x]) continue;
        if (x < b.x0) side[0]++;
        if (x > b.x1) side[1]++;
        if (y < b.y0) side[2]++;
        if (y > b.y1) side[3]++;
    ...
    const sideMin = Math.max(3, Math.round(Math.min(bw, bh) * 0.25));
    deterministicEyelet = side.every((count) => count >= sideMin);
  }
  if (deterministicEyelet && !plan.eyelet?.present) {
    throw new Error("semantic_eyelet_missing");
  }
  if (plan.eyelet?.present) {
    const byId = new Map(plan.regions.map((r) => [r.id, r]));
    const band = byId.get(plan.eyelet.bandRegionId), hole = byId.get(plan.eyelet.holeRegionId);
    if (Number(plan.eyelet.confidence) < confidenceFloor ||
        !band || band.class !== "polished" || band.role !== "eyelet_band") throw new Error("semantic_eyelet_band_conflict");
    if (!hole || hole.class !== "cutout" || hole.role !== "eyelet_hole") throw new Error("semantic_eyelet_hole_conflict");
    const hasInside = plan.relations.some((r) => r.type === "inside" && r.parentId === band.id && r.childId === hole.id);
    if (!hasInside) throw new Error("semantic_eyelet_relation_missing");
  } else if (plan.regions.some((r) => r.role === "eyelet_band" || r.role === "eyelet_hole")) {
    throw new Error("semantic_eyelet_presence_conflict");
  }
  return plan;
}

async function studioValidateZonesAgainstSpec(zones, materialSpecBuffer, sharp) {
  const declared = (Array.isArray(zones) ? zones : []).filter((z) => z.intent === "engrave" || z.intent === "cutout");
  if (!declared.length) return true;
  const { data, info } = await sharp(materialSpecBuffer).removeAlpha().raw().toBuffer({ resolveWithObject: true });
  const w = info.width, h = info.height, ch = info.channels;
  for (const z of declared) {
    const x0 = Math.floor(z.x * w), y0 = Math.floor(z.y * h);
    const x1 = Math.max(x0 + 1, Math.ceil((z.x + z.w) * w));
    const y1 = Math.max(y0 + 1, Math.ceil((z.y + z.h) * h));
    let total = 0, matching = 0;
    ...
    for (let y = y0; y < Math.min(h, y1); y += step) for (let x = x0; x < Math.min(w, x1); x += step) {
      const v = data[(y * w + x) * ch]; total++;
      if (z.intent === "engrave" ? Math.abs(v - STUDIO_MASK_ENGRAVED[0]) <= 24 : v > 230) matching++;
    }
    if (!total || matching < Math.max(2, total * 0.005)) throw new Error("render_zone_spec_conflict");
  }
  return true;
}
...
async function handleStudioPrecheck({ body, event, origin }) {
  ...
  if (!(await studioBudgetOk("pc", uid, STUDIO_MAX_GENS_HOUR_UID)) ||
      !(await studioBudgetOk("pcip", studioClientIp(event), STUDIO_MAX_GENS_HOUR_IP))) {
...
const STUDIO_GENERATION_LEASE_MS = 10 * 60 * 1000;
function studioGenerationOperationId(sessionId, n, runId) {
  return `custom_charm_generation:${sessionId}:v${n}:${runId}`;
}

async function studioClaimGeneration(sRef, vRef, uid, n, runId, initial) {
  const db = getDb(), now = Date.now();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    if (!sSnap.exists || sSnap.data()?.uid !== uid || sSnap.data()?.deleted === true || sSnap.data()?.deletePending === true) {
      return { state: "session_deleted" };
    }
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    if (d.deletePending === true) return { state: "version_deleted", data: d };
    const same = String(d.generationRunId || "") === runId;
    if (same && d.status === "done") return { state: "done", data: d };
    if (d.generationProviderAmbiguous === true && Number(d.generationRetryAfterMs || 0) > now) {
      return { state: "provider_result_pending", data: d };
    }
    if (d.generationRefundPending === true || d.generationRecoveryState === "reconciling") {
      return { state: "refund_pending", data: d };
    }
    if (same && d.status === "failed") return { state: "failed", data: d };
    if (same && ["queued", "generating"].includes(d.status) && Number(d.generationLeaseUntil || 0) > now) {
      return { state: "existing", data: d };
    }
    if (same && ["queued", "generating"].includes(d.status)) return { state: "interrupted", data: d };
    if (!same && ["queued", "generating"].includes(d.status)) {
      const rejections = studioRenderRejections(d.generationRejections, runId, "generation_in_progress", now);
      tx.set(vRef, { generationRejections: rejections }, { merge: true });
      return { state: "busy", data: d };
    }
    if (!same && d.status === "done") return { state: "version_exists", data: d };
    tx.set(vRef, {
      ...initial, n, status: "queued", stage: "queued", generationRunId: runId,
      generationLeaseUntil: now + STUDIO_GENERATION_LEASE_MS,
      generationDebitPending: false, generationDebited: false,
      generationRefundPending: false, generationRefunded: false,
      generationRecoveryState: null, generationRecoveryLeaseUntil: 0,
      generationProviderAmbiguous: false, generationRetryAfterMs: 0,
      error: null,
    }, { merge: true });
    return { state: "claimed", data: { ...d, ...initial } };
  });
}

async function studioRecordGenerationRejection(sRef, vRef, uid, runId, error) {
  const db = getDb(), at = Date.now();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    if (!sSnap.exists || sSnap.data()?.uid !== uid || sSnap.data()?.deleted === true || sSnap.data()?.deletePending === true) return false;
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    tx.set(vRef, {
      generationRejections: studioRenderRejections(d.generationRejections, runId, error, at),
    }, { merge: true });
    return true;
  });
}

async function studioGenerationPatch(vRef, runId, patch, terminal) {
  const db = getDb(), now = Date.now();
  return db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    if (d.deletePending === true || String(d.generationRunId || "") !== runId || !["queued", "generating"].includes(d.status) ||
        d.generationRecoveryState === "reconciling") return false;
    tx.set(vRef, {
      ...patch, generationRunId: runId,
      generationLeaseUntil: terminal ? 0 : now + STUDIO_GENERATION_LEASE_MS,
    }, { merge: true });
    return true;
  });
}

async function studioFinalizeGenerationForLiveSession(sRef, vRef, n, runId, patch) {
  const db = getDb();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    ...
    if (!sSnap.exists || s.deleted === true || s.deletePending === true || v.deletePending === true || String(v.generationRunId || "") !== runId ||
        !["queued", "generating"].includes(v.status) || v.generationRecoveryState === "reconciling") {
      return false;
    }
    tx.set(vRef, { ...patch, n, generationRunId: runId, generationLeaseUntil: 0 }, { merge: true });
    tx.set(sRef, { currentVersion: n, approved: false,
      approvedRenderRunId: null, approvedVersionNumber: null, approvedMetal: null,
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    return true;
  });
}

async function studioFailGeneration(sRef, vRef, n, runId, error, extra = {}) {
  const db = getDb();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    if (!sSnap.exists || sSnap.data()?.deleted === true) return false;
    const vSnap = await tx.get(vRef), v = vSnap.exists ? vSnap.data() : {};
    if (String(v.generationRunId || "") !== runId ||
        !["queued", "generating"].includes(v.status) || v.generationRecoveryState === "reconciling") return false;
    tx.set(vRef, {
      ...extra, n, status: "failed", stage: "failed", generationRunId: runId,
      generationLeaseUntil: 0, error: String(error).slice(0, 300),
      failedAt: admin.firestore.FieldValue.serverTimestamp(),
    }, { merge: true });
    return true;
  });
...
/* A background worker can be terminated after its wallet transaction but
   before its catch block runs. Status reads and the next queue preflight use
   this lease recovery to settle that deterministic ledger operation exactly
   once. Recovery first takes ownership transactionally; all worker patches
   then fail while the refund is in progress. */
async function studioReconcileStaleGeneration(vRef, uid, sessionId) {
  const db = getDb(), now = Date.now(), recoveryLeaseMs = 60 * 1000;
  const recoveryToken = studioSha256(`${uid}:${sessionId}:${vRef.id}:${now}:${Math.random()}`).slice(0, 32);
  const claimed = await db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef);
    if (!snap.exists) return { state: "none" };
    const d = snap.data() || {};
    const retryRefund = d.status === "failed" && d.generationRefundPending === true;
    const expired = ["queued", "generating"].includes(d.status) && Number(d.generationLeaseUntil || 0) < now;
    if (!retryRefund && !expired) return { state: "none" };
    if (d.generationRecoveryState === "reconciling" && Number(d.generationRecoveryLeaseUntil || 0) >= now) {
      return { state: "existing" };
    }
    tx.set(vRef, {
      generationRecoveryState: "reconciling",
      generationRecoveryToken: recoveryToken,
      generationRecoveryLeaseUntil: now + recoveryLeaseMs,
      generationRefundPending: d.generationDebited === true || d.generationDebitPending === true ||
        d.generationRefundPending === true,
    }, { merge: true });
    return { state: "claimed", data: d };
  });
  if (claimed.state !== "claimed") return claimed.state === "existing";

  const d = claimed.data || {}, runId = String(d.generationRunId || "");
  const owesSettlement = d.generationDebited === true || d.generationDebitPending === true ||
    d.generationRefundPending === true;
  const op = String(d.generationOperationId || studioGenerationOperationId(sessionId, vRef.id, runId));
  const refund = owesSettlement
    ? await studioRefundResult(uid, Number(d.generationCost) || 0, sessionId, op)
    : { settled: true, refunded: false, reason: "not_debited" };

  await db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef), current = snap.exists ? snap.data() : {};
    if (String(current.generationRunId || "") !== runId ||
        current.generationRecoveryState !== "reconciling" ||
        String(current.generationRecoveryToken || "") !== recoveryToken) return;
    const settled = refund.settled === true;
    tx.set(vRef, {
      status: "failed", stage: "failed", generationLeaseUntil: 0,
      error: current.error || "generation_interrupted",
      generationRefunded: refund.refunded === true,
      generationRefundPending: !settled && owesSettlement,
      generationDebitPending: settled ? false : current.generationDebitPending === true,
      generationDebited: settled ? false : current.generationDebited === true,
      generationRecoveryState: null, generationRecoveryToken: null,
      generationRecoveryLeaseUntil: 0,
      generationRefundReason: refund.reason || null,
      failedAt: admin.firestore.FieldValue.serverTimestamp(),
    }, { merge: true });
  });
  return true;
...
async function handleStudioGenerate({ kind, body, event, origin }) {
  const uid = await requireStudioUser(event);
  const cfg = await studioConfig();
  const cost = studioCreditCost(cfg.generateCost, 1);
  const db = getDb();
  ...
  const n = Number(body?.versionNumber);
  if (!Number.isInteger(n) || n < 1) return studioJson(400, { ok: false, error: "missing_version" }, origin);
  const vRef = sRef.collection("versions").doc(String(n));
  const generationRunId = String(body?.generationRunId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 80);
  if (!generationRunId) return studioJson(400, { ok: false, error: "missing_generation_run" }, origin);
  const isRefine = kind === "custom_charm_refine";
  const baseVersionNumber = Number(body?.baseVersionNumber);
  if (isRefine && (!Number.isInteger(baseVersionNumber) || baseVersionNumber < 1 || baseVersionNumber >= n)) {
    await studioRecordGenerationRejection(sRef, vRef, uid, generationRunId, "base_version_invalid").catch(() => {});
    return studioJson(400, { ok: false, error: "base_version_invalid", n, generationRunId }, origin);
  }
  const operationId = studioGenerationOperationId(sessionId, n, generationRunId);
  const instructions = studioCleanText(body?.instructions);
  const metal = studioCleanText(body?.metal || session.metal || "gold");
  const claim = await studioClaimGeneration(sRef, vRef, uid, n, generationRunId, {
    instructions, refineText: studioCleanText(body?.refineText), metal,
    generationOperationId: operationId, generationCost: cost,
    baseVersionNumber: isRefine ? baseVersionNumber : null,
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
  });
  if (claim.state === "done") return studioJson(200, { ok: true, idempotent: true, n,
    storagePath: claim.data?.storagePath, downloadURL: claim.data?.downloadURL, generationRunId }, origin);
  if (claim.state === "existing") return studioJson(202, { ok: true, queued: true, idempotent: true, n, generationRunId }, origin);
  if (claim.state !== "claimed") {
    const claimError = claim.state === "busy" ? "generation_in_progress" : claim.state;
    await studioRecordGenerationRejection(sRef, vRef, uid, generationRunId, claimError).catch(() => {});
    return studioJson(409, { ok: false, error: claimError, n, generationRunId }, origin);
  }
  ...
  if (!(await studioBudgetOk("gen", uid, STUDIO_MAX_GENS_HOUR_UID)) ||
    ...
    await studioFailGeneration(sRef, vRef, n, generationRunId, "rate_limited");
    return studioJson(429, { ok: false, error: "rate_limited" }, origin);
  }
  if (!(await studioCanAfford(uid, cost))) {
    await studioFailGeneration(sRef, vRef, n, generationRunId, "out_of_credits");
    return studioJson(402, { ok: false, error: "out_of_credits" }, origin);
  }

  // 1) atomic debit, BEFORE any upstream call
  let debitAttempted = false, debited = false, publishedStoragePath = "";
  try {
    if (!(await studioGenerationPatch(vRef, generationRunId, { generationDebitPending: true, status: "generating" }))) {
      return studioJson(409, { ok: false, error: "generation_superseded", generationRunId }, origin);
    }
    debitAttempted = true;
    const debitResult = await studioDebit(uid, cost, sessionId, operationId);
    if (debitResult?.alreadyApplied) {
      const reused = new Error("generation_operation_reused");
      reused.operationReused = true;
      throw reused;
    }
    debited = debitResult?.debited === true;
    await studioGenerationPatch(vRef, generationRunId, {
      generationDebitPending: false, generationDebited: true, status: "generating",
    });
  } catch (err) {
    if (err?.status === 402) {
      await studioFailGeneration(sRef, vRef, n, generationRunId, "out_of_credits", {
    ...
    const refund = err?.operationReused === true
      ? { settled: true, refunded: false, reason: "operation_reused" }
      : debitAttempted
      ? await studioRefundResult(uid, cost, sessionId, operationId)
      : { settled: true, refunded: false, reason: "not_attempted" };
    await studioFailGeneration(sRef, vRef, n, generationRunId, err?.message || err, {
      generationDebitPending: refund.settled ? false : debitAttempted && !debited,
      generationDebited: refund.settled ? false : debited,
      generationRefunded: refund.refunded === true,
      generationRefundPending: debitAttempted && refund.settled !== true,
      generationRefundReason: refund.reason || null,
    });
    return studioJson(500, { ok: false, error: "generation_failed", generationRunId,
      refunded: refund.refunded === true, refundPending: refund.settled !== true }, origin);
  }
  const generationStartedAt = Date.now();

  try {
    if (!(await studioGenerationPatch(vRef, generationRunId, { status: "generating", stage: "planning" }))) throw new Error("generation_superseded");
    ...
    if (ref.type === "upload" && ref.path) {
      ...
      const r = await fetchBufferWithTimeout(studioAbsoluteUrl(ref.item.image), {}, 60000);
    ...
    if (isRefine) {
      try {
        const baseSnap = await sRef.collection("versions").doc(String(baseVersionNumber)).get();
        const baseData = baseSnap.exists ? baseSnap.data() : {};
        if (baseData.status !== "done") throw new Error("base_version_not_ready");
        const basePath = String(baseData.storagePath ||
          `custom-studio/${uid}/uploads/designs/${sessionId}/v${baseVersionNumber}.png`);
        const prev = await storagePathToBuffer(basePath);
    ...
    if (!(await studioGenerationPatch(vRef, generationRunId, { stage: "generating" }))) throw new Error("generation_superseded");
    ...
    let outBuf = await callImageModelEdits({
      ...
      backgroundPolicy: null,
      requestTimeoutMs: Math.min(480000, Math.max(30000,
        Number(cfg.generationProviderTimeoutMs) || 300000)),
    });

    if (!(await studioGenerationPatch(vRef, generationRunId, { stage: "polishing" }))) throw new Error("generation_superseded");
    ...
    const drawingSHA256 = studioSha256(outBuf);
    ...
    const latestSession = await sRef.get();
    if (!latestSession.exists || latestSession.data()?.deleted === true) throw new Error("session_deleted");
    const runSlug = `${generationRunId.slice(0, 40)}-${studioSha256(generationRunId).slice(0, 16)}`;
    const storagePath = `custom-studio/${uid}/uploads/designs/${sessionId}/v${n}-${runSlug}.png`;
    publishedStoragePath = storagePath;
    const bucket = getBucket();
    const token = newDownloadToken();
    await bucket.file(storagePath).save(outBuf, {
      ...
      metadata: { metadata: { firebaseStorageDownloadTokens: token, uid, sessionId,
        version: String(n), generationRunId } },
    });
    const downloadURL = tokenDownloadURLFor(bucket.name, storagePath, token);

    let generationFinalized;
    try {
      generationFinalized = await studioFinalizeGenerationForLiveSession(sRef, vRef, n, generationRunId, {
        status: "done", stage: "done", storagePath, downloadURL,
        drawingSHA256,
        prompt: String(effectivePrompt).slice(0, 4000),
        model: studioModelConfig.id,
        generationDebitPending: false, generationDebited: false,
        generationRefundPending: false,
        doneAt: admin.firestore.FieldValue.serverTimestamp(),
      });
    } catch (e) {
      e.finalizeAmbiguous = true;
      throw e;
    }
    if (!generationFinalized) throw new Error("generation_superseded_before_publish");

    return studioJson(200, { ok: true, n, storagePath, downloadURL, generationRunId }, origin);
  } catch (err) {
    let terminalReadSucceeded = false;
    try {
      const terminal = await vRef.get(), d = terminal.exists ? terminal.data() : {};
      terminalReadSucceeded = true;
      if (String(d.generationRunId || "") === generationRunId && d.status === "done" && d.downloadURL) {
        return studioJson(200, { ok: true, idempotent: true, n,
          storagePath: d.storagePath, downloadURL: d.downloadURL, generationRunId }, origin);
      }
    } catch (_e) {}
    if (err?.finalizeAmbiguous === true && !terminalReadSucceeded) {
      console.error("[studio] generation finalization acknowledgement ambiguous; lease reconciliation owns outcome");
      return studioJson(202, { ok: true, queued: true, ambiguous: true, n, generationRunId }, origin);
    }
    if (publishedStoragePath) {
      try { await getBucket().file(publishedStoragePath).delete({ ignoreNotFound: true }); } catch (_e) {}
    }
    // A failed generation must never cost a credit.
    const refund = debitAttempted
      ? await studioRefundResult(uid, cost, sessionId, operationId)
      : { settled: true, refunded: false, reason: "not_attempted" };
    await studioFailGeneration(sRef, vRef, n, generationRunId, err?.message || err, {
      generationDebitPending: refund.settled ? false : debitAttempted && !debited,
      ...
      generationRefundPending: debitAttempted && refund.settled !== true,
      generationRefundReason: refund.reason || null,
      generationProviderAmbiguous: err?.ambiguous === true,
      generationRetryAfterMs: err?.ambiguous === true ? Date.now() + 10 * 60 * 1000 : 0,
      generationTotalMs: Date.now() - generationStartedAt,
    });
    await studioFinalizePendingSessionDelete(sRef, uid, sessionId).catch(() => {});
    console.error("[studio] generate failed:", err?.message || err);
    return studioJson(500, { ok: false, error: "generation_failed", generationRunId,
      refunded: refund.refunded === true, refundPending: debitAttempted && refund.settled !== true }, origin);
  }
}
...
function studioSpecCutMask(plan, zones) {
  ...
  const detected = cuts.length ? studioLabel(cut, w, h) : null;
  const detectedSize = detected ? new Uint32Array(detected.count + 1) : null;
  const detectedCovered = detected ? new Uint8Array(detected.count + 1) : null;
  const detectedCoreSize = detected ? new Uint32Array(detected.count + 1) : null;
  const detectedCoreCovered = detected ? new Uint32Array(detected.count + 1) : null;
  const zoneCovered = cuts.length ? new Uint8Array(n) : null;
  if (detected) for (let i = 0; i < n; i++) {
    const L = detected.labels[i]; if (!L) continue;
    detectedSize[L]++;
    const x = i % w, y = (i / w) | 0;
    /* A blue perimeter may be connected to a filled blue opening. Measure
       coverage on the component's interior, not merely whether a zone touched
       one pixel. Thin perimeter/bridge pixels have no four-neighbour core;
       every real filled opening does. */
    if (x > 0 && x + 1 < w && y > 0 && y + 1 < h &&
        cut[i - 1] && cut[i + 1] && cut[i - w] && cut[i + w]) {
      detectedCoreSize[L]++;
    }
  }
  for (const z of cuts) {
    ...
    for (let y = y0; y < y1; y++) for (let x = x0; x < x1; x++) {
      const i = y * w + x;
      if (blue[i]) { out[i] = 1; zoneCovered[i] = 1; }
      if (detected?.labels[i]) detectedCovered[detected.labels[i]] = 1;
    }
  }
  if (detected) {
    const meaningful = Math.max(12, Math.round(n * 0.00001));
    for (let i = 0; i < n; i++) {
      const L = detected.labels[i]; if (!L || !zoneCovered[i]) continue;
      const x = i % w, y = (i / w) | 0;
      if (x > 0 && x + 1 < w && y > 0 && y + 1 < h &&
          cut[i - 1] && cut[i + 1] && cut[i - w] && cut[i + w]) {
        detectedCoreCovered[L]++;
      }
    }
    for (let L = 1; L <= detected.count; L++) {
      if (detectedSize[L] >= meaningful && !detectedCovered[L]) throw new Error("cutout_zones_incomplete");
      if (detectedCoreSize[L] >= meaningful) {
        const uncoveredCore = detectedCoreSize[L] - detectedCoreCovered[L];
        if (uncoveredCore > Math.max(4, Math.floor(detectedCoreSize[L] * 0.10))) {
          throw new Error("cutout_zones_incomplete");
        }
      }
    }
  }
...
/* Direct import of the customer's documented grayscale contract. This path
   is intentionally detected by palette, so normal BLUE/RED/BLACK production
   drawings continue through studioDrawingPlan unchanged. A nested pair of
   closed light-grey eyelet contours represents an annular metal band: the
   white region between them is filled by even/odd nesting, while the inner
   white centre remains a true opening. Unresolved grayscale maps fail later
   instead of silently falling back to colour interpretation. */
async function studioDirectGrayscaleSpec(sharp, drawingBuf) {
  if (!sharp) return null;
  const guarded = sharp(drawingBuf, { limitInputPixels: 4_500_000, sequentialRead: true });
  const metadata = await guarded.metadata();
  const mw = Number(metadata.width) || 0, mh = Number(metadata.height) || 0;
  if (mw < 32 || mh < 32 || mw > 4096 || mh > 4096 || mw * mh > 4_500_000) {
    throw new Error("grayscale_spec_dimensions_invalid");
  }
  const { data, info } = await sharp(drawingBuf, { limitInputPixels: 4_500_000, sequentialRead: true })
  ...
  if (w < 32 || h < 32 || w > 4096 || h > 4096 || n > 4_500_000) throw new Error("grayscale_spec_dimensions_invalid");
  let chromatic = 0, light = 0, dark = 0, nonWhite = 0, palettePx = 0;
  for (let i = 0; i < n; i++) {
    const p = i * ch, r = data[p], g = data[p + 1], b = data[p + 2];
    const hi = Math.max(r, g, b), lo = Math.min(r, g, b), v = (r + g + b) / 3;
    if (hi - lo > 8) chromatic++;
    if (Math.min(Math.abs(v - 96), Math.abs(v - 176), Math.abs(v - 255)) <= 20) palettePx++;
    if (v < 230) {
      nonWhite++;
      if (v < 136) dark++; else light++;
    }
  }
  if (nonWhite < n * 0.01 || light < n * 0.005 || palettePx < n * 0.97 ||
      chromatic > Math.max(24, nonWhite * 0.005)) return null;

  const white = new Uint8Array(n);
  for (let i = 0; i < n; i++) {
    const p = i * ch, v = (data[p] + data[p + 1] + data[p + 2]) / 3;
    if (v >= 230) white[i] = 1;
  }
  const { labels, count } = studioLabel(white, w, h);
  if (count > 256) throw new Error("grayscale_spec_too_complex");
  const components = Array.from({ length: count + 1 }, () => ({ size: 0, x0: w, y0: h, x1: -1, y1: -1, edge: false, sample: -1, sx: 0, sy: 0 }));
  for (let i = 0; i < n; i++) {
    const L = labels[i]; if (!L) continue;
    const c = components[L], x = i % w, y = (i / w) | 0;
    if (c.sample < 0) c.sample = i;
    c.sx += x; c.sy += y;
    c.size++; c.x0 = Math.min(c.x0, x); c.x1 = Math.max(c.x1, x);
    c.y0 = Math.min(c.y0, y); c.y1 = Math.max(c.y1, y);
    if (!x || !y || x === w - 1 || y === h - 1) c.edge = true;
  }

  /* A bounding box is not proof of nesting: a C-shaped white region can
     surround another component's box. The production eyelet is attached at
     its base, so its white annulus is not itself a closed mathematical ring;
     require instead a compact TOP eyelet candidate that surrounds the child
     in all eight angular sectors and on all four sides. */
  const encloses = (candidateLabel, childLabel) => {
    const a = components[candidateLabel], b = components[childLabel];
    if (!a?.size || !b?.size || b.sample < 0 ||
        b.x0 <= a.x0 || b.x1 >= a.x1 || b.y0 <= a.y0 || b.y1 >= a.y1) return false;
    const aw = a.x1 - a.x0 + 1, ah = a.y1 - a.y0 + 1;
    const bw = b.x1 - b.x0 + 1, bh = b.y1 - b.y0 + 1;
    if (a.y1 > h * 0.35 || aw / ah < 0.60 || aw / ah > 1.65 ||
        bw / bh < 0.65 || bw / bh > 1.55) return false;
    const cx = b.sx / b.size, cy = b.sy / b.size;
    if (Math.abs(cx - (a.x0 + a.x1) / 2) > aw * 0.16 ||
        Math.abs(cy - (a.y0 + a.y1) / 2) > ah * 0.16) return false;
    let sectors = 0, sides = 0;
    for (let y = a.y0; y <= a.y1; y++) for (let x = a.x0; x <= a.x1; x++) {
      if (labels[y * w + x] !== candidateLabel) continue;
      const dx = x - cx, dy = y - cy;
      if (dx || dy) {
        let angle = Math.atan2(dy, dx); if (angle < 0) angle += Math.PI * 2;
        sectors |= 1 << Math.min(7, Math.floor(angle / (Math.PI / 4)));
      }
      if (x < b.x0) sides |= 1;
      if (x > b.x1) sides |= 2;
      if (y < b.y0) sides |= 4;
    ...
    return sectors === 0xff && sides === 0x0f;
  };
  const fillWhiteLabel = new Uint8Array(count + 1);
  const candidates = [];
  for (let L = 1; L <= count; L++) {
    const c = components[L], cw = c.x1 - c.x0 + 1, chh = c.y1 - c.y0 + 1;
    if (!c.edge && c.size >= 20 && c.y1 <= h * 0.35 && cw / Math.max(1, chh) >= 0.55 && cw / Math.max(1, chh) <= 1.75) {
      candidates.push(L);
    }
  }
  /* Do not truncate this census by area. A valid top eyelet can sit beside
     many larger through-openings; an area-based top-N silently dropped its
     centre and turned the annular band into a cut-out. The global component
     cap above bounds the work, and `encloses` rejects non-nested bboxes before
     it scans any candidate pixels. */
  candidates.sort((a, b) => components[a].y0 - components[b].y0 ||
    Math.abs((components[a].sx / components[a].size) - w / 2) -
    Math.abs((components[b].sx / components[b].size) - w / 2));
  for (const L of candidates) {
    const a = components[L];
    for (const K of candidates) {
      if (K === L) continue;
      const b = components[K];
      if (b.edge || !b.size || b.size >= a.size) continue;
      if (encloses(L, K)) {
        fillWhiteLabel[L] = 1;
        break;
      }
  ...
  /* The production raster has a dark antialiased silhouette stroke. It is a
     boundary rendering artefact, not a surface engraving. Only dark connected
     components that touch the EXTERIOR white neighbourhood are reclassified
     as polished boundary metal; interior dark components remain engraving.
     This preserves the documented three-state contract without engraving the
     charm's outline. */
  const darkMask = new Uint8Array(n), exteriorWhite = new Uint8Array(n);
  for (let i = 0; i < n; i++) {
    const p = i * ch, v = (data[p] + data[p + 1] + data[p + 2]) / 3;
    if (v < 136) darkMask[i] = 1;
    if (labels[i] && components[labels[i]].edge) exteriorWhite[i] = 1;
  }
  const darkInfo = studioLabel(darkMask, w, h);
  if (darkInfo.count > 256) throw new Error("grayscale_spec_too_complex");
  const darkSize = new Uint32Array(darkInfo.count + 1);
  const darkCanonical = new Uint32Array(darkInfo.count + 1);
  const darkValueSum = new Float64Array(darkInfo.count + 1);
  const exteriorNear = new Uint32Array(darkInfo.count + 1);
  const darkBox = Array.from({ length: darkInfo.count + 1 }, () => ({ x0: w, y0: h, x1: -1, y1: -1 }));
  for (let i = 0; i < n; i++) {
    const L = darkInfo.labels[i]; if (!L) continue;
    darkSize[L]++;
    const src = i * ch, sourceValue = (data[src] + data[src + 1] + data[src + 2]) / 3;
    darkValueSum[L] += sourceValue;
    if (Math.abs(sourceValue - 96) <= 2) darkCanonical[L]++;
    const x = i % w, y = (i / w) | 0;
    const box = darkBox[L]; box.x0 = Math.min(box.x0, x); box.x1 = Math.max(box.x1, x);
    ...
    for (let dy = -2; dy <= 2 && !near; dy++) {
      ...
      for (let dx = -2; dx <= 2; dx++) {
        ...
        if (exteriorWhite[yy * w + xx]) { near = true; break; }
      }
    }
    if (near) exteriorNear[L]++;
  }
  const structuralDark = new Uint8Array(darkInfo.count + 1);
  let structuralEdgePx = 0;
  for (let L = 1; L <= darkInfo.count; L++) {
    const box = darkBox[L], area = Math.max(1, (box.x1 - box.x0 + 1) * (box.y1 - box.y0 + 1));
    const sparseSilhouette = darkSize[L] / area < 0.06 &&
      ((box.x1 - box.x0 + 1) > w * 0.20 || (box.y1 - box.y0 + 1) > h * 0.20);
    const tinyAntialias = darkSize[L] <= 64 && exteriorNear[L] / Math.max(1, darkSize[L]) >= 0.70;
    const canonicalFraction = darkCanonical[L] / Math.max(1, darkSize[L]);
    const meanValue = darkValueSum[L] / Math.max(1, darkSize[L]);
    /* Canonical dark-grey pixels are explicit engraving data. The supplied
       raster's silhouette is instead a mixed antialias component (mostly
       values above 96); only that noncanonical edge artefact may be cleaned. */
    const antialiasedBoundary = canonicalFraction < 0.70 && meanValue > 99.5;
    if (antialiasedBoundary && darkSize[L] < n * 0.01 && exteriorNear[L] / Math.max(1, darkSize[L]) >= 0.05 &&
        (sparseSilhouette || tinyAntialias)) {
      structuralDark[L] = 1; structuralEdgePx += darkSize[L];
    }
  }

  const out = Buffer.alloc(n * 3, 255);
  const metalMask = new Uint8Array(n), engravedMask = new Uint8Array(n);
  let facePx = 0, holePx = 0, workPx = 0, reconstructedPx = 0;
  for (let i = 0; i < n; i++) {
    const p0 = i * ch, v = (data[p0] + data[p0 + 1] + data[p0 + 2]) / 3;
    const enclosedWhite = labels[i] && !components[labels[i]].edge;
    const reconstructed = enclosedWhite && fillWhiteLabel[labels[i]];
    if (v >= 230 && !reconstructed) { if (enclosedWhite) holePx++; continue; }
    const engraved = v < 136 && !reconstructed && !structuralDark[darkInfo.labels[i]];
    const tone = engraved ? STUDIO_MASK_ENGRAVED[0] : STUDIO_MASK_POLISHED[0];
    const p = i * 3; out[p] = out[p + 1] = out[p + 2] = tone;
    metalMask[i] = 1; facePx++;
    if (engraved) { engravedMask[i] = 1; workPx++; }
    if (reconstructed) reconstructedPx++;
  }
  if (!facePx || !holePx) throw new Error("grayscale_spec_incomplete");
  const topologyBefore = studioMaskTopology(metalMask, w, h);
  /* One charm must be one connected piece of metal. A detached inner eyelet
     ring is the signature of an unresolved nested-white ambiguity, not a
     manufacturable interpretation. */
  if (topologyBefore.foregroundComponents !== 1) {
    throw new Error("grayscale_spec_disconnected_metal");
  }
  const buf = await sharp(out, { raw: { width: w, height: h, channels: 3 } }).png().toBuffer();
  return {
    buf, facePx, holePx, workPx, reconstructedPx, structuralEdgePx, w, h,
    sourceMode: "direct-grayscale-v3",
    topologyBefore,
    engravedTopology: studioMaskTopology(engravedMask, w, h),
  };
}
...
function studioMaskTopology(mask, w, h) {
  const labels = new Int32Array(w * h); labels.fill(-1);
  let foregroundComponents = 0;
  for (let start = 0; start < mask.length; start++) {
    if (!mask[start] || labels[start] >= 0) continue;
    const stack = [start]; labels[start] = foregroundComponents;
    while (stack.length) {
      const i = stack.pop(), x = i % w, y = (i / w) | 0;
      if (x && mask[i - 1] && labels[i - 1] < 0) { labels[i - 1] = foregroundComponents; stack.push(i - 1); }
      if (x + 1 < w && mask[i + 1] && labels[i + 1] < 0) { labels[i + 1] = foregroundComponents; stack.push(i + 1); }
      if (y && mask[i - w] && labels[i - w] < 0) { labels[i - w] = foregroundComponents; stack.push(i - w); }
      if (y + 1 < h && mask[i + w] && labels[i + w] < 0) { labels[i + w] = foregroundComponents; stack.push(i + w); }
    }
  ...
  for (let start = 0; start < mask.length; start++) {
    ...
    while (stack.length) {
      const i = stack.pop(), x = i % w, y = (i / w) | 0;
      if (!x || !y || x === w - 1 || y === h - 1) touchesEdge = true;
      if (x && !mask[i - 1] && !seen[i - 1]) { seen[i - 1] = 1; stack.push(i - 1); }
      if (x + 1 < w && !mask[i + 1] && !seen[i + 1]) { seen[i + 1] = 1; stack.push(i + 1); }
      if (y && !mask[i - w] && !seen[i - w]) { seen[i - w] = 1; stack.push(i - w); }
      if (y + 1 < h && !mask[i + w] && !seen[i + w]) { seen[i + w] = 1; stack.push(i + w); }
    }
    if (!touchesEdge) enclosedHoles++;
  }
  return { foregroundComponents, enclosedHoles };
}
...
async function studioRefineMask(sharp, maskBuf, targetLong) {
  ...
  const topologyBefore = studioMaskTopology(metal, w, h);
  const engravedBefore = studioMaskTopology(engraved, w, h);
  if (!topologyBefore.foregroundComponents) throw new Error("mask_empty");

  const scale = Math.max(1, targetLong / Math.max(w, h));
  const tw = Math.round(w * scale), th = Math.round(h * scale);
  /* Blur in SOURCE-pixel units before resampling.  Blurring a field after a
     446x618 map has been enlarged to a ~3M-pixel 2K canvas is mathematically
     the same scale-space operation when sigma is multiplied by `scale`, but it
     costs hundreds of millions of JavaScript kernel taps and can consume most
     of a background-function window before the renderer is even called.
     Keeping sigma at 0.85 source pixels makes runtime depend on the uploaded
     drawing, not the requested proof size; the exact topology guard below is
     unchanged and remains the final authority. */
  const sourceSigma = 0.85;

  /* Smooth the occupancy union once. Holes are the inverse of this union and
     are never independently filled/despeckled. Engraving is classified only
     inside the fixed metal union, preventing white seams or invented metal. */
  const upMetalField = studioBilinearField(
    studioGaussField(studioSignedField(metal, w, h), w, h, sourceSigma),
    w, h, tw, th);
  const upEngravedField = engravedBefore.foregroundComponents
    ? studioBilinearField(
        studioGaussField(studioSignedField(engraved, w, h), w, h, sourceSigma),
        w, h, tw, th)
    : null;

  const rgb = Buffer.alloc(tw * th * 3, ABSENT);
  const outMetal = new Uint8Array(tw * th), outEngraved = new Uint8Array(tw * th);
  for (let i = 0; i < tw * th; i++) {
    const p = i * 3;
    if (upMetalField[i] >= 0) continue;
    outMetal[i] = 1;
    if (upEngravedField && upEngravedField[i] < 0) {
      outEngraved[i] = 1;
      rgb[p] = rgb[p + 1] = rgb[p + 2] = ENGRAVED;
    } else {
      rgb[p] = rgb[p + 1] = rgb[p + 2] = POLISHED;
    }
  }
  const topologyAfter = studioMaskTopology(outMetal, tw, th);
  const engravedAfter = studioMaskTopology(outEngraved, tw, th);
  if (topologyAfter.foregroundComponents !== topologyBefore.foregroundComponents ||
      topologyAfter.enclosedHoles !== topologyBefore.enclosedHoles ||
      engravedAfter.foregroundComponents !== engravedBefore.foregroundComponents ||
      engravedAfter.enclosedHoles !== engravedBefore.enclosedHoles) {
    /* Smoothing is cosmetic; topology is manufacturing data. Fall back to an
       exact nearest-neighbour enlargement so the model still receives the
       configured 2K instruction map without a single component/hole/class
       being merged or invented. */
    const safe = await sharp(maskBuf).resize(tw, th, { fit: "fill", kernel: "nearest" }).png().toBuffer();
    return {
      buf: safe, w: tw, h: th,
      topologyBefore, topologyAfter: topologyBefore,
      engravedBefore, engravedAfter: engravedBefore,
      refineMode: "nearest-topology-safe",
      rejectedSmoothTopology: { metal: topologyAfter, engraved: engravedAfter },
    };
  }
  const buf = await sharp(rgb, { raw: { width: tw, height: th, channels: 3 } }).png().toBuffer();
  return { buf, w: tw, h: th, topologyBefore, topologyAfter, engravedBefore, engravedAfter, refineMode: "signed-distance" };
}
...
function studioQuantile(values, q) {
  if (!values.length) return null;
...
function studioRgbLab(r8, g8, b8) {
  const lin = (v) => {
    v /= 255;
...
function studioHueDistance(a, b) {
  const d = Math.abs(a - b) % 360;
  return Math.min(d, 360 - d);
}

async function studioGoldQa(sharp, renderBuf, specBuf, metal) {
  const materialKey = studioMetalKey(metal);
  const report = { version: "material-structure-finish-v3", applicable: true,
    colorApplicable: studioUsesApproved14kCalibration(materialKey), materialKey,
    confident: false, pass: true, reasons: [] };
  if (!sharp || !specBuf) { report.applicable = false; return report; }
  try {
    const [render, spec] = await Promise.all([
      sharp(renderBuf).ensureAlpha().raw().toBuffer({ resolveWithObject: true }),
      sharp(specBuf).removeAlpha().raw().toBuffer({ resolveWithObject: true }),
    ]);
    const rw = render.info.width, rh = render.info.height, rc = render.info.channels;
    const sw = spec.info.width, sh = spec.info.height, sc = spec.info.channels;
    const rp = render.data, sp = spec.data;
    report.outputAspectRatio = rw / rh;
    if (Math.abs(report.outputAspectRatio - 1) > 0.03) report.reasons.push("output_not_square");
    let rx0 = rw, ry0 = rh, rx1 = -1, ry1 = -1, borderPx = 0, whiteBorderPx = 0, transparentPx = 0;
    const materialCandidate = new Uint8Array(rw * rh);
    for (let y = 0; y < rh; y++) for (let x = 0; x < rw; x++) {
      const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
      if (rc > 3 && rp[p + 3] < 250) transparentPx++;
      const chroma = Math.max(r, g, b) - Math.min(r, g, b);
      const value = (r + g + b) / 3;
      const border = x < rw * 0.035 || x >= rw * 0.965 || y < rh * 0.035 || y >= rh * 0.965;
      if (border) {
        borderPx++;
        if (r >= 238 && g >= 238 && b >= 238 && chroma <= 10) whiteBorderPx++;
      }
      const isMaterial = materialKey === "silver"
        ? value >= 65 && value < 247 && chroma <= 38
        : materialKey === "rose"
          ? value >= 65 && value < 247 && chroma >= 5 && r >= g + 2 && g >= b - 10
          : chroma >= 9 && r >= g - 6 && g > b + 4 && (r + g + b) > 180;
      if (isMaterial) materialCandidate[y * rw + x] = 1;
    }
    report.transparentFraction = transparentPx / Math.max(1, rw * rh);
    if (report.transparentFraction > 0.0005) report.reasons.push("transparent_background_or_subject");
    const materialLabels = studioLabel(materialCandidate, rw, rh);
    const materialSizes = new Uint32Array(materialLabels.count + 1);
    for (let i = 0; i < materialCandidate.length; i++) if (materialLabels.labels[i]) materialSizes[materialLabels.labels[i]]++;
    let largestMaterial = 0;
    for (let L = 1; L <= materialLabels.count; L++) if (!largestMaterial || materialSizes[L] > materialSizes[largestMaterial]) largestMaterial = L;
    const renderMetalMask = new Uint8Array(rw * rh);
    for (let i = 0; i < renderMetalMask.length; i++) if (materialLabels.labels[i] === largestMaterial) renderMetalMask[i] = 1;
    /* Added jump rings are small relative to a 2K canvas but enormous next
       to genuine antialias speckles. The old 0.05% floor let a visible 992px
       annulus pass; 0.005% still leaves broad headroom over the approved
       fixture's 14px largest stray gold-like speck. */
    const extraMaterialFloor = Math.max(64, Math.round(rw * rh * 0.00005));
    report.extraMaterialComponents = [];
    for (let L = 1; L <= materialLabels.count; L++) {
      if (L !== largestMaterial && materialSizes[L] >= extraMaterialFloor) report.extraMaterialComponents.push(materialSizes[L]);
    }
    if (report.extraMaterialComponents.length) report.reasons.push("disconnected_material_artifact");
    for (let y = 0; y < rh; y++) for (let x = 0; x < rw; x++) if (renderMetalMask[y * rw + x]) {
      if (x < rx0) rx0 = x; if (y < ry0) ry0 = y; if (x > rx1) rx1 = x; if (y > ry1) ry1 = y;
    }
    report.whiteGroundFraction = borderPx ? whiteBorderPx / borderPx : 0;
    if (report.whiteGroundFraction < 0.90) report.reasons.push("white_ground_missing");
    if (rx1 <= rx0 || ry1 <= ry0) { report.pass = false; report.reasons.push("gold_surface_not_found"); return report; }
    /* Audit the whole canvas, not only the outermost border and the charm's
       normalised box. Dark lettering, props and fragments in a side margin
       are forbidden even when they are not gold-coloured. A small expanded
       subject box and the exact lower-right contact-shadow corridor are the
       only non-white areas permitted outside the detected main charm. */
    const exteriorArtifactMask = new Uint8Array(rw * rh);
    const subjectMargin = Math.max(8, Math.round(Math.max(rw, rh) * 0.018));
    const outsideMetal = new Uint8Array(rw * rh);
    for (let i = 0; i < outsideMetal.length; i++) outsideMetal[i] = renderMetalMask[i] ? 0 : 1;
    const distanceFromMetal = studioChamfer(outsideMetal, rw, rh);
    const allowedShadowX0 = Math.max(0, Math.floor(rx0 + (rx1 - rx0 + 1) * 0.35));
    const allowedShadowY0 = Math.max(0, Math.floor(ry1 - rh * 0.035));
    const allowedShadowX1 = Math.min(rw - 1, Math.ceil(rx1 + rw * 0.12));
    const allowedShadowY1 = Math.min(rh - 1, Math.ceil(ry1 + rh * 0.09));
    const shadowCandidate = new Uint8Array(rw * rh);
    for (let y = allowedShadowY0; y <= allowedShadowY1; y++) {
      for (let x = allowedShadowX0; x <= allowedShadowX1; x++) {
        const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
        const value = (r + g + b) / 3, chroma = Math.max(r, g, b) - Math.min(r, g, b);
        if (chroma <= 14 && value >= 165 && value < 246) shadowCandidate[y * rw + x] = 1;
      }
    }
    const shadowLabels = studioLabel(shadowCandidate, rw, rh);
    const allowedShadowLabels = new Uint8Array(shadowLabels.count + 1);
    for (let i = 0; i < shadowCandidate.length; i++) {
      const L = shadowLabels.labels[i];
      if (L && distanceFromMetal[i] <= subjectMargin * 1.75) allowedShadowLabels[L] = 1;
    }
    for (let y = 0; y < rh; y++) for (let x = 0; x < rw; x++) {
      const i = y * rw + x;
      const insideSubject = distanceFromMetal[i] <= subjectMargin;
      const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
      const value = (r + g + b) / 3, chroma = Math.max(r, g, b) - Math.min(r, g, b);
      const plausibleShadow = !!allowedShadowLabels[shadowLabels.labels[i]];
      if (insideSubject || plausibleShadow) continue;
      if (value < 230 || chroma > 14) exteriorArtifactMask[i] = 1;
    }
    const exteriorLabels = studioLabel(exteriorArtifactMask, rw, rh);
    const exteriorSizes = new Uint32Array(exteriorLabels.count + 1);
    for (let i = 0; i < exteriorArtifactMask.length; i++) {
      if (exteriorLabels.labels[i]) exteriorSizes[exteriorLabels.labels[i]]++;
    }
    const exteriorArtifactFloor = Math.max(24, Math.round(rw * rh * 0.00001));
    report.exteriorArtifactComponents = [];
    for (let L = 1; L <= exteriorLabels.count; L++) {
      if (exteriorSizes[L] >= exteriorArtifactFloor) report.exteriorArtifactComponents.push(exteriorSizes[L]);
    }
    if (report.exteriorArtifactComponents.length) report.reasons.push("exterior_artifact_present");
    let sx0 = sw, sy0 = sh, sx1 = -1, sy1 = -1, specEngravedPx = 0;
    const specMetalMask = new Uint8Array(sw * sh);
    for (let y = 0; y < sh; y++) for (let x = 0; x < sw; x++) {
      const v = sp[(y * sw + x) * sc];
      if (Math.abs(v - STUDIO_MASK_ENGRAVED[0]) <= 24) specEngravedPx++;
      if (v < 230) {
        specMetalMask[y * sw + x] = 1;
        if (x < sx0) sx0 = x; if (y < sy0) sy0 = y; if (x > sx1) sx1 = x; if (y > sy1) sy1 = y;
      }
    }
    if (sx1 <= sx0 || sy1 <= sy0) { report.pass = false; report.reasons.push("spec_surface_not_found"); return report; }

    /* Compare the gold occupancy to the material map after normalising each
       to its own bounds. A small tolerance admits antialiasing, reflections
       and the requested 3–6° tilt; a filled rectangle/canvas cannot pass. */
    const N = 192;
    const rBox = { x0: rx0, y0: ry0, x1: rx1, y1: ry1, w: rx1 - rx0 + 1, h: ry1 - ry0 + 1 };
    const sBox = { x0: sx0, y0: sy0, x1: sx1, y1: sy1, w: sx1 - sx0 + 1, h: sy1 - sy0 + 1 };
    report.specAspectRatio = sBox.w / sBox.h;
    report.renderAspectRatio = rBox.w / rBox.h;
    report.aspectRatioDelta = Math.abs(Math.log(report.renderAspectRatio / report.specAspectRatio));
    report.specCanvasFill = Math.max(sBox.w / sw, sBox.h / sh);
    report.renderCanvasFill = Math.max(rBox.w / rw, rBox.h / rh);
    report.canvasFillDelta = Math.abs(report.renderCanvasFill - report.specCanvasFill);
    if (report.aspectRatioDelta > 0.06) report.reasons.push("proportion_distorted");
    if (report.canvasFillDelta > 0.18) report.reasons.push("subject_scale_mismatch");
    let rNorm = studioNormalise(renderMetalMask, rw, rh, rBox, N);
    const sNorm = studioNormalise(specMetalMask, sw, sh, sBox, N);
    /* Close one-pixel reflection gaps without filling a real opening. */
    const erode = (mask, width, height) => {
      const out = new Uint8Array(width * height);
      for (let y = 0; y < height; y++) for (let x = 0; x < width; x++) {
        const i = y * width + x;
        out[i] = mask[i] && x && y && x + 1 < width && y + 1 < height &&
          mask[i - 1] && mask[i + 1] && mask[i - width] && mask[i + width] ? 1 : 0;
      }
      return out;
    };
    let expectedMetalCore = sNorm;
    for (let k = 0; k < 3; k++) expectedMetalCore = erode(expectedMetalCore, N, N);
    const undeclaredDarkMask = new Uint8Array(N * N);
    for (let ny = 0; ny < N; ny++) for (let nx = 0; nx < N; nx++) {
      const q = ny * N + nx;
      if (!expectedMetalCore[q]) continue;
      const x = Math.max(0, Math.min(rw - 1, Math.round(rBox.x0 + (nx + 0.5) * rBox.w / N)));
      const y = Math.max(0, Math.min(rh - 1, Math.round(rBox.y0 + (ny + 0.5) * rBox.h / N)));
      const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
      const value = (r + g + b) / 3, chroma = Math.max(r, g, b) - Math.min(r, g, b);
      if (value < 65 && chroma <= 16) undeclaredDarkMask[q] = 1;
    }
    const darkLabels = studioLabel(undeclaredDarkMask, N, N);
    const darkSizes = new Uint16Array(darkLabels.count + 1);
    for (let q = 0; q < undeclaredDarkMask.length; q++) if (darkLabels.labels[q]) darkSizes[darkLabels.labels[q]]++;
    report.undeclaredDarkSurfaceComponents = [];
    for (let L = 1; L <= darkLabels.count; L++) {
      if (darkSizes[L] >= 4) report.undeclaredDarkSurfaceComponents.push(darkSizes[L]);
    }
    if (report.undeclaredDarkSurfaceComponents.length) report.reasons.push("undeclared_surface_mark");
    rNorm = erode(studioDilate(rNorm, N, N, 1), N, N);
    const rNear = studioDilate(rNorm, N, N, 3), sNear = studioDilate(sNorm, N, N, 3);
    let rCount = 0, sCount = 0, rMatch = 0, sMatch = 0;
    for (let i = 0; i < N * N; i++) {
      if (rNorm[i]) { rCount++; if (sNear[i]) rMatch++; }
      if (sNorm[i]) { sCount++; if (rNear[i]) sMatch++; }
    }
    report.structurePrecision = rCount ? rMatch / rCount : 0;
    report.structureRecall = sCount ? sMatch / sCount : 0;
    report.structureIoU = studioIoU(rNorm, sNorm, N * N);
    report.specTopology = studioMaskTopology(sNorm, N, N);
    report.renderTopology = studioMaskTopology(rNorm, N, N);
    if (report.structurePrecision < 0.80 || report.structureRecall < 0.80 || report.structureIoU < 0.68) {
      report.reasons.push("structure_mismatch");
    }
    /* Hole counts alone are insufficient: a model could fill the knife and
       eyelet, then invent two holes elsewhere. Match every enclosed component
       by registered pixels, centroid and area, and reject extras. */
    const enclosed = (metalMask) => {
      const absent = new Uint8Array(N * N);
      for (let i = 0; i < absent.length; i++) absent[i] = metalMask[i] ? 0 : 1;
      const labeled = studioLabel(absent, N, N);
      const comps = Array.from({ length: labeled.count + 1 }, () => ({ size: 0, sx: 0, sy: 0, edge: false }));
      for (let i = 0; i < absent.length; i++) {
        const L = labeled.labels[i]; if (!L) continue;
        const x = i % N, y = (i / N) | 0, c = comps[L];
        c.size++; c.sx += x; c.sy += y;
        if (!x || !y || x === N - 1 || y === N - 1) c.edge = true;
      }
      return comps.map((c, label) => ({ ...c, label, cx: c.size ? c.sx / c.size : 0, cy: c.size ? c.sy / c.size : 0 }))
        .filter((c) => c.label && !c.edge && c.size >= N * N * 0.0008)
        .map((c) => {
          const mask = new Uint8Array(N * N);
          for (let i = 0; i < mask.length; i++) if (labeled.labels[i] === c.label) mask[i] = 1;
          return { ...c, mask, near: studioDilate(mask, N, N, 3) };
        });
    };
    const specHoles = enclosed(sNorm), renderHoles = enclosed(rNorm), usedRenderHoles = new Set();
    let openingMatches = 0;
    for (const expected of specHoles) {
      let best = null;
      for (let j = 0; j < renderHoles.length; j++) {
        if (usedRenderHoles.has(j)) continue;
        const actual = renderHoles[j];
        const centreDistance = Math.hypot(expected.cx - actual.cx, expected.cy - actual.cy) / N;
        const areaRatio = actual.size / expected.size;
        if (centreDistance > 0.075 || areaRatio < 0.45 || areaRatio > 2.2) continue;
        let expectedHit = 0, actualHit = 0;
        for (let i = 0; i < N * N; i++) {
          if (expected.mask[i] && actual.near[i]) expectedHit++;
          if (actual.mask[i] && expected.near[i]) actualHit++;
        }
        const overlap = Math.min(expectedHit / expected.size, actualHit / actual.size);
        if (overlap >= 0.68 && (!best || overlap > best.overlap)) best = { j, overlap };
      }
      if (best) { usedRenderHoles.add(best.j); openingMatches++; }
    }
    report.specOpenings = specHoles.length;
    report.renderOpenings = renderHoles.length;
    report.openingMatches = openingMatches;
    if (openingMatches !== specHoles.length) report.reasons.push("opening_missing_or_moved");
    if (usedRenderHoles.size !== renderHoles.length) report.reasons.push("opening_invented");

    const isWhiteGroundAt = (x, y) => {
      const p = (Math.max(0, Math.min(rh - 1, y)) * rw + Math.max(0, Math.min(rw - 1, x))) * rc;
      const r = rp[p], g = rp[p + 1], b = rp[p + 2];
      return r >= 230 && g >= 230 && b >= 230 && Math.max(r, g, b) - Math.min(r, g, b) <= 14;
    };
    const sampleRegisteredWhite = (mask) => {
      const core = erode(erode(mask, N, N), N, N);
      let total = 0, whitePx = 0;
      for (let q = 0; q < N * N; q++) {
        if (!core[q]) continue;
        const nx = q % N, ny = (q / N) | 0;
        const x = Math.round(rBox.x0 + (nx + 0.5) * rBox.w / N);
        const y = Math.round(rBox.y0 + (ny + 0.5) * rBox.h / N);
        total++; if (isWhiteGroundAt(x, y)) whitePx++;
      }
      return { total, fraction: total ? whitePx / total : 0 };
    };
    const specAbsent = new Uint8Array(N * N);
    for (let i = 0; i < specAbsent.length; i++) specAbsent[i] = sNorm[i] ? 0 : 1;
    const absentInfo = studioLabel(specAbsent, N, N), edgeAbsent = new Uint8Array(absentInfo.count + 1);
    for (let q = 0; q < N * N; q++) {
      const L = absentInfo.labels[q]; if (!L) continue;
      const x = q % N, y = (q / N) | 0;
      if (!x || !y || x === N - 1 || y === N - 1) edgeAbsent[L] = 1;
    }
    const exteriorMask = new Uint8Array(N * N);
    for (let q = 0; q < N * N; q++) if (edgeAbsent[absentInfo.labels[q]]) exteriorMask[q] = 1;
    const exteriorWhite = sampleRegisteredWhite(exteriorMask);
    report.exteriorGroundWhiteFraction = exteriorWhite.fraction;
    if (exteriorWhite.total < 100 || exteriorWhite.fraction < 0.88) report.reasons.push("studio_ground_not_white");
    const openingWhiteFractions = specHoles.map((hole) => sampleRegisteredWhite(hole.mask).fraction);
    report.openingWhiteFractions = openingWhiteFractions;
    if (openingWhiteFractions.some((fraction) => fraction < 0.70)) report.reasons.push("opening_ground_not_white");
    const samples = { polished: { L: [], a: [], b: [] }, engraved: { L: [], a: [], b: [] } };
    const step = Math.max(1, Math.floor(Math.max(sw, sh) / 700));
    for (let sy = sy0; sy <= sy1; sy += step) for (let sx = sx0; sx <= sx1; sx += step) {
      const sv = sp[(sy * sw + sx) * sc];
      const kind = Math.abs(sv - STUDIO_MASK_ENGRAVED[0]) <= 24 ? "engraved"
        : Math.abs(sv - STUDIO_MASK_POLISHED[0]) <= 24 ? "polished" : "";
      if (!kind) continue;
      const u = (sx - sx0) / Math.max(1, sx1 - sx0), v = (sy - sy0) / Math.max(1, sy1 - sy0);
      const x = Math.max(0, Math.min(rw - 1, Math.round(rx0 + u * (rx1 - rx0))));
      const y = Math.max(0, Math.min(rh - 1, Math.round(ry0 + v * (ry1 - ry0))));
      const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
      const sampleChroma = Math.max(r, g, b) - Math.min(r, g, b);
      if (r + g + b < 150 || (report.colorApplicable ? sampleChroma < 8 : (r > 248 && g > 248 && b > 248))) continue;
      const lab = studioRgbLab(r, g, b);
      samples[kind].L.push(lab.L); samples[kind].a.push(lab.a); samples[kind].b.push(lab.b);
    }
    const summarize = (s) => {
      const L = studioMedian(s.L), a = studioMedian(s.a), b = studioMedian(s.b);
      const hue = a == null || b == null ? null : (Math.atan2(b, a) * 180 / Math.PI + 360) % 360;
      return { count: s.L.length, L, a, b, hue, aOverB: b > 0 ? a / b : null };
    };
    const calibratedOutlierFraction = (s) => {
      let bad = 0, total = Math.min(s.a.length, s.b.length);
      for (let i = 0; i < total; i++) {
        const a = s.a[i], b = s.b[i];
        const hue = (Math.atan2(b, a) * 180 / Math.PI + 360) % 360;
        if (!(b > 0) || hue < 78 || hue > 100 || a / b > 0.25) bad++;
      }
      return total ? bad / total : 1;
    };
    report.polished = summarize(samples.polished);
    report.engraved = summarize(samples.engraved);
    report.polishedColourOutlierFraction = calibratedOutlierFraction(samples.polished);
    report.engravedColourOutlierFraction = calibratedOutlierFraction(samples.engraved);
    report.polishedLightnessIqr = studioQuantile(samples.polished.L, 0.75) - studioQuantile(samples.polished.L, 0.25);
    report.engravedLightnessIqr = studioQuantile(samples.engraved.L, 0.75) - studioQuantile(samples.engraved.L, 0.25);
    report.hasEngraving = specEngravedPx > 0;

    /* Distribution spread alone can be faked by two flat vector swatches.
       Sample a registered spatial grid and require several lightness levels
       plus a meaningful amount of small, local illumination change inside
       each material class. Real polished/matte metal has continuous highlight
       falloff; a flat fill with a hard drop shadow does not. */
    const finishClass = new Uint8Array(N * N);
    const finishL = new Float32Array(N * N); finishL.fill(Number.NaN);
    const finishBins = { polished: new Set(), engraved: new Set() };
    for (let ny = 0; ny < N; ny++) for (let nx = 0; nx < N; nx++) {
      const q = ny * N + nx;
      const sx = Math.max(0, Math.min(sw - 1, Math.round(sBox.x0 + (nx + 0.5) * sBox.w / N)));
      const sy = Math.max(0, Math.min(sh - 1, Math.round(sBox.y0 + (ny + 0.5) * sBox.h / N)));
      const sv = sp[(sy * sw + sx) * sc];
      const kind = Math.abs(sv - STUDIO_MASK_ENGRAVED[0]) <= 24 ? "engraved"
        : Math.abs(sv - STUDIO_MASK_POLISHED[0]) <= 24 ? "polished" : "";
      if (!kind) continue;
      const x = Math.max(0, Math.min(rw - 1, Math.round(rBox.x0 + (nx + 0.5) * rBox.w / N)));
      const y = Math.max(0, Math.min(rh - 1, Math.round(rBox.y0 + (ny + 0.5) * rBox.h / N)));
      if (!renderMetalMask[y * rw + x]) continue;
      const p = (y * rw + x) * rc;
      const lab = studioRgbLab(rp[p], rp[p + 1], rp[p + 2]);
      finishClass[q] = kind === "polished" ? 1 : 2;
      finishL[q] = lab.L;
      finishBins[kind].add(Math.round(lab.L * 2)); // half-L* bins
    }
    const spatialFinish = (code, kind) => {
      let pairs = 0, soft = 0, hard = 0;
      for (let y = 0; y < N; y++) for (let x = 0; x < N; x++) {
        const i = y * N + x; if (finishClass[i] !== code) continue;
        for (const j of [x + 1 < N ? i + 1 : -1, y + 1 < N ? i + N : -1]) {
          if (j < 0 || finishClass[j] !== code || !Number.isFinite(finishL[i]) || !Number.isFinite(finishL[j])) continue;
          const d = Math.abs(finishL[i] - finishL[j]); pairs++;
          if (d >= 0.10 && d <= 5.0) soft++;
          if (d > 10) hard++;
        }
      }
      return { lightnessBins: finishBins[kind].size, pairs,
        softGradientFraction: pairs ? soft / pairs : 0,
        hardGradientFraction: pairs ? hard / pairs : 0 };
    };
    report.finishSpatial = {
      polished: spatialFinish(1, "polished"),
      engraved: spatialFinish(2, "engraved"),
    };

    /* Aggregate medians can hide a lost recognition island (for example the
       polished fork recoloured to match its engraved field). Audit every
       significant connected material-class component independently. */
    const classRegions = [];
    for (const kind of ["polished", "engraved"]) {
      const target = kind === "polished" ? STUDIO_MASK_POLISHED[0] : STUDIO_MASK_ENGRAVED[0];
      const classMask = new Uint8Array(sw * sh);
      let classTotal = 0;
      for (let i = 0; i < sw * sh; i++) {
        if (Math.abs(sp[i * sc] - target) <= 24) { classMask[i] = 1; classTotal++; }
      }
      const labeled = studioLabel(classMask, sw, sh), sizes = new Uint32Array(labeled.count + 1);
      for (let i = 0; i < classMask.length; i++) if (labeled.labels[i]) sizes[labeled.labels[i]]++;
      const minRegion = Math.max(40, Math.round(classTotal * 0.012), Math.round(sw * sh * 0.00025));
      for (let L = 1; L <= labeled.count; L++) {
        if (sizes[L] < minRegion) continue;
        const regionSamples = { L: [], a: [], b: [], labs: [] };
        const componentStep = Math.max(1, Math.floor(Math.sqrt(sizes[L] / 2500)));
        for (let sy = sy0; sy <= sy1; sy += componentStep) for (let sx = sx0; sx <= sx1; sx += componentStep) {
          if (labeled.labels[sy * sw + sx] !== L) continue;
          const u = (sx - sx0) / Math.max(1, sx1 - sx0), v = (sy - sy0) / Math.max(1, sy1 - sy0);
          const x = Math.max(0, Math.min(rw - 1, Math.round(rx0 + u * (rx1 - rx0))));
          const y = Math.max(0, Math.min(rh - 1, Math.round(ry0 + v * (ry1 - ry0))));
          const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
          ...
          if (r + g + b < 150 || (report.colorApplicable ? sampleChroma < 8 : (r > 248 && g > 248 && b > 248))) continue;
          const lab = studioRgbLab(r, g, b);
          regionSamples.L.push(lab.L); regionSamples.a.push(lab.a); regionSamples.b.push(lab.b);
          regionSamples.labs.push(lab);
        }
        let expectedWins = 0;
        for (const lab of regionSamples.labs) {
          const dp = Math.hypot(lab.L - report.polished.L, lab.a - report.polished.a, lab.b - report.polished.b);
          const de = Math.hypot(lab.L - report.engraved.L, lab.a - report.engraved.a, lab.b - report.engraved.b);
          if ((kind === "polished" && dp <= de) || (kind === "engraved" && de <= dp)) expectedWins++;
        }
        classRegions.push({ kind, specPx: sizes[L], ...summarize(regionSamples),
          expectedClassFraction: regionSamples.labs.length ? expectedWins / regionSamples.labs.length : 0 });
      }
    }
    report.classRegions = classRegions;
    const calibratedFinishConfidence = !report.colorApplicable ||
      (report.polished.count >= 80 && (!report.hasEngraving || report.engraved.count >= 30) &&
       classRegions.every((r) => r.count >= 20));
    report.confident = rCount >= N * N * 0.02 && sCount >= N * N * 0.02 && calibratedFinishConfidence;
    if (!report.confident) { report.pass = false; report.reasons.push("insufficient_region_samples"); return report; }
    if (report.colorApplicable && (!(report.polished.b > 0) || report.polished.hue < 78 || report.polished.hue > 100 || report.polished.aOverB > 0.25)) report.reasons.push("gold_too_rose_or_copper");
    if (report.colorApplicable && (report.polishedColourOutlierFraction > 0.05 ||
        (report.hasEngraving && report.engravedColourOutlierFraction > 0.05))) {
      report.reasons.push("gold_colour_outliers");
    }
    if (report.colorApplicable && (report.polishedLightnessIqr < 1.0 ||
        (report.hasEngraving && report.engravedLightnessIqr < 1.0))) {
      report.reasons.push("finish_too_flat");
    }
    if (report.colorApplicable && (report.finishSpatial.polished.lightnessBins < 6 || report.finishSpatial.polished.softGradientFraction < 0.30 ||
        (report.hasEngraving && (report.finishSpatial.engraved.lightnessBins < 6 ||
          report.finishSpatial.engraved.softGradientFraction < 0.30)))) {
      report.reasons.push("finish_too_synthetic");
    }
    if (report.colorApplicable && (report.finishSpatial.polished.hardGradientFraction < 0.012 ||
        (report.hasEngraving && report.finishSpatial.engraved.hardGradientFraction < 0.020))) {
      report.reasons.push("metallic_texture_missing");
    }
    if (report.hasEngraving) {
      if (report.colorApplicable && (!(report.engraved.b > 0) || report.engraved.hue < 78 || report.engraved.hue > 100 || report.engraved.aOverB > 0.25)) report.reasons.push("engraving_too_copper");
      if (report.colorApplicable && studioHueDistance(report.polished.hue, report.engraved.hue) > 7) report.reasons.push("engraving_hue_mismatch");
      const deltaL = report.polished.L - report.engraved.L;
      report.lightnessDelta = deltaL;
      if (report.colorApplicable && (deltaL > 12 || deltaL < 0.5)) report.reasons.push("engraving_contrast_wrong");
    }
    for (const region of report.colorApplicable ? classRegions : []) {
      if (report.colorApplicable && (!(region.b > 0) || region.hue < 78 || region.hue > 100 || region.aOverB > 0.25)) {
        report.reasons.push("material_region_colour_mismatch");
        break;
      }
      if (report.hasEngraving && region.kind === "polished" && region.L - report.engraved.L < 0.75) {
        report.reasons.push("polished_region_lost");
        break;
      }
      if (report.hasEngraving && region.kind === "engraved" && report.polished.L - region.L < 0.5) {
        report.reasons.push("engraved_region_lost");
        break;
      }
      const classCoverageFloor = region.kind === "polished" ? 0.75 : 0.60;
      if (report.hasEngraving && region.expectedClassFraction < classCoverageFloor) {
        report.reasons.push("material_region_partially_lost");
        break;
      }
    }
    /* A physical charm on the requested white sweep has a small neutral
       contact shadow below/right. This conservative signal plus nonzero
       within-material lightness spread rejects a flat two-tone vector fill. */
    let shadowTotal = 0, shadowPx = 0;
    const shadowBands = [0, 0, 0];
    const shadowX0 = Math.max(0, Math.floor(rx0 + rBox.w * 0.35));
    const shadowY0 = Math.max(0, Math.floor(ry1 - rh * 0.035));
    const shadowX1 = Math.min(rw, Math.ceil(rx1 + rw * 0.12));
    const shadowY1 = Math.min(rh, Math.ceil(ry1 + rh * 0.09));
    for (let y = shadowY0; y < shadowY1; y++) for (let x = shadowX0; x < shadowX1; x++) {
      if (renderMetalMask[y * rw + x]) continue;
      const p = (y * rw + x) * rc, r = rp[p], g = rp[p + 1], b = rp[p + 2];
      const chroma = Math.max(r, g, b) - Math.min(r, g, b), value = (r + g + b) / 3;
      shadowTotal++;
      if (chroma <= 14 && value >= 165 && value < 246) {
        shadowPx++;
        shadowBands[value < 205 ? 0 : value < 230 ? 1 : 2]++;
      }
    }
    report.contactShadowFraction = shadowTotal ? shadowPx / shadowTotal : 0;
    report.contactShadowBands = shadowBands.map((v) => shadowTotal ? v / shadowTotal : 0);
    const visibleShadowBands = report.contactShadowBands.filter((v) => v >= 0.0005).length;
    if (report.colorApplicable && (report.contactShadowFraction < 0.003 || visibleShadowBands < 2)) {
      report.reasons.push("contact_shadow_missing");
    }
    /* A contact shadow is a small soft grounding cue, not a second object.
       A huge hard rectangle can otherwise satisfy the minimum and two-band
       tests while remaining visibly impossible. The 20% cap is over five
       times the approved fixture's measured 3.8% shadow. */
    if (report.colorApplicable && report.contactShadowFraction > 0.20) {
      report.reasons.push("contact_shadow_implausible");
    }
    report.pass = report.reasons.length === 0;
    return report;
  } catch (err) {
    report.pass = false;
    report.reasons.push("qa_unavailable");
    report.error = String(err?.message || err).slice(0, 160);
    return report;
  }
}

function studioQaScore(report) {
  if (!report?.confident) return -1000;
  let score = report.pass ? 100 : 0;
  score -= (report.reasons || []).length * 20;
  if (Number.isFinite(report.polished?.hue)) score -= Math.abs(85 - report.polished.hue);
  if (Number.isFinite(report.engraved?.hue)) score -= Math.abs(85 - report.engraved.hue);
  if (Number.isFinite(report.lightnessDelta)) score -= Math.max(0, Math.abs(report.lightnessDelta - 4) - 4);
  return score;
}
 ...
 * ===================================================================== */
const STUDIO_RENDER_LEASE_MS = 8 * 60 * 1000;
function studioRenderOperationId(sessionId, versionNumber, renderRunId) {
  return `custom_charm_render:${sessionId}:v${versionNumber}:${renderRunId}`;
}

function studioRenderRejections(current, runId, error, at) {
  const entries = Object.entries(current && typeof current === "object" ? current : {})
    .filter(([id, value]) => /^[A-Za-z0-9_-]{1,80}$/.test(id) && value && Number(value.at) > at - 24 * 60 * 60 * 1000)
    .sort((a, b) => Number(b[1].at) - Number(a[1].at))
    .slice(0, 10);
  const out = Object.fromEntries(entries);
  out[runId] = { error: String(error).slice(0, 120), at };
  return out;
}

async function studioRecordRenderRejection(sRef, vRef, uid, renderRunId, error) {
  const db = getDb(), at = Date.now();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    if (!sSnap.exists || sSnap.data()?.uid !== uid || sSnap.data()?.deleted === true || sSnap.data()?.deletePending === true) return false;
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    tx.set(vRef, {
      renderRejectedRunId: renderRunId,
      renderRejectedError: String(error).slice(0, 300),
      renderRejectedAt: at,
      renderRejections: studioRenderRejections(d.renderRejections, renderRunId, error, at),
    }, { merge: true });
  });
}

async function studioClaimRender(sRef, vRef, uid, renderRunId, initial) {
  const db = getDb(), now = Date.now();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    if (!sSnap.exists || sSnap.data()?.uid !== uid || sSnap.data()?.deleted === true || sSnap.data()?.deletePending === true) {
    ...
    const snap = await tx.get(vRef);
    if (!snap.exists || snap.data()?.status !== "done") return { state: "version_not_ready" };
    const d = snap.data() || {};
    if (d.deletePending === true) return { state: "version_deleted", data: d };
    const same = String(d.renderRunId || "") === renderRunId;
    if (same && d.renderStatus === "done") return { state: "done", data: d };
    if (same && d.renderStatus === "failed") return { state: "failed", data: d };
    if (!same && d.renderProviderAmbiguous === true && Number(d.renderRetryAfterMs || 0) > now) {
      return { state: "ambiguous_cooldown", data: d };
    }
    if (d.renderRefundPending === true || d.renderRecoveryState === "reconciling") {
      return { state: "refund_pending", data: d };
    }
    if (same && d.renderStatus === "rendering" && Number(d.renderLeaseUntil || 0) > now) {
      return { state: "existing", data: d };
    }
    if (same && d.renderStatus === "rendering") {
      /* An expired delivery may have timed out after the provider accepted
         it. Never replay that ambiguous call under the same id. Reconcile and
         require a fresh customer run instead. */
      return { state: "interrupted", data: d };
    }
    if (!same && d.renderStatus === "rendering" && Number(d.renderLeaseUntil || 0) > now) {
      tx.set(vRef, {
        renderRejectedRunId: renderRunId,
        renderRejectedError: "render_in_progress",
        renderRejectedAt: now,
        renderRejections: studioRenderRejections(d.renderRejections, renderRunId, "render_in_progress", now),
      }, { merge: true });
      return { state: "busy", data: d };
    }
    /* Never overwrite an expired paid run. Recovery first takes ownership
       transactionally, settles its ledger operation, and only then may a new
       run claim this version. */
    if (!same && d.renderStatus === "rendering") return { state: "stale", data: d };
    tx.set(vRef, {
      ...initial,
      renderStatus: "rendering",
      renderStage: "planning",
      renderRunId,
      renderLeaseUntil: now + STUDIO_RENDER_LEASE_MS,
      renderStartedAtMs: now,
      renderError: null,
      renderDebitPending: false,
      renderDebited: false,
      renderRefunded: false,
      renderRefundPending: false,
      renderRecoveryState: null,
      renderRecoveryLeaseUntil: 0,
      renderProviderAmbiguous: false,
      renderRetryAfterMs: 0,
      renderRejectedRunId: null,
      renderRejectedError: null,
    }, { merge: true });
    return { state: "claimed", data: { ...d, ...initial } };
  });
}

async function studioRenderPatch(vRef, renderRunId, patch) {
  const db = getDb(), now = Date.now();
  return db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    if (d?.deletePending === true || String(d?.renderRunId || "") !== renderRunId || d?.renderStatus !== "rendering" ||
        d?.renderRecoveryState === "reconciling") return false;
    tx.set(vRef, { ...patch, renderRunId, renderLeaseUntil: now + STUDIO_RENDER_LEASE_MS }, { merge: true });
    return true;
  });
}

async function studioFinalizeRender(sRef, vRef, renderRunId, patch) {
  const db = getDb();
  return db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    const snap = await tx.get(vRef), d = snap.exists ? snap.data() : {};
    if (!sSnap.exists || sSnap.data()?.deleted === true || sSnap.data()?.deletePending === true) return false;
    if (d?.deletePending === true || String(d?.renderRunId || "") !== renderRunId || d?.renderStatus !== "rendering" ||
        d?.renderRecoveryState === "reconciling") return false;
    tx.set(vRef, {
      ...patch,
      renderStatus: "done", renderStage: "done", renderRunId,
      renderLeaseUntil: 0, renderDebitPending: false, renderDebited: false,
      renderRefunded: false, renderRefundPending: false,
      renderRecoveryState: null, renderRecoveryLeaseUntil: 0,
...
async function studioFailRender(sRef, vRef, error, renderRunId, extra = {}) {
  try {
    const db = getDb();
    return await db.runTransaction(async (tx) => {
      const sSnap = await tx.get(sRef);
      ...
      if (String(d?.renderRunId || "") !== String(renderRunId || "")) {
        ...
        tx.set(vRef, {
          renderRejectedRunId: String(renderRunId || ""),
          renderRejectedError: String(error).slice(0, 300),
          renderRejectedAt: rejectedAt,
          renderRejections: studioRenderRejections(d.renderRejections, String(renderRunId || ""), error, rejectedAt),
        }, { merge: true });
        return false;
      }
      if (d.renderStatus !== "rendering" || d.renderRecoveryState === "reconciling") return false;
      tx.set(vRef, {
        ...extra,
        renderStatus: "failed",
        /* Keep the last recognised progress stage; renderStatus drives failure. */
        renderStage: ["queued", "planning", "rendering", "engraving", "polishing"].includes(d.renderStage) ? d.renderStage : "planning",
...
async function studioReconcileStaleRender(vRef, uid, sessionId) {
  const db = getDb(), now = Date.now(), recoveryLeaseMs = 60 * 1000;
  const claimed = await db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef);
    if (!snap.exists) return { state: "none" };
    const d = snap.data() || {};
    const retryRefund = d.renderStatus === "failed" && d.renderRefundPending === true;
    const expiredRender = d.renderStatus === "rendering" && Number(d.renderLeaseUntil || 0) < now;
    if (!retryRefund && !expiredRender) return { state: "none" };
    if (d.renderRecoveryState === "reconciling" && Number(d.renderRecoveryLeaseUntil || 0) >= now) {
      return { state: "existing" };
    }
    tx.set(vRef, {
      renderRecoveryState: "reconciling",
      renderRecoveryLeaseUntil: now + recoveryLeaseMs,
      renderRefundPending: d.renderDebited === true || d.renderDebitPending === true || d.renderRefundPending === true,
    }, { merge: true });
    return { state: "claimed", data: d };
  });
  if (claimed.state !== "claimed") return claimed.state === "existing";

  const d = claimed.data || {}, runId = String(d.renderRunId || "");
  const owesSettlement = d.renderDebited === true || d.renderDebitPending === true || d.renderRefundPending === true;
  const op = String(d.renderOperationId || studioRenderOperationId(sessionId, vRef.id, runId));
  const refund = owesSettlement
    ? await studioRefundResult(uid, Number(d.renderCost) || 0, sessionId, op)
    : { settled: true, refunded: false, reason: "not_debited" };

  await db.runTransaction(async (tx) => {
    const snap = await tx.get(vRef), current = snap.exists ? snap.data() : {};
    if (String(current.renderRunId || "") !== runId || current.renderRecoveryState !== "reconciling") return;
    const settled = refund.settled === true;
    tx.set(vRef, {
      renderStatus: "failed",
      renderStage: ["queued", "planning", "rendering", "engraving", "polishing"].includes(current.renderStage)
        ? current.renderStage : "planning",
      renderLeaseUntil: 0,
      renderError: current.renderError || "render_interrupted",
      renderRefunded: refund.refunded === true,
      renderRefundPending: !settled && owesSettlement,
      /* Preserve the charge evidence until its ledger operation is known to
         be settled. A later status poll will retry the idempotent refund. */
      renderDebitPending: settled ? false : current.renderDebitPending === true,
      renderDebited: settled ? false : current.renderDebited === true,
      renderRecoveryState: null,
      renderRecoveryLeaseUntil: 0,
      renderRefundReason: refund.reason || null,
    }, { merge: true });
  });
  return true;
}
...
async function handleStudioRender({ body, event, origin }) {
  ...
  const highCost = Math.max(cost, studioCreditCost(cfg.renderHighCost, cost * 2));
  const renderCost = quality === "high" ? highCost : cost;

  const sessionId = String(body?.sessionId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 60);
  if (!sessionId) return studioJson(400, { ok: false, error: "missing_session" }, origin);
  const renderRunId = String(body?.renderRunId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 80);
  if (!renderRunId) return studioJson(400, { ok: false, error: "missing_render_run" }, origin);
  ...
  if (!Number.isInteger(n) || n < 1) return studioJson(400, { ok: false, error: "missing_version" }, origin);
  const vRef = sRef.collection("versions").doc(String(n));
  const requestedMetal = String(body?.metal || session.metal || "gold").trim().toLowerCase();
  if (!["silver", "gold", "rose", "solid10", "solid14"].includes(requestedMetal)) {
    await studioRecordRenderRejection(sRef, vRef, uid, renderRunId, "invalid_metal").catch(() => {});
    return studioJson(400, { ok: false, error: "invalid_metal", renderRunId }, origin);
  }
  const metal = requestedMetal;
  const operationId = studioRenderOperationId(sessionId, n, renderRunId);
  const requestedModel = quality === "high" ? (cfg.renderHighModel || "gpt-image-2") : studioRenderModelId(cfg);
  let modelConfig = null;
  try { modelConfig = resolveImageModel(requestedModel); } catch (_e) {}

  const claimInitial = {
    renderMetal: metal, renderQuality: quality,
    renderModel: modelConfig?.id || String(requestedModel || ""),
    renderCost, renderOperationId: operationId,
    renderPipelineVersion: STUDIO_RENDER_PIPELINE_VERSION,
    renderFinishVersion: STUDIO_RENDER_FINISH_VERSION,
    /* Every proof artifact belongs to exactly one run. Clear the complete prior
       bundle at claim time so a failure before mask publication cannot expose
       the previous run's map under this run id. */
    renderURL: null, renderPath: null, renderSHA256: null,
    renderSpecURL: null, renderSpecPath: null, renderSpecSHA256: null,
    renderSpecMode: null, renderSpecSourceMode: null,
    renderSpecFacePx: 0, renderSpecHolePx: 0, renderSpecWorkPx: 0,
    renderSpecReconstructedPx: 0, renderSpecTopologyBefore: null,
    renderSpecTopologyAfter: null, renderSpecRefineMode: null,
    renderSpecRejectedSmoothTopology: null, renderSpecRefineSkipped: null,
    renderSpecRoiMode: null, renderSpecRoiGuard: null, renderSpecRoiSignalPx: 0,
    renderSpecRoiBox: null, renderSpecSignalBox: null,
    renderSemanticModel: null, renderSemanticPlan: null,
    renderDrawingSHA256: null, renderPromptSHA256: null,
    renderQa: null, renderProviderCalls: 0, renderColorRepairAttempted: false,
    renderColorRepairApplied: false,
  };
  let claim = await studioClaimRender(sRef, vRef, uid, renderRunId, claimInitial);
  /* A different expired run must be settled before this run can own the
     version. Retry the claim once after the transactional reconciler; never
     overwrite the old debit markers merely to make progress. */
  if (claim.state === "stale" || claim.state === "refund_pending") {
    await studioReconcileStaleRender(vRef, uid, sessionId);
    claim = await studioClaimRender(sRef, vRef, uid, renderRunId, claimInitial);
  }
  if (claim.state === "version_not_ready") {
    await studioRecordRenderRejection(sRef, vRef, uid, renderRunId, "version_not_ready").catch(() => {});
    return studioJson(409, { ok: false, error: "version_not_ready", renderRunId }, origin);
  }
  if (claim.state === "busy") return studioJson(409, { ok: false, error: "render_in_progress", renderRunId }, origin);
  if (claim.state === "ambiguous_cooldown") {
    await studioRecordRenderRejection(sRef, vRef, uid, renderRunId, "provider_result_pending").catch(() => {});
    return studioJson(409, { ok: false, error: "provider_result_pending", renderRunId,
                             retryAfterMs: Number(claim.data?.renderRetryAfterMs || 0) }, origin);
  }
  if (claim.state === "stale" || claim.state === "refund_pending") {
    await studioRecordRenderRejection(sRef, vRef, uid, renderRunId, "refund_pending").catch(() => {});
    return studioJson(409, { ok: false, error: "refund_pending", renderRunId }, origin);
  }
  if (claim.state === "existing") return studioJson(202, { ok: true, queued: true, idempotent: true, renderRunId }, origin);
  if (claim.state === "interrupted") {
    await studioReconcileStaleRender(vRef, uid, sessionId);
    return studioJson(409, { ok: false, error: "render_interrupted", renderRunId }, origin);
  }
  if (claim.state === "done") {
    const d = claim.data || {};
    return studioJson(200, { ok: true, idempotent: true, n, renderRunId,
      renderURL: d.renderURL, renderPath: d.renderPath, renderSpecURL: d.renderSpecURL,
      renderSpecPath: d.renderSpecPath, renderMetal: d.renderMetal,
      renderQuality: d.renderQuality, renderModel: d.renderModel, renderCost: d.renderCost }, origin);
  }
  if (claim.state === "failed") return studioJson(409, { ok: false, error: claim.data?.renderError || "render_failed", renderRunId }, origin);

  if (!modelConfig) {
    await studioFailRender(sRef, vRef, quality === "high" ? "high_unavailable" : "render_unavailable", renderRunId);
    return studioJson(503, { ok: false, error: "render_unavailable", renderRunId }, origin);
  }
  let imageApiKey = "";
  try { imageApiKey = apiKeyForImageModel(modelConfig); }
  catch (_e) {
    await studioFailRender(sRef, vRef, quality === "high" ? "high_unavailable" : "render_unavailable", renderRunId);
    return studioJson(503, { ok: false, error: "render_unavailable", renderRunId }, origin);
  }
  if (!(await studioBudgetOk("gen", uid, STUDIO_MAX_GENS_HOUR_UID)) ||
      !(await studioBudgetOk("genip", studioClientIp(event), STUDIO_MAX_GENS_HOUR_IP))) {
    await studioFailRender(sRef, vRef, "rate_limited", renderRunId);
    return studioJson(429, { ok: false, error: "rate_limited", renderRunId }, origin);
  }
  if (!(await studioCanAfford(uid, renderCost))) {
    await studioFailRender(sRef, vRef, "out_of_credits", renderRunId);
    return studioJson(402, { ok: false, error: "out_of_credits", renderRunId }, origin);
  }

  let debited = false, debitAttempted = false, renderSpecURL = "", renderSpecPath = "",
    publishedRenderPath = "", qaReport = null, providerCalls = 0,
    colorRepairAttempted = false, colorRepairApplied = false;
  const startedAt = Date.now();
  /* Keep the advertised runtime inside the ownership lease, and reserve a
     meaningful tail for deterministic QA, PNG metadata, Storage and the
     final Firestore CAS. Provider timeouts alone are not an end-to-end
     deadline. */
  const renderRuntimeMs = Math.min(STUDIO_RENDER_LEASE_MS - 60000,
    Math.max(60000, Number(cfg.renderMaxRuntimeMs) || 330000));
  const renderDeadlineMs = startedAt + renderRuntimeMs;
  const postProviderReserveMs = Math.min(120000,
    Math.max(45000, Math.floor(renderRuntimeMs * 0.32)));
  const requireRenderTime = (reserveMs = 0) => {
    const remaining = renderDeadlineMs - Date.now() - reserveMs;
    if (remaining <= 0) throw new Error("render_deadline_exhausted");
    return remaining;
  };
  try {
    const bwPath = String(claim.data?.storagePath ||
      `custom-studio/${uid}/uploads/designs/${sessionId}/v${n}.png`);
    const bw = await storagePathToBuffer(bwPath);
    const sharpMod = studioSharp();
    if (!sharpMod && cfg.renderRequireDeterministicSpec !== false) throw new Error("deterministic_spec_unavailable");
    let studioPlan = null;
    const renderZones = studioValidateRenderZones(body?.zones);
    let materialSpec = sharpMod ? await studioDirectGrayscaleSpec(sharpMod, bw.buffer) : null;
    if (!materialSpec && sharpMod) {
      studioPlan = await studioDrawingPlan(sharpMod, bw.buffer);
      materialSpec = studioPlan ? await studioMaterialSpec(sharpMod, studioPlan, metal, renderZones) : null;
      if (materialSpec) materialSpec.sourceMode = "production-colour-v1";
    }
    if (!materialSpec?.buf && cfg.renderRequireDeterministicSpec !== false) throw new Error("deterministic_spec_failed");
    if (materialSpec && materialSpec.buf) {
      ...
      if (floor) {
        try {
          ...
          materialSpec.topologyBefore = refined.topologyBefore;
          materialSpec.topologyAfter = refined.topologyAfter;
          materialSpec.refineMode = refined.refineMode;
          materialSpec.rejectedSmoothTopology = refined.rejectedSmoothTopology || null;
        } catch (e) {
          /* The exact unrefined map is safer than a cosmetically smoother map
             whose components or holes changed. */
          materialSpec.refineSkipped = String(e?.message || e).slice(0, 80);
        }
      }
    }
    const specUsed = !!(materialSpec && materialSpec.buf);
    if (!specUsed && cfg.renderRequireDeterministicSpec !== false) throw new Error("deterministic_spec_required");
    if (specUsed) await studioValidateZonesAgainstSpec(renderZones, materialSpec.buf, sharpMod);

    /* Publish the deterministic manufacturing map as soon as it is valid.
       Style-reference and semantic checks happen after this point and can
       deliberately fail closed. Keeping the run-scoped map on the version
       document makes those failures diagnosable in the Gold-mask tab without
       charging a credit or exposing another run's artifact. */
    const runSlug = `${renderRunId.slice(0, 40)}-${studioSha256(renderRunId).slice(0, 16)}`;
    if (specUsed) {
      try {
        renderSpecPath = `custom-studio/${uid}/uploads/designs/${sessionId}/v${n}-${runSlug}-spec.png`;
        ...
        await specBucket.file(renderSpecPath).save(materialSpec.buf, {
          ...
          metadata: { metadata: { firebaseStorageDownloadTokens: specToken, uid, sessionId,
                                  version: String(n), spec: "1", renderRunId } },
        });
        renderSpecURL = tokenDownloadURLFor(specBucket.name, renderSpecPath, specToken);
        if (!(await studioRenderPatch(vRef, renderRunId, {
          renderSpecURL, renderSpecPath,
          renderSpecMode: "deterministic-material",
          renderSpecSourceMode: materialSpec?.sourceMode || "unknown",
        }))) throw new Error("render_superseded_after_spec");
      } catch (e) {
        if (String(e?.message || e) === "render_superseded_after_spec") {
          if (renderSpecPath) {
            try { await getBucket().file(renderSpecPath).delete({ ignoreNotFound: true }); } catch (_e) {}
          }
          renderSpecURL = ""; renderSpecPath = "";
          throw e;
        }
        ...
        renderSpecURL = ""; renderSpecPath = "";
        throw new Error("render_spec_storage_failed");
      }
    }

    let renderStyleRef = null;
    if (specUsed && studioUsesApproved14kCalibration(metal)) {
      renderStyleRef = await ensureStudioGoldStyleReference();
      if ((!renderStyleRef?.buffer?.length || renderStyleRef.sha256 !== STUDIO_GOLD_STYLE_REFERENCE_SHA256) && cfg.renderRequireStyleReference !== false) {
        throw new Error("verified_style_reference_required");
      }
    }
    let semanticPlan = null;
    if (specUsed && cfg.renderSemanticPreflight !== false) {
      const semanticConfidenceMin = Math.max(0, Math.min(1, Number(cfg.renderSemanticConfidenceMin) || 0.55));
      semanticPlan = await studioSemanticPreflight(materialSpec.buf, cfg);
      await studioValidateSemanticAgainstSpec(semanticPlan, materialSpec.buf, sharpMod, semanticConfidenceMin);
      if (semanticPlan.overallConfidence < semanticConfidenceMin &&
          cfg.renderRequireSemanticPreflight !== false) throw new Error("semantic_preflight_low_confidence");
    }
    if (!semanticPlan && cfg.renderRequireSemanticPreflight !== false) throw new Error("semantic_preflight_required");

    const drawingHash = studioSha256(bw.buffer), specHash = specUsed ? studioSha256(materialSpec.buf) : "";
    /* Debit only after every deterministic/preflight dependency has passed
       and immediately before the first paid image call. */
    const latestSession = await sRef.get();
    if (!latestSession.exists || latestSession.data()?.deleted === true) throw new Error("session_deleted");
    if (!(await studioRenderPatch(vRef, renderRunId, { renderDebitPending: true }))) {
      throw new Error("render_superseded_before_debit");
    }
    debitAttempted = true;
    const debitResult = await studioDebit(uid, renderCost, sessionId, operationId);
    if (debitResult?.alreadyApplied) {
      const reused = new Error("render_operation_reused");
      reused.operationReused = true;
      throw reused;
    }
    debited = debitResult?.debited === true;
    if (!(await studioRenderPatch(vRef, renderRunId, {
      ...
      renderDebitPending: false, renderDebited: true, renderDebitedAtMs: Date.now(),
      renderSpecURL, renderSpecPath,
      renderSpecMode: specUsed ? "deterministic-material" : "legacy-colour-fallback-disabled",
      renderSpecSourceMode: materialSpec?.sourceMode || "unknown",
      ...
      renderSpecReconstructedPx: Number(materialSpec?.reconstructedPx || 0),
      renderSpecTopologyBefore: materialSpec?.topologyBefore || null,
      renderSpecTopologyAfter: materialSpec?.topologyAfter || null,
      renderSpecRefineMode: materialSpec?.refineMode || "none",
      renderSpecRejectedSmoothTopology: materialSpec?.rejectedSmoothTopology || null,
      renderSpecRefineSkipped: materialSpec?.refineSkipped || null,
      ...
      renderStyleRefSHA256: renderStyleRef?.sha256 || null,
      /* One rerender upgrades a legacy drawing into the immutable proof
         contract: this hash is computed from the exact source bytes just
         downloaded for this run. */
      drawingSHA256: drawingHash,
      renderDrawingSHA256: drawingHash,
      renderSpecSHA256: specHash,
      renderSemanticVersion: STUDIO_SEMANTIC_VERSION,
      renderSemanticModel: semanticPlan?.model || null,
      renderSemanticPlan: semanticPlan ? {
        version: semanticPlan.version, subjectLabel: semanticPlan.subjectLabel,
        overallConfidence: semanticPlan.overallConfidence, eyelet: semanticPlan.eyelet,
        regions: semanticPlan.regions, relations: semanticPlan.relations, ambiguities: semanticPlan.ambiguities,
      } : null,
      renderPlanningMs: Date.now() - startedAt,
    }))) throw new Error("render_superseded_during_planning");

    const images = [{ buffer: materialSpec.buf, mime: "image/png", filename: "material-spec.png" }];
    if (renderStyleRef?.buffer?.length) images.push({ buffer: renderStyleRef.buffer,
    ...
    const effectivePrompt = buildMaterialSpecToCharmPrompt({ metal, hasReference: images.length > 1 });
    const finalPromptSuffix = buildStudioRenderFinalLock({ metal, semanticPlan });
    const promptAuditHash = (promptText, suffixText, role, auditImages) => studioSha256(Buffer.from(JSON.stringify({
      pipeline: STUDIO_RENDER_PIPELINE_VERSION,
      finish: STUDIO_RENDER_FINISH_VERSION,
      model: modelConfig.id,
      provider: modelConfig.provider,
      role,
      prompt: promptText,
      finalSuffix: suffixText,
      imageSHA256: auditImages.map((img) => studioSha256(img.buffer)),
    })));
    let promptHash = promptAuditHash(effectivePrompt, finalPromptSuffix, "material_spec_to_charm", images);
    const providerTimeout = () => {
      const remaining = requireRenderTime(postProviderReserveMs);
      if (remaining < 30000) throw new Error("render_deadline_exhausted");
      return Math.min(Math.max(30000, Number(cfg.renderProviderTimeoutMs) || 150000), remaining);
    };
    providerCalls = 1;
    if (!(await studioRenderPatch(vRef, renderRunId, {
      renderStage: "rendering", renderPromptSHA256: promptHash, renderProviderCalls: providerCalls,
    }))) throw new Error("render_superseded_before_provider");
    let outBuf = await callImageModelEdits({
      apiKey: imageApiKey,
      model: modelConfig.id,
      prompt: effectivePrompt,
      ...
      backgroundPolicy: null,
      finalPromptSuffix,
      requestTimeoutMs: providerTimeout(),
    });
    if (!(await studioRenderPatch(vRef, renderRunId, { renderStage: "polishing", renderPromptSHA256: promptHash }))) {
      throw new Error("render_superseded_after_provider");
    }

    qaReport = await studioGoldQa(sharpMod, outBuf, materialSpec.buf, metal);
    const qaMode = ["observe", "repair", "enforce"].includes(String(cfg.renderQaMode)) ? String(cfg.renderQaMode) : "enforce";
    const colorRepairReasons = new Set(["gold_too_rose_or_copper", "engraving_too_copper", "engraving_hue_mismatch",
      "gold_colour_outliers", "engraving_contrast_wrong", "material_region_colour_mismatch", "polished_region_lost", "engraved_region_lost",
      "finish_too_flat", "finish_too_synthetic", "metallic_texture_missing", "contact_shadow_missing",
      "contact_shadow_implausible"]);
    const canRetry = qaReport.colorApplicable && qaReport.confident && !qaReport.pass &&
      qaReport.reasons.length > 0 && qaReport.reasons.every((reason) => colorRepairReasons.has(reason)) &&
      (qaMode === "repair" || qaMode === "enforce") &&
      Math.max(0, Math.min(1, Number(cfg.renderMaxQaRetries) || 0)) > 0 &&
      renderDeadlineMs - Date.now() > postProviderReserveMs + 30000;
    if (canRetry) {
      const retryImages = [{ buffer: outBuf, mime: "image/png", filename: "charm-to-correct.png" }];
      if (renderStyleRef?.buffer?.length) retryImages.push({ buffer: renderStyleRef.buffer,
        mime: renderStyleRef.mime || STUDIO_GOLD_STYLE_REFERENCE_MIME,
        filename: renderStyleRef.filename || STUDIO_GOLD_STYLE_REFERENCE_FILENAME });
      const retryPrompt = "FINISH CORRECTION ONLY. Keep the finished charm geometry and camera exactly as supplied. Correct the calibrated yellow-gold hue, same-alloy engraving closeness, realistic polished reflections, fine matte engraving texture, and add or correct only the one required soft neutral contact shadow.";
      const retrySuffix = buildStudioRenderFinalLock({ metal, semanticPlan, retryReason: qaReport.reasons.join(", ") });
      providerCalls = 2;
      colorRepairAttempted = true;
      const retryPromptHash = promptAuditHash(retryPrompt, retrySuffix, "color_only_charm_repair", retryImages);
      if (!(await studioRenderPatch(vRef, renderRunId, {
        renderProviderCalls: providerCalls, renderColorRepairAttempted: true,
        renderRepairPromptSHA256: retryPromptHash,
      }))) throw new Error("render_superseded_before_repair");
      const retryBuf = await callImageModelEdits({
        apiKey: imageApiKey, model: modelConfig.id,
        prompt: retryPrompt,
        size: studioImageSizeString(studioRenderTier(cfg)), imageSizeTier: studioRenderTier(cfg),
        quality: "high", output_format: "png", images: retryImages,
        imageRoles: "color_only_charm_repair", charmGeometryPolicy: null, backgroundPolicy: null,
        finalPromptSuffix: retrySuffix,
        requestTimeoutMs: providerTimeout(),
      });
      const retryQa = await studioGoldQa(sharpMod, retryBuf, materialSpec.buf, metal);
      /* A colour-only correction is accepted only when the complete QA suite
         passes. A prettier hue must never win if the retry moved an opening,
         distorted the silhouette or flattened the finish. */
      if (retryQa.pass) {
        outBuf = retryBuf; qaReport = retryQa;
        promptHash = retryPromptHash;
        colorRepairApplied = true;
      }
    }
    if (qaMode === "enforce" && qaReport.applicable && (!qaReport.confident || !qaReport.pass)) {
      const error = new Error(qaReport.confident ? "render_qa_failed" : "render_qa_inconclusive");
      error.qaReport = qaReport;
      throw error;
    }
    requireRenderTime(30000);
    await studioRenderPatch(vRef, renderRunId, {
      renderProviderCalls: providerCalls,
      renderColorRepairAttempted: colorRepairAttempted,
      renderColorRepairApplied: colorRepairApplied,
      renderQaMode: qaMode, renderQa: qaReport,
      renderPromptSHA256: promptHash,
      renderCutMode: "off", renderCheck: "disabled_unsafe_segmentation",
      renderPolishingMs: Date.now() - startedAt,
    });
    ...
    const renderSHA256 = studioSha256(outBuf);
    requireRenderTime(20000);
    const beforePublish = await sRef.get();
    if (!beforePublish.exists || beforePublish.data()?.deleted === true) throw new Error("session_deleted");
    const renderPath = `custom-studio/${uid}/uploads/designs/${sessionId}/v${n}-${runSlug}-charm.png`;
    publishedRenderPath = renderPath;
    const bucket = getBucket();
    const token = newDownloadToken();
    ...
    let finalized;
    try {
      finalized = await studioFinalizeRender(sRef, vRef, renderRunId, {
        renderURL, renderPath, renderMetal: metal,
        renderSpecURL, renderSpecPath,
        renderQuality: quality, renderModel: modelConfig.id, renderCost,
        renderSHA256,
        renderOperationId: operationId,
        renderTotalMs: Date.now() - startedAt,
      });
    } catch (e) {
      /* Firestore can commit and still return an ambiguous transport error.
         The object is authoritative if that transaction committed, so the
         catch path must not delete/refund it until a successful read proves
         otherwise. Status/lease reconciliation owns an unreadable outcome. */
      e.finalizeAmbiguous = true;
      throw e;
    }
    if (!finalized) throw new Error("render_superseded_before_publish");

    return studioJson(200, { ok: true, n, renderURL, renderPath, renderMetal: metal,
                             renderSpecURL, renderSpecPath,
                             renderQuality: quality, renderModel: modelConfig.id,
    ...
    try {
      const terminal = await vRef.get(), d = terminal.exists ? terminal.data() : {};
      terminalReadSucceeded = true;
      if (String(d.renderRunId || "") === renderRunId && d.renderStatus === "done" && d.renderURL) {
        return studioJson(200, { ok: true, idempotent: true, n, renderRunId,
          renderURL: d.renderURL, renderPath: d.renderPath, renderSpecURL: d.renderSpecURL,
          renderSpecPath: d.renderSpecPath, renderMetal: d.renderMetal,
          renderQuality: d.renderQuality, renderModel: d.renderModel, renderCost: d.renderCost }, origin);
    ...
    if (err?.finalizeAmbiguous === true && !terminalReadSucceeded) {
      console.error("[studio] render finalization acknowledgement ambiguous; lease reconciliation owns outcome");
      return studioJson(202, { ok: true, queued: true, ambiguous: true, n, renderRunId }, origin);
    }
    if (publishedRenderPath) {
      try { await getBucket().file(publishedRenderPath).delete({ ignoreNotFound: true }); } catch (_e) {}
    }
    if (renderSpecPath) {
      let keepDiagnosticSpec = false;
      try {
        const [liveSession, liveVersion] = await Promise.all([sRef.get(), vRef.get()]);
        keepDiagnosticSpec = liveSession.exists && liveSession.data()?.deleted !== true &&
          liveVersion.exists && String(liveVersion.data()?.renderRunId || "") === renderRunId;
      } catch (_e) {}
      if (!keepDiagnosticSpec) {
        try { await getBucket().file(renderSpecPath).delete({ ignoreNotFound: true }); } catch (_e) {}
        renderSpecURL = ""; renderSpecPath = "";
      }
    }
    const refund = err?.operationReused === true
      ? { settled: true, refunded: false, reason: "operation_reused" }
      : debitAttempted
      ? await studioRefundResult(uid, renderCost, sessionId, operationId)
      : { settled: true, refunded: false, reason: "not_attempted" };
    await studioFailRender(sRef, vRef, String(err?.message || err), renderRunId, {
      renderDebitPending: refund.settled ? false : debitAttempted && !debited,
      renderDebited: refund.settled ? false : debited,
      renderRefunded: refund.refunded === true,
      renderRefundPending: debitAttempted && refund.settled !== true,
      renderRefundReason: refund.reason || null,
      renderProviderAmbiguous: err?.ambiguous === true,
      renderProviderCalls: providerCalls,
      renderColorRepairAttempted: colorRepairAttempted,
      renderColorRepairApplied: colorRepairApplied,
      renderRetryAfterMs: err?.ambiguous === true ? Date.now() + 10 * 60 * 1000 : 0,
      renderSpecURL, renderSpecPath,
      renderQa: err?.qaReport || qaReport || null,
      renderTotalMs: Date.now() - startedAt,
    });
    await studioFinalizePendingSessionDelete(sRef, uid, sessionId).catch(() => {});
    console.error("[studio] render failed:", err?.message || err);
    return studioJson(500, { ok: false, error: "render_failed", renderRunId,
                             refunded: refund.refunded === true,
                             refundPending: debitAttempted && refund.settled !== true }, origin);
  }
}
...
async function handleStudioSessionStatus({ body, event, origin }) {
  ...
  const sRef = db.collection(STUDIO_SESSIONS_COLL).doc(sessionId);
  const sSnap = await sRef.get();
  if (!sSnap.exists || sSnap.data().uid !== uid || sSnap.data().deleted === true) {
    return studioJson(403, { ok: false, error: "forbidden" }, origin);
  }
  const vRef = sRef.collection("versions").doc(n);
  await studioReconcileStaleGeneration(vRef, uid, sessionId).catch((e) => {
    console.error("[studio] stale generation reconciliation failed:", e?.message || e);
  });
  await studioReconcileStaleRender(vRef, uid, sessionId).catch((e) => {
    console.error("[studio] stale render reconciliation failed:", e?.message || e);
  });
...
/* ── SERVER-OWNED DELETION ────────────────────────────────────────────────
   A browser must never delete a version document that is the only durable
   evidence of a debit/refund operation.  Whole-design and generated-version
   deletion therefore pass through this authenticated gate.  `deletePending`
   is written on the parent first: generation/render claims also read that
   parent transactionally, so no new paid operation can start after deletion
   begins. Existing work is allowed to reach its catch/refund path; until every
   ledger flag is settled the tombstone is deliberately not finalized. */
function studioVersionBlocksDelete(v) {
  /* A browser-created placeholder can say `queued` before the server has
     claimed it. Only a server lease or ledger flag proves work is in flight. */
  const generationActive = ["queued", "generating"].includes(String(v?.status || "")) &&
    (Number(v?.generationLeaseUntil || 0) > 0 || v?.generationDebitPending === true || v?.generationDebited === true);
  const renderActive = String(v?.renderStatus || "") === "rendering" &&
    (Number(v?.renderLeaseUntil || 0) > 0 || v?.renderDebitPending === true || v?.renderDebited === true);
  return generationActive || renderActive ||
    v?.generationDebitPending === true || v?.generationDebited === true ||
    v?.generationRefundPending === true || v?.generationRecoveryState === "reconciling" ||
    v?.renderDebitPending === true || v?.renderDebited === true ||
    v?.renderRefundPending === true || v?.renderRecoveryState === "reconciling";
}

function studioOwnedDeletePaths(uid, sessionId, session, versionDocs) {
  const values = [];
  const designPrefix = `custom-studio/${uid}/uploads/designs/${sessionId}/`;
  const add = (value) => {
    const p = String(value || "").trim();
    /* References and library assets may be reused verbatim by another design.
       Only run-scoped outputs below this exact session prefix are provably
       owned here and safe to remove without reference counting. */
    if (!p || !p.startsWith(designPrefix)) return;
    values.push(p);
  };
  const addVersion = (v) => {
    if (!v || typeof v !== "object") return;
    add(v.path || v.storagePath); add(v.renderPath); add(v.renderSpecPath);
  };
  (Array.isArray(session?.versions) ? session.versions : []).forEach(addVersion);
  (versionDocs || []).forEach((doc) => addVersion(doc.data ? doc.data() : doc));
  return Array.from(new Set(values));
}

async function studioUnsharedDeletePaths(uid, sourceSessionId, paths, { skipSourceDoc = false } = {}) {
  const candidates = new Set((paths || []).map(String).filter(Boolean));
  if (!candidates.size) return [];
  const shared = new Set();
  const scan = (value, seen = new Set()) => {
    if (shared.size === candidates.size || value == null) return;
    if (typeof value === "string") {
      if (candidates.has(value)) shared.add(value);
      else for (const path of candidates) {
        if (value.includes(path) || value.includes(encodeURIComponent(path))) shared.add(path);
      }
      return;
    }
    if (typeof value !== "object" || seen.has(value)) return;
    seen.add(value);
    if (Array.isArray(value)) value.forEach((item) => scan(item, seen));
    else Object.values(value).forEach((item) => scan(item, seen));
  };
  try {
    const sessions = await getDb().collection(STUDIO_SESSIONS_COLL).where("uid", "==", uid).get();
    for (const doc of sessions.docs) {
      if (skipSourceDoc && doc.id === sourceSessionId) continue;
      const data = doc.data() || {};
      if (data.deleted === true) continue;
      scan(data);
      if (shared.size === candidates.size) break;
    }
    return Array.from(candidates).filter((path) => !shared.has(path));
  } catch (err) {
    /* Reference discovery is a safety check. If it is unavailable, retain
       the bytes rather than break an image layer another saved design owns. */
    console.error("[studio] shared asset scan failed; retaining candidate paths:", err?.message || err);
    return [];
  }
}

async function studioDeleteStoragePaths(paths) {
  const bucket = getBucket();
  for (let start = 0; start < paths.length; start += 12) {
    const slice = paths.slice(start, start + 12);
    await Promise.all(slice.map((p) => bucket.file(p).delete({ ignoreNotFound: true })));
  }
}

async function studioDeleteVersionDocs(db, docs) {
...
async function studioReconcileDeleteBlockers(sRef, uid, sessionId) {
  const snapshot = await sRef.collection("versions").get();
  const jobs = [];
  const now = Date.now();
  for (const doc of snapshot.docs) {
    const v = doc.data() || {};
    if ((["queued", "generating"].includes(String(v.status || "")) && Number(v.generationLeaseUntil || 0) < now) ||
        v.generationRefundPending === true) {
      jobs.push(() => studioReconcileStaleGeneration(doc.ref, uid, sessionId));
    }
    if ((String(v.renderStatus || "") === "rendering" && Number(v.renderLeaseUntil || 0) < now) ||
        v.renderRefundPending === true) {
      jobs.push(() => studioReconcileStaleRender(doc.ref, uid, sessionId));
    }
  }
  for (let start = 0; start < jobs.length; start += 5) {
    await Promise.all(jobs.slice(start, start + 5).map((job) => job().catch(() => false)));
  }
}

async function studioFinalizePendingSessionDelete(sRef, uid, sessionId) {
  const db = getDb(), now = Date.now();
  const versions = await sRef.collection("versions").get();
  const blockers = versions.docs.filter((doc) => studioVersionBlocksDelete(doc.data() || {}));
  if (blockers.length) return { state: "pending", blockers: blockers.length };

  const deleteToken = studioSha256(`${uid}:${sessionId}:delete`).slice(0, 40);
  const claim = await db.runTransaction(async (tx) => {
    const snap = await tx.get(sRef), s = snap.exists ? snap.data() : {};
    if (!snap.exists || s.uid !== uid) return { state: "forbidden" };
    if (s.deleted === true) {
      if (["pending", "failed"].includes(String(s.deleteCleanupState || ""))) {
        return { state: "cleanup", session: s };
      }
      return { state: "deleted" };
    }
    if (s.deletePending !== true) return { state: "not_pending" };
    if (s.status === "ordered") {
      tx.set(sRef, { deletePending: false, deleteFinalizeState: "cancelled_ordered",
        deleteFinalizeLeaseUntil: 0, updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
      return { state: "ordered" };
    }
    if (s.deleteFinalizeState === "deleting" && String(s.deleteFinalizeToken || "") !== deleteToken &&
        Number(s.deleteFinalizeLeaseUntil || 0) > now) return { state: "existing" };
    tx.set(sRef, { deleteFinalizeState: "deleting", deleteFinalizeToken: deleteToken,
      deleteFinalizeLeaseUntil: now + 2 * 60 * 1000,
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    return { state: "claimed", session: s };
  });
  if (!['claimed', 'cleanup'].includes(claim.state)) return claim;

  const paths = studioOwnedDeletePaths(uid, sessionId, claim.session, versions.docs);
  try {
    /* A version document retains only the latest rerender paths. Enumerating
       this session-owned prefix also removes older successful runs without
       touching shared library/reference files or immutable order proofs. */
    const designPrefix = `custom-studio/${uid}/uploads/designs/${sessionId}/`;
    const [storedFiles] = await getBucket().getFiles({ prefix: designPrefix });
    const allPaths = Array.from(new Set(paths.concat((storedFiles || []).map((file) => String(file.name || ""))
      .filter((path) => path.startsWith(designPrefix)))));
    const unsharedPaths = await studioUnsharedDeletePaths(uid, sessionId, allPaths, { skipSourceDoc: true });
    paths.splice(0, paths.length, ...unsharedPaths);
  } catch (err) {
    await sRef.set({ deleteFinalizeState: "failed", deleteFinalizeLeaseUntil: 0,
      deleteError: String(err?.message || err).slice(0, 200),
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(() => {});
    throw err;
  }

  let commitState = claim.state === "cleanup" ? "deleted" : "", commitError = null;
  if (claim.state === "claimed") {
    try {
      /* The tombstone wins its Firestore race with an order webhook BEFORE
         any bytes are removed. If ordered won first, nothing destructive has
         happened and the delete request is cancelled. */
      commitState = await db.runTransaction(async (tx) => {
        const snap = await tx.get(sRef), s = snap.exists ? snap.data() : {};
        ...
        if (!snap.exists || s.uid !== uid || s.deletePending !== true ||
            String(s.deleteFinalizeToken || "") !== deleteToken) return "superseded";
        /* Overwrite, do not merge: no stale asset URL or editable design state
           survives. Cleanup is retryable from the deterministic session
           prefix if Storage/Firestore deletion later has a transient error. */
        tx.set(sRef, { uid, deleted: true, deletePending: false, status: "deleted",
          name: "Deleted design", deletedAt: admin.firestore.FieldValue.serverTimestamp(),
          deleteCleanupState: "pending", deleteCleanupError: null,
          updatedAt: Date.now(), updatedAtServer: admin.firestore.FieldValue.serverTimestamp() });
        return "deleted";
      });
  ...
  if (!commitState) {
    try {
      const terminal = await sRef.get();
      if (terminal.exists && terminal.data()?.deleted === true) commitState = "deleted";
      else if (terminal.exists && terminal.data()?.status === "ordered") commitState = "ordered";
      else commitState = "pending";
    } catch (_e) {
      if (commitError) return { state: "ambiguous" };
      commitState = "pending";
    }
  }
  if (commitState === "ordered") return { state: "ordered" };
  if (commitState !== "deleted") return { state: "pending", blockers: 0 };
  try {
    await studioDeleteStoragePaths(paths);
    await studioDeleteVersionDocs(db, versions.docs);
    await sRef.set({ deleteCleanupState: "complete", deleteCleanupError: null,
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    return { state: "deleted", deletedPaths: paths.length, cleanupComplete: true };
  } catch (err) {
    await sRef.set({ deleteCleanupState: "failed",
      deleteCleanupError: String(err?.message || err).slice(0, 200),
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(() => {});
    return { state: "deleted", deletedPaths: 0, cleanupPending: true };
  }
}

async function handleStudioSessionDelete({ body, event, origin }) {
  const uid = await requireStudioUser(event), db = getDb();
  const sessionId = String(body?.sessionId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 60);
  if (!sessionId) return studioJson(400, { ok: false, error: "missing_session" }, origin);
  const sRef = db.collection(STUDIO_SESSIONS_COLL).doc(sessionId);
  const requested = await db.runTransaction(async (tx) => {
    const snap = await tx.get(sRef), s = snap.exists ? snap.data() : {};
    if (!snap.exists || s.uid !== uid) return { state: "forbidden" };
    if (s.deleted === true) return { state: ["pending", "failed"].includes(String(s.deleteCleanupState || ""))
      ? "cleanup" : "deleted" };
    if (s.status === "ordered") return { state: "ordered" };
    if (s.deletePending === true) return { state: "pending" };
    tx.set(sRef, { deletePending: true, deleteRequestedAt: admin.firestore.FieldValue.serverTimestamp(),
      approved: false, approvedRenderRunId: null, approvedVersionNumber: null, approvedMetal: null,
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    return { state: "pending" };
  });
  if (requested.state === "forbidden") return studioJson(403, { ok: false, error: "forbidden" }, origin);
  if (requested.state === "ordered") return studioJson(409, { ok: false, error: "ordered_design_locked" }, origin);
  if (requested.state === "deleted") return studioJson(200, { ok: true, deleted: true, idempotent: true }, origin);

  if (requested.state !== "cleanup") {
    await studioReconcileDeleteBlockers(sRef, uid, sessionId).catch((err) => {
      console.error("[studio] delete reconciliation failed:", err?.message || err);
    });
  }
  const result = await studioFinalizePendingSessionDelete(sRef, uid, sessionId);
  if (result.state === "deleted") return studioJson(200, { ok: true, deleted: true, ...result }, origin);
  if (result.state === "ambiguous") return studioJson(202, { ok: true, deletePending: true, ambiguous: true }, origin);
  if (result.state === "forbidden") return studioJson(403, { ok: false, error: "forbidden" }, origin);
  if (result.state === "ordered") return studioJson(409, { ok: false, error: "ordered_design_locked" }, origin);
  return studioJson(202, { ok: true, deletePending: true, blockers: result.blockers || 0 }, origin);
}

async function handleStudioVersionDelete({ body, event, origin }) {
  const uid = await requireStudioUser(event), db = getDb();
  const sessionId = String(body?.sessionId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 60);
  const n = Number(body?.versionNumber);
  if (!sessionId || !Number.isInteger(n) || n < 1 || n > 400) {
    return studioJson(400, { ok: false, error: "bad_version_delete" }, origin);
  }
  const sRef = db.collection(STUDIO_SESSIONS_COLL).doc(sessionId);
  const vRef = sRef.collection("versions").doc(String(n));
  const deleteToken = studioSha256(`${uid}:${sessionId}:v${n}:delete`).slice(0, 40);
  const claimed = await db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef), vSnap = await tx.get(vRef);
    const s = sSnap.exists ? sSnap.data() : {}, v = vSnap.exists ? vSnap.data() : {};
    if (!sSnap.exists || s.uid !== uid || s.deleted === true || s.deletePending === true) return { state: "forbidden" };
    if (s.status === "ordered") return { state: "ordered" };
    const parentVersion = (Array.isArray(s.versions) ? s.versions : []).find((item) => Number(item?.n) === n) || null;
    const versions = (Array.isArray(s.versions) ? s.versions : []).filter((item) => Number(item?.n) !== n);
    const clearsApproval = Number(s.approvedVersionNumber) === n;
    const parentPatch = { versions,
      deletedVersionNumbers: admin.firestore.FieldValue.arrayUnion(n),
      markups: { [String(n)]: admin.firestore.FieldValue.delete() },
      mkHistory: { [String(n)]: admin.firestore.FieldValue.delete() },
      currentVersion: Math.min(Number(s.currentVersion) || 0, Math.max(0, versions.length - 1)),
      nextVersionNumber: Math.max(Number(s.nextVersionNumber) || 1, n + 1),
      ...(clearsApproval ? { approved: false, approvedRenderRunId: null,
        approvedVersionNumber: null, approvedMetal: null, status: "draft", maxStep: Math.min(2, Number(s.maxStep) || 2) } : {}),
      updatedAtServer: admin.firestore.FieldValue.serverTimestamp() };
    if (!vSnap.exists || v.deleted === true) {
      /* Keep a compact server tombstone. Without it, a delayed Kick worker
         accepted just before this request could recreate and charge a
         version that the delete endpoint already reported as gone. */
      tx.set(vRef, { deleted: true, deletePending: true, deleteToken,
        deletedAt: admin.firestore.FieldValue.serverTimestamp() });
      tx.set(sRef, parentPatch, { merge: true });
      return { state: "deleted", version: parentVersion || v };
    }
    if (studioVersionBlocksDelete(v)) return { state: "busy" };
    if (v.deletePending === true && String(v.deleteToken || "") !== deleteToken) return { state: "busy" };
    tx.set(vRef, { deletePending: true, deleteToken,
      deleteRequestedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    tx.set(sRef, parentPatch, { merge: true });
    return { state: "claimed", version: v };
  });
  if (claimed.state === "forbidden") return studioJson(403, { ok: false, error: "forbidden" }, origin);
  if (claimed.state === "ordered") return studioJson(409, { ok: false, error: "ordered_design_locked" }, origin);
  if (claimed.state === "busy") return studioJson(409, { ok: false, error: "version_operation_in_progress" }, origin);
  if (claimed.state === "deleted") {
    const candidates = studioOwnedDeletePaths(uid, sessionId, {}, [claimed.version].filter(Boolean));
    const paths = await studioUnsharedDeletePaths(uid, sessionId, candidates);
    if (paths.length) await studioDeleteStoragePaths(paths);
    return studioJson(200, { ok: true, deleted: true, idempotent: true }, origin);
  }
  const candidates = studioOwnedDeletePaths(uid, sessionId, {}, [claimed.version]);
  const paths = await studioUnsharedDeletePaths(uid, sessionId, candidates);
  await studioDeleteStoragePaths(paths);
  let deleted = false, deleteError = null;
  try {
    deleted = await db.runTransaction(async (tx) => {
      const snap = await tx.get(vRef), v = snap.exists ? snap.data() : {};
      if (!snap.exists) return true;
      if (v.deletePending !== true || String(v.deleteToken || "") !== deleteToken) return false;
      tx.set(vRef, { deleted: true, deletePending: true, deleteToken,
        deletedAt: admin.firestore.FieldValue.serverTimestamp() });
      return true;
    });
  } catch (err) { deleteError = err; }
  if (deleted) return studioJson(200, { ok: true, deleted: true }, origin);
  try {
    const terminal = await vRef.get();
    if (!terminal.exists) return studioJson(200, { ok: true, deleted: true, idempotent: true }, origin);
    return studioJson(409, { ok: false, error: "version_delete_superseded" }, origin);
  } catch (_e) {
    if (deleteError) return studioJson(202, { ok: true, deletePending: true, ambiguous: true }, origin);
    return studioJson(409, { ok: false, error: "version_delete_superseded" }, origin);
  }
}

const _studioProductCache = new Map();
async function studioStorefrontProduct(handle, { fresh = false } = {}) {
  const key = String(handle || "").replace(/[^a-z0-9-]/gi, "");
  if (!key) throw new Error("bad_product_handle");
  const cached = _studioProductCache.get(key);
  if (!fresh && cached && cached.expiresAt > Date.now()) return cached.data;
  const configured = String(process.env.STUDIO_STOREFRONT_BASE || "https://britesjewelry.com").replace(/\/+$/, "");
  if (!/^https:\/\/[A-Za-z0-9.-]+(?::\d+)?$/.test(configured)) throw new Error("bad_storefront_base");
  const resp = await fetchWithTimeout(`${configured}/products/${key}.js`, {
    method: "GET", headers: { Accept: "application/json", "Cache-Control": "no-cache" },
  }, 10000);
  const raw = await resp.text();
  if (!resp.ok) throw new Error(`storefront_product_${resp.status}`);
  const data = JSON.parse(raw);
  if (!data || !Array.isArray(data.variants)) throw new Error("storefront_product_invalid");
  _studioProductCache.set(key, { data, expiresAt: Date.now() + 60000 });
  return data;
}

async function studioVerifyCommerceManifest(commerce) {
  const charmHandles = {
    charm: "custom-charm-only", huggie: "custom-charm-huggie",
    chain: "custom-charm-chain", beady: "custom-charm-beady", studs: "custom-charm-studs",
  };
  const requests = [
    { handle: charmHandles[commerce.format], id: commerce.charmVariantId,
      cents: commerce.charmUnitCents,
      options: [({ silver: "Sterling Silver", gold: "14k Gold Filled", rose: "14k Rose Gold Filled",
        solid10: "10k Solid Gold", solid14: "14k Solid Gold" })[commerce.metal],
        commerce.sizeMm == null ? null : `${commerce.sizeMm} mm`].filter(Boolean) },
    { handle: "custom-design-fee", id: commerce.designFeeVariantId, cents: commerce.designFeeCents, options: [], defaultOnly: true },
  ];
  if (commerce.engravingVariantId) requests.push({ handle: "custom-charm-engraving",
    id: commerce.engravingVariantId, cents: commerce.engravingUnitCents, options: [], defaultOnly: true });
  if (commerce.heartVariantId) requests.push({ handle: "chain-extender-tiny-heart",
    id: commerce.heartVariantId, cents: commerce.heartUnitCents, options: [], defaultOnly: true });
  /* A proof claim is a committing price/availability decision. Never accept
     the preview cache: Shopify may have changed the variant seconds ago. */
  const products = await Promise.all(requests.map((r) => studioStorefrontProduct(r.handle, { fresh: true })));
  requests.forEach((request, index) => {
    const product = products[index];
    const variant = product.variants.find((v) => String(v.id) === String(request.id));
    if (!variant || variant.available !== true || Number(variant.price) !== Number(request.cents)) {
      throw new Error("commerce_variant_unavailable_or_changed");
    }
    const options = (variant.options || [variant.option1, variant.option2, variant.option3])
      .filter((value) => value != null && String(value).trim() !== "")
      .map((value) => String(value).trim().toLowerCase());
    const expectedOptions = request.options.map((value) => String(value).trim().toLowerCase());
    const defaultOptionVector = options.length === 0 ||
      (options.length === 1 && options[0] === "default title");
    if (request.defaultOnly && (!defaultOptionVector || product.variants.filter((v) => v.available === true).length !== 1)) {
      throw new Error("commerce_default_variant_ambiguous");
    }
    /* Shopify option order is part of the variant identity. `includes` let a
       forged manifest append or permute options while still being described
       as server-validated. Default-title add-ons intentionally have no
       expected vector and are validated by exact variant id/price instead. */
    if (expectedOptions.length && (options.length !== expectedOptions.length ||
        expectedOptions.some((expected, optionIndex) => options[optionIndex] !== expected))) {
      throw new Error("commerce_variant_options_mismatch");
    }
  });
}

/* An approved proof is copied into an immutable, run-bound location before
   Shopify receives it. Cart line properties are user-controlled and design
   assets can be deleted or replaced in another tab; neither is a safe
   fulfillment record. The opaque claim ID points to this server-validated
   snapshot, while the customer-facing URLs also come from that snapshot. */
async function handleStudioProofClaim({ body, event, origin }) {
  const uid = await requireStudioUser(event);
  const db = getDb();
  const sessionId = String(body?.sessionId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 60);
  const n = Number(body?.versionNumber);
  const renderRunId = String(body?.renderRunId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 80);
  const metal = String(body?.metal || "").toLowerCase();
  if (!sessionId || !Number.isInteger(n) || n < 1 || !renderRunId ||
      !["silver", "gold", "rose", "solid10", "solid14"].includes(metal)) {
    return studioJson(400, { ok: false, error: "bad_proof_claim" }, origin);
  }
  const rawCommerce = body?.commerce || {};
  const variantId = (value, required) => {
    const id = value == null ? "" : String(value);
    if (!id && !required) return null;
    return /^\d{1,24}$/.test(id) ? id : "";
  };
  const cents = (value) => Number.isInteger(Number(value)) && Number(value) >= 0 && Number(value) <= 10000000
    ? Number(value) : null;
  const quantity = Number(rawCommerce.quantity);
  const commerce = {
    metal,
    format: ["charm", "huggie", "chain", "beady", "studs"].includes(String(rawCommerce.format))
      ? String(rawCommerce.format) : "",
    sizeMm: rawCommerce.sizeMm == null ? null : Number(rawCommerce.sizeMm),
    quantity,
    charmVariantId: variantId(rawCommerce.charmVariantId, true),
    charmUnitCents: cents(rawCommerce.charmUnitCents),
    designFeeVariantId: variantId(rawCommerce.designFeeVariantId, true),
    designFeeCents: cents(rawCommerce.designFeeCents),
    engravingVariantId: variantId(rawCommerce.engravingVariantId, false),
    engravingUnitCents: cents(rawCommerce.engravingUnitCents),
    engravingText: String(rawCommerce.engravingText || "").replace(/[\u0000-\u001f\u007f]/g, " ").trim().slice(0, 100),
    chainExtender: rawCommerce.chainExtender === true,
    heartVariantId: variantId(rawCommerce.heartVariantId, false),
    heartUnitCents: cents(rawCommerce.heartUnitCents),
  };
  const sizedFormat = ["charm", "chain", "beady"].includes(commerce.format);
  const engravableFormat = ["charm", "chain", "beady"].includes(commerce.format);
  const extenderFormat = ["chain", "beady"].includes(commerce.format);
  const hasEngraving = !!commerce.engravingVariantId || !!commerce.engravingText ||
    Number(commerce.engravingUnitCents || 0) !== 0;
  const hasExtender = commerce.chainExtender || !!commerce.heartVariantId ||
    Number(commerce.heartUnitCents || 0) !== 0;
  if (!commerce.format || !Number.isInteger(quantity) || quantity < 1 || quantity > 9 ||
      !commerce.charmVariantId || commerce.charmUnitCents == null ||
      !commerce.designFeeVariantId || commerce.designFeeCents == null ||
      (sizedFormat && (!Number.isFinite(commerce.sizeMm) || commerce.sizeMm < 5 || commerce.sizeMm > 50)) ||
      (!sizedFormat && commerce.sizeMm != null) ||
      (!engravableFormat && hasEngraving) ||
      (!extenderFormat && hasExtender) ||
      (commerce.engravingVariantId && !commerce.engravingText) ||
      (commerce.engravingVariantId && commerce.engravingUnitCents == null) ||
      (!commerce.engravingVariantId && (commerce.engravingText || commerce.engravingUnitCents !== 0)) ||
      (commerce.heartVariantId && (!commerce.chainExtender || commerce.heartUnitCents == null)) ||
      (!commerce.heartVariantId && commerce.heartUnitCents !== 0)) {
    return studioJson(400, { ok: false, error: "bad_commerce_manifest" }, origin);
  }
  try {
    await studioVerifyCommerceManifest(commerce);
  } catch (err) {
    console.error("[studio] commerce verification failed:", err?.message || err);
    return studioJson(409, { ok: false, error: "commerce_unavailable_or_changed" }, origin);
  }
  const proofBundleSHA256 = studioSha256(Buffer.from(JSON.stringify(commerce)));

  const sRef = db.collection(STUDIO_SESSIONS_COLL).doc(sessionId);
  const vRef = sRef.collection("versions").doc(String(n));
  const claimId = `pc_${studioSha256(`${uid}:${sessionId}:${n}:${renderRunId}:${metal}:${proofBundleSHA256}`).slice(0, 48)}`;
  const claimRef = db.collection("customProofClaims").doc(claimId);
  const now = Date.now(), leaseMs = 2 * 60 * 1000;
  const copyAttemptToken = studioSha256(`${claimId}:${now}:${Math.random()}`).slice(0, 32);
  const claimed = await db.runTransaction(async (tx) => {
    const sSnap = await tx.get(sRef);
    const vSnap = await tx.get(vRef);
    ...
    const s = sSnap.exists ? sSnap.data() : {};
    const v = vSnap.exists ? vSnap.data() : {};
    if (!sSnap.exists || s.uid !== uid || s.deleted === true || s.deletePending === true ||
        !vSnap.exists || v.deleted === true || v.deletePending === true) {
      return { state: "forbidden" };
    }
    if (s.approved !== true || String(s.approvedRenderRunId || "") !== renderRunId ||
        Number(s.approvedVersionNumber) !== n || String(s.approvedMetal || "") !== metal ||
        v.status !== "done" || v.renderStatus !== "done" ||
        String(v.renderRunId || "") !== renderRunId || String(v.renderMetal || "") !== metal) {
      return { state: "proof_not_approved" };
    }
    if (!v.storagePath || !v.downloadURL || !v.renderPath || !v.renderURL || !v.renderSHA256 ||
        !v.renderSpecPath || !v.renderSpecURL || !v.renderSpecSHA256 ||
        !v.drawingSHA256 || String(v.drawingSHA256) !== String(v.renderDrawingSHA256 || "")) {
      return { state: "legacy_proof_requires_rerender" };
    }
    const c = cSnap.exists ? cSnap.data() : {};
    if (c.status === "ready") return { state: "ready", data: c };
    if (c.status === "copying" && Number(c.leaseUntil || 0) > now) return { state: "copying" };
    const sourcePrefix = `custom-studio/${uid}/uploads/designs/${sessionId}/`;
    const sourcePaths = [v.storagePath, v.renderPath, v.renderSpecPath].filter(Boolean).map(String);
    if (sourcePaths.some((p) => !p.startsWith(sourcePrefix))) return { state: "invalid_source" };
    tx.set(claimRef, {
      claimId, status: "copying", leaseUntil: now + leaseMs,
      copyAttemptToken,
      uid, sessionId, versionNumber: n, renderRunId, metal,
      commerce, proofBundleSHA256,
      sourceDrawingPath: String(v.storagePath), sourceRenderPath: String(v.renderPath),
      sourceSpecPath: v.renderSpecPath ? String(v.renderSpecPath) : null,
      createdAt: c.createdAt || admin.firestore.FieldValue.serverTimestamp(),
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    }, { merge: true });
    return { state: "claimed", data: v, isNew: !cSnap.exists };
  });

  if (claimed.state === "ready") return studioJson(200, { ok: true, idempotent: true,
    proofClaimId: claimId, ...claimed.data }, origin);
  if (claimed.state === "forbidden") return studioJson(403, { ok: false, error: "forbidden" }, origin);
  if (claimed.state !== "claimed") return studioJson(409, { ok: false,
    error: claimed.state === "copying" ? "proof_claim_in_progress" : claimed.state }, origin);
  /* Charge the abuse budget only after ownership, approval and the
     deterministic claim race have all succeeded. Invalid requests and the
     losing half of a concurrent idempotent pair remain free. */
  if (claimed.isNew &&
      (!(await studioBudgetOk("proof", uid, STUDIO_MAX_PROOF_CLAIMS_HOUR_UID)) ||
       !(await studioBudgetOk("proofip", studioClientIp(event), STUDIO_MAX_PROOF_CLAIMS_HOUR_IP)))) {
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(claimRef), current = snap.exists ? snap.data() : {};
      if (current.status === "copying" && String(current.copyAttemptToken || "") === copyAttemptToken) {
        tx.delete(claimRef);
      }
    }).catch(() => {});
    return studioJson(429, { ok: false, error: "proof_claim_rate_limited" }, origin);
  }

  const v = claimed.data || {};
  /* Keep `uploads` in customer-facing proof URLs: both existing cart
     templates use that path segment plus the image extension to render a
     thumbnail instead of printing a long URL. */
  const prefix = `custom-studio/${uid}/uploads/order-proofs/${claimId}/${copyAttemptToken}`;
  const drawingPath = `${prefix}/drawing.png`;
  const renderPath = `${prefix}/charm.png`;
  const specPath = v.renderSpecPath ? `${prefix}/material-spec.png` : "";
  const written = [];
  try {
    const bucket = getBucket();
    const copyOne = async (sourcePath, targetPath, role, expectedSHA256) => {
      const [bytes] = await bucket.file(String(sourcePath)).download();
      if (!expectedSHA256 || studioSha256(bytes) !== String(expectedSHA256)) {
        throw new Error(`${role}_source_hash_mismatch`);
      }
      const target = bucket.file(targetPath);
      const token = newDownloadToken();
      /* Register the target before the write. If Storage commits but its
         acknowledgement is lost, cleanup still knows the exact object. */
      written.push(targetPath);
      await target.save(bytes, {
        resumable: false,
        contentType: "image/png",
        metadata: { metadata: { firebaseStorageDownloadTokens: token, uid, sessionId,
          version: String(n), renderRunId, proofClaimId: claimId, role } },
      });
      return tokenDownloadURLFor(bucket.name, targetPath, token);
    };
    const drawingURL = await copyOne(v.storagePath, drawingPath, "drawing", v.drawingSHA256);
    const renderURL = await copyOne(v.renderPath, renderPath, "charm", v.renderSHA256);
    const specURL = specPath ? await copyOne(v.renderSpecPath, specPath, "material-spec", v.renderSpecSHA256) : "";
    const auditSHA256 = studioSha256(Buffer.from(JSON.stringify({
      claimId, uid, sessionId, n, renderRunId, metal, drawingPath, renderPath, specPath,
    })));
    const ready = {
      status: "ready", leaseUntil: 0, proofClaimId: claimId, proofAuditSHA256: auditSHA256,
      copyAttemptToken,
      uid, sessionId, versionNumber: n, renderRunId, metal,
      commerce, proofBundleSHA256,
      drawingURL, drawingPath, renderURL, renderPath,
      specURL: specURL || null, specPath: specPath || null,
      readyAt: admin.firestore.FieldValue.serverTimestamp(),
      updatedAt: admin.firestore.FieldValue.serverTimestamp(),
    };
    /* A transaction acknowledgement may fail after Firestore committed it.
       Never delete the immutable proof copies on that ambiguous path: first
       read the claim and let the durable `ready` record own the outcome. */
    let finalized = false, finalizeError = null;
    try {
      finalized = await db.runTransaction(async (tx) => {
        const [snap, sessionSnap, versionSnap] = await Promise.all([
          tx.get(claimRef), tx.get(sRef), tx.get(vRef),
        ]);
        const current = snap.exists ? snap.data() : {};
        const currentSession = sessionSnap.exists ? sessionSnap.data() : {};
        const currentVersion = versionSnap.exists ? versionSnap.data() : {};
        if (current.status !== "copying" || String(current.copyAttemptToken || "") !== copyAttemptToken) {
          return false;
        }
        const stillApproved = sessionSnap.exists && versionSnap.exists &&
          currentSession.uid === uid && currentSession.deleted !== true && currentSession.deletePending !== true &&
          currentSession.approved === true && Number(currentSession.approvedVersionNumber) === n &&
          String(currentSession.approvedRenderRunId || "") === renderRunId &&
          String(currentSession.approvedMetal || "") === metal && currentVersion.deletePending !== true &&
          currentVersion.status === "done" && currentVersion.renderStatus === "done" &&
          String(currentVersion.renderRunId || "") === renderRunId &&
          String(currentVersion.renderMetal || "") === metal;
        if (!stillApproved) {
          tx.set(claimRef, { status: "failed", leaseUntil: 0, error: "proof_revoked_during_copy",
            updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
          return false;
        }
        tx.set(claimRef, ready, { merge: true });
        return true;
      });
    } catch (err) {
      finalizeError = err;
    ...
    if (!finalized) {
      let data = {}, terminalReadSucceeded = false;
      try {
        const current = await claimRef.get();
        data = current.exists ? current.data() : {};
        terminalReadSucceeded = true;
      } catch (_e) {}
      if (terminalReadSucceeded && data.status === "ready" &&
          String(data.copyAttemptToken || "") === copyAttemptToken) {
        return studioJson(200, { ok: true, idempotent: true,
          proofClaimId: claimId, ...data }, origin);
      }
      if (!terminalReadSucceeded && finalizeError) {
        console.error("[studio] proof finalization acknowledgement ambiguous; claim status owns outcome");
        return studioJson(202, { ok: true, queued: true, ambiguous: true,
          proofClaimId: claimId, copyAttemptToken }, origin);
      }
      /* A successful terminal read proved that our attempt is not the ready
         owner. Its run-scoped copies are therefore unreachable and safe to
         remove; a different ready attempt, if present, has different paths. */
      await Promise.all(written.map(async (path) => {
        try { await getBucket().file(path).delete({ ignoreNotFound: true }); } catch (_e) {}
      }));
      if (data.status === "ready") return studioJson(200, { ok: true, idempotent: true,
        proofClaimId: claimId, ...data }, origin);
      return studioJson(409, { ok: false, error: "proof_claim_superseded" }, origin);
    }
    return studioJson(200, { ok: true, ...ready, readyAt: Date.now(), updatedAt: Date.now() }, origin);
  } catch (err) {
    await Promise.all(written.map(async (path) => {
      try { await getBucket().file(path).delete({ ignoreNotFound: true }); } catch (_e) {}
    }));
    await db.runTransaction(async (tx) => {
      const snap = await tx.get(claimRef), current = snap.exists ? snap.data() : {};
      if (current.status !== "copying" || String(current.copyAttemptToken || "") !== copyAttemptToken) return;
      tx.set(claimRef, { status: "failed", leaseUntil: 0,
        error: String(err?.message || err).slice(0, 200),
        updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
    }).catch(() => {});
    console.error("[studio] proof claim failed:", err?.message || err);
    return studioJson(500, { ok: false, error: "proof_claim_failed" }, origin);
  }
}
...
async function handleStudioCompose({ body, event, origin }) {
  ...
  const n = Number(body?.versionNumber);
  const composeRunId = String(body?.composeRunId || "").replace(/[^A-Za-z0-9_-]/g, "").slice(0, 80);
  if (!Number.isInteger(n) || n < 1 || n > 400 || !composeRunId) {
    return studioJson(400, { ok: false, error: "bad_compose_request" }, origin);
  }
  const vRef = sRef.collection("versions").doc(String(n));
  ...
  const claim = await studioClaimGeneration(sRef, vRef, uid, n, composeRunId, {
    composed: true, generationCost: 0,
    generationOperationId: `custom_charm_compose:${sessionId}:v${n}:${composeRunId}`,
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
    /* A new drawing can never inherit a prior proof. */
    renderStatus: null, renderStage: null, renderURL: null, renderPath: null,
    renderSpecURL: null, renderSpecPath: null, renderRunId: null, renderMetal: null,
  });
  if (claim.state === "done") return studioJson(200, { ok: true, idempotent: true, n,
    storagePath: claim.data?.storagePath, downloadURL: claim.data?.downloadURL,
    composeRunId }, origin);
  if (claim.state === "existing") return studioJson(202, { ok: true, queued: true,
    idempotent: true, n, composeRunId }, origin);
  if (claim.state !== "claimed") {
    const error = claim.state === "busy" ? "generation_in_progress" : claim.state;
    await studioRecordGenerationRejection(sRef, vRef, uid, composeRunId, error).catch(() => {});
    return studioJson(409, { ok: false, error, n, composeRunId }, origin);
  }

  if (!(await studioBudgetOk("gen", uid, STUDIO_MAX_GENS_HOUR_UID)) ||
      !(await studioBudgetOk("genip", studioClientIp(event), STUDIO_MAX_GENS_HOUR_IP))) {
    await studioFailGeneration(sRef, vRef, n, composeRunId, "rate_limited");
    return studioJson(429, { ok: false, error: "rate_limited", composeRunId }, origin);
  }

  let publishedStoragePath = "";
  try {
    if (!(await studioGenerationPatch(vRef, composeRunId, { status: "generating", stage: "generating" }))) {
      throw new Error("compose_superseded");
    }
    const liveSession = await sRef.get();
    if (!liveSession.exists || liveSession.data()?.deleted === true) throw new Error("session_deleted");
    const runSlug = `${composeRunId.slice(0, 40)}-${studioSha256(composeRunId).slice(0, 16)}`;
    const storagePath = `custom-studio/${uid}/uploads/designs/${sessionId}/v${n}-${runSlug}.png`;
    publishedStoragePath = storagePath;
    const bucket = getBucket();
    const token = newDownloadToken();
    await bucket.file(storagePath).save(img.buffer, {
      ...
      contentType: "image/png",
      metadata: { metadata: { firebaseStorageDownloadTokens: token, uid, sessionId,
                              version: String(n), composed: "1", composeRunId } },
    });
    const downloadURL = tokenDownloadURLFor(bucket.name, storagePath, token);
    const drawingSHA256 = studioSha256(img.buffer);
    let finalized;
    try {
      finalized = await studioFinalizeGenerationForLiveSession(sRef, vRef, n, composeRunId, {
        status: "done", stage: "done", storagePath, downloadURL,
        composed: true, drawingSHA256, model: "studio-composer",
        prompt: "Composed locally from the customer's own vector items — exactly as drawn.",
        generationDebitPending: false, generationDebited: false, generationRefundPending: false,
        doneAt: admin.firestore.FieldValue.serverTimestamp(),
      });
    } catch (e) {
      e.finalizeAmbiguous = true;
    ...
    if (!finalized) throw new Error("compose_superseded_before_publish");
    return studioJson(200, { ok: true, n, storagePath, downloadURL, composeRunId }, origin);
  } catch (err) {
    let terminalReadSucceeded = false;
    try {
      const terminal = await vRef.get(), d = terminal.exists ? terminal.data() : {};
      terminalReadSucceeded = true;
      if (String(d.generationRunId || "") === composeRunId && d.status === "done" && d.downloadURL) {
        return studioJson(200, { ok: true, idempotent: true, n,
          storagePath: d.storagePath, downloadURL: d.downloadURL, composeRunId }, origin);
      }
    } catch (_e) {}
    if (err?.finalizeAmbiguous === true && !terminalReadSucceeded) {
      console.error("[studio] compose finalization acknowledgement ambiguous; status owns outcome");
      return studioJson(202, { ok: true, queued: true, ambiguous: true, n, composeRunId }, origin);
    }
    if (publishedStoragePath) {
      try { await getBucket().file(publishedStoragePath).delete({ ignoreNotFound: true }); } catch (_e) {}
    }
    await studioFailGeneration(sRef, vRef, n, composeRunId, err?.message || err, {
      generationDebitPending: false, generationDebited: false, generationRefundPending: false,
    });
    await studioFinalizePendingSessionDelete(sRef, uid, sessionId).catch(() => {});
    console.error("[studio] compose file:", err?.message || err);
    return studioJson(500, { ok: false, error: "compose_failed", composeRunId }, origin);
  }
}
...
async function handleStudioKind({ kind, body, event }) {
  ...
  try {
    ...
    if (kind === "custom_proof_claim") return await handleStudioProofClaim({ body, event, origin });
    if (kind === "custom_session_delete") return await handleStudioSessionDelete({ body, event, origin });
    if (kind === "custom_version_delete") return await handleStudioVersionDelete({ body, event, origin });
