...
async function callGeminiGenerateContentImage({
  ...
  // run_set_async edits tasks, etc.) still outputs at 2K, and the
  // batch path's own JSONL builder is still locked to 2K, so every
  // image-producing surface that does not ask for something else
  // is unchanged.
  //
  // The tier used to be a hard constant. It is now a DEFAULT with
  // an explicit opt-out, because the Custom Charm Studio needs 1K:
  // its render input is a flat deterministic material proof, not a
  // photograph, so the extra tier buys nothing and costs latency
  // on every press. A caller that passes nothing gets 2K exactly
  // as before; only an explicit imageSizeTier moves it, and an
  // unrecognised value falls back to 2K rather than to whatever
...
const STUDIO_DEFAULT_CONFIG = {
  ...
  /* ── ONE MODEL, ONE TIER, FOR EVERY STUDIO IMAGE ──────────────────────
     Both studio steps — the production drawing and the metal render — run on
     the same model and the same size tier, set here rather than in two places
     in code. gemini-3-pro-image is the studio default; the 1K tier is
     deliberate rather than a saving: the render's input is a flat
     deterministic material proof, and 2K bought latency on every press
     without buying accuracy. Values are "1K" | "2K" | "4K"; anything else
     falls back to 2K inside the image call. No new environment variable —
     GEMINI_STUDIO_IMAGE_MODEL is still read if it is already set, below this
     key so Firestore stays the faster lever. */
  studioImageModel: "gemini-3-pro-image",
  /* ── 2K IN, 1K OUT ────────────────────────────────────────────────────
     The two studio steps are not the same job, so they do not get the same
     tier.

     studioDrawingTier is the INPUT side. The production drawing is what the
     renderer is later handed, and it is also what code measures: studioPenWidth
     and the 2.5×-pen test that separates a cut-out from a cut edge both read
     the pen off the drawing itself, so halving the canvas halves the pen and
     leaves a hairline with fewer pixels to be judged on. It stays at 2K.
     ...
     presentation only, at real latency on every press. It renders at 1K from
     the 2K input, which is the whole point: full-resolution instructions, a
     lighter picture.

     Values are "1K" | "2K" | "4K"; anything else falls back to 2K inside the
     image call, so a typo can never quietly drop quality. No new environment
     variable — GEMINI_STUDIO_IMAGE_MODEL is still read if it is already set,
     below studioImageModel so Firestore stays the faster lever. */
...
async function handleStudioRender({ body, event, origin }) {
  ...
  try {
    ...
    /* ── THE GOLD MASK IS EVIDENCE, SO IT IS KEPT ─────────────────────────
       This proof is the last instruction the model receives and the only
       image in the chain that states, unambiguously and without colour
       vocabulary, what is metal, what is worked and what is not there at all.
       It was being built, sent and thrown away — so when a render came back
       wrong there was no way to tell a bad proof from a bad render, which are
