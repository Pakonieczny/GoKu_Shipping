...
async function reviewCreativeApproval(id, hash) {
  ...
  await fb().db.runTransaction(async tx=>{const s=await tx.get(ref);if(!s.exists)throw new Error("Draft not found.");const it=s.data(),c=it.creative||{};
    ...
    if(!(c.groups||[]).length||c.groups.some(g=>!g.review||g.review.pass!==true&&!g.review.operatorReviewRequired))throw new Error("Every group must pass its quality review.");
    const reviewedGroups=c.groups.map(g=>g.review.operatorReviewRequired?{...g,review:{...g.review,pass:true,operatorReviewedAt:Date.now(),reviewedBy:"authenticated operator"}}:g);
    tx.update(ref,{"creative.groups":reviewedGroups,"creative.review":{payloadHash:hash,assetHash:creativeHash({groups:c.groups.map(g=>g.placementAssets?{assets:g.assets||{},placementAssets:g.placementAssets}:g.assets||{}),logo:c.logo||null}),at:Date.now(),by:"authenticated operator"}});
  });return {ok:true,id};
}
...
async function deleteAdDesignGeneratedImage(input){return _designEngine().deleteGeneratedImage(input);}
...
async function _verifyPublishedAd(campaignId,group,product,approval){
  const ref=_gaqlString(group.ref),[groups,links,signals,filters]=await Promise.all([
    gaql(`SELECT campaign.id, campaign.status, campaign.shopping_setting.merchant_id, campaign.shopping_setting.feed_label, asset_group.resource_name, asset_group.final_urls, asset_group.final_mobile_urls, asset_group.status, asset_group.primary_status FROM asset_group WHERE asset_group.resource_name = ${ref} AND campaign.id = ${campaignId}`),
    gaql(`SELECT asset_group_asset.asset, asset_group_asset.field_type, asset_group_asset.status, asset_group_asset.primary_status, asset_group_asset.policy_summary.approval_status, asset_group_asset.policy_summary.review_status, asset.resource_name, asset.text_asset.text, asset.call_to_action_asset.call_to_action, asset.image_asset.full_size.width_pixels, asset.image_asset.full_size.height_pixels FROM asset_group_asset WHERE asset_group_asset.asset_group = ${ref} AND asset_group_asset.status != 'REMOVED'`),
    gaql(`SELECT asset_group_signal.search_theme.text FROM asset_group_signal WHERE asset_group_signal.asset_group = ${ref}`),
    gaql(`SELECT asset_group_listing_group_filter.type, asset_group_listing_group_filter.case_value.product_item_id.value FROM asset_group_listing_group_filter WHERE asset_group_listing_group_filter.asset_group = ${ref}`)
  ]);
  ...
  const destinationUpdate=operations.map(o=>o.assetGroupOperation?.update).find(g=>g?.resourceName===group.ref&&Array.isArray(g.finalUrls));
  if(update&&!attachments.length&&!destinationUpdate)issues.push('The reviewed update has no verifiable attachments for this Google group.');
  if(!groups.length)issues.push('The uploaded group is not visible in Google yet.');
  if(destinationUpdate?JSON.stringify(current.assetGroup?.finalUrls||[])!==JSON.stringify(destinationUpdate.finalUrls)||JSON.stringify(current.assetGroup?.finalMobileUrls||[])!==JSON.stringify(destinationUpdate.finalMobileUrls||[]):destination!==product.url&&destination!==group.url)issues.push('The Google destination does not match this product ad.');
...
async function _adDesignDeliveryFresh({workspaceId,start,end}={}){
  ...
  let verification=null,verificationError=null;if(campaignId&&group.channel==='pmax'&&applied){const record=await fb().db.collection(COL.approvals).doc(applied.approvalId).get();try{verification=await _verifyPublishedAd(campaignId,group,product,record.data());}catch(e){verificationError='Google verification could not be completed: '+e.message;}}
...
async function prepareAdDesignPublication({workspaceId,target='ads',formats=[],includeCopy=false,includeProductDestination=false,offerId=null,merchantIdentity=null,queueOnly=false}={}){
  const {ref,w,products,product,group}=await _adDesignPublicationContext(workspaceId);if(w.context.draftGroups)throw new Error('Review and create all product groups together in Approvals.');if(target==='ads'&&group.requiresProductSplit)throw new Error('Split this shared group into product groups before publishing product-specific assets. Your design stays saved.');const selection={formats:[...new Set(formats)].sort(),copy:includeCopy===true,...(includeProductDestination===true?{destination:true}:{})};
  if(selection.destination&&(target!=='ads'||!w.context.campaignId||selection.copy||selection.formats.length))throw new Error('Review the product destination separately for an existing product group.');
  ...
  if(!['ads','merchant'].includes(target)||selection.formats.some(k=>!['square','landscape','portrait'].includes(k))||!selection.copy&&!selection.formats.length&&!selection.destination)throw new Error('Select an image format, messaging or product destination to update.');
  ...
  for(const device of ['desktop','mobile'])for(const format of selection.formats){const chosen=placements.find(p=>p.device===device&&p.format===format),asset=chosen&&chosen.asset||(result.placementAssets||{})[device]&&result.placementAssets[device][format]||(result.assets||{})[format];if(!asset)throw new Error('Save or generate the '+format+' image before approving it.');
    if(target==='ads'&&chosen?.artwork)throw new Error('Choose the clean product photograph for responsive Google Ads. Publish its headline, logo and native Shop now button separately; a button drawn into an image cannot be interactive.');
    ...
    const bytes=await _loadCreativeAsset(asset),meta=await require('sharp')(bytes).metadata();if(bytes.length!==asset.bytes||meta.width!==asset.width||meta.height!==asset.height)throw new Error('A saved image changed. Save its crop again before approving.');const ratio={square:1,landscape:1.91,portrait:.8}[format];if(Math.abs(meta.width/meta.height-ratio)>.015||meta.width<({square:300,landscape:600,portrait:480}[format])||meta.height<({square:300,landscape:314,portrait:600}[format])||bytes.length>5*1024*1024)throw new Error('The '+format+' image does not meet Google’s dimensions or file-size requirements. Save its correct crop first.');assets[device][format]=asset;}
  ...
  if(queueOnly){
    if(target!=='ads'||!selection.copy||selection.formats.length<(group.channel==='search'&&w.context.campaignId?2:3))throw new Error('Send the complete ad with all formats and messaging.');
    if(!_copyValid(result.copy,group.channel==='pmax'||!w.context.campaignId))throw new Error('Complete the messaging, including a short headline and description, before sending to Approval.');
  ...
  if(target==='merchant'){
    ...
    const item={type:'adDesignUpdate',summary:'Update '+product.title+' · '+[...selection.formats,selection.copy?'messaging':'',selection.destination?'product destination':''].filter(Boolean).join(', '),payload,vetted:false,status:'PENDING',createdAt:fb().FV.serverTimestamp()};
    versionReviewGate.assertVersionOperationScope(item,w.sourceSnapshot,CID);
    approvalItem=item;
  }else{
    if(selection.formats.length!==3||!selection.copy)throw new Error('The first ad needs all three image formats and messaging.');
    approvalId=await _prepareFirstAdDesignApproval({workspaceId,w,product,group,result,id,sourceHash});
...
async function publishAdDesignPublication({workspaceId,id,hash,confirmed=false}={}){
  ...
  const {ref,w,product}=await _adDesignPublicationContext(workspaceId),pubRef=ref.collection('publications').doc(id);let p;
