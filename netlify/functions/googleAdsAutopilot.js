...
async function adDesignDelivery({workspaceId,start,end}={}){
  ...
  let rows,imageQuery,totalResource='asset_group',totalRef=group.ref;
  ...
  return {ok:true,available:true,deviceAvailable,deviceError,deviceRows:deviceMetrics,totals,totalsError,range,currency:ctx.budgetCurrency,timeZone:ctx.accountTimezone,basis:'Google Ads interaction date',scope:group.channel==='search'?'Ad group image assets; shared by ads in this group':'This asset group',checkedAt:Date.now(),publications:safePublications,
...
async function publishAdDesignPublication({workspaceId,id,hash,confirmed=false}={}){
  ...
  try{
    ...
    if(applied&&p.target==='ads')try{const ap=await fb().db.collection(COL.approvals).doc(p.approvalId).get(),campaignId=w.context.campaignId||(ap.data().publishedCampaignIds||[])[0]||((ap.data().learningPublication||{}).campaignIds||[])[0];publishedCampaignId=campaignId||null;if(campaignId){const basis=await _verifiedCampaignAnalysisBasis({campaignId});if(w.context.campaignId)await ref.update({sourceVersion:basis.version,snapshotHash:basis.snapshotHash,sourceSnapshot:basis.snapshot,publication:{id,target:p.target,status:'APPLIED',confirmedAt:Date.now()}});else {await ref.update({publication:{id,target:p.target,status:'APPLIED',campaignId,confirmedAt:Date.now()}});const urlKey=url=>String(url||'').replace(/[?#].*$/,'').replace(/\/$/,''),groups=(basis.snapshot.components.assetGroups||[]).filter(g=>(g.finalUrls||[]).some(url=>urlKey(url)===urlKey(product.url))).map(g=>({ref:g.resourceName}));if(groups.length)await _designEngine().linkPublishedWorkspaceGallery({workspaceId,campaignId,groups});}}}catch(e){versionWarning='Google accepted the update, but its new version could not yet be read. Refresh sources before another update.';}
