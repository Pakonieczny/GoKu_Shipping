...
async function _verifyPublishedAd(campaignId,group,product,approval){
  const ref=_gaqlString(group.ref),[groups,links,signals,filters]=await Promise.all([
    gaql(`SELECT campaign.id, campaign.status, campaign.shopping_setting.merchant_id, campaign.shopping_setting.feed_label, asset_group.resource_name, asset_group.final_urls, asset_group.status, asset_group.primary_status FROM asset_group WHERE asset_group.resource_name = ${ref} AND campaign.id = ${campaignId}`),
  ...
  for(const [format,field]of Object.entries(mapping)){
    ...
    for(const image of wanted){const receipt=(approval.assetReceipts||[]).find(r=>r.hash===image.hash);if(!receipt||!assets.some(a=>a.resourceName===receipt.resourceName&&a.fieldType===field&&a.width===image.width&&a.height===image.height))issues.push('The exact '+format+' upload is not confirmed.');}
  }
...
async function _prepareFirstAdDesignApproval({workspaceId,w,product,group,result,id,sourceHash}){
  ...
  const prior=await ap.get();if(prior.exists&&prior.data().creative?.publicationSourceHash===sourceHash)return approvalId;
