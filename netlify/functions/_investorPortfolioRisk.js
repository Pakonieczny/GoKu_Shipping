...
function candidateFromResearch(r, advBySymbol) {
  ...
  const m = r.mandate || r.proposal || (r.memo && r.memo.mandate) || r;
  if (upper(get(m, "action.kind") || m.decision || m.action || r.action) !== "BUY") return null;
  ...
  return {
    ...
    limitPriceMicros: first(get(m, "action.entry.limitPriceMicros"), get(m, "entry.limitPriceMicros"), m.limitPriceMicros),
    lossBoundaryPriceMicros: first(get(m, "action.protection.lossBoundaryPriceMicros"), get(m, "protection.lossBoundaryPriceMicros"), m.lossBoundaryPriceMicros),
    costPerShareMicros: first(get(m, "forecast.basis.estimatedRoundTripCostMicrosPerShare"), get(m, "entry.costPerShareMicros"), m.costPerShareMicros, r.costPerShareMicros, "0"),
