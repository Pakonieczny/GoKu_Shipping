...
const RELAX_LIMITS = {
  ...
  /* Profit taking. Bounded so a value cannot be set to something that never
     fires (a 90% target) or that sells on noise (a 0.1% one). */
  takeProfitPct:          { min: 0.5, max: 20, dflt: 2 },
  /* The arm must exceed the give-back, or the "trailing stop" can still sell a
     winner at a loss — which is what arming at +3% and giving back 4% did. */
  trailingArmsAtPct:      { min: 0.25, max: 10, dflt: 1.5 },
  trailingStopPct:        { min: -10, max: -0.25, dflt: -1 },
};
...
function paperLearningConfig(base, ctrl = {}) {
  ...
  /* Also non-numeric, and also silently dropped by clampRelax if left to the
     generic loop. "either" is the shipped value; "both" restores the v17
     conjunction that admitted one company out of 270. */
  if (req.sessionTrendMode === "either" || req.sessionTrendMode === "both") {
    if (cfg.sessionTrendMode !== req.sessionTrendMode) {
      applied.sessionTrendMode = { from: cfg.sessionTrendMode || "both", to: req.sessionTrendMode };
      cfg.sessionTrendMode = req.sessionTrendMode;
    }
  }
  /* A loss is closed by a rule about money, never by the ranking. Declared so
     the frozen policy records it; the code default already holds. */
  if (req.holdLosersThroughRankExit === false && cfg.holdLosersThroughRankExit !== false) {
    cfg.holdLosersThroughRankExit = false;
    applied.holdLosersThroughRankExit = { from: true, to: false };
  }
...
module.exports = {
  ...
  "exploratoryAuto": {
    ...
    "paperLearningDefaults": {
      ...
      "_sessionTrendNote": "either = the company must be above its 200-day average OR have a rising 50-day line, and carry at most one downtrend warning. Requiring BOTH admitted one company out of 270 measured, which is not a strategy, it is a refusal to trade.",
      "takeProfitPct": 2,
      "trailingArmsAtPct": 1.5,
      "trailingStopPct": -1,
      "_profitNote": "The desk had NO profit target: takeProfitPct was null, so it never sold because a position had made money, and the trailing stop armed at +3% while giving back 4% - which could turn a +3% position into a -1% sale. It now banks 2%, and arms at +1.5% giving back 1% so a gain cannot become a loss.",
    ...
    "portfolioControls": {
      "unmarkedExposureHaltPctOfNav": 20,
      "_unmarkedNote": "How much of the account may be held in positions without a validated current mark before new purchases stop. Below this the positions are carried at cost, named in the console, and buying continues; above it the account halts.",
      "maxOpenPositions": null,
      "_maxOpenPositionsNote": "null means NO LIMIT on how many companies may be held at once. The book is bounded by capital (position size, gross ceiling, cash), never by a count.",
