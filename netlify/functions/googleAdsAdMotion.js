...
If a camera idea needs an unseen part of the piece, hold the piece still and move the scene instead.
The metal holds the exact colour, tone and finish of the reference: warm yellow gold stays warm, rich, luminous gold. Never silver, white, grey, green-tinted, chalky or desaturated, and never washed by a scene colour or grade.

RULE 2 - BRIGHT AND CHEERFUL.
Light it like a sunny day: open, airy, luminous, with clean daylight, lively colour and an upbeat feel.
Never dark, dim, moody, overcast, gloomy or heavy with shadow. Never grey, muddy, hazy, foggy, misty, bloomed or milky.
The background stays rich but quieter than the jewelry through framing, depth of field and contrast placement. Never quiet it by draining its colour or going dark.

RULE 3 - ONE CONTINUOUS TAKE.
Film it as a single unbroken shot. No cuts, no jump cuts, no dissolves, no scene changes, no lighting changes, no camera teleporting to a new position. Everything happens inside that one move.
Perform the camera approach named in the treatment, not a generic push-in or zoom. Move the camera through the scene only: never orbit, spin, rotate or turn the jewelry itself.
...
RULE 4 - FRAME.
The scenery reaches all four edges. No dark strip along any edge, no vignette, no rounded corners, no border of any kind, and nothing that appears or disappears at the edges during the shot.
${orientation==='portrait'?'Fill the 9:16 frame. Keep the whole piece in the lower middle at about 55-65% of frame width, and keep the upper third open, plain, softly lit background with nothing in it.':'Fill the 16:9 frame. Keep the whole piece toward the right, away from the edges, at about 45-55% of frame height, and keep the left third open, plain, softly lit background with nothing in it.'}

RULE 5 - EVERY SURFACE IS PLAIN AND UNMARKED.
Film only bare, undecorated things: stone, sand, water, foliage, plain fabric, unpainted wood, smooth ceramic, clean glass.
Every surface is blank and unmarked, exactly as it looks before anything is applied to it. No panels, cards, plaques, frames, tiles, badges, icons, buttons or rectangles float anywhere in the scene.
Show only real physical scenery and the jewelry. Nothing is ever laid over the picture.${job.repairOf||job.fixOf?'\n\nCORRECT THESE EARLIER ISSUES without altering the piece: '+scrubText((job.repairIssues||[]).join(' '),[],[{find:String(job.title||''),to:'piece'}]):''}`;
}
...
function createMotionService(D){
 ...
 async function run(input){
  ...
  try{
   ...
   const held=job.letteringBlocked===true||job.quality?.footageLettering===true;
   await save({phase:'ready',leaseUntil:0,completedAt:Date.now(),error:null,progress:{pct:100,label:held?'Held · lettering was rendered inside the footage; repair the affected film':(job.pipelineVersion>=2?'Three video formats':'Saved video formats')+' ready to preview'}});return {ok:true,workspaceId:job.workspaceId,jobId:job.id};
...
module.exports={createMotionService,renderVariants,cropFilter,captionCopy,captionLayers,motionRequest,motionPrompt,scrubText,dropLettered,dropClauses,scrubDirection,validateDirection,resolveDirection,copyFixRequest,validateCopyFix,qualityPass,MODEL,SECONDS};
