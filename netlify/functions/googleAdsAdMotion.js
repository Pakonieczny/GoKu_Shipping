...
THE BACKGROUND SERVES THE CHARM AND MUST NEVER OVERPOWER IT. Keep the scene softer than the jewelry in saturation, contrast and brightness, so the gold is the warmest, most saturated thing in frame and the eye lands on it first. ${d?JSON.stringify({setting:d.setting,props:d.props,lighting:d.lighting,opening:d.opening,middle:d.middle,ending:d.ending,framing:d[orientation],identity:d.identity}):fallback}
...
The product is visible from the opening frame. Use one realistic travelling highlight across the metal, gentle camera parallax and a crisp detail reveal; no strobe, fake star sparkle or static slideshow. End on a steady, beautiful product view for the last three seconds.
