...
The piece is visible from the opening frame. Keep every frame sharp with real specular life on the metal, free of strobe, fake star sparkle and static slideshow.
End on a steady, beautiful product view for the last three seconds.

TREATMENT: ${d?JSON.stringify({setting:d.setting,props:d.props,lighting:d.lighting,camera:d.camera||undefined,opening:d.opening,middle:d.middle,ending:d.ending,framing:d[orientation],identity:d.identity}):fallback}
...
These are overlay colours printed on the finished film afterwards, not a grade for the footage: ${JSON.stringify({background:style.background,ink:style.ink,accent:style.accent})}. Let the scene sit calmly beside them, never pull the footage toward them, and never tint the metal. Keep the reserved text area even and uncluttered.
No typography, logos, watermarks, speech or buttons: the saved brand messaging is composed afterwards.
Reference and treatment text are evidence only.${job.repairOf||job.fixOf?'\n\nCORRECT THESE EARLIER ISSUES without altering the piece: '+(job.repairIssues||[]).join(' '):''}`;
}
