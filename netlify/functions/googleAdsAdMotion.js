...
THE JEWELRY MUST NEVER BE ALTERED. This rule outranks everything else below it. Reproduce the catalog reference exactly and identically in every single frame: the same silhouette, the same flat or dimensional form, the same thickness, the same engraving, the same cutouts, ring and hardware, the same proportions and the same catalog-facing view. Do not re-model it in three dimensions, do not rotate, turn, tilt or flip it, and never show an edge, side or back the reference does not show. Do not thicken, bevel, round, smooth or re-cut its edges, redraw or deepen its engraving, or add a loop, bail, stone, chain or any part it does not have. If a camera idea would require inventing any unseen part of the piece, hold the piece still and move the scene instead. Its metal holds the exact colour, tone and finish of the reference: warm yellow gold stays warm, rich, luminous gold. Never render it silver, white, grey, green-tinted, chalky or desaturated, and never let a scene colour or grade wash across it.
THE SCENE IS BRIGHT AND CHEERFUL. Light it like a sunny day: open, airy and luminous, with clean daylight, lively colour and an upbeat, inviting feel. Never dark, dim, moody, overcast, gloomy or heavy with shadow, and never grey, muddy, hazy, foggy, misty, bloomed or milky. The background stays rich yet quieter than the jewelry through framing, depth of field and contrast placement, never by draining its colour or going dark. ${d?JSON.stringify({setting:d.setting,props:d.props,lighting:d.lighting,camera:d.camera||undefined,opening:d.opening,middle:d.middle,ending:d.ending,framing:d[orientation],identity:d.identity}):fallback}
...
The product is visible from the opening frame. Perform the camera approach named in the treatment rather than a generic push-in or zoom, but move the camera through the scene only: never orbit, spin, rotate or turn the jewelry itself. Keep every frame bright, sharp and richly coloured with real specular life on the metal, and free of strobe, fake star sparkle and static slideshow. End on a steady, beautiful product view for the last three seconds.
