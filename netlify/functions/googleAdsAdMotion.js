...
The piece is flat, laser-cut sheet metal about as thick as a credit card. Seen face-on its edge is almost invisible; at a slight angle it shows only a hairline sliver, no deeper than about one fortieth of the charm's length. It is stamped flat sheet, never a cast or moulded figure: no chunky rim, no bevel, no rounded or extruded body, no added depth.
Its engraving is cut into the metal: real recessed lines that read as cut depth catching light. Never draw the detail on top as a printed, embossed or superficial line.
...
RULE 4 - FRAME.
The scenery reaches all four edges. No black bars, no letterboxing, no pillarboxing, no vignette, no rounded corners, no frame or border of any kind, and nothing that appears or disappears at the edges during the shot.
${orientation==='portrait'?'Fill the 9:16 frame. Keep the whole piece in the lower middle at about 55-65% of frame width, and leave the upper third empty for typography.':'Fill the 16:9 frame. Keep the whole piece toward the right, away from the edges, and leave the left third empty for typography.'}
...
module.exports={createMotionService,renderVariants,cropFilter,captionCopy,captionLayers,motionRequest,motionPrompt,scrubText,scrubDirection,validateDirection,resolveDirection,copyFixRequest,validateCopyFix,qualityPass,MODEL,SECONDS};
