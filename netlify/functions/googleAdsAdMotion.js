...
async function renderVariants(bytes,orientation,plan={}){
 const dir=await fs.mkdtemp(path.join(os.tmpdir(),'brites-motion-'));try{
  ...
  for(const device of ['mobile','desktop'])for(const format of policy.video.formats){
   ...
   const fullCanvas=plan.renderVersion>=5&&!photoMotion,geo=fullCanvas?composition.geometry(format,plan.composition):null;
   ...
   if(plan.pipelineVersion>=2){
    ...
    const animated=fullCanvas?overlays.map((o,i)=>`[${i+1}:v]format=rgba,fade=t=in:st=${o.start}:d=0.35:alpha=1${o.end<SECONDS?',fade=t=out:st='+(o.end-.25)+':d=0.25:alpha=1':''}[c${i}];${i?'[v'+(i-1)+']':'[0:v]'}[c${i}]overlay=x=0:y='12*pow(1-min(1,max(0,(t-${o.start})/0.45)),3)':enable='gte(t,${o.start})*lt(t,${o.end})'[v${i}]`).join(';'):null;
...
function createMotionService(D){
 ...
 async function recompose(input){
  ...
  const target=jobs(ref).doc('motion_'+hash('recompose:6:'+input.recomposeOf).slice(0,40));
  await D.fb().db.runTransaction(async tx=>{const current=await tx.get(ref),source=await tx.get(jobs(ref).doc(input.recomposeOf)),existing=await tx.get(target);scope(current.data(),input);if(!source.exists)throw Error('Saved videos were not found.');const parent=source.data();scope(current.data(),parent);
   if(parent.resetAt||parent.renderVersion>=6||parent.inFlight||parent.leaseUntil>Date.now()||!['ready','needs_attention'].includes(parent.phase)||!['portrait','landscape'].every(k=>parent.masters?.[k]?.status==='completed'&&parent.masters[k].asset))throw Error('Finish the existing video work before updating its captions.');
   ...
   tx.set(target,{...clone(parent),id:target.id,pipelineVersion:2,renderVersion:6,compositionBlocked:false,composition:parent.composition||null,recomposeOf:parent.id,stageUsage:[],variants:[],completedAt:null,quality:null,publication:null,inFlight:null,phase:'queued',createdAt:Date.now(),updatedAt:Date.now(),leaseUntil:0,owner:null,error:null,estimatedUsd:0,progress:{pct:65,label:'Planning full-canvas layouts from saved films'}});
