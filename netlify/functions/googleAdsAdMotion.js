...
function createMotionService(D){
 ...
 async function recompose(input){
  ...
  const target=jobs(ref).doc('motion_'+hash('recompose:4:'+input.recomposeOf).slice(0,40));
  await D.fb().db.runTransaction(async tx=>{const current=await tx.get(ref),source=await tx.get(jobs(ref).doc(input.recomposeOf)),existing=await tx.get(target);scope(current.data(),input);if(!source.exists)throw Error('Saved videos were not found.');const parent=source.data();scope(current.data(),parent);
   if(parent.resetAt||parent.renderVersion>=4||parent.inFlight||parent.leaseUntil>Date.now()||!['ready','needs_attention'].includes(parent.phase)||!['portrait','landscape'].every(k=>parent.masters?.[k]?.status==='completed'&&parent.masters[k].asset))throw Error('Finish the existing video work before updating its captions.');
   ...
   tx.set(target,{...clone(parent),id:target.id,pipelineVersion:2,renderVersion:4,recomposeOf:parent.id,stageUsage:[],variants:[],completedAt:null,quality:null,publication:null,inFlight:null,phase:'queued',createdAt:Date.now(),updatedAt:Date.now(),leaseUntil:0,owner:null,error:null,estimatedUsd:0,progress:{pct:65,label:'Reusing saved masters with protected product framing'}});
