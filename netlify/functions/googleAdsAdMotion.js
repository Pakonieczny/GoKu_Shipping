...
async function renderVariants(bytes,orientation,plan={}){
 const dir=await fs.mkdtemp(path.join(os.tmpdir(),'brites-motion-'));try{
  ...
  for(const device of ['mobile','desktop'])for(const format of policy.video.formats){
   ...
   const safeFilter=plan.renderVersion>=3&&!photoMotion?`[0:v]split=2[bg][hero];[bg]scale=16:16,boxblur=4:3,scale=${format.width}:${format.height}[back];[hero]scale=${format.width}:${Math.floor(format.height*.70/2)*2}:force_original_aspect_ratio=decrease[front];[back][front]overlay=(W-w)/2:(${Math.floor(format.height*.70/2)*2}-h)/2,setsar=1,fps=24,tpad=stop_mode=clone:stop_duration=10`:null;
   await ffmpeg(['-y','-i',input,...(safeFilter?['-filter_complex',safeFilter]:['-vf',filter]),'-t',String(SECONDS),'-an','-c:v','libx264','-preset','veryfast','-crf','20','-pix_fmt','yuv420p','-movflags','+faststart','-metadata','comment='+(photoMotion?'Photograph motion':'AI-generated product video')+'; Brites Jewelry',file]);
...
async function captionLayers(plan,format){
 ...
 for(const beat of captionCopy(plan)){
  ...
  const fontFiles=['CormorantGaramond.ttf','OpenSans-Regular.ttf','OpenSans-Bold.ttf'].map(name=>path.join(__dirname,'fonts',name));
...
function createMotionService(D){
 ...
 async function repair(input){
  ...
  await D.fb().db.runTransaction(async tx=>{const current=await tx.get(ref),source=await tx.get(parentRef),existing=await tx.get(target);scope(current.data(),input);if(!source.exists)throw Error('The original animation was not found.');const parent=source.data();scope(current.data(),parent);
   if(parent.resetAt||(!input.explicitRerun&&((parent.repairOf&&parent.pipelineVersion>=PIPELINE)||qualityPass(parent.quality)))||!['needs_attention','ready'].includes(parent.phase)||!parent.quality||parent.inFlight||parent.leaseUntil>Date.now())throw Error('Only a completed, failed quality review can receive this bounded repair.');
 ...
 async function recompose(input){
  ...
  const target=jobs(ref).doc('motion_'+hash('recompose:3:'+input.recomposeOf).slice(0,40));
  await D.fb().db.runTransaction(async tx=>{const current=await tx.get(ref),source=await tx.get(jobs(ref).doc(input.recomposeOf)),existing=await tx.get(target);scope(current.data(),input);if(!source.exists)throw Error('Saved videos were not found.');const parent=source.data();scope(current.data(),parent);
   if(parent.resetAt||parent.renderVersion>=3||parent.inFlight||parent.leaseUntil>Date.now()||!['ready','needs_attention'].includes(parent.phase)||!['portrait','landscape'].every(k=>parent.masters?.[k]?.status==='completed'&&parent.masters[k].asset))throw Error('Finish the existing video work before updating its captions.');
   ...
   tx.set(target,{...clone(parent),id:target.id,pipelineVersion:2,renderVersion:3,recomposeOf:parent.id,stageUsage:[],variants:[],completedAt:null,quality:null,publication:null,inFlight:null,phase:'queued',createdAt:Date.now(),updatedAt:Date.now(),leaseUntil:0,owner:null,error:null,estimatedUsd:0,progress:{pct:65,label:'Reusing saved masters with protected product framing'}});
 ...
 async function run(input){
  ...
  try{
   ...
   for(const [orientation,m]of Object.entries(job.masters)){
    ...
    const variants=await (D.renderVariants||renderVariants)(String(job.motionMode||'').startsWith('photograph')?await D.loadAsset(m.source):await D.loadVideo(m.asset),orientation,{...job.plan,pipelineVersion:job.pipelineVersion,renderVersion:job.renderVersion,motionMode:job.motionMode});
