      ...
      if (shouldResetForRound2) {
        try {
          ...
          await db.collection(THREADS_COLL).doc(threadId).update({
            ...
            // Audit fixes F4/F5/F7 — the link-reply and manual-listing fields
            // belong to the previous round as well.
            customListingReplyText       : FV.delete(),
            customListingReplyDraftId    : FV.delete(),
            customListingReplyStatus     : FV.delete(),
            customListingReplyQueuedAt   : FV.delete(),
            customListingReplyEnqueuedAt : FV.delete(),
            customListingReplyError      : FV.delete(),
            customListingReplySettledAt  : FV.delete(),
            customListingManualMode      : FV.delete(),
            customListingManualRequestedAt: FV.delete(),
            customListingManualCreatedAt : FV.delete(),
            customListingPriceSource     : FV.delete(),
            // Start of the new round: the listing creator only uses photos
            // the customer sends from here on.
            salesRoundStartedAt          : FV.serverTimestamp(),
      ...
      if (activeSalesStage || freshSalesLead) {
        ...
        if (typeof inboundText === "string" && inboundText.length > 12) {
          try {
            ...
            const lookupMod = (() => {
              // Audit fix F12 — findEtsyUrlsInText / lookupListingByUrl live in
              // etsyMailListingLookup.js (the catalog module never exported them).
            ...
            if (lookupMod && typeof lookupMod.findEtsyUrlsInText === "function") {
              ...
              if (urls.length > 0) {
                ...
                const lookups = await Promise.all(
                  toFetch.map(({ url }) =>
                    lookupMod.lookupListingByUrl({ url, threadId, preferMirror: true })   // audit fix F12: mirror first, 0 Etsy calls for our listings
        ...
        try {
          ...
          // Audit fix F10 — every run carries its own id (the inbox passes
          // manualRunId; automatic runs get one here), so the poll below only
          // accepts the agent's write for THIS run.
          const salesRunId = manualRunId ||
            ("auto_" + Date.now().toString(36) + "_" + Math.random().toString(36).slice(2, 8));
          ...
          let draftDoc = null;
          let draftBusyDuringRun = null;   // audit fix F1
          ...
          if (draftBusyDuringRun) {
            await writeAudit({
              threadId, draftId,
              eventType: "auto_pipeline_skipped",
              payload  : { reason: `operator reply ${draftBusyDuringRun} during the sales turn`, stage: "sales_agent_poll" }
            });
            await db.collection(THREADS_COLL).doc(threadId).set({
              lastAutoDecision  : "skipped_draft_in_flight",
              lastAutoDecisionAt: FV.serverTimestamp(),
            ...
            return ok({ threadId, decision: "skipped_draft_in_flight", durationMs: Date.now() - tStart });
          }
          ...
          if (!agentCompleted) {
            ...
            // Audit fix F11 — record the outcome the inbox's AI-draft button
            // already looks for (thread.lastAutoDecision), and end the
            // "in_progress" claim so the reaper does not re-run the turn.
            await db.collection(THREADS_COLL).doc(threadId).set({
              lastAutoDecision  : "sales_agent_timeout",
              lastAutoDecisionAt: FV.serverTimestamp(),
              updatedAt         : FV.serverTimestamp()
            }, { merge: true });
            return ok({
          ...
          let salesAutoSendFailed = false;   // audit fix F10
          ...
          // Audit fix F10 — every exit of the sales path ends the claim.
          // Auto-send off, an enqueue error or an empty draft used to leave
          // "in_progress" (rail shows "AI thinking…") until the stale-claim
          // reaper cleared it AND reset lastAutoProcessedInboundAt.
          await db.runTransaction(async (tx) => {
            const s = await tx.get(threadRef);
            if (s.exists && (s.data() || {}).lastAutoDecision === "in_progress") {
              tx.set(threadRef, {
                lastAutoDecision  : salesAutoSendFailed ? "sales_auto_send_failed"
                                  : (autoCfg.salesAutoSendEnabled ? "sales_auto_send_skipped" : "sales_draft_ready"),
                lastAutoDecisionAt: FV.serverTimestamp(),
                updatedAt         : FV.serverTimestamp()
              }, { merge: true });
            }
          }).catch(e => console.warn("sales claim finalize failed (non-fatal):", e.message));
          ...
          await db.collection(THREADS_COLL).doc(threadId).set({
            ...
            lastAutoDecision         : "sales_agent_failed",      // audit fix F11: the inbox checks this
            lastAutoDecisionAt       : FV.serverTimestamp(),
            updatedAt                : FV.serverTimestamp()
          }, { merge: true });
