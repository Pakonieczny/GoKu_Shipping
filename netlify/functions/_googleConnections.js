...
const ADS_RESOURCES = [
  ...
  { resource: "asset_field_type_view", used: true, family: "creative",
    query: "SELECT asset_field_type_view.field_type, segments.device, metrics.impressions, metrics.clicks, metrics.conversions, metrics.conversions_value, metrics.cost_micros FROM asset_field_type_view WHERE " + RECENT + " LIMIT 5",
    why: "Results grouped by the slot an asset filled. This is Google's own answer to which aspect ratio earns, and it is what the delivery report and the campaign analysis read." },
