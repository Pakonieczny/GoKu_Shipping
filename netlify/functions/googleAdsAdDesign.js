...
function createAdDesignService(deps) {
  ...
  async function appliedResponsive(w,input){
    const refs=[refFor(input.workspaceId)],campaign=String(w.context?.campaignId||w.sourceSnapshot?.campaignId||'');
    for(const id of [...new Set(w.context?.legacyEditorWorkspaceIds||[])].slice(0,12)){
      if(!campaign||id===input.workspaceId||!token(id))continue;
      const ref=refFor(id),row=await ref.get(),prior=row.exists?row.data():null;
      if(prior&&!prior.archivedAt&&String(prior.context?.campaignId||prior.sourceSnapshot?.campaignId||'')===campaign&&(prior.context?.groups||[]).some(g=>g.ref===input.groupRef))refs.push(ref);
    }
    for(const ref of refs){
      const rows=await ref.collection('editorAIJobs').get(),jobs=rows.docs.map(d=>d.data()).filter(j=>j.phase==='ready'&&j.nativeAppliedAt&&!j.resetAt&&productKey(j.scope?.productId)===productKey(input.productId)&&j.scope?.groupRef===input.groupRef).sort((a,b)=>b.nativeAppliedAt-a.nativeAppliedAt);
      for(const job of jobs){const row=await ref.collection('editorAIJobs').doc(job.id).collection('data').doc('result').get(),result=row.exists?row.data():null;if(result?.responsive)return {ref,job,result};}
    }
    return null;
  }
  ...
  async function editorResponsiveState(input={}){
    ...
    const applied=await appliedResponsive(w,input);if(!applied)throw new Error('No applied AI layout is saved for this product and group.');const {job,result}=applied;
    if(applied.ref.id!==input.workspaceId)for(const source of result.sources){
      const row=await applied.ref.collection('editorSources').doc(source.id).get(),photo=row.exists?row.data():null;
      if(!photo||productKey(photo.productId)!==productKey(input.productId)||photo.groupRef!==input.groupRef||photo.asset?.hash!==source.asset?.hash)throw new Error('A saved AI photo does not match this product and group.');
      const target=refFor(input.workspaceId).collection('editorSources').doc(source.id),existing=await target.get();
  ...
  async function editorAIStatus(input={}){
    ...
    const flightReceipt=job.inFlight?.key?await target.collection('data').doc(/(?:quality|quality_v1)$/.test(job.inFlight.key)?job.inFlight.key+'_response':job.inFlight.key).get():receipt;
    ...
    const weightedReview=await target.collection('data').doc('ad_quality_v1').get();
  ...
  async function editorAIResume(input={}){
    ...
    if(input.reviewProofs){
      const current=await target.get(),candidateRow=await target.collection('data').doc('candidate').get(),existing=await target.collection('data').doc('ad_proofs_v1').get();
      ...
      if(existing.exists){if(existing.data().candidateHash!==input.candidateHash)throw new Error('A different set of layouts already has a saved review.');}
      else{
        if(current.data().phase!=='awaiting_review')throw new Error('This design is not waiting for rendered ad proofs.');
        const expected=[{...candidate.artboard,key:'active',device:candidate.device},...require('../../brites-ad-responsive').variants.map(b=>({...b,key:b.device+'_'+b.key}))];
        if(!Array.isArray(input.reviewProofs)||input.reviewProofs.length!==expected.length||JSON.stringify(input.reviewProofs).length>4000000)throw new Error('Supply the complete bounded set of rendered ad layouts.');
        const images=[];
        ...
        await f().db.runTransaction(async tx=>{const row=await tx.get(target),saved=await tx.get(target.collection('data').doc('ad_proofs_v1')),c=await tx.get(target.collection('data').doc('candidate'));
          ...
          tx.set(target.collection('data').doc('ad_proofs_v1'),clean(proof));
        });
      }
    }
    await f().db.runTransaction(async tx=>{const row=await tx.get(target),receipt=await tx.get(target.collection('data').doc('response'));if(!row.exists)throw new Error('The AI request was not found.');const job=row.data();editorScope(w,job.scope);
      ...
      const flightReceipt=job.inFlight?.key?await tx.get(target.collection('data').doc(/(?:quality|quality_v1)$/.test(job.inFlight.key)?job.inFlight.key+'_response':job.inFlight.key)):receipt;
  ...
  async function editorAIStart(input={}){
    ...
    await f().db.runTransaction(async tx=>{const latest=await tx.get(ref),exists=await tx.get(target);editorScope(latest.data(),input);if(latest.data().sourceSetId!==w.sourceSetId)throw new Error('The product sources changed. Reopen the editor.');if(exists.exists){if(exists.data().inputHash!==inputHash)throw new Error('AI request ID conflict.');cached=true;return;}
      ...
      tx.set(target.collection('data').doc('request'),clean({...request,previewAsset:asset,sources,publicationBaseline:sha({placements:chosenPlacements(w),messaging:w.messaging||null})}));tx.update(ref,{editorAI:{id,designKey,at:now}});
  ...
  async function editorAIRun({workspaceId,jobId}={}){
    ...
    const save=async patch=>{Object.assign(job,patch,{updatedAt:Date.now(),...(patch.phase==='ready'||patch.phase==='needs_attention'?{}:{leaseUntil:Date.now()+8*60000})});await f().db.runTransaction(async tx=>{const row=await tx.get(target);if(row.data()?.owner!==owner)throw new Error('AI design worker ownership changed.');tx.update(target,clean(job));});};
  ...
  async function responsiveEditorResult({workspaceId,jobId,job,request,evidence,output,target,ref,w,save,saveData,verify,usage}){
    const research=require('./googleAdsAdDesignResearch'),plan=research.validateResponsivePlan({output,request,evidence}),responsive=require('../../brites-ad-responsive'),products=await productsFor(ref,w),product=products.find(p=>String(p.id)===request.productId);
    ...
    const repaired=await target.collection('data').doc('scene_repair').get();
    if(repaired.exists){
      const repair=repaired.data();await recordCost('scene_repair',repair);
      const sourceId='scene_'+sha([workspaceId,jobId,'scene_repair']).slice(0,40),source={id:sourceId,productId:request.productId,groupRef:request.groupRef,title:product.title+' · saved corrected scene',width:repair.asset.width,height:repair.asset.height,asset:repair.asset,source:{kind:'library',imageId:'generated_'+sha(repair.asset.path).slice(0,32)}};
  ...
  async function editorAIApply(input={}){
    ...
    const target=editorAIRef(input.workspaceId,input.jobId);
    await f().db.runTransaction(async tx=>{
      ...
      if(latest.job&&(active(latest.job)||latest.job.inFlight))throw new Error('Another design is still updating this ad.');
  ...
  async function resetFailures({workspaceId,all=false,cursor,before}={}){
    ...
    for(const ref of refs){
      ...
      for(const candidate of failed.docs){const result=await f().db.runTransaction(async tx=>{
        ...
        if(job.inFlight){const receipt=await tx.get(candidate.ref.collection('data').doc(/(?:quality|quality_v1)$/.test(job.inFlight.key)?job.inFlight.key+'_response':job.inFlight.key||'response'));if(!receipt.exists)return -1;}
