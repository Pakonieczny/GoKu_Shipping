...
function createAdDesignService(deps) {
  ...
  async function editorAIStatus(input={}){
    ...
    for(const version of [9,8,7,6,5,4,3,2,1]){weightedReview=await target.collection('data').doc('ad_quality_v'+version).get();if(weightedReview.exists){reviewVersion=version;break;}}
  ...
  async function editorAIResume(input={}){
    ...
    if(input.reviewProofs){
      const current=await target.get(),candidateRow=await target.collection('data').doc('candidate').get(),existing=await target.collection('data').doc('ad_proofs_v9').get();
      ...
      if(existing.exists){if(existing.data().candidateHash!==input.candidateHash)throw new Error('A different set of layouts already has a saved review.');}
        ...
        await f().db.runTransaction(async tx=>{const row=await tx.get(target),saved=await tx.get(target.collection('data').doc('ad_proofs_v9')),c=await tx.get(target.collection('data').doc('candidate'));
          ...
          tx.set(target.collection('data').doc('ad_proofs_v9'),clean(proof));
        });
      }
    }
  ...
  async function responsiveEditorResult({workspaceId,jobId,job,request,evidence,output,target,ref,w,save,saveData,verify,usage}){
    ...
    const rubric=require('./googleAdsAdQuality'),candidateHash=sha(candidate),proofRow=await target.collection('data').doc('ad_proofs_v9').get();
    if(!proofRow.exists||proofRow.data().candidateHash!==candidateHash)return {...candidate,candidateHash,reviewPending:true};
    const proof=proofRow.data(),key='ad_quality_v9',raw=await target.collection('data').doc(key+'_response').get();
  ...
  async function resetFailures({workspaceId,all=false,cursor,before}={}){
    ...
    for(const ref of refs){
      ...
      for(const candidate of failed.docs){const result=await f().db.runTransaction(async tx=>{
        ...
        if(job.inFlight){const receipt=await tx.get(candidate.ref.collection('data').doc(/(?:quality|quality_v\d+|copy_refine_v\d+|subject_focus_[a-f0-9]{24})$/.test(job.inFlight.key)?job.inFlight.key+'_response':job.inFlight.key||'response'));if(!receipt.exists)return -1;}
