...
async function judgeFamily(imgs) {
  // CRITERIA: charm-first matching (owner directive). The ONLY judgment is whether
  // the CHARM ITSELF matches: type/subject, exact outline/silhouette + pose,
  // engraving/cutouts/surface detail, proportions, and symbolic meaning (zodiac/
  // birth flower/Norse). Format, chain, mounting, metal and size are explicitly
  // irrelevant. Image detail "high" so small charms in zoomed-out shots resolve.
  ...
  for (const im of imgs) content.push({ type: "image_url", image_url: { url: im.url, detail: "high" } }); /* small charms in zoomed-out shots need full resolution */
