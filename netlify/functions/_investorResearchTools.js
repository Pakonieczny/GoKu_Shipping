...
function productionBindings({ accountId, sectorOf = null, marks = null } = {}) {
  ...
  return {
    filings: F ? async ({ symbol, asOfMs, concepts }) => {
      ...
      return { cik: r.cik, asOfMs: r.asOfMs, truncated: r.truncated, facts: r.facts.map((f) => ({ factId: f.factId, taxonomy: f.taxonomy, concept: f.concept, unit: f.unit, period: f.period, valueScaled: f.valueScaled, scale: f.scale, form: f.form, filedDate: f.filedDate, accession: f.accession })), lineage: r.lineage.slice(0, 200) };
    } : null,
    ...
    market: H && D ? async ({ symbol, asOfMs, lookbackDays }) => {
      ...
      return { symbol, sector: row ? row.sector : null, sectorDriver: driver, price: own.returns, provenance: own.provenance, bars: own.bars.slice(-60),
    ...
    portfolio: P ? async ({ asOfMs }) => P.getSnapshot({ accountId, asOfMs }) : null,
    portfolioRisk: P && PR ? async ({ symbol, asOfMs, proposedQuantityUnits, limitPriceMicros, lossBoundaryPriceMicros }) => {
      const snap = await P.snapshot({ accountId, asOfMs });
      const policy = POLICY.loadActiveSync({});
      const row = rosterRow(symbol);
      ...
      return PR.runScenarios({ portfolio: snap, candidates: [{ symbol, sector: row ? row.sector : null, proposedQuantityUnits, limitPriceMicros, lossBoundaryPriceMicros,
        ...
        policy, marks: marks || {}, advBySymbol: marks || {}, sectorOf: sector, clusterOf: null });
    } : null,
  };
}
