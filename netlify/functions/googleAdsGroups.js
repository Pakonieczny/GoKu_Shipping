...
function createGroupsService(D){
  ...
  async function index(input={}){
    ...
    const reportingTree=input.reportingTree===true,adChannels=reportingTree?"('SEARCH','DISPLAY')":"('SEARCH')";
    const cacheKey=JSON.stringify([campaignId,range,reportingTree]);const prior=cache.get(cacheKey);if(!input.force&&prior&&Date.now()-prior.at<180000)return prior.result;
    ...
    const [pmax,search,pmaxMetrics,searchMetrics,filters,ads,offerMetrics]=await Promise.all([
      ...
      read('Ad groups',`SELECT campaign.id, campaign.name, campaign.status, campaign.advertising_channel_type, ad_group.resource_name, ad_group.name, ad_group.status, ad_group.primary_status FROM ad_group WHERE campaign.advertising_channel_type IN ${adChannels} AND campaign.status != 'REMOVED' AND ad_group.status != 'REMOVED'${filter} LIMIT 5000`),
      ...
      read('Ad group metrics',`SELECT ad_group.resource_name, ${metricFields} FROM ad_group WHERE campaign.advertising_channel_type IN ${adChannels} AND segments.date BETWEEN '${range.start}' AND '${range.end}'${filter} LIMIT 5000`),
      ...
      read('Ad destinations',`SELECT ad_group.resource_name, ad_group_ad.ad.final_urls FROM ad_group_ad WHERE campaign.advertising_channel_type IN ${adChannels} AND ad_group_ad.status != 'REMOVED'${filter} LIMIT 5000`),
      reportingTree?read('Listing metrics',`SELECT asset_group.resource_name, asset_group_listing_group_filter.case_value.product_item_id.value, ${metricFields} FROM asset_group_product_group_view WHERE asset_group_listing_group_filter.type = 'UNIT_INCLUDED' AND segments.date BETWEEN '${range.start}' AND '${range.end}'${filter} LIMIT 5000`):Promise.resolve(null)
    ]);
    if(!pmax&&!search)throw Error('Group data could not be loaded. Refresh to retry.');
    const groupRows=[...(pmax||[]).map(r=>({r,g:r.assetGroup,channel:'pmax'})),...(search||[]).map(r=>({r,g:r.adGroup,channel:r.campaign.advertisingChannelType==='DISPLAY'?'display':'search'}))];
    ...
    const groups=groupRows.map(({r,g,channel})=>{const row={ref:g.resourceName,id:g.resourceName.split('/').pop(),name:g.name,channel,campaignId:String(r.campaign.id),campaignName:r.campaign.name,campaignStatus:r.campaign.status,status:g.status,primaryStatus:g.primaryStatus||null,urls:g.finalUrls||[]};row.mapping=mapping(row,(filters||[]).map(r=>({assetGroup:r.assetGroupListingGroupFilter.assetGroup,type:r.assetGroupListingGroupFilter.type,caseValue:r.assetGroupListingGroupFilter.caseValue})),(ads||[]).map(r=>({adGroup:r.adGroup.resourceName,finalUrls:r.adGroupAd.ad.finalUrls})));if(channel==='pmax'&&!filters||channel!=='pmax'&&!ads)row.mapping={...row.mapping,status:'unavailable',reason:'Live product mapping is unavailable.'};row.metrics=(channel==='pmax'?pmaxMetrics:searchMetrics)?metricMaps[channel==='pmax'?'pmax':'search'].get(row.ref)||totals({}):null;row.listingMetrics=offerMetrics?offerMetrics.filter(x=>x.assetGroup?.resourceName===row.ref&&x.assetGroupListingGroupFilter?.caseValue?.productItemId?.value).map(x=>({itemId:x.assetGroupListingGroupFilter.caseValue.productItemId.value,metrics:totals(x.metrics||{})})):null;row.serving=row.campaignStatus!=='ENABLED'?'Campaign '+row.campaignStatus.toLowerCase():row.status!=='ENABLED'?row.status.toLowerCase():row.primaryStatus||'Enabled';return row;});
