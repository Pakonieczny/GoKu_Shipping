...
async function freezeDecisions({ D, accountId, tradingDate, control, seriesReader, closeBySymbol, benchmarkCloseMicros, nowMs }) {
  ...
  let pinned;
  try { pinned=await require("./_investorDecisionContext").read({runId:run.managerRunId,admin:D}); }
  catch { return {skipped:true,reason:"FROZEN_DECISION_INPUTS_UNAVAILABLE",written:0}; }
  const closes=Object.fromEntries(pinned.cards.cards.map(c=>[c.symbol,c.marketObservation && c.marketObservation.priceMicros || c.price && c.price.closeMicros || null]));
  benchmarkCloseMicros=pinned.marketState.observations.find(x=>x.symbol === "SPY")?.priceMicros || null;
  const frozen = await LEARN.freezeDecisionOutcomeRecords({ accountId, managerRunId: run.managerRunId, tradingDate, cutoffMs: run.cutoffMs || null, decisions, proposalsBySymbol, envelopesBySymbol, closeBySymbol: closes,
    marketBySymbol:Object.fromEntries(pinned.cards.cards.map(c=>[c.symbol,c.marketObservation])),benchmarkObservation:pinned.marketState.observations.find(x=>x.symbol==="SPY") || null, benchmarkCloseMicros, versions: { policyHash: run.policyHash || null, universeHash: run.universeHash || null, universeVersion: run.universeVersion || null, contextManifestHash: run.contextManifestHash || null }, admin: D });
...
async function buildKpiInputs({ D, accountId, tradingDate, control, snapshot, benchmarkSeries, fills, trades, nowMs, marks, freeze, resolve }) {
  ...
  const resolved = rows(await D.col(D.COL.forecasts).where("lastResolutionAtMs", "==", nowMs).get());
  const scored=resolved.filter(f=>f.actualTrade && f.actualTrade.scoring && f.actualTrade.scoring.crps && !f.actualTrade.scoring.crps.error && f.outcomeBuckets);
  if(scored.length) inputs.forecasts=scored.map(f=>({buckets:f.outcomeBuckets,realizedMicros:f.actualTrade.forecastOutcomes[`h${f.actualTrade.scoring.horizon}`].closeMicros}));
  const fillEvents=resolved.filter(f=>f.actualTrade && f.actualTrade.fillScoring).map(f=>f.actualTrade.fillScoring);
  if(fillEvents.length) inputs.calibrationEvents=fillEvents;
...
async function runPostclose({ accountId, tradingDate, admin = null, seriesReader = null, nowMs = Date.now(), kpiInputs = {} } = {}) {
  ...
  await step("resolve", () => LEARN.resolveDue({ admin: D, seriesReader: reader, benchmarkReader: reader, nowMs }));
