...
function buildEvidencePackage(j,c) {
  ...
  const data={campaignId:j.campaignId,sourceVersion:j.sourceVersion,snapshotHash:j.snapshotHash,range:j.range,snapshot:j.snapshot,evidence:c.evidence,limitations:c.warnings,coverage,
    ...
    history:c.improvement||null,assetOutcomes:c.data.assets||[],queries:c.data.queries,audiences:c.data.audiences,devices:c.data.devices,video:c.data.video||null,geography:c.data.geography,landing:c.data.landing,tracking:c.health,previous:c.data.previous,catalog,merchant:c.merchant?{rows:c.merchant,coverage:c.merchant._diag||null}:null,merchantOwnership:c.merchantOwnership,storeSales:c.storeSales,pages:c.pages};
...
function makeAnalysisEngine(D) {
  ...
  async function collectEvidence(j,save) {
    ...
    const [assetRows,queryRows,audiences,devices,geo,landing,previous,videoRows]=await Promise.all([
      ...
      hasVideo?read('video','Google Ads','Video watch-through, views and view rate in the selected range',()=>D.gaql(`SELECT video.id, video.title, video.duration_millis, video.channel_id, metrics.impressions, metrics.video_views, metrics.video_view_rate, metrics.video_quartile_p25_rate, metrics.video_quartile_p50_rate, metrics.video_quartile_p75_rate, metrics.video_quartile_p100_rate, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.conversions_value FROM video WHERE ${RANGE} LIMIT 200`)):Promise.resolve(null)
    ]);
    ...
    else if((videoRows||[]).length)warnings.push('Video views, view rate and watch-through quartiles are Google-reported delivery measures. They are not proof that a view caused a purchase.');
