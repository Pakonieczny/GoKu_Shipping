...
function buildEvidencePackage(j,c) {
  ...
  const data={campaignId:j.campaignId,sourceVersion:j.sourceVersion,snapshotHash:j.snapshotHash,range:j.range,snapshot:j.snapshot,evidence:c.evidence,limitations:c.warnings,coverage,
    ...
    history:c.improvement||null,assetOutcomes:c.data.assets||[],queries:c.data.queries,audiences:c.data.audiences,devices:c.data.devices,video:c.data.video||null,shapes:c.data.shapes||null,geography:c.data.geography,landing:c.data.landing,tracking:c.health,previous:c.data.previous,catalog,merchant:c.merchant?{rows:c.merchant,coverage:c.merchant._diag||null}:null,merchantOwnership:c.merchantOwnership,storeSales:c.storeSales,pages:c.pages};
...
function makeAnalysisEngine(D) {
  ...
  async function collectEvidence(j,save) {
    ...
    // A campaign with no linked video has no video outcomes to miss; an empty
    // read there would be reported as missing evidence rather than as N/A.
    const hasVideo=((j.snapshot.components||{}).assetLinks||[]).some(l=>String(l.fieldType||'')==='YOUTUBE_VIDEO');
    const [assetRows,queryRows,audiences,devices,geo,landing,previous,videoRows,shapeRows]=await Promise.all([
      ...
      hasVideo?read('video','Google Ads','Video watch-through in the selected range',()=>D.gaql(`SELECT video.id, video.title, video.duration_millis, video.channel_id, metrics.impressions, metrics.video_quartile_p25_rate, metrics.video_quartile_p50_rate, metrics.video_quartile_p75_rate, metrics.video_quartile_p100_rate, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.conversions_value FROM video WHERE ${RANGE} LIMIT 200`)):Promise.resolve(null),
      read('shapes','Google Ads','Results grouped by the asset slot each creative filled, split by device',()=>D.gaql(`SELECT asset_field_type_view.field_type, segments.device, metrics.impressions, metrics.clicks, metrics.cost_micros, metrics.conversions, metrics.conversions_value FROM asset_field_type_view WHERE ${RANGE} LIMIT 200`))
    ]);
    ...
    if((shapeRows||[]).length)warnings.push('Shape results are grouped by the asset slot each creative filled. One impression can combine several assets, so shapes are comparable with each other and must never be summed into a campaign total.');
    if(!hasVideo)evidence.push({id:'video',domain:'Google Ads',label:'Video outcomes',status:'not_applicable',detail:'This saved version links no YouTube video asset, so there are no video outcomes to report.',checkedAt:Date.now()});
    else if((videoRows||[]).length)warnings.push('Video watch-through quartiles are Google-reported delivery measures. They are not proof that a view caused a purchase. This API version exposes no view count for the video resource.');
