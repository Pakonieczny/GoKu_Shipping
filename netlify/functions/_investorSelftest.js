...
function runFixtures() {
  ...
  cases.push(fixture("legacy_baseline_is_frozen_with_full_hashes_and_the_legacy_surface_is_labelled", () => {
    ...
    /* Archived Investor JSON has no decision authority. Shared-site Netlify
       configuration is preserved under AGENTS.md, not asserted absent. */
  ...
  cases.push(fixture("fund_manager_policy_is_hashed_routing_is_fixed_and_the_mandate_contract_validates", () => {
    ...
    if (P.ROLE_MODELS.facts.model !== "gpt-5.6-terra" || P.ROLE_MODELS.manager.model !== "gpt-6-astra") return false;
    ...
    if (a.calendarId !== "XNYS" || P.POLICY_VERSION !== "fund-manager-v3-terra-high") return false;
  ...
  cases.push(fixture("guidance_claims_supersede_by_metric_and_period_and_earnings_dates_are_point_in_time", async () => {
    ...
    const first = await E.recordClaims({ symbol: "abc", documentId: "doc1", documentVersionId: "v1", sourceId: "sec:8k", publishedAtMs: t1, firstSeenAtMs:t1, nowMs:t1,
    ...
    const again = await E.recordClaims({ symbol: "ABC", documentId: "doc1", documentVersionId: "v1", publishedAtMs: t1, firstSeenAtMs:t1, nowMs:t1, admin: fake, claims: [
    ...
    const second = await E.recordClaims({ symbol: "ABC", documentId: "doc2", documentVersionId: "v2", publishedAtMs: t2, firstSeenAtMs:t2, nowMs:t2, admin: fake, claims: [
  ...
  cases.push(fixture("compact_card_has_the_blueprint_shape_declares_missing_data_and_carries_no_score", async () => {
    ...
    const bars = [];
    let c = 40;
    for (let i = 400; i >= 1; i -= 1) { const d = new Date(asOf - i * 864e5).toISOString().slice(0, 10); c *= 1 + ((i % 7) - 3) / 400; bars.push({ date: d, c: Number(c.toFixed(4)) }); }
  ...
  cases.push(fixture("gateway_routes_by_fixed_role_settles_exact_cents_and_records_every_request", async () => {
    ...
    const { transport, calls } = scriptedTransport([
      ...
      () => completed("gpt-6-astra", reviewOutput(symbols, [{ symbol: "BBB", researchPriority: 1, completionClass: "BUY_REQUIRED", reason: "inflection", reviewDirective: "RESEARCH_NOW" }])),
    ]);
    ...
    const post = calls[0];
    if (post.method !== "POST" || post.body.model !== "gpt-6-astra" || post.body.store !== false || post.body.background !== true) throw new Error(`post body ${JSON.stringify(post.body).slice(0, 200)}`);
    ...
    if (r.costMinor !== "90") throw new Error(`cost ${r.costMinor}`);
    const spend = await G.spendToday();
    if (spend.spentMinor !== 90 || spend.reservedMinor !== 0 || spend.calls !== 1 || spend.tokens.reasoning !== 6000 || spend.byRole.manager.calls !== 1) throw new Error(`ledger ${JSON.stringify(spend)}`);
    ...
    if (rec.role !== "manager" || rec.model !== "gpt-6-astra" || rec.costMinor !== "90" || rec.policyHash !== P.policyIdentity().policyHash) throw new Error("record identity");
    ...
    for (const fn of ["reviewUniverse", "researchCompany", "finalizePortfolio", "reviseEntry", "reviseHolding", "finalizeEventRevision", "repairCoverageStructure"]) if (P.ROLE_MODELS[O.ROLE_OF[fn]].model !== "gpt-6-astra") throw new Error(`${fn} is not Sol`);
    if (O.MODELS.classify.model !== "gpt-5.6-terra" || O.MODELS.classify.reasoning.effort !== "high") throw new Error("legacy table still names Terra");
    return true;
  }));
  ...
  cases.push(fixture("gateway_failures_are_never_decisions_refusal_truncation_schema_budget_and_substitution", async () => {
    ...
    /* refusal */
    const refusal = await run([{ data: { id: "r1", status: "completed", model: "gpt-6-astra", output: [{ type: "message", content: [{ type: "refusal", refusal: "cannot" }] }], usage: { input_tokens: 1000, output_tokens: 10 } } }]);
    assertFailure(refusal.r, "model_refusal");
    if ((await refusal.G.spendToday()).spentMinor !== 1 || (await refusal.G.spendToday()).calls !== 1) throw new Error("refusal not settled at its billed cost");
    /* truncation */
    const trunc = await run([completed("gpt-6-astra", reviewOutput(symbols), {}, { status: "incomplete", incomplete_details: { reason: "max_output_tokens" } })]);
    assertFailure(trunc.r, "output_truncated");
    /* schema-invalid: a workflow value in the decision column and a missing field */
    const bad = await run([completed("gpt-6-astra", { ...reviewOutput(symbols), coverage: [{ ...coverageRow("AAA"), provisionalDisposition: "RESEARCH_NOW" }] })]);
  ...
  cases.push(fixture("gateway_tools_are_allowlisted_bounded_and_scoped_and_citations_must_exist", async () => {
    ...
    let t = scriptedTransport([
      { data: { id: "r1", status: "completed", model: "gpt-6-astra", output: [{ type: "function_call", call_id: "c1", name: "getFilingFactsAsOf", arguments: JSON.stringify({ symbol: "ABC", asOfMs: cutoffMs, concepts: ["Revenues"] }) }], usage: { input_tokens: 1000, output_tokens: 100 } } },
      completed("gpt-6-astra", memo("ABC"), { input_tokens: 2000, output_tokens: 3000 }),
    ]);
    ...
    let n = 0;
    t = scriptedTransport([() => { n += 1; return { data: { id: `r${n}`, status: "completed", model: "gpt-6-astra", output: [{ type: "function_call", call_id: `c${n}`, name: "getFilingFactsAsOf", arguments: JSON.stringify({ symbol: "ABC", asOfMs: cutoffMs, concepts: [] }) }], usage: {} } }; }]);
    G = O.withDeps({ admin: fakeAdmin(), fetchImpl: t.transport, env: { OPENAI_API_KEY: "k" } });
    ...
    t = scriptedTransport([completed("gpt-6-astra", memo("ABC", "claim_made_up"))]);
    G = O.withDeps({ admin: fakeAdmin(), fetchImpl: t.transport, env: { OPENAI_API_KEY: "k" } });
    ...
    t = scriptedTransport([completed("gpt-6-astra", { ...memo("ABC"), proposedDecision: "BUY" })]);
    G = O.withDeps({ admin: fakeAdmin(), fetchImpl: t.transport, env: { OPENAI_API_KEY: "k" } });
    ...
    t = scriptedTransport([completed("gpt-5.6-terra", { schemaVersion: "fact-extraction.v1", abstained: false, abstainReason: "", contradictions: [], claims: [
    ...
    if (t.calls[0].body.model !== "gpt-5.6-terra" || t.calls[0].body.reasoning?.effort !== "high") throw new Error("extraction did not route to Luna without reasoning");
    /* Luna cost at $0.20/M in + $1.20/M out: 3000 in, 500 out → $0.0012 → 0 cents, recorded exactly in nano-dollars */
    if (ex.costMinor !== "1") throw new Error(`luna cost ${ex.costMinor}`);
  ...
  cases.push(fixture("gateway_partitions_a_large_roster_into_sol_blocks_without_filtering_and_synthesis_checks_ranks", async () => {
    ...
    const { transport, calls } = scriptedTransport([({ body }) => {
      ...
      return completed("gpt-6-astra", reviewOutput(m.symbols, [{ symbol: m.symbols[0], researchPriority: 1, completionClass: "OPTIONAL_DISCOVERY", reason: "r", reviewDirective: "RESEARCH_NOW" }]), { input_tokens: 10000, output_tokens: 2000 });
    }]);
    ...
    if (calls.length !== plan.blocks.length || calls.some((c) => c.body.model !== "gpt-6-astra")) throw new Error("a block did not go to Sol");
    ...
    if (r.costMinor !== String(calls.length * 20)) throw new Error(`block cost ${r.costMinor}`);
    /* final synthesis: duplicate capital ranks and BUY on a held name are rejected; a clean basket passes */
    const synth = (decisions, mandates) => ({ schemaVersion: "portfolio-synthesis.v1", planClass: "EXPANSION", decisions, holdingAnalysis:[], expansionMandates: mandates, comparisonNote: "n" });
    ...
    let t = scriptedTransport([completed("gpt-6-astra", synth([row("AAA", "BUY", 1), row("BBB", "BUY", 1)], [mandate("AAA"), mandate("BBB")]))]);
    let F = O.withDeps({ admin: fakeAdmin(), fetchImpl: t.transport, env: { OPENAI_API_KEY: "k" } });
    const dup = await F.finalizePortfolio({ coverage: [], researchResults: [{ symbol:"AAA", verifiedValuation:fixtureValuation(mandate("AAA")).binding, memo: { symbol: "AAA", factualPremises: [{ claimId: "claim_1" }, { claimId: "claim_2" }, { claimId: "claim_3" }] } }], holdings: [], background: false });
    if (dup.ok !== false || dup.error !== "capital_ranks_not_unique") throw new Error("duplicate ranks accepted");
    t = scriptedTransport([completed("gpt-6-astra", synth([row("HLD", "BUY", 1)], [mandate("HLD")]))]);
    ...
    t = scriptedTransport([completed("gpt-6-astra", synth([row("AAA", "BUY", 1), row("BBB", "WATCH", null, "UNFUNDED")], [mandate("AAA")]))]);
    F = O.withDeps({ admin: fakeAdmin(), fetchImpl: t.transport, env: { OPENAI_API_KEY: "k" } });
    const clean = await F.finalizePortfolio({ coverage: [], researchResults: [{ symbol:"AAA", verifiedValuation:fixtureValuation(mandate("AAA")).binding, memo: { symbol: "AAA", factualPremises: [{ claimId: "claim_1" }, { claimId: "claim_2" }, { claimId: "claim_3" }] } }], holdings: [], background: false });
  ...
  function fixtureValuation(proposal) {
    const valuation={horizonTradingDays:proposal.forecast.horizonTradingDays,method:"comparable_multiples",methodRationale:"fixture arithmetic",forwardInputBasis:"assumption_only",guidanceClaimId:null,preferredValuationZone:null,
      scenarios:proposal.forecast.outcomeBuckets.map(b=>({id:b.id,probabilityPpm:b.probabilityPpm,realizationPpm:"1000000",terminalPriceMicros:b.terminalPriceMicros,
        assumptions:[{name:"metricPerShareMicros",value:b.terminalPriceMicros,unit:"micros"},{name:"multipleMilli",value:"1000",unit:"milli"}]}))};
    const binding=require("./_investorDecisionValidation").verifyResearch({symbol:proposal.symbol,proposedDecision:"BUY",mandate:proposal,valuation},{dossierHash:"fixture-frozen-dossier"});
  ...
  function meetingWorld({ reviewMissing = ["BBB"], reviewFails = false, synthesisBuy = true, heldMandateWidens = false } = {}) {
    ...
    const gateway = {
      ...
      researchCompany: async ({ dossier }) => { calls.research += 1; return { ok: true, symbol: dossier.symbol, costMinor: "120", verifiedValuation:fixtureValuation(mandateFor(dossier.symbol)).binding, requestId: "mr_r", outputHash: "h".repeat(64), memo: { schemaVersion: "research-memo.v1", symbol: dossier.symbol, asOf: new Date(t0).toISOString(),
        checklist: { business: "b", whatChanged: "c", agreementDisagreement: "a", risks: "r", valuationFramework: "v", disconfirmingEvidence: "d", returnAndHorizon: "h", versusAlternatives: "x", mandateOrAbstain: "m" },
        ...
        inferences: [], valuation: fixtureValuation(mandateFor(dossier.symbol)).valuation, bearCase: "bear", thesisHealth: "INTACT", proposedDecision: "BUY", reasonCode: null, mandate: mandateFor(dossier.symbol) } }; },
      finalizePortfolio: async ({ researchResults, holdings }) => { calls.synthesis += 1; const s = researchResults[0] && researchResults[0].symbol || "AAA"; return { ok: true, costMinor: "80", requestId: "mr_f", synthesis: { schemaVersion: "portfolio-synthesis.v1", planClass: "EXPANSION",
        ...
        holdingAnalysis:holdings.map(h=>({symbol:h.symbol,decision:"HOLD",reasonCode:null,revisionResult:"UNCHANGED",researchDirective:"NONE",thesisHealth:"INTACT",emergency:{emergencyReductionRank:1,emergencyRankAsOf:new Date(t0).toISOString(),emergencyRankExpiresAfterSession:"2026-09-08",rationale:"x"},rationale:"hold",mandate:null})), expansionMandates: synthesisBuy ? [mandateFor(s)] : [], comparisonNote: "n" } }; },
    };
    ...
    const seed = async () => {
      for (const symbol of symbols) {
        ...
        await D.persistVersion(v, { admin: fake, sourceAtMs: t0 - 3600e3, nowMs:t0-3600e3 });
      }
  ...
  cases.push(fixture('operator_review_resumes_saved_failed_decision_without_duplicate_screening',async()=>{
    ...
    const wrong=await W.mutate('runManagerReview',{reason:'OPERATOR',resumeRunId:'different_review'});assert.equal(wrong.statusCode,409);assert.equal((await ref.get()).data().status,'dead');
    const r=await W.mutate('runManagerReview',{reason:'OPERATOR',resumeRunId:id});assert.equal(r.statusCode,200,JSON.stringify(r.body));assert.equal(r.body.data.runId,id);assert.equal(r.body.data.resumed,true);
    ...
    const again=await W.mutate('runManagerReview',{reason:'OPERATOR',resumeRunId:id});assert.equal(again.statusCode,200);assert.equal(again.body.data.duplicate,true);assert.equal(again.body.data.jobId,'saved_job');
    await ref.set({status:'complete'},{merge:true});const ended=await W.mutate('runManagerReview',{reason:'OPERATOR',resumeRunId:id});assert.equal(ended.statusCode,409);
    return true;
  }));
  ...
  cases.push(fixture("paper_meeting_real_gateway_joint_research_resume_keeps_holdings_and_costs",async()=>{
    ...
    const gateway=O.withDeps({admin:W.fake,env:{OPENAI_API_KEY:'fixture'},now:()=>W.t0,fetchImpl:async(url,opts)=>{
      ...
      if(name==='paper_shortlist_v1'){assert.equal(b.model,'gpt-5.6-terra');assert.equal(b.reasoning.effort,'high');assert.deepEqual(data('eligible_source_cards').map(x=>x.symbol),['AAA','CCC']);output={schemaVersion:'paper-shortlist.v1',selected:['AAA','CCC']};}
      ...
      else if(name==='prepared_research_document_v1'){const source=data('source_packet');assert.equal(b.model,'gpt-5.6-terra');assert.equal(b.reasoning.effort,'high');output={schemaVersion:'prepared-research-document.v1',symbol:source.baseline.symbol,sections:Object.fromEntries(H.SECTIONS.map(k=>[k,{summary:'Evidence incomplete',evidenceIds:[],missing:['Insufficient evidence']}]))};}
  ...
  cases.push(fixture("unowned_paper_news_does_not_purchase_standalone_research",async()=>{
    const assert=require('assert/strict'),fake=fakeAdmin(),C=require('./_investorDecisionContext');
  ...
  cases.push(fixture("operator_pause_blocks_new_ai_posts_without_changing_budget_or_execution",async()=>{
    const assert=require('assert/strict'),fake=fakeAdmin(),O=require('./_investorOpenai');let calls=0;
    await fake.col(fake.COL.control).doc('control').set({managerState:'PAUSED',accountMode:'PAPER_AI',executorState:'MONITORING',budget:{dailyReservationMinor:'1000'}});
    const gateway=O.withDeps({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async()=>{calls++;throw Error('must not call provider');}});
    const r=await gateway.extractFacts({symbol:'AME',documentVersions:[{versionId:'f1',canonicalText:'Reported revenue increased by ten percent this quarter.'}]});
    assert.equal(calls,0);assert.equal(r.ok,false);assert.equal((await gateway.spendToday()).reservedMinor,0);assert.equal((await gateway.spendToday()).spentMinor,0);
    assert.equal((await fake.col(fake.COL.control).doc('control').get()).data().executorState,'MONITORING');return true;
  }));

  cases.push(fixture("terra_high_and_whole_filing_evidence_survive_preparation",async()=>{
    const assert=require('assert/strict'),P=require('./_investorPolicy'),T=require('./_investorResearchTools'),H=require('./_investorResearchHandoff');
    assert.equal(P.ROLE_MODELS.facts.model,'gpt-5.6-terra');assert.equal(P.ROLE_MODELS.facts.reasoning.effort,'high');assert.equal(P.ROLE_MODELS.verification.reasoning.effort,'high');
    assert.equal(P.ROLE_MODELS.manager.model,'gpt-6-astra');assert.equal(P.costMinor({model:'gpt-5.6-terra',ordinaryInputTokens:100000,outputTokens:10000}).amountMinor,'32');
    const facts=Array.from({length:100},(_,i)=>({factId:'f'+i,filedDate:'2026-09-01',concept:'Revenue',valueScaled:'12345',unit:'USD',detail:'x'.repeat(100)}));
    const bound=T.allowlisted({filings:async()=>({facts,lineage:facts.map(f=>({factId:f.factId,accession:'dated-sec-record'}))})},{...P.TOOL_POLICY,maxResultBytes:2000},{symbol:'AAA',cutoffMs:Date.UTC(2026,8,9)});
    const filings=await bound.tools.getFilingFactsAsOf.execute({symbol:'AAA',asOfMs:Date.UTC(2026,8,9),concepts:[]});
    assert(filings.truncated&&filings.facts.length>0&&filings.facts.length<100);assert(Buffer.byteLength(JSON.stringify(filings))<=2000);
    const source=H.sourcePacket({symbol:'AAA',cutoffMs:Date.UTC(2026,8,9),dossierVersionId:'dossier',claims:[]},{filings});
    const doc=H.bindDocument({schemaVersion:'prepared-research-document.v1',symbol:'AAA',sections:Object.fromEntries(H.SECTIONS.map(k=>[k,{summary:'Uncited summary',evidenceIds:[],missing:[]}]))},source);
    assert.equal(doc.evidence.filter(e=>e.kind==='financial_fact').length,filings.facts.length);assert(doc.missing.some(x=>x.truncated));H.assertDocument(doc);return true;
  }));
  ...
  cases.push(fixture("shared_paper_meeting_uses_joint_buy_pass_plan_and_preserves_holdings",async()=>{
    ...
    const gateway=O.withDeps({admin:W.fake,env:{OPENAI_API_KEY:'fixture'},now:()=>W.t0,fetchImpl:async(url,opts)=>{
      ...
      else if(name==='paper_shortlist_v1'){assert.equal(b.model,'gpt-5.6-terra');assert.equal(b.reasoning.effort,'high');assert.deepEqual(data('eligible_source_cards').map(x=>x.symbol),['AAA','CCC']);output={schemaVersion:'paper-shortlist.v1',selected:['AAA','CCC']};}
      else if(name==='universe_review_v1'){const m=JSON.parse(user.match(/UNIVERSE_MANIFEST=(.*)/)[1]),r=await baseReview({cards:data('universe_cards'),universeManifest:m,holdings:data('holding_packets')});output={schemaVersion:'universe-review.v1',universeVersion:m.universeVersion,universeHash:m.universeHash,eligibleCount:3,coverage:r.coverage,holdingAnalysis:r.holdingAnalysis,researchRequests:r.researchRequests,managerNote:'One top finalist; existing holding retained.'};}
      else if(name==='prepared_research_document_v1'){const source=data('source_packet');assert.equal(b.model,'gpt-5.6-terra');assert.equal(b.reasoning.effort,'high');output={schemaVersion:'prepared-research-document.v1',symbol:source.baseline.symbol,sections:Object.fromEntries(H.SECTIONS.map(k=>[k,{summary:'Evidence incomplete',evidenceIds:[],missing:['Insufficient evidence']}]))};}
  ...
  cases.push(fixture("manager_meeting_fails_closed_with_typed_no_buy_reasons_and_holding_protection_is_never_widened_from_a_delta_review", async () => {
    ...
    if (!out2.done || W2.calls.review !== 1 || !out2.summary.noBuyReasons.some(x=>x.code==="COVERAGE_INCOMPLETE")) throw new Error(`cards gate ${JSON.stringify(out2).slice(0, 200)}`);
  ...
  function stagingWorld() {
    ...
    const stage = async (args) => require("./_investorMandate").stagePortfolioPlan({ accountId: "paper-1", policy: P.loadActiveSync({}), managerRunId: "run_1", admin: fake, marks, nowMs, eligibleSymbols: ["AAA", "BBB", "CCC", "DDD"], verifiedValuations:Object.fromEntries((args.proposals || []).filter(p=>p.decision==="BUY").map(p=>[p.symbol,fixtureValuation(p).binding])), ...args });
  ...
  cases.push(fixture("forecast_record_freezes_the_declared_basis_and_resolves_by_the_point_in_time_clock", async () => {
    ...
    const rec = L.outcomeRecordFor({ managerRunId: "run_1", tradingDate: "2026-09-04", decision: { symbol: "AAA", decision: "BUY", capitalRank: 1, reasonCode: null }, proposal, cutoffMs, referenceCloseMicros:"100000000", benchmarkCloseMicros: "500000000", versions: { policyHash: "p1" } });
    if (rec.referencePriceMicros !== "100000000" || rec.referencePriceType !== "COMMON_DECISION_MARK") throw new Error("BUY reference must be the entry limit");
    ...
    if (watch.referencePriceType !== "COMMON_DECISION_MARK" || watch.targetPriceMicros !== null) throw new Error("WATCH reference must be the cutoff close");
    ...
    const r = L.resolveAgainstBars({ record: rec, series, benchmarkSeries: bench, nowMs:Date.UTC(2026,10,1) });
    ...
    if (r.resolved.h1.returnBps !== "0") throw new Error(`h1 return ${r.resolved.h1.returnBps}`);
    ...
    if (L.barsAfter(series, cutoffMs)[0].date !== "2026-09-04") throw new Error("bars on the reference day leaked");
    return true;
  }));
  ...
  cases.push(fixture("counterfactual_and_anti_anchoring_rows_use_the_same_clock_and_costs", async () => {
    ...
    const row = L.counterfactualRow({ managerRunId: "run_1", tradingDate: "2026-09-04", horizon: 20,
      selected: [{ symbol: "AAA", returnBps: "300", characteristics:{sector:"a",volatilityBps:"2000"} }, { symbol: "BBB", returnBps: "-100",characteristics:{sector:"b",volatilityBps:"2000"} }], unselected: [{ symbol: "CCC", returnBps: "500",characteristics:{sector:"a",volatilityBps:"2000"} }, { symbol: "DDD", returnBps: "100",characteristics:{sector:"b",volatilityBps:"2000"} }, { symbol: "EEE", returnBps: "0" }], benchmarkReturnBps: "50" });
    if (!row.counterfactualId || row.horizonTradingDays !== 20 || row.selectedCount !== 2 || row.unselectedCount !== 3) throw new Error("row identity");
    if (row.selectedMeanBps !== "100" || row.unselectedMeanBps !== "300") throw new Error(`means ${row.selectedMeanBps}/${row.unselectedMeanBps}`);
    if (row.selectedMinusUnselectedBps !== "-200" || row.selectedMinusBenchmarkBps !== "50") throw new Error(`lift ${row.selectedMinusUnselectedBps}/${row.selectedMinusBenchmarkBps}`);
  ...
  cases.push(fixture("freeze_is_idempotent_and_resolve_due_writes_forecasts_and_counterfactuals", async () => {
    ...
    if (h5.selectedMinusUnselectedBps !== null || h5.matchedCount !== 0) throw new Error("unmatched names must not produce selection-lift claims");
  ...
  cases.push(fixture("postclose_marks_records_nav_freezes_decisions_resolves_and_derives_alerts_without_a_model", async () => {
    ...
    await require("./_investorDecisionContext").freeze({runId:"run_1",admin:fake,context:{cards:{cards:[{symbol:"AAA",marketObservation:{priceMicros:"90000000"}},{symbol:"BBB",marketObservation:{priceMicros:"50000000"}},{symbol:"CCC",marketObservation:{priceMicros:"20000000"}}]},marketState:{observations:[{symbol:"SPY",priceMicros:"500000000"}]}}});
    ...
    if (!bbb || bbb.referencePriceMicros !== "50000000" || bbb.referencePriceType !== "COMMON_DECISION_MARK" || bbb.benchmark.referenceCloseMicros == null) throw new Error(`BBB record ${JSON.stringify(bbb).slice(0, 300)}`);
    if (out.steps.resolve.examined !== 3 || out.steps.resolve.advanced !== 3) throw new Error(`the first close after a premarket decision must resolve that evening: ${JSON.stringify(out.steps.resolve)}`);
  ...
  cases.push(fixture("operator_paper_test_runs_shared_fills_once_and_preserves_ai_pass",async()=>{
    ...
    const args={symbol:'CCC',sourcePlanId:'saved_pass'},first=await W.mutate('runPaperPurchaseTest',args);assert.equal(first.statusCode,200,JSON.stringify(first.body));
    const again=await W.mutate('runPaperPurchaseTest',args);assert.equal(again.statusCode,200);assert.equal(first.body.data.planId,again.body.data.planId);
    assert.equal((await W.fake.col(W.C.portfolioPlans).doc('saved_pass').get()).data().investments.CCC.decision,'PASS');
    ...
    await E.tickSharedPaper({admin:W.fake,accountId:'paper-1',control:W.ctrl(),barsBySymbol:{CCC:[bar]},provenanceBySymbol:provenance,nowMs:at+21*60000});
    const pos=(await W.fake.col(W.C.positions).doc('paper-1_CCC').get()).data();assert(pos.open&&pos.protectionAcknowledged);assert.equal(pos.decisionAuthority,'OPERATOR_PAPER_TEST');assert(pos.excludeFromLearning);
    assert.equal([...W.fake.docs.values()].filter(f=>f.schemaVersion==='fill.v2'&&f.symbol==='CCC').length,1);assert((await E.assertConservation('paper-1',{admin:W.fake})).pass);
    W.ctrl().buyState='FROZEN';assert.equal((await W.mutate('runPaperPurchaseTest',args)).statusCode,409);
    return true;
  }));
  ...
  cases.push(fixture("api_v2_contract_compiles_strictly_and_rejects_unknown_fields_oversized_pages_and_unsafe_numbers", () => {
    ...
    if (S.READ_ACTIONS.length !== 34 || S.MUTATION_ACTIONS.length !== 41) throw new Error(`actions ${S.READ_ACTIONS.length}/${S.MUTATION_ACTIONS.length}`);
  ...
  cases.push(fixture("api_v2_reads_answer_bounded_envelopes_and_typed_view_models_from_a_seeded_world", async () => {
    ...
    if (!sys.data.identities.fixtures.matchesBuild || sys.data.identities.roles.investment.model !== "gpt-6-astra" || sys.data.configuration.riskMandate.bounds == null) throw new Error("system identities");
  ...
  cases.push(fixture("sessions_are_server_state_with_httponly_cookie_rotating_csrf_reauth_and_origin_enforcement", async () => {
    ...
    try {
      ...
      const created2 = await SESSION.handle(ev({ action: "create", passcode }), { admin: fake, nowMs: Date.now() });
  ...
  cases.push(fixture("console_has_five_views_stable_ids_shared_vectors_and_self_contained_assets", () => {
    const S = require("./_investorApiSchemas");
    const { html, script } = consoleSource();
    if ((html.match(/<script>/g) || []).length !== 1 || (html.match(/<style>/g) || []).length !== 1 || /type="text\/plain"/.test(html)) throw new Error("One executable script and one applied stylesheet inside the HTML");
    if (/<script[^>]+src=|<link[^>]+rel="stylesheet"|investorUi\?asset=/.test(html)) throw new Error("The interface must render without an asset function or supporting files");
  ...
  cases.push(fixture("console_inline_assets_match_the_embedded_content_security_policy", () => {
    const crypto = require("crypto");
    const { script, style, html } = consoleSource();
    const csp = (html.match(/<meta http-equiv="Content-Security-Policy" content="([^"]+)">/) || [])[1];
    if (!csp) throw new Error("The HTML must carry its own content security policy");
    const h = (s) => `'sha256-${crypto.createHash("sha256").update(s, "utf8").digest("base64")}'`;
    if (!csp.includes(`script-src 'self' ${h(script)}`) || !csp.includes(`style-src 'self' ${h(style)}`)) throw new Error("Inline asset hash mismatch: the browser would block this interface");
    for (const d of ["default-src 'self'", "connect-src 'self'", "object-src 'none'", "base-uri 'none'", "form-action 'self'"]) if (!csp.includes(d)) throw new Error(`csp directive missing: ${d}`);
  ...
  cases.push(fixture("standalone_ui_keeps_all_advanced_controls_without_an_asset_loader", () => {
    ...
    if(!script.includes("drawer.history || (d && d.chart)") || !script.includes("drawer.chartEpoch") || !script.includes("loadCompanyIntraday"))throw Error("chart source/race protection");
    return true;
  }));

  cases.push(fixture("ui_health_cards_render_extra_data_without_aborting_all_settings", () => {
    const {script}=consoleSource();const start=script.indexOf("function healthCard("),end=script.indexOf("\nfunction renderSystem(",start);let rendered;
    const context={host:"health",extra:[{kind:"source-table"}],$:x=>x,cardHead:()=>({kind:"heading"}),setChildren:(host,parts)=>{rendered=parts;},stateBox:()=>({kind:"state"}),append:(node,child)=>node.appendChild(child)};
    require("vm").runInNewContext(script.slice(start,end)+"\nhealthCard(host,'Sources','link',null,extra);",context);
    if(!rendered || rendered.length!==3 || rendered[2][0].kind!=="source-table")throw Error("health card did not preserve extra data");
    return true;
  }));

  cases.push(fixture("ui_run_progress_accepts_older_numeric_counts_and_retains_portfolio_explanation", () => {
    const {runView}=require("./_investorApiV2");
    const v=runView({managerRunId:"run-progress",status:"running",stage:"maintenance",eligibleCount:304,coverage:{completedCount:290,eligibleCount:304,missing:14,duplicates:0,unknown:0},maintenance:{actionable:2,actionRequired:1},investmentNote:"Keep cash available until entry prices are met."});
    if(v.coverage.completedCount!==290 || v.coverage.missingCount!==14 || !Array.isArray(v.maintenance.actionRequired) || !v.investmentNote.includes("entry prices"))throw Error("progress view lost its state");
    return true;
  }));
  ...
  cases.push(fixture("end_to_end_meeting_activation_execution_postclose_and_console_agree_on_one_trading_day", async () => {
    ...
    if (pc.steps.marks.marked !== 2 || pc.steps.freeze.written !== 0 || pc.steps.freeze.skipped !== 3 || pc.steps.freeze.envelopes !== 1 || pc.steps.freeze.proposals !== 1) throw new Error(`postclose steps ${JSON.stringify(pc.steps)}`);
    const L = require("./_investorLearning");
    const fc = fake.docs.get(`${C.forecasts}/${L.forecastIdFor({ managerRunId: claim.runId, symbol: "AAA" })}`);
    if (!fc || fc.referencePriceType !== "COMMON_DECISION_MARK" || fc.conditionalForecast.basis.referencePriceMicros !== "43250000" || fc.status !== "PENDING" || fc.lossBoundaryPriceMicros !== "37250000") throw new Error(`forecast ${JSON.stringify(fc).slice(0, 300)}`);
    ...
    if (journal.totalCount !== 3 || !jAAA || jAAA.decision !== "BUY" || !jAAA.forecast || jAAA.forecast.referenceType !== "COMMON_DECISION_MARK" || jAAA.attributionAuthority !== "AI_MANDATE") throw new Error(`journal ${JSON.stringify(jAAA).slice(0, 300)}`);
  ...
  /* Material decision-process regressions. These exercise rejected inputs and
     persisted effects, not just the presence of a new configuration field. */
  cases.push(fixture("material_canonical_buy_has_real_price_quantity_and_scalar_risk_marks",()=>{
    const P=require("./_investorPolicy"),PR=require("./_investorPortfolioRisk"),C=require("./_investorDecisionContext");
    const p={...P.EXAMPLE_MANDATE_PROPOSAL,symbol:"AAA"};
    const candidate=PR.candidateFromResearch({symbol:"AAA",memo:{mandate:p},liquidity:{advMinor:"10000000000",spreadBps:"10"}});
    if(candidate.limitPriceMicros!=="43250000" || candidate.proposedQuantityUnits!=="86" || candidate.lossBoundaryPriceMicros!=="37250000" || candidate.advMinor!=="10000000000")throw Error(JSON.stringify(candidate));
    const input=C.riskInputs({AAA:{advMinor:"1000",spreadBps:"10"}},{positions:[{symbol:"HLD",markMicros:"50000000"}]});
    if(input.marks.HLD!=="50000000" || input.advBySymbol.AAA!=="1000")throw Error("risk units mixed");
    return true;
  }));
  cases.push(fixture("material_issuer_millions_are_normalized_and_late_claims_never_backdate",async()=>{
    const C=require("./_investorDecisionContext"),E=require("./_investorEvidence"),fake=fakeAdmin();
    if(C.moneyMicros("4200","USD_millions")!==C.moneyMicros("4200000000","USD") || C.moneyMicros("25","percent")!==null)throw Error("unit scaling");
    const t=Date.UTC(2026,8,4,12);
    await E.recordClaims({symbol:"AAA",documentVersionId:"late",firstSeenAtMs:t+1000,nowMs:t+2000,publishedAtMs:t-864e5,admin:fake,claims:[{claimType:"GUIDANCE",quote:"Revenue guidance raised to 4200 million dollars",text:"raised guidance",metric:"revenue",periodLabel:"FY2026",lowValue:"4200",highValue:"4300",unit:"USD_millions"}]});
    if((await E.claimsForCompany("AAA",{asOfMs:t,admin:fake})).length)throw Error("late discovery leaked");
    const visible=await E.claimsForCompany("AAA",{asOfMs:t+2001,admin:fake});
    if(visible.length!==1 || visible[0].normalizedLowMicros!=="4200000000000000")throw Error("normalized claim absent");
    return true;
  }));
  cases.push(fixture("material_buy_requires_positive_cost_adjusted_return_and_verified_forward_scenarios",()=>{
    const P=require("./_investorPolicy"),MD=require("./_investorMandate"),DV=require("./_investorDecisionValidation");
    const p=JSON.parse(JSON.stringify({...P.EXAMPLE_MANDATE_PROPOSAL,symbol:"AAA"})), v=fixtureValuation(p);
    if(!DV.assertMandate(p,v.binding))throw Error("valid binding rejected");
    const rejected=(fn,code)=>{let got;try{fn();}catch(e){got=e.code;}if(got!==code)throw Error(`${got} instead of ${code}`);};
    rejected(()=>DV.assertMandate({...p,forecast:{...p.forecast,horizonTradingDays:20}},v.binding),"VALUATION_FORECAST_HORIZON_MISMATCH");
    const wrong=JSON.parse(JSON.stringify(v.valuation));wrong.scenarios[0].terminalPriceMicros="36000001";
    rejected(()=>DV.verifyResearch({symbol:"AAA",proposedDecision:"BUY",mandate:p,valuation:wrong},{}),"TERMINAL_PRICE_NOT_CALCULATOR_BOUND");
    rejected(()=>DV.verifyResearch({symbol:"AAA",proposedDecision:"BUY",mandate:p,valuation:{...v.valuation,method:"reverse_dcf"}},{}),"REVERSE_DCF_IS_EXPECTATIONS_NOT_FORWARD_FORECAST");
    rejected(()=>DV.verifyResearch({symbol:"AAA",proposedDecision:"BUY",mandate:p,valuation:null},{}),"BUY_REQUIRES_VERIFIED_VALUATION");
    p.forecast.outcomeBuckets=p.forecast.outcomeBuckets.map(b=>({...b,terminalPriceMicros:"40000000"}));
    if(!MD.validateProposal(p,{nowMs:Date.UTC(2026,8,4,13)}).errors.includes("BUY_EXPECTED_RETURN_NOT_POSITIVE_AFTER_COSTS"))throw Error("negative expected return accepted");
    return true;
  }));
  cases.push(fixture("material_final_synthesis_sell_reaches_real_mandate_outbox_without_schema_pollution",async()=>{
    const W=stagingWorld(),G=require("./_investorManager"),PP=require("./_investorPortfolio"),MD=require("./_investorMandate");
    W.fake.docs.set(`${W.fake.COL.positions}/paper-1_AAA`,{accountId:"paper-1",symbol:"AAA",open:true,qty:10,quantityUnits:"10",entryPriceUsd:40,lastMarkUsd:43,lossBoundaryPriceMicros:"37250000",costBasisCents:40000});
    const proposal={...W.proposal("AAA"),decision:"SELL",allocation:null,forecast:null,action:{kind:"SELL",entry:null,protection:null,exit:{quantityUnits:"10",orderType:"MARKETABLE_LIMIT",limitPriceMicros:null,collarBps:"75",authorizedSessionDates:["2026-09-04"],onHaltOrNonfill:"KEEP_PROTECTION_AND_ALERT"}}};
    const analysis={symbol:"AAA",decision:"SELL",revisionResult:"REVISED",mandate:proposal,emergency:{emergencyReductionRank:1,emergencyRankAsOf:new Date(W.nowMs).toISOString(),emergencyRankExpiresAfterSession:"2026-09-04"}};
    const out=await G.stageHoldingAnalyses({analyses:[analysis],holdings:["AAA"],accountId:"paper-1",cutoff:{cutoffMs:W.nowMs},policy:W.P.loadActiveSync({}),managerRunId:"final-sell",nowMs:W.nowMs,
      deps:{admin:W.fake,portfolio:PP,mandate:MD,universe:{tradeTier:[{symbol:"AAA",sector:"sw"}]},claimVerifier:{verifyAndPersistBatch:async({proposals})=>W.verify(proposals)},history:{readDailyWithMeta:async()=>({series:[]})}}});
    if(out.staged.status!=="COMMITTED" || out.acceptedAnalysis[0].decision!=="SELL")throw Error(JSON.stringify(out));
    if(W.docsIn(W.fake.COL.executionOutbox).filter(x=>x.kind==="APPLY_DESIRED_ORDER_SET").length!==1)throw Error("sell has no execution effect");
    return true;
  }));
  cases.push(fixture("material_resumed_meeting_uses_frozen_evidence_and_portfolio_bytes",async()=>{
    const G=require("./_investorManager"),C=require("./_investorDecisionContext"),W=meetingWorld({reviewMissing:[]});await W.seed();
    const claim={jobId:"freeze_test",runId:"freeze_test",payload:{accountId:"paper-1",tradingDate:"2026-09-04"}};let n=0;
    const first=await G.runManagerMeeting({claim,deps:W.deps,control:{engineMode:"manager"},budget:()=>n++===0?600000:1000});
  ...
  cases.push(fixture("material_astra_background_tools_resume_and_replay_encrypted_reasoning_once",async()=>{
    const O=require("./_investorOpenai"),C=require("./_investorDecisionContext"),fake=fakeAdmin(),cutoffMs=Date.UTC(2026,8,4,13);
    const memo={schemaVersion:"research-memo.v1",symbol:"AAA",asOf:new Date(cutoffMs).toISOString(),checklist:Object.fromEntries(["business","whatChanged","agreementDisagreement","risks","valuationFramework","disconfirmingEvidence","returnAndHorizon","versusAlternatives","mandateOrAbstain"].map(k=>[k,"insufficient edge"])),factualPremises:[],inferences:[],valuation:null,bearCase:"uncertainty",thesisHealth:"UNCERTAIN",proposedDecision:"WATCH",reasonCode:"UNCERTAINTY",mandate:null};
    // use a valid canonical thesis-health value
    memo.thesisHealth="INTACT";
    let executions=0;
    const tool={getFilingFactsAsOf:{description:"facts",parameters:{type:"object",additionalProperties:false,required:["symbol","asOfMs"],properties:{symbol:{type:"string"},asOfMs:{type:"number"}}},execute:async()=>{executions++;return {facts:[]};}}};
    const t=scriptedTransport([{data:{id:"bg1",status:"queued",model:"gpt-6-astra"}},
      {data:{id:"bg1",status:"completed",model:"gpt-6-astra",output:[{type:"reasoning",id:"rs1",encrypted_content:"opaque-fixture",summary:[]},{type:"function_call",call_id:"call1",name:"getFilingFactsAsOf",arguments:JSON.stringify({symbol:"AAA",asOfMs:cutoffMs})}],usage:{input_tokens:1000,output_tokens:100}}},
      {data:{id:"bg2",status:"queued",model:"gpt-6-astra"}},completed("gpt-6-astra",memo,{input_tokens:2000,output_tokens:3000})]);
    const G=O.withDeps({admin:fake,fetchImpl:t.transport,env:{OPENAI_API_KEY:"fixture"}}),args={dossier:{symbol:"AAA",dossierHash:"pinned",claimIds:[]},tools:tool,cutoffMs};
    const a=await G.researchCompany(args),b=await G.researchCompany(args),c=await G.researchCompany(args);
    if(!a.pending || !b.pending || !c.ok || c.costMinor!=="19" || executions!==1 || t.calls.map(x=>x.method).join()!=="POST,GET,POST,GET")throw Error(JSON.stringify({a,b,c,executions,calls:t.calls.map(x=>x.method)}).slice(0,800));
    if(!t.calls[2].body.input.some(x=>x.type==="reasoning" && x.encrypted_content==="opaque-fixture") || !t.calls[0].body.include.includes("reasoning.encrypted_content"))throw Error("reasoning state lost");
    await G.researchCompany(args);if(t.calls.length!==4)throw Error("completed research bought again");
    return true;
  }));
  cases.push(fixture("material_daytime_market_state_distinguishes_delayed_breadth_from_missing_data",async()=>{
    const C=require("./_investorDecisionContext"),cutoffMs=Date.UTC(2026,8,4,15), daily=[{date:"2026-09-03",c:100,v:1000}],old={t:"2026-09-04T14:30:00Z",c:102,knownAtMs:cutoffMs-1};
    const deps={admin:fakeAdmin(),decisionMarketCache:new Map(),history:{readDailyWithMeta:async()=>({series:daily,provenance:{provider:"fixture",adjustment:"split_only"}})},market:{activeProvider:()=>({delayMinutes:15}),readRecentBarsWithMeta:async()=>({bars:[old,{t:"2026-09-04T14:55:00Z",c:999,knownAtMs:cutoffMs-1}],provenance:{feed:"delayed_sip"}})}};
    const obs=await C.marketObservation("AAA",{cutoffMs,deps});
    if(obs.priceMicros!=="102000000" || obs.freshness!=="STALE_INTRADAY")throw Error(JSON.stringify(obs));
    const fresh={...obs,freshness:"DELAYED_INTRADAY"};
    const state=await C.marketState({cards:[{marketObservation:fresh},{marketObservation:null}],cutoffMs,deps});
    if(state.breadth.observedCount!==1 || state.breadth.missingCount!==1 || state.breadth.advancing!==1 || state.creditAndRates.directRatesAndSpreadsAvailable!==false)throw Error("invented breadth or rates");
    return true;
  }));
  cases.push(fixture("material_daytime_new_documents_are_extracted_and_built_before_event_routing",async()=>{
    const worker=require("./investorEvents-background"),fake=fakeAdmin(),order=[],docs=[],now=Date.now();
    fake.docs.set(`${fake.COL.control}/control`,{engineMode:"manager"});
    const source={source_id:"test.gov",scope:"global",kind:"feed",url:"https://example.test/feed",full_text_allowed:true};
    const deps={A:fake,U:{tradeTier:[{symbol:"AAA",sector:"sw"}],researchTier:[]},P:{snapshot:async()=>({positions:[{symbol:"AAA"}],workingOrders:[]})},
      J:{checkpoint:async()=>{},segmentBudgetRemainingMs:()=>600000,renewRunLease:async()=>{}},M:{sessionState:()=>({date:"2026-09-04"}),fetchBarsChunked:async()=>({bars:{AAA:[{t:new Date(now).toISOString(),c:100}]},provider:"fixture"}),writeBars:async()=>order.push("market")},
      IS:{SOURCE_REGISTRY:{test:source},profileFor:r=>r,createSweep:()=>({}),fetchGlobalOnce:async()=>({ok:true,res:{text:"feed",sha256:"a".repeat(64)}}),sweepIo:()=>({}),parseFeed:()=>[{title:"AAA guidance cut",link:"https://example.test/a",summary:"Company lowered its revenue guidance."}],matchesProfile:()=>true,sourceMeta:()=>({}),resolveIdentity:async r=>({...r,sourceIds:[]}),pollCompany:async()=>{}},
      E:{recordVersion:async()=>{docs.push({documentId:"d",versionId:"v",sourceId:"test.gov",decisionKnownAtMs:Date.now()+1,canonicalText:"Company lowered its revenue guidance.",title:"AAA guidance cut"});return{created:true};},documentsForCompany:async()=>docs,recordClaims:async()=>order.push("claims")},
      O:{extractFacts:async()=>{order.push("extract");return{ok:true,model:"gpt-5.6-luna",claims:[]};}},D:{buildBaseline:async()=>{order.push("build");return{version:{}};},persistVersion:async()=>order.push("persist")},R:{routeEvidenceEvent:async()=>order.push("route")}};
    const result=await worker.run({payload:{accountId:"paper-1"}},{deps});
    if(result.yielded || result.summary.changed[0]!=="AAA" || order.indexOf("route")<=order.indexOf("persist") || order.indexOf("extract")>=order.indexOf("build") || !order.includes("market"))throw Error(JSON.stringify({result,order}));
    const symbols=worker.pollingSymbols({held:["HELD"],pending:["PENDING"],watch:["WATCH"],eligible:Array.from({length:20},(_,i)=>`E${i}`),slot:1});
    if(!symbols.includes("HELD") || !symbols.includes("PENDING") || !symbols.includes("E8"))throw Error("polling priorities");return true;
  }));
  cases.push(fixture("material_event_review_is_idempotent_and_does_not_consume_another_symbols_delta",async()=>{
    const D=require("./_investorDossier"),fake=fakeAdmin();fake.docs.set(`${fake.COL.dossiers}/AAA`,{pendingDeltaCount:2});
    fake.docs.set(`${fake.COL.evidenceDeltas}/a`,{symbol:"AAA",managerMateriality:"pending"});fake.docs.set(`${fake.COL.evidenceDeltas}/b`,{symbol:"BBB",managerMateriality:"pending"});
    const args={symbol:"AAA",deltaIds:["a","a","b"],managerRunId:"run",admin:fake};
    if((await D.markDeltasReviewed(args)).reviewed!==1 || (await D.markDeltasReviewed(args)).reviewed!==0 || fake.docs.get(`${fake.COL.dossiers}/AAA`).pendingDeltaCount!==1 || fake.docs.get(`${fake.COL.evidenceDeltas}/b`).managerMateriality!=="pending")throw Error("duplicate review decremented twice");return true;
  }));
  cases.push(fixture("material_learning_rebases_corporate_actions_and_never_scores_an_unfilled_limit_as_a_trade",async()=>{
    const L=require("./_investorLearning"),P=require("./_investorPolicy"),fake=fakeAdmin();
    const rebased=L.rebaseSeries({series:[{date:"2026-09-03",c:50},{date:"2026-09-04",c:55}],provenance:{provider:"fixture",adjustment:"split_only"}},{date:"2026-09-03",closeMicros:"100000000",provider:"fixture",adjustment:"split_only"});
    if(!rebased.ok || rebased.series[1].c!==110)throw Error("split created fictitious loss");
    const rec={symbol:"AAA",accountId:"paper-1",decision:"BUY",mandateVersionId:"m1",conditionalForecast:P.EXAMPLE_MANDATE_PROPOSAL.forecast,fillProbabilityByExpiryPpm:"650000",entryExpiryMs:1};
    const outcome=await L.actualTradeOutcome({record:rec,series:[],nowMs:10000,admin:fake});
    if(outcome.status!=="NOT_FILLED" || outcome.scoring!==null || outcome.fillScoring.occurred!==false)throw Error("unfilled limit scored as return");
    const cal=L.calibrationSummary(Array.from({length:5},()=>({actualTrade:{scoring:{errorBps:"100",horizon:20}}})));
    if(cal.calibrated || cal.sampleSize!==5 || cal.shrinkageAdjustedErrorBps!=="14")throw Error("small sample overconfidence");
    const mixed=L.calibrationSummary([{actualTrade:{scoring:{errorBps:"100",horizon:20}}},{actualTrade:{scoring:{errorBps:"200",horizon:60}}}]);
    if(!mixed.byHorizon || mixed.byHorizon[20].sampleSize!==1)throw Error("horizons pooled");return true;
  }));

  cases.push(fixture("material_clamped_basket_returns_actual_capacity_to_one_resynthesis",async()=>{
    const G=require("./_investorManager"),W=meetingWorld({reviewMissing:[]});await W.seed();
    let attempts=0,syntheses=0;const finalize=W.deps.gateway.finalizePortfolio;
    W.deps.gateway.finalizePortfolio=async args=>{
      syntheses++;if(syntheses===2 && (!args.resynthesisFeedback || args.resynthesisFeedback.envelopes[0].authorizedQuantityUnits!=="80" || args.resynthesisFeedback.rejectedBasket.length!==1))throw Error("capacity feedback omitted");
      const out=await finalize(args);if(syntheses===2)out.synthesis.expansionMandates=out.synthesis.expansionMandates.map(m=>({...m,allocation:{...m.allocation,proposedQuantityUnits:"80"}}));return out;
    };
    W.deps.mandate.stagePortfolioPlan=async({proposals})=>++attempts===1?{status:"NEEDS_SOL_RESYNTHESIS",reason:"MATERIAL_ACTIVATION_CLAMP",envelopes:[{symbol:"AAA",authorizedQuantityUnits:"80"}]}:{status:"COMMITTED",planId:"corrected"};
    const out=await G.runManagerMeeting({claim:{jobId:"clamp",runId:"clamp",payload:{accountId:"paper-1",tradingDate:"2026-09-04"}},deps:W.deps,control:{engineMode:"manager"},budget:()=>600000});
    if(!out.done || out.summary.activation.status!=="COMMITTED" || attempts!==2 || syntheses!==2)throw Error("resynthesis failed or unbounded");return true;
  }));
  cases.push(fixture("material_standalone_research_keeps_its_cutoff_and_inputs_across_background_yields",async()=>{
    const B=require("./investorManager-background"),C=require("./_investorDecisionContext"),W=meetingWorld({reviewMissing:[]});await W.seed();
    W.deps.tools=require("./_investorResearchTools");let calls=0,hash;
    const research=W.deps.gateway.researchCompany;
    W.deps.gateway.researchCompany=async args=>{calls++;const next=C.hash({dossier:args.dossier,portfolio:args.portfolio,cutoffMs:args.cutoffMs});if(calls===1){hash=next;return{pending:true};}if(hash!==next)throw Error("resumed research changed identity");return research(args);};
    const claim={jobId:"focused",runId:"focused",payload:{symbol:"AAA",accountId:"paper-1"}};
    if(!(await B.runFocusedResearch(claim,{engineMode:"manager"},W.deps)).pending)throw Error("expected pending");
    W.deps.now=()=>W.t0+600000;W.fake.docs.delete(`${W.fake.COL.dossiers}/AAA`);
    const second=await B.runFocusedResearch(claim,{engineMode:"manager"},W.deps);
    if(!second.ok || calls!==2 || second.standaloneBuy!==false)throw Error(JSON.stringify(second));return true;
  }));
  cases.push(fixture("material_filled_forecast_calibration_and_actual_stop_pnl_are_separate",async()=>{
    const L=require("./_investorLearning"),P=require("./_investorPolicy"),fake=fakeAdmin(),cutoffMs=Date.UTC(2026,8,4,12,45),nowMs=Date.UTC(2026,8,4,21);
    const proposal=JSON.parse(JSON.stringify({...P.EXAMPLE_MANDATE_PROPOSAL,symbol:"AAA"}));
    proposal.forecast.horizonTradingDays=1;Object.assign(proposal.forecast.basis,{horizonTradingDays:1,referencePriceMicros:"100000000",returnConvention:"ARITHMETIC_PRICE_RETURN",corporateActionPolicy:"SPLIT_ADJUSTED_ONLY"});
    proposal.forecast.outcomeBuckets=proposal.forecast.outcomeBuckets.map(b=>({...b,terminalPriceMicros:"110000000"}));
    const rec=L.outcomeRecordFor({managerRunId:"actual",tradingDate:"2026-09-04",decision:{symbol:"AAA",decision:"BUY"},proposal,referenceCloseMicros:"100000000",cutoffMs});
    Object.assign(rec,{accountId:"paper-1",mandateVersionId:"m1",entryExpiryMs:Date.UTC(2026,8,4,20)});
    fake.docs.set(`${fake.COL.fills}/entry`,{fillId:"entry",symbol:"AAA",accountId:"paper-1",role:"ENTRY",side:"buy",mandateVersionId:"m1",positionLifecycleId:"life",quantityUnits:"10",priceMicros:"100000000",feeMinor:"100",eventAtMs:Date.UTC(2026,8,4,14),receivedAtMs:Date.UTC(2026,8,4,14)});
    fake.docs.set(`${fake.COL.fills}/stop`,{fillId:"stop",symbol:"AAA",accountId:"paper-1",role:"STOP",side:"sell",mandateVersionId:"m2",positionLifecycleId:"life",quantityUnits:"10",priceMicros:"98000000",feeMinor:"100",realizedMinor:"-2200",closedPosition:true,eventAtMs:Date.UTC(2026,8,4,15),receivedAtMs:Date.UTC(2026,8,4,15)});
    const out=await L.actualTradeOutcome({record:rec,series:{series:[{date:"2026-09-04",h:999,l:1,c:110}],provenance:{provider:"fixture",adjustment:"split_only"}},nowMs,admin:fake});
...
async function simulatorAdversarial({only=null}={}) {
  const assert=require('assert/strict'),A=require('./_investorAdmin'),RealSim=require('./_investorEvals').Simulator,P=require('./_investorPolicy');
  const Sim={...RealSim,create(options={}) {
    let creating=false;const realNow=options.wallNow||Date.now;
    const svc=RealSim.create({...options,wallNow:()=>creating?Math.max(realNow(),Date.UTC(2026,8,15)):realNow()}),create=svc.createBatch;
    svc.createBatch=async(input,owner,key)=>{
      creating=true;let b;try{b=await create({...input,companyRange:'range2'},owner,key);}finally{creating=false;}
      const br=options.admin.col('InvestorAI_SimulationBatches').doc(b.batchId),config=await svc.readJSON(br,b.configRef);
  ...
  await check('learning_pinned_instructions_reach_metered_simulation_requests',async()=>{
    let sent;const S=require('./_investorSimulationStrategy'),x=await metered(async(url,opts)=>{sent=JSON.parse(opts.body);return {ok:true,status:200,json:async()=>({id:'resp_strategy',status:'completed',usage:{input_tokens:1000,output_tokens:100},output:[]})};});x.run.strategy={versionId:'strategy_fixture',rules:{...S.DEFAULT,entryDelayMinutes:15}};
    await x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',stage:'manager_decision',body:{model:'gpt-6-astra',input:[{role:'user',content:'fixture'}],max_output_tokens:1024}});assert(sent.instructions.includes('strategy_fixture'));assert(sent.instructions.includes('"entryDelayMinutes":15'));assert.equal(sent.reasoning.effort,'medium');assert.equal(x.posts(),1);
  });
  await check('learning_api_authentication_idempotence_and_pause_gate',async()=>{
    const fake=database(),V2=require('./_investorApiV2');await fake.col(A.COL.control).doc('control').set({fixturesPass:true,fixturesCommit:process.env.COMMIT_REF||process.env.DEPLOY_ID||'local'});
    const body={apiVersion:'investor.v2',requestId:'req_learning',action:'simulationAnalysisStart',params:{spendLimitUsd:10},idempotencyKey:'learning_'+'k'.repeat(24),csrfToken:'c'.repeat(24)};
    assert.equal((await V2.dispatch({body,admin:fake,authOverride:{ok:false,code:'SESSION_EXPIRED',message:'expired'}})).statusCode,401);
    const authOverride={ok:true,subject:'operator'},first=await V2.dispatch({body,admin:fake,authOverride}),second=await V2.dispatch({body,admin:fake,authOverride});assert.equal(first.statusCode,200,JSON.stringify(first.body));assert.equal(second.body.data.analysisId,first.body.data.analysisId);assert.equal((await fake.col('InvestorAI_Simulations').get()).size,0);
    await fake.col(A.COL.control).doc('control').set({fixturesPass:false},{merge:true});const pause={...body,action:'simulationAnalysisControl',params:{analysisId:first.body.data.analysisId,command:'pause'},idempotencyKey:'pause_learning_'+'k'.repeat(24)};assert.equal((await V2.dispatch({body:pause,admin:fake,authOverride})).statusCode,200);
    const view=await V2.dispatch({body:{apiVersion:'investor.v2',requestId:'req_learning_read',action:'simulationAnalysisOverview',params:{}},admin:fake,authOverride});assert.equal(view.body.data.current.paused,true);
  });
  await check('learning_holistic_pipeline_cost_versions_cache_and_manual_only',async()=>{
    const L=require('./_investorSimulationLearning'),S=require('./_investorSimulationStrategy'),fake=database();let wall=now,posts=0,gets=0;const responses=new Map();
    for(let i=0;i<9;i++)await fake.col('InvestorAI_Simulations').doc('sim_'+String(i).padStart(24,'0')).set({runId:'sim_'+String(i).padStart(24,'0'),owner:'operator',createdAtMs:now-100,date:i<5?'2026-04-01':'2026-04-02',status:'complete',returnBps:i%2?100:-20,executionVersion:i===8?'legacy-unverified':'observed-exits.v2',spentNano:100,companyRange:'range2',sessions:[]});
    await fake.col('InvestorAI_Simulations').doc('other').set({owner:'intruder',createdAtMs:now-100,status:'complete'});
    await fake.col('InvestorAI_Simulations').doc('future').set({owner:'operator',createdAtMs:now+100000,status:'complete'});
    const svc=L.create({admin:fake,now:()=>wall,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{
      if(url.endsWith('/input_tokens'))return {ok:true,json:async()=>({input_tokens:1000})};
      if(opts.method==='GET'){gets++;return {ok:true,json:async()=>responses.get(url.split('/').at(-1))};}
      const b=JSON.parse(opts.body),input=JSON.parse(b.input[1].content);posts++;assert.equal(b.model,'gpt-6-astra');assert.equal(b.reasoning.effort,'high');
      assert.equal(b.tools,undefined,'core reviews must not search');assert.equal(input.dataset.coverage.total,9);const report=b.text.format.schema.properties.patterns?{summary:'Combined patterns',patterns:[{finding:'Opening noise recurs',supportingRunIds:[input.dataset.records[0].id],contraryEvidence:'Only two dates',testOnFutureData:'Compare delays on future dates'}],criticalGaps:[],limitations:['Correlated dates']}:{title:'Test an opening delay',summary:'Delay entry by five minutes.',changeRecommended:true,why:'Experimental and reversible',changes:[{variable:'entryDelayMinutes',reason:'Avoid initial noise',supportingRunIds:['sim_'+String(0).padStart(24,'0')],contraryEvidence:'Small sample'}],limitations:['Correlated dates'],rules:{...S.DEFAULT,entryDelayMinutes:5}};
      const id='resp_'+posts;responses.set(id,{id,status:'completed',usage:{input_tokens:1000,output_tokens:100,input_tokens_details:{cached_tokens:200,cache_write_tokens:100},output_tokens_details:{reasoning_tokens:70}},output:[...(input.records?[{type:'web_search_call'}]:[]),{type:'message',content:[{type:'output_text',text:JSON.stringify(report)}]}]});return {ok:true,json:async()=>({id,status:'queued'})};
    }});
    const a=await svc.start('operator','first',{spendLimitUsd:1});assert.equal(posts,0);assert.equal((await svc.start('operator','first',{spendLimitUsd:1})).analysisId,a.analysisId);await assert.rejects(()=>svc.start('operator','different',{}),/already running/);await assert.rejects(()=>svc.view('intruder',{analysisId:a.analysisId}),/another operator/);
    const tasks=[];await svc.schedule(async j=>{tasks.push(j.task);return {upstream:202};});assert.deepEqual(tasks,['simulation_analysis']);assert.equal(posts,0);
    for(let i=0;i<35;i++){await svc.execute(a.analysisId);wall+=1000;const r=(await svc.view('operator')).current;if(r.status==='needs_attention')throw Error(JSON.stringify(r.error));if(r.status==='complete')break;}
    let v=await svc.view('operator',{evidencePage:0});assert.equal(v.current.status,'complete');assert.equal(v.current.recordCount,9);assert.equal(v.current.eligibleCount,8);assert.equal(v.current.trainingThroughDate,'2026-04-02');assert.equal(v.current.statistics.groups.length,2);assert.equal(v.current.activated,true);assert.equal(v.versions.length,1);assert.equal(v.evidence,null);assert.equal(v.current.holisticStatistics.eligible.count,8);assert.equal(posts,2);assert.equal(gets,2);
    const charge=RealSim.price('gpt-6-astra',{input_tokens:1000,output_tokens:100,input_tokens_details:{cached_tokens:200,cache_write_tokens:100}},'standard');assert.equal(v.current.spentNano,2*charge);assert.equal(v.current.reservedNano,0);assert.equal(v.current.searchCalls,0);assert.equal(v.current.outputTokens,200,'reasoning is included once');assert.equal((await fake.col('InvestorAI_Simulations').get()).size,11,'analysis must never start simulations');
    const version=v.activeVersionId;await assert.rejects(()=>svc.select('intruder',version),/not found/);await svc.select('operator','baseline');assert.equal((await svc.view('operator')).activeVersionId,'baseline');
    const b=await svc.start('operator','second',{spendLimitUsd:1});await svc.select('operator',version);
    for(let i=0;i<30;i++){await svc.execute(b.analysisId);wall++;if((await svc.view('operator')).current.status==='complete')break;}
    v=await svc.view('operator');assert.equal(v.current.status,'complete');assert.equal(posts,3,'identical source research is reused across analyses');assert.equal(v.current.spentNano,charge);assert.equal(v.versions.length,2);assert.equal(v.current.activated,false,'a later manual selection wins');assert.equal(v.activeVersionId,version);
    const batch=await RealSim.create({admin:fake,wallNow:()=>Date.UTC(2026,8,7)}).createBatch({companyRange:'top1',count:1,from:'2026-04-01',to:'2026-04-01'},'operator','pinned');const rr=(await fake.col('InvestorAI_Simulations').doc(batch.runIds[0]).get()).data();assert.equal(rr.strategyVersionId,version);assert.equal(rr.evaluationMode,'training_replay');await svc.select('operator','baseline');assert.equal((await fake.col('InvestorAI_Simulations').doc(rr.runId).get()).data().strategyVersionId,version,'in-flight version is immutable');
  });
  await check('learning_spend_threshold_pause_resume_and_uncertain_submission_no_duplicate',async()=>{
    const L=require('./_investorSimulationLearning'),fake=database();let posts=0;const svc=L.create({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{if(url.endsWith('/input_tokens'))return {ok:true,json:async()=>({input_tokens:1000})};posts++;throw Error('network lost after send');}});
    const a=await svc.start('operator','threshold',{spendLimitUsd:1}),ref=fake.col(L.RUNS).doc(a.analysisId),io=RealSim.create({admin:fake});await ref.collection('pages').doc('00000000').set({dataRef:await io.saveJSON(ref,'rows',[{runId:RID,status:'complete',cohort:'baseline',returnBps:0}])});await ref.set({status:'researching',pageCount:1,spentNano:1e9},{merge:true});
    await svc.executeBatch(a.analysisId);assert.equal(posts,0);assert.equal((await ref.get()).data().error.code,'BUDGET_EXHAUSTED');await svc.control('operator',a.analysisId,'resume',2);await svc.control('operator',a.analysisId,'pause');await svc.execute(a.analysisId);assert.equal(posts,0);await svc.control('operator',a.analysisId,'resume');await svc.execute(a.analysisId);assert.equal(posts,1);await svc.control('operator',a.analysisId,'resume');await svc.execute(a.analysisId);assert.equal(posts,1,'ambiguous submission must not be purchased twice');assert((await ref.get()).data().reservedNano>0);
  });
  await check('learning_parallel_collection_handles_296_records_without_minute_roundtrips',async()=>{
    const L=require('./_investorSimulationLearning'),fake=database();let posts=0;
    for(let i=0;i<296;i++)await fake.col('InvestorAI_Simulations').doc('sim_'+String(i).padStart(24,'0')).set({runId:'sim_'+String(i).padStart(24,'0'),owner:'operator',date:'2026-04-01',status:'incomplete',error:{message:'Budget failure'},managerCheckpointRef:'must_not_load_failed_evidence'});
    const svc=L.create({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url)=>{if(url.endsWith('/responses'))posts++;throw Error('Stop at first token count after local work');}});
    const a=await svc.start('operator','collection-speed');await svc.executeBatch(a.analysisId);
    const v=await svc.view('operator');assert.equal(v.current.recordCount,0);assert.equal(v.current.statistics.failed,0);assert.equal(v.current.excludedCount,296);assert.equal(v.current.status,'no_data');assert.equal(v.current.eligibleCount,0);assert(v.current.collectionCompletedAtMs);assert.equal(posts,0);assert.equal(v.current.reviewIndex,0,'no empty evidence groups are created');assert.equal(v.current.pageCount,0);
    assert.equal((await fake.col('InvestorAI_Simulations').get()).size,296);
  });
  await check('learning_resume_recovers_paid_verbose_answer_and_compacts_only_unpurchased_pages',async()=>{
    const L=require('./_investorSimulationLearning'),fake=database(),io=RealSim.create({admin:fake});let paid=0;
    const svc=L.create({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async()=>{paid++;throw Error('Saved answers must not be repurchased');}}),a=await svc.start('operator','legacy-recovery'),ref=fake.col(L.RUNS).doc(a.analysisId);
    const row=i=>({runId:'sim_'+String(i).padStart(24,'0'),date:'2026-04-01',status:i===0?'complete':'incomplete',returnBps:i===0?50:null,eligible:i===0,cohort:'legacy',shortlistInput:{huge:'x'.repeat(200000)}});
    for(let i=0;i<25;i++)await ref.collection('pages').doc(String(i).padStart(8,'0')).set({index:i,count:1,dataRef:await io.saveJSON(ref,'old_'+i,[row(i)])});
    const report={summary:'Paid review',cases:[{runId:row(0).runId,finding:'A'.repeat(4000),marketContext:'Observed context',evidenceGaps:'Incomplete',sources:[]}],hypotheses:[]};
    const responseRef=await io.saveJSON(ref,'paid',{id:'resp_paid',status:'completed',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify(report)}]}]});
    await ref.collection('requests').doc('review_0').set({key:'review_0',status:'settled',responseRef,actualNano:5619000000});
    await ref.set({pipelineVersion:null,status:'needs_attention',resumeStatus:'researching',pageCount:25,recordCount:25,spentNano:5619000000},{merge:true});
    let view=await svc.view('operator');assert(view.current.retainedAnswer.text.includes('AAAA'));assert.equal(view.current.requests[0].actualNano,5619000000);
    await svc.control('operator',a.analysisId,'resume');await svc.execute(a.analysisId);let saved=(await ref.get()).data();assert.equal(saved.datasetCollection,'completed_pages_v3');assert.equal(saved.pageCount,1);assert.equal(saved.statistics.total,1);assert.equal(saved.excludedCount,24);
    await svc.execute(a.analysisId);view=await svc.view('operator',{evidencePage:0});assert.equal(view.evidence.cases[0].finding.length,4000,'paid prose retained in full');assert.equal(view.current.reviewIndex,1);assert.equal(view.current.spentNano,5619000000);assert.equal(paid,0);assert.equal((await ref.collection('pages').get()).size,0,'obsolete groups are physically removed');assert.equal((await ref.collection('artifacts').doc(responseRef).get()).exists,true,'valid paid completed-outcome response is retained');assert.equal((await ref.collection('requests').doc('review_0').get()).data().actualNano,5619000000,'cost ledger remains intact');
    await svc.execute(a.analysisId);assert.equal(paid,0,'uncompleted-only groups require no paid historical search');assert.equal((await ref.get()).data().status,'paused');const fresh=await svc.start('operator','separate-after-cleanup');assert.notEqual(fresh.analysisId,a.analysisId,'a cleaned historical run does not block a new separate analysis');assert.equal(paid,0);
  });
  await check('learning_cleanup_deletes_failed_groups_and_chunks_only_for_the_authenticated_owner',async()=>{
    const L=require('./_investorSimulationLearning'),fake=database(),io=RealSim.create({admin:fake}),svc=L.create({admin:fake,fetchImpl:async()=>{throw Error('Cleanup cannot buy AI work');}});
    const a=await svc.start('operator','remove-failed'),other=await svc.start('other','keep-other'),ref=fake.col(L.RUNS).doc(a.analysisId),otherRef=fake.col(L.RUNS).doc(other.analysisId);
    const failed={runId:RID,status:'incomplete',returnBps:null,cohort:'legacy'},dataRef=await io.saveJSON(ref,'old_rows',[failed]),reportRef=await io.saveJSON(ref,'old_review',{summary:'Failed operational run',cases:[{runId:RID,finding:'Failed',marketContext:'None',evidenceGaps:'No outcome',sources:[]}],hypotheses:[]});
    await ref.collection('pages').doc('00000000').set({dataRef,index:0});await ref.collection('reviews').doc('00000000').set({reportRef});
    await fake.col('InvestorAI_SimulationResearchCache').doc('old_cache').set({analysisId:a.analysisId,reportRef});
    await ref.collection('requests').doc('review_0').set({key:'review_0',status:'settled',responseRef:reportRef,actualNano:5619000000});
    await ref.set({pipelineVersion:null,status:'needs_attention',resumeStatus:'researching',pageCount:1,recordCount:1,reviewIndex:1,spentNano:5619000000},{merge:true});await otherRef.set({pipelineVersion:null},{merge:true});
    await fake.col('InvestorAI_Simulations').doc(RID).set({...failed,owner:'operator'});
    await assert.rejects(()=>svc.control('other',a.analysisId,'remove_failed_evidence'),/another operator/);
    assert.equal((await svc.requestCleanup('operator')).queued,1);assert.equal((await otherRef.get()).data().cleanupRequested,undefined);
    await svc.execute(a.analysisId);const v=await svc.view('operator',{includeInventory:true,evidencePage:0});
    assert.equal(v.current.status,'no_data');assert.equal(v.current.recordCount,0);assert.equal(v.current.pageCount,0);assert.equal(v.current.spentNano,5619000000);assert.equal(v.current.cleanupRequested,false);assert.equal(v.evidence,null);
    assert.equal((await ref.collection('pages').get()).size,0);assert.equal((await ref.collection('reviews').get()).size,0);assert.equal((await ref.collection('artifacts').doc(dataRef).get()).exists,false);assert.equal((await ref.collection('artifacts').doc(dataRef).collection('chunks').get()).size,0);assert.equal((await fake.col('InvestorAI_SimulationResearchCache').doc('old_cache').get()).exists,false);
    assert.equal((await ref.collection('requests').doc('review_0').get()).data().responseRef,null);assert.equal((await ref.collection('requests').doc('review_0').get()).data().actualNano,5619000000);assert.equal((await fake.col('InvestorAI_Simulations').doc(RID).get()).exists,true,'simulation source records are separate from learning groups');assert.equal(v.inventory.completed,0);assert.equal(v.inventory.excluded,1);
  });
  await check('learning_cleanup_retry_and_historical_pagination_preserve_each_run',async()=>{
    const L=require('./_investorSimulationLearning'),fake=database(),io=RealSim.create({admin:fake}),svc=L.create({admin:fake}),a=await svc.start('operator','cleanup-retry'),ref=fake.col(L.RUNS).doc(a.analysisId);
    const dataRef=await io.saveJSON(ref,'failed',[{runId:RID,status:'incomplete',returnBps:null}]);await ref.collection('pages').doc('00000000').set({index:0,dataRef});await ref.set({pipelineVersion:null,status:'needs_attention',pageCount:1,recordCount:1},{merge:true});await svc.requestCleanup('operator');
    const batch=fake.batch;let failOnce=true;fake.batch=()=>{const b=batch(),commit=b.commit,remove=b.delete;let deleting=false;b.delete=(...args)=>{deleting=true;return remove(...args);};b.commit=async()=>{if(failOnce&&deleting){failOnce=false;throw Error('Transient delete failure');}return commit();};return b;};
    await svc.execute(a.analysisId);assert((await ref.get()).data().cleanupGarbage,'cleanup publication retains a restartable deletion manifest');await svc.execute(a.analysisId);assert.equal((await ref.get()).data().status,'no_data');assert.equal((await ref.get()).data().cleanupRequested,false);assert.equal((await ref.collection('artifacts').doc(dataRef).collection('chunks').get()).size,0);
    for(let i=0;i<30;i++){const id='analysis_'+String(i).padStart(24,'0');await fake.col(L.RUNS).doc(id).set({analysisId:id,owner:'operator',pipelineVersion:'completed.v3',createdAtMs:i,status:'complete',recordCount:i});}
    const selected='analysis_'+String(29).padStart(24,'0'),v=await svc.view('operator',{analysisId:selected});assert.equal(v.current.analysisId,selected);assert(v.history.some(r=>r.analysisId===selected));assert.equal(v.nextCursor,'analysis_'+String(24).padStart(24,'0'));const next=await svc.view('operator',{analysisId:selected,after:v.nextCursor});assert(next.history.some(r=>r.analysisId==='analysis_'+String(25).padStart(24,'0')));assert.equal(next.current.analysisId,selected);
  });
  await check('learning_contract_errors_are_visible_and_invalid_rules_never_activate',async()=>{
    const L=require('./_investorSimulationLearning'),S=require('./_investorSimulationStrategy'),fake=database(),io=RealSim.create({admin:fake}),svc=L.create({admin:fake});
    const a=await svc.start('operator','invalid-final'),ref=fake.col(L.RUNS).doc(a.analysisId),report={title:'Invalid',summary:'Useful saved findings',changeRecommended:true,why:'Wrong rule',changes:[],limitations:[],rules:{...S.DEFAULT,allocationScalePct:101}};
    const responseRef=await io.saveJSON(ref,'bad_final',{id:'resp_bad',status:'completed',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify(report)}]}]});
    await ref.collection('requests').doc('final').set({key:'final',status:'settled',responseRef,actualNano:100});await ref.set({status:'synthesizing',eligibleCount:1,spentNano:100},{merge:true});await svc.execute(a.analysisId);
    const v=await svc.view('operator');assert.equal(v.current.status,'needs_attention');assert(v.current.error.message.includes('$.rules.allocationScalePct'));assert(v.current.retainedAnswer.text.includes('Useful saved findings'));assert.equal(v.versions.length,0);assert.equal(v.activeVersionId,'baseline');
  });
  await check('learning_evidence_is_compact_and_price_sampling_is_explicit',async()=>{
    const L=require('./_investorSimulationLearning'),bars=Array.from({length:78},(_,i)=>[new Date(now+i*300000).toISOString(),100,i===7?200:101,i===13?20:99,100,500]);
    const raw={runId:RID,status:'complete',positions:[{symbol:'A',heldMs:7200000}],shortlistInput:{raw:'x'.repeat(500000)},preparedDocuments:[{text:'y'.repeat(100000)}],pricePath:{columns:['time','open','high','low','close','volume'],sessions:[{date:'2026-04-01',series:{A:{bars,coverage:'complete'}}}]}};
    const packet=L.compactEvidence(raw);assert(JSON.stringify(packet).length<10000);assert.equal(raw.shortlistInput.raw.length,500000);assert.equal(packet.positions[0].heldMs,7200000);assert(packet.pricePath.sampling.includes('Not an executable replay'));assert(packet.pricePath.sessions[0].series.A.bars.some(b=>b[2]===200));assert(packet.pricePath.sessions[0].series.A.bars.some(b=>b[3]===20));assert.equal(packet.pricePath.sessions[0].series.A.originalBarCount,78);
    const groups=L.evidenceGroups(Array.from({length:30},(_,i)=>({runId:String(i),status:i%2?'complete':'incomplete',returnBps:i%2?100:null})));assert.equal(groups.flat().length,15);assert(groups.every(g=>g.length<=12&&g.every(r=>r.status==='complete')));
  });
  await check('learning_statistics_keep_losses_missingness_and_overlapping_date_dependence',async()=>{
    const T=require('./_investorLearningStatistics');
    const row=(i,date,end,ret,more={})=>({runId:String(i),date,sessions:[{date},{date:end}],status:'complete',eligible:true,returnBps:ret,cohort:'c',strategyVersionId:'baseline',execution:'observed-exits.v2',companyRange:'range2',...more});
    const rows=[row(1,'2026-04-01','2026-04-03',100),row(2,'2026-04-02','2026-04-06',-100),row(3,'2026-04-06','2026-04-08',0),row(4,'2026-05-01','2026-05-01',40,{benchmarkReturnBps:20}),row(5,'2026-05-02','2026-05-02',999,{eligible:false}),row(6,'2026-05-03','2026-05-03',999,{status:'incomplete'})];
    const a=T.analyze(rows);assert.equal(a.eligible.count,4);assert.equal(a.eligible.losses,1);assert.equal(a.eligible.flat,1);assert.equal(a.eligible.meanReturnBps,10);assert.equal(a.eligible.meanExcessBps,20);assert.equal(a.eligible.benchmarkCount,1);assert.equal(a.eligible.uncertainty.blocks,2);assert.equal(a.eligible.uncertainty.blockWeightedMeanBps,20);assert.equal(a.eligible.uncertainty.interval95Bps,null);assert.equal(a.diagnostic.count,1);
    assert.equal(T.analyze([...rows,...Array.from({length:100},()=>({...rows[0]}))]).eligible.uncertainty.blocks,2,'repeated runs cannot create independent dates');
    const spread=Array.from({length:24},(_,i)=>row(i,new Date(Date.UTC(2025,i,1)).toISOString().slice(0,10),new Date(Date.UTC(2025,i,1)).toISOString().slice(0,10),i*10-120));const u=T.uncertainty(spread);assert.equal(u.blocks,24);assert(u.interval95Bps[0]<u.interval95Bps[1]);assert.deepEqual(u,T.uncertainty(spread));assert.deepEqual(T.uncertainty(spread.map(r=>({...r,returnBps:50}))).interval95Bps,[50,50]);
    const comparison=T.analyze([row('b','2026-04-01','2026-04-03',10),row('a','2026-04-01','2026-04-03',30,{strategyVersionId:'test'}),row('x','2026-04-01','2026-04-03',999,{strategyVersionId:'test',companyRange:'top1'})]);assert.equal(comparison.matchedStrategies[0].meanReturnBps,20);assert.equal(comparison.matchedStrategies[0].matchedWindows,1);
  });
  await check('learning_combined_packet_covers_296_outcomes_and_bounds_large_evidence',async()=>{
    const T=require('./_investorLearningStatistics');const rows=Array.from({length:296},(_,i)=>({runId:'sim_'+String(i).padStart(24,'0'),date:'2026-04-01',status:'complete',eligible:true,returnBps:i-148,cohort:'observed|baseline|range2',positions:[{symbol:'A',returnBps:i,holdingSessions:2,outlook:'x'.repeat(10000)}],preparedDocuments:[{text:'saved research '.repeat(10000)}]}));
    const p=T.packet(rows,'baseline');assert.equal(p.statistics.eligible.count,296);assert.equal(p.coverage.total,296);assert.equal(p.coverage.savedDocumentCount,1);assert.equal(p.records.length,296);assert(Buffer.byteLength(JSON.stringify(p))<180000);assert.equal(rows[0].positions[0].outlook.length,10000);
    const huge=T.packet(Array.from({length:2000},(_,i)=>({...rows[i%296],runId:'id'+i,cohort:'cohort'+i})),'baseline');assert.equal(huge.statistics.eligible.count,2000);assert.equal(huge.coverage.total,2000);assert(huge.coverage.mode.includes('aggregated'));assert(Buffer.byteLength(JSON.stringify(huge))<180000);
  });
  await check('learning_296_outcomes_purchase_only_two_core_reviews_and_one_optional_gap_pass',async()=>{
    const L=require('./_investorSimulationLearning'),S=require('./_investorSimulationStrategy'),fake=database();let posts=0,searchRequests=0;const bodies=[];
    for(let i=0;i<296;i++)await fake.col('InvestorAI_Simulations').doc('sim_'+String(i).padStart(24,'0')).set({runId:'sim_'+String(i).padStart(24,'0'),owner:'operator',date:'2026-04-01',status:'complete',returnBps:i-148,executionVersion:'observed-exits.v2',companyRange:'range2'});
    const svc=L.create({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{
      if(url.endsWith('/input_tokens'))return {ok:true,json:async()=>({input_tokens:12000})};const b=JSON.parse(opts.body),input=JSON.parse(b.input[1].content);posts++;bodies.push(b);let report;
      if(b.tools){searchRequests++;assert.equal(b.max_tool_calls,3);assert.equal(b.max_output_tokens,16000);assert.equal(b.tools[0].return_token_budget,'default');assert.equal(input.affectedOutcomes[0].date,'2026-04-01');report={summary:'Gap checked',findings:[{question:'Was a market-wide event scheduled?',answer:'Not established',sources:[]}],limitations:['Unresolved']};}
      else if(b.text.format.schema.properties.patterns){assert.equal(input.dataset.coverage.total,296);assert.equal(input.dataset.statistics.eligible.count,296);report={summary:'Portfolio-wide pattern',patterns:[],criticalGaps:[{question:'Was a market-wide event scheduled?',whyMaterial:'Could explain shared timing risk',affectedRunIds:['sim_'+String(0).padStart(24,'0')],savedEvidenceInsufficient:'No calendar saved'}],limitations:[]};}
      else {assert.equal(input.dataset.coverage.total,296);assert.equal(input.gapResearch.summary,'Gap checked');report={title:'Holistic report',summary:'All outcomes compared',changeRecommended:false,why:'Evidence insufficient',changes:[],limitations:['Unresolved context'],rules:S.DEFAULT};}
      return {ok:true,json:async()=>({id:'r'+posts,status:'completed',usage:{input_tokens:12000,output_tokens:1000},output:[...(b.tools?[{type:'web_search_call'}]:[]),{type:'message',content:[{type:'output_text',text:JSON.stringify(report)}]}]})};
    }});
    const a=await svc.start('operator','large');await svc.executeBatch(a.analysisId);let v=(await svc.view('operator')).current;if(v.status!=='complete'){await svc.executeBatch(a.analysisId);v=(await svc.view('operator')).current;}assert.equal(v.status,'complete',JSON.stringify(v.error));assert.equal(posts,3);assert.equal(searchRequests,1);assert.equal(v.searchCalls,1);assert.equal(v.recordCount,296);assert.equal(v.requests.length,3);assert.equal(v.spentNano,3*RealSim.price('gpt-6-astra',{input_tokens:12000,output_tokens:1000},'standard')+10000000);assert(v.spentNano<6e9);assert.equal((await fake.col('InvestorAI_Simulations').get()).size,296);
    // Conservative configured token envelope, not a claimed live charge.
    const estimated=bodies.reduce((sum,b)=>sum+12000*12500+b.max_output_tokens*50000+(b.tools?3*10000000:0),0);assert(estimated<6e9);
  });
  await check('learning_budget_skips_optional_research_and_still_completes_final_report',async()=>{
    const L=require('./_investorSimulationLearning'),S=require('./_investorSimulationStrategy'),fake=database(),io=RealSim.create({admin:fake});let posts=0;
    const svc=L.create({admin:fake,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{if(url.endsWith('/input_tokens'))return {ok:true,json:async()=>({input_tokens:1000})};const b=JSON.parse(opts.body);posts++;assert.equal(b.tools,undefined);const input=JSON.parse(b.input[1].content);assert.equal(input.gapResearch,null);assert(input.gapDecision.includes('unresolved gaps'));return {ok:true,json:async()=>({id:'final_budget',status:'completed',usage:{input_tokens:1000,output_tokens:100},output:[{type:'message',content:[{type:'output_text',text:JSON.stringify({title:'Complete',summary:'Useful patterns with limits',changeRecommended:false,why:'Unresolved gaps',changes:[],limitations:['Gap retained'],rules:S.DEFAULT})}]}]})};}});
    const a=await svc.start('operator','optional-budget'),ref=fake.col(L.RUNS).doc(a.analysisId),rows=[{runId:RID,date:'2026-04-01',status:'complete',returnBps:10,eligible:true,cohort:'c'}];await ref.collection('pages').doc('00000000').set({dataRef:await io.saveJSON(ref,'data',rows)});await ref.set({status:'researching',spentNano:3e9,aggregateRef:await io.saveJSON(ref,'aggregate',require('./_investorLearningStatistics').packet(rows)),holisticReviewRef:await io.saveJSON(ref,'review',{summary:'Pattern',patterns:[],criticalGaps:[{question:'Unknown historical event',whyMaterial:'Timing',affectedRunIds:[RID],savedEvidenceInsufficient:'No source'}],limitations:[]}),recordCount:1,eligibleCount:1},{merge:true});await svc.executeBatch(a.analysisId);const v=(await svc.view('operator')).current;assert.equal(v.status,'complete',JSON.stringify(v.error));assert.equal(posts,1);assert(v.spentNano<6e9);assert.equal(v.gapDecision,'cache_checked');
  });
  await check('learning_recovers_paid_final_without_any_new_ai_purchase',async()=>{
    const L=require('./_investorSimulationLearning'),S=require('./_investorSimulationStrategy'),fake=database(),io=RealSim.create({admin:fake}),svc=L.create({admin:fake,fetchImpl:async()=>{throw Error('Must reuse paid answer');}}),a=await svc.start('operator','paid-final'),ref=fake.col(L.RUNS).doc(a.analysisId);
    const rows=[{runId:RID,date:'2026-04-01',status:'complete',returnBps:10,eligible:true,cohort:'c'}];await ref.collection('pages').doc('00000000').set({dataRef:await io.saveJSON(ref,'data',rows)});const report={title:'Paid final',summary:'Previously purchased',changeRecommended:false,why:'Keep baseline',changes:[],limitations:[],rules:S.DEFAULT};await ref.collection('requests').doc('completed_final').set({key:'completed_final',status:'settled',actualNano:4e9,responseRef:await io.saveJSON(ref,'paid',{status:'completed',output:[{type:'message',content:[{type:'output_text',text:JSON.stringify(report)}]}]})});await ref.set({status:'synthesizing',spentNano:4e9},{merge:true});await svc.executeBatch(a.analysisId);const v=(await svc.view('operator')).current;assert.equal(v.status,'complete',JSON.stringify(v.error));assert.equal(v.report.title,'Paid final');assert.equal(v.spentNano,4e9);assert.equal(v.requests.length,1);
  });
  await check('learning_strategy_contract_horizon_and_delayed_execution',async()=>{
    const S=require('./_investorSimulationStrategy'),H=require('./_investorSimulationHorizon'),Hand=require('./_investorResearchHandoff'),E=require('./_investorExecution'),C=require('./_investorDecisionContext'),M=require('./_investorMarket');
    assert.equal(S.hash(S.DEFAULT),S.hash(Object.fromEntries(Object.entries(S.DEFAULT).reverse())));assert.throws(()=>S.validate({...S.DEFAULT,allocationScalePct:101}),/Invalid/);assert.throws(()=>S.validate({...S.DEFAULT,entryDelayMinutes:3}),/five-minute/);assert.throws(()=>S.validate({...S.DEFAULT,extra:1}),/exactly/);
    const rules={...S.DEFAULT,entryDelayMinutes:10,entryWindowMinutes:30,pullbackBps:100,allocationScalePct:50,stopScalePct:200,targetScalePct:50,maxHoldingSessions:1},strategy={versionId:'strategy_fixture',rules,rulesHash:S.hash(rules)},policy=H.policyFor('top1',strategy);assert(Hand.supportedInvestmentPolicy(policy));assert.equal(Hand.supportedInvestmentPolicy({...policy,maxTotalUsd:100000}),null);assert.equal(H.choose(horizonFixture(3),policy),1);
    const date='2026-04-01',sessions=H.sessions(date),fake=database(),content={policy,sessions,cutoffMs:M.nyWallClockToUtcMs(date,P.CUTOFFS_ET.evidenceFreezeMin),investments:{TEST:{allocationUsd:20000,holdingSessions:1,holdingReason:'Short horizon',horizonAnalysis:horizonFixture(1),conviction:'MEDIUM',sizingReason:'Limited exposure',takeProfitBps:2000,stopLossBps:500}}},plan={...content,planHash:C.hash(content)},scope={runId:RID,investmentPolicy:policy,clock:()=>content.cutoffMs,collection:fake.col,transaction:fake.runTransaction,batch:fake.batch,executionSpreadBps:0,feePerShareMicros:0};
    await fake.col(A.COL.accounts).doc(RID).set({accountId:RID,startingNavCents:10000000,balanceCents:{cash:10000000,contributed_capital:-10000000}});await A.withSimulationScope(scope,()=>E.saveRequiredSimulationPlan({plan,admin:fake,accountId:RID,managerRunId:'fixture'}));
    const tick=async(min,price)=>{const at=sessions[0].openMs+min*60000;return A.withSimulationScope(scope,()=>E.tickRequiredSimulation({admin:fake,accountId:RID,barsBySymbol:{TEST:[{t:new Date(at).toISOString(),o:price,h:price+1,l:price-1,c:price,v:100000}]},nowMs:at+1200000}));};
    await fake.col(A.COL.ledger).doc('initial_funding').set({accountId:RID,legs:[{account:'cash',amountCents:10000000},{account:'contributed_capital',amountCents:-10000000}]});await tick(0,100);await tick(5,98);assert.equal((await fake.col(A.COL.fills).get()).size,0);await tick(10,99);let fills=(await fake.col(A.COL.fills).get()).docs.map(d=>d.data());assert.equal(fills.length,1);assert.equal(fills[0].quantityUnits,'101');const pos=(await fake.col(A.COL.positions).get()).docs[0].data();assert.equal(pos.allocationUsd,10000);assert.equal(pos.stopLossBps,1000);assert.equal(pos.takeProfitBps,1000);await tick(10,99);assert.equal((await fake.col(A.COL.fills).get()).size,1);await tick(15,110);assert.equal((await fake.col(A.COL.fills).get()).size,2);assert((await E.assertConservation(RID,{admin:fake})).pass);
  });
  await check('company_summary_reads_overlap_keep_cutoff_and_order_report_counts_and_reuse_pointers',async()=>{
    const fake=database(),D=require('./_investorDossier'),cutoffMs=Date.UTC(2026,6,1),symbols=Array.from({length:25},(_,i)=>'TEST'+i),cache=new Map(),progress=[];
    for(const symbol of symbols){const old=D.composeVersion({symbol,identity:{symbol,sector:'Technology'},asOfMs:cutoffMs-1000});await fake.col(A.COL.dossierVersions).doc(symbol+'_old').set(old);await fake.col(A.COL.dossierVersions).doc(symbol+'_future').set({...old,asOfMs:cutoffMs+86400000,previousVersionId:symbol+'_old',identity:{...old.identity,name:'FUTURE_SECRET'}});await fake.col(A.COL.dossiers).doc(symbol).set({symbol,currentVersionId:symbol+'_future',asOfMs:cutoffMs-1000});}
    let active=0,peak=0,reads=0;const raw=fake.col.bind(fake);fake.col=name=>{const c=raw(name);if(name!==A.COL.dossiers)return c;return {...c,doc:id=>{const r=c.doc(id);return {...r,get:async()=>{reads++;active++;peak=Math.max(peak,active);await new Promise(resolve=>setTimeout(resolve,1));const out=await r.get();active--;return out;}};}};};
    const deps={admin:fake,decisionMarketCache:new Map(),history:{readDailyWithMetaFor:async()=>({series:[]})},market:{readRecentBarsWithMeta:async()=>({bars:[]}),activeProvider:()=>({delayMinutes:15})},progress:async p=>progress.push(p)};
    const result=await D.compactCards({symbols,cutoff:{cutoffMs},admin:fake,deps,pointerCache:cache});assert.deepEqual(result.cards.map(c=>c.symbol),symbols);assert(!JSON.stringify(result).includes('FUTURE_SECRET'));assert(peak>1);assert.equal(reads,25);assert.deepEqual(progress.map(p=>p.done),[0,20,25]);
    const health=await D.dossierHealth({symbols,admin:fake,nowMs:cutoffMs,pointerCache:cache});assert.equal(health.present,25);assert.equal(reads,25,'freshness reuses this snapshot rather than rereading every company');
    await assert.rejects(()=>D.compactCards({symbols,cutoff:{cutoffMs},admin:fake,deps:{...deps,progress:async()=>{throw Error('paused');}}}),/paused/);assert.equal(reads,25);
  });
  ...
  await check('higher_budget_resumes_existing_run_without_resetting_costs_or_checkpoints',async()=>{
    ...
    const saved=await svc.saveJSON(rr,'checkpoint',{stage:'research',data:{}});
    ...
    await assert.rejects(()=>svc.control({runId:b.runIds[0],command:'retry'},'intruder'),/another operator/);
    await svc.control({runId:b.runIds[0],command:'retry'},'operator');const r=await svc.getRun(b.runIds[0]);
    assert.equal(r.status,'queued');assert.equal(r.clockMs,123456);assert.equal(r.managerCheckpointRef,saved);assert.equal(r.spentNano,900000000);assert.equal(r.reservedNano,1234);assert.equal(r.ceilingNano,Sim.CEILING);assert.equal(r.budgetFailure,null);assert.equal(r.error,null);
    assert.equal((await svc.detail(r.runId,'operator')).run.ceilingNano,Sim.CEILING);
  });
  ...
  await check('cold_simulation_worker_loads_market_credentials_after_verified_claim',async()=>{
    ...
    const originals={load:M.loadMarketSettings,auth:AUTH.loadAuthSecrets,claim:J.claimOnce,complete:J.complete,yield:J.yieldSegment,create:RealSim.create};
    ...
    try {
      ...
      RealSim.create=()=>({execute:async runId=>{assert(loaded,'saved market credentials were not loaded');assert.equal(runId,RID);executed++;return {done:true};},prepareRepository:async(batchId,unitId)=>{assert(loaded);assert.equal(batchId,BID);assert.equal(unitId,'company_A');prepared++;return {done:true};}});
      ...
      RealSim.create=()=>({execute:async()=>{throw Object.assign(Error('10 ABORTED: contention'),{code:10});}});
    ...
    } finally {M.loadMarketSettings=originals.load;AUTH.loadAuthSecrets=originals.auth;J.claimOnce=originals.claim;J.complete=originals.complete;J.yieldSegment=originals.yield;RealSim.create=originals.create;}
  });
  ...
  await check('pre_increase_saved_response_keeps_original_identity_and_reservation',async()=>{
    ...
    const key=require('./_investorDecisionContext').hash({body,clock:x.run.clockMs}),qref=x.ref.collection('requests').doc(key),oldReserve=106250000;
    await qref.set({key,status:'pending',responseId:completed.id,model:body.model,tier:'flex',maxOutput:4000,reservation:oldReserve,rates:Sim.rate(body.model,'flex',1000),stage:'manager_review'});
  ...
  for(const stage of ['manager_document','manager_decision'])await check('truncated_required_'+stage+'_retry_increases_room_and_preserves_charges',async()=>{
    let succeed=false,lastBody;const x=await metered(async(u,o)=>{lastBody=o.body?JSON.parse(o.body):lastBody;return {ok:true,status:200,json:async()=>succeed?{...completed,id:'resp_replacement'}:{...completed,id:'resp_truncated_required',status:'incomplete',incomplete_details:{reason:'max_output_tokens'}}};});
    const b=await x.svc.createBatch({count:1,from:'2026-04-23',to:'2026-04-23'},'operator','required-truncated-'+stage);await x.ref.set({batchId:b.batchId},{merge:true});
    await assert.rejects(()=>x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body,stage}),e=>e.code==='SIMULATION_AI_RESPONSE_FAILED');
    const originalMax=lastBody.max_output_tokens;
    await x.ref.set({status:'incomplete',leaseUntil:0,initialized:true,error:{code:'SIMULATION_AI_RESPONSE_FAILED'}},{merge:true});
    await assert.rejects(()=>x.meter.drain({recovery:true}),e=>e.code==='SIMULATION_AI_RESPONSE_FAILED','a required initial answer must never be ignored');
    await x.svc.control({runId:RID,command:'retry'},'operator');Object.assign(x.run,(await x.ref.get()).data());succeed=true;
    await x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body,stage});
    assert.equal(lastBody.max_output_tokens,Math.ceil(originalMax*1.5));assert.equal(x.posts(),2);
    await x.meter.drain({recovery:true});assert.equal((await x.ref.get()).data().spentNano,60000000);assert.equal((await x.ref.get()).data().pendingAiCount,0);
    await x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body,stage});assert.equal(x.posts(),2);
  });
  ...
  await check('shortlist_uses_only_cutoff_evidence_validates_50_and_reuses_paid_response_after_pause',async()=>{
    ...
    assert.equal(await x.svc.prepareShortlist(x.run,config,packets,opts),null);assert.equal(x.posts(),1);assert.equal(sent.model,'gpt-5.6-terra');assert.equal(sent.reasoning.effort,'high');assert.equal(sent.max_output_tokens,24000);assert.equal(sent.service_tier,'default');assert(!JSON.stringify(sent).includes('FUTURE_SECRET'));assert(!JSON.stringify(sent).includes('99999'));
    ...
    const result=await x.svc.prepareShortlist(x.run,config,packets,opts);assert.equal(result.selected.length,50);assert.equal(result.reasoning,'high');assert.equal(x.posts(),1);assert.equal(result.cutoffMs,cutoffMs);
    ...
    const settled=(await x.ref.get()).data();assert.equal(settled.reservedNano,0);assert.equal(settled.pendingAiCount,0);assert.equal(settled.spentNano,14000000);assert.equal(settled.costByStage.shortlist.spentNano,14000000);assert.equal(settled.costByStage.shortlist.requests,1);
  ...
  await check('full_saved_universe_to_luna_50_to_astra_manager_replay_and_costs',async()=>{
    const fake=database(),D=require('./_investorDossier'),M=require('./_investorMarket'),date='2026-09-03',cutoff=M.nyWallClockToUtcMs(date,P.CUTOFFS_ET.evidenceFreezeMin),calls=[];let selected,savedResponse,wall=Date.now();
    const svc=Sim.create({admin:fake,wallNow:()=>wall,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{
      if(opts.method==='GET')return {ok:true,status:200,json:async()=>savedResponse};
      ...
      let output;
      if(b.model==='gpt-5.6-terra') {const input=JSON.parse(user);assert.equal(input.profiles.length,304);assert(input.profiles.every(row=>row[5]===100));selected=input.profiles.slice(0,50).map(row=>({symbol:row[0],reason:'Historical profile selected for deeper comparison'}));output={selected};}
    ...
    const first=await svc.execute(batch.runIds[0]);assert(first.yielded);assert.equal(calls.length,1);assert.equal((await svc.getRun(batch.runIds[0])).aiRecovery,true);
    // Reproduce the old deployed dead end: uncertain request, incomplete card and lost pending count.
    const rr=fake.col('InvestorAI_Simulations').doc(batch.runIds[0]),qr=(await rr.collection('requests').get()).docs[0].ref;await qr.set({status:'uncertain'},{merge:true});
    await rr.set({status:'incomplete',pendingAiCount:0,aiRecovery:false,error:{code:'SIMULATION_USAGE_UNKNOWN',message:'Provider did not report token usage'}},{merge:true});
    await br.set({status:'incomplete'},{merge:true});wall+=10000;const launches=[];await svc.schedule({batchId:batch.batchId,dispatch:async job=>{launches.push(job);return {upstream:202};}});assert(launches.some(j=>j.runId===batch.runIds[0]));
  ...
  function horizonFixture(chosen=3) {
    return Object.fromEntries([1,2,3].map(n=>['session'+n,{expectedReturnBps:n<=chosen?100+(n-1)*130:0,downsideBps:100+(n-1)*20,opportunityCostBps:(n-1)*20,uncertaintyPenaltyBps:10,evidenceConfidence:n<=chosen?'MEDIUM':'LOW',reason:'Dated catalyst timing, adverse gaps and alternate uses of capital considered.'}]));
  }
  async function researchPipeline({failure=null,prepared=false,measure=false,event=false,required=false,picks=2,horizon=false}={}) {
    const fake=database(),D=require('./_investorDossier'),M=require('./_investorMarket'),date=horizon?'2026-08-28':'2026-09-03',cutoff=M.nyWallClockToUtcMs(date,P.CUTOFFS_ET.evidenceFreezeMin),calls=[],responses=new Map(),toolRounds=new Map();
    let finalists=[],bad=failure,missingUsage=failure==='usage',wall=Date.now();const transportErrors=[];
    const svc=(horizon?RealSim:Sim).create({admin:fake,wallNow:()=>wall,env:{OPENAI_API_KEY:'fixture'},publicFetch:async()=>{throw Error('No live source access in pipeline test');},fetchImpl:async(url,opts)=>{try {
      ...
      if(name==='shared_shortlist_v1'){
        ...
        const m=JSON.parse(user.match(/UNIVERSE_MANIFEST=(.*)/)[1]);assert.equal(m.symbols.length,50);finalists=m.symbols.slice(0,picks);
      ...
      } else if(name==='prepared_research_document_v1') {
        assert.equal(b.model,'gpt-5.6-terra');assert(!b.tools);
    ...
    for(const unit of units.filter(x=>x.data().kind==='company')) {
      ...
      if(horizon)data.push({collection:A.COL.claims,id:symbol+'_future',knownAtMs:cutoff+3600000,data:{symbol,text:'FUTURE_HORIZON_SECRET',publishedAtMs:cutoff+3600000}});
      const artifact=await svc.saveJSON(parent,'company',{data,daily,provenance:{provider:'fixture',adjustment:'split_only'}});await unit.ref.set({status:'ready',pointer:{cacheId:parent.id,artifact}},{merge:true});priceSymbols[symbol]={bars:providerSession(date,{extra:false}),coverage:{regular:78,expected:78,missing:0}};
    }
    for(const unit of units.filter(x=>x.data().kind==='prices')) {
      const sessionDate=unit.data().date,index=horizon?require('./_investorSimulationHorizon').sessions(date).findIndex(s=>s.date===sessionDate):0;
      const symbols=horizon?Object.fromEntries(Object.keys(priceSymbols).map(symbol=>[symbol,{bars:providerSession(sessionDate,{extra:false}).map(b=>({...b,o:b.o+index*3,h:b.h+index*3,l:b.l+index*3,c:b.c+index*3})),coverage:{regular:78,expected:78,missing:0}}])):priceSymbols;
      const prices=fake.col('InvestorAI_SimulationScenarios').doc('research_session_'+sessionDate),artifact=await svc.saveJSON(prices,'prices',{symbols,provenance:{provider:'fixture'}});
      await unit.ref.set({status:'ready',pointer:{cacheId:prices.id,artifact}},{merge:true});
    }
    if(picks===2&&!horizon){config.aiPlan={...config.aiPlan,version:'shortlist-50.v1',maxResearchCompanies:2,outputTokens:{...config.aiPlan.outputTokens,decidePreparedPortfolio:18000}};config.investmentPolicy=require('./_investorResearchHandoff').INVESTMENT_POLICY;await br.set({configRef:await svc.saveJSON(br,'saved_legacy_config',config)},{merge:true});}
  ...
  await check('horizon_ranges_required_dates_calendar_and_choice_validation',async()=>{
    ...
    estimates.session2.expectedReturnBps=126;assert.equal(H.choose(estimates),2);estimates.session2.evidenceConfidence='LOW';assert.equal(H.choose(estimates),1);
    assert.throws(()=>H.validate({holdingSessions:3,holdingReason:'Hope of recovery',horizonAnalysis:horizonFixture(1)}),/HORIZON/);
    const recent=RealSim.create({admin:database(),wallNow:()=>Date.UTC(2026,8,6)});await assert.rejects(()=>recent.createBatch({companyRange:'range2',count:1,from:'2026-09-04',to:'2026-09-04'},'operator','future'),/enough distinct/);
  });
  await check('horizon_deadlines_liquidity_gap_stop_resume_and_basis_protection',async()=>{
    const H=require('./_investorSimulationHorizon'),E=require('./_investorExecution'),C=require('./_investorDecisionContext'),M=require('./_investorMarket');
    async function setup(date='2025-11-26',holding=3) {
      const fake=database(),policy=H.policyFor('range1'),schedule=H.sessions(date),content={policy,sessions:schedule,cutoffMs:M.nyWallClockToUtcMs(date,P.CUTOFFS_ET.evidenceFreezeMin),investments:{TEST:{allocationUsd:10000,holdingSessions:holding,holdingReason:'Supported timing',horizonAnalysis:horizonFixture(holding),conviction:'MEDIUM',sizingReason:'Controlled exposure',takeProfitBps:5000,stopLossBps:500}}},plan={...content,planHash:C.hash(content)};
      const scope={runId:RID,investmentPolicy:policy,clock:()=>content.cutoffMs,collection:fake.col,transaction:fake.runTransaction,batch:fake.batch,executionSpreadBps:10,feePerShareMicros:5000};
      await fake.col(A.COL.accounts).doc(RID).set({accountId:RID,startingNavCents:10000000,balanceCents:{cash:10000000,contributed_capital:-10000000}});
      await A.withSimulationScope(scope,()=>E.saveRequiredSimulationPlan({plan,admin:fake,accountId:RID,managerRunId:'fixture'}));
      const tick=(at,bar={o:100,h:101,l:99,c:100,v:100000})=>A.withSimulationScope(scope,()=>E.tickRequiredSimulation({admin:fake,accountId:RID,barsBySymbol:{TEST:bar?[{...bar,t:new Date(at).toISOString()}]:[]},nowMs:at+1200000}));
      const fills=async()=>(await fake.col(A.COL.fills).get()).docs.map(d=>d.data());
      return {fake,schedule,scope,tick,fills};
    }
    const x=await setup();await x.tick(x.schedule[0].openMs);await x.tick(x.schedule[0].closeMs-300000);assert.equal((await x.fills()).length,1);
    // Recovery sees the same bar a second time: no duplicate debit or purchase.
    await x.tick(x.schedule[0].closeMs-300000);await x.tick(x.schedule[1].openMs);await x.tick(x.schedule[1].closeMs-300000);assert.equal((await x.fills()).length,1);
    ...
    for(const last of [null,{o:100,h:101,l:99,c:100,v:1},{o:100,h:101,l:99,c:100,v:100000,halted:true}]) {
      ...
      await assert.rejects(()=>missing.tick(missing.schedule[0].closeMs-300000,last),/HISTORICAL_HOLD_EXIT_UNAVAILABLE|no sufficiently liquid/);assert.equal((await missing.fills()).length,1);
    }
    const late=await setup();await late.tick(late.schedule[1].openMs);assert.equal((await late.fills()).length,0,'entry cannot migrate to a future session');
    const basis=await setup();await basis.tick(basis.schedule[0].openMs);await assert.rejects(()=>basis.tick(basis.schedule[1].openMs,{o:50,h:51,l:49,c:50,v:100000}),/share-basis/);assert.equal((await basis.fills()).length,1,'split-like changes cannot become false investment losses');
  });
  for(const picks of [1,3,8])await check('horizon_'+picks+'_picks_full_pipeline_fixed_deadlines_no_future_AI_or_rebuys',async()=>{
    ...
    const curve=(await x.ref.collection('curve').get()).docs.map(d=>d.data());assert(curve.every(p=>plan.sessions.some(s=>p.atMs>=s.openMs&&p.atMs<=s.closeMs+1200000)));
    const paid=(await x.ref.collection('requests').get()).docs.map(d=>d.data());assert(paid.every(q=>q.clockMs===plan.cutoffMs),'all purchased decisions precede replay');
    const before=JSON.stringify(fills);await x.svc.execute(x.runId);assert.equal(JSON.stringify((await x.ref.collection(A.COL.fills).get()).docs.map(x=>x.data())),before);assert.equal(x.calls.length,picks+3);
  });
  for(const picks of [4,7])await check('diversified_'+picks+'_picks_real_pipeline_caps_total_and_reuses_paid_plan',async()=>{
    const x=await researchPipeline({prepared:true,required:true,picks}),run=await x.drive();
    assert.equal(run.status,'complete',JSON.stringify(run.error));assert.equal(x.calls.length,picks+3);
    const plan=(await x.ref.collection(A.COL.portfolioPlans).doc('required_investment').get()).data();
    assert.equal(plan.policy.version,'required-investment.v2');assert.equal(Object.keys(plan.investments).length,picks);
    assert(Object.values(plan.investments).reduce((n,p)=>n+p.allocationUsd,0)<=95000);
    const fills=(await x.ref.collection(A.COL.fills).get()).docs.map(d=>d.data()).filter(f=>f.side==='buy');
    assert.equal(fills.length,picks);const deployed=fills.reduce((n,f)=>n+Number(f.notionalMinor)+Number(f.feeMinor),0);
    assert(deployed<=9500000&&deployed>9400000);assert(run.spentNano<Sim.CEILING);assert.equal(run.pendingAiCount,0);assert.equal(run.reservedNano,0);
    await x.svc.execute(x.runId);assert.equal(x.calls.length,picks+3);
  });
  await check('diversified_policy_rejects_excess_cap_wrong_counts_and_policy_mismatch_before_fills',async()=>{
    const H=require('./_investorResearchHandoff'),E=require('./_investorExecution'),C=require('./_investorDecisionContext');
    const policy=H.DIVERSIFIED_POLICY,rows=n=>Object.fromEntries(Array.from({length:n},(_,i)=>['S'+i,{allocationUsd:20000}]));
    H.assertInvestmentAllocations(rows(4),policy);
    for(const n of [0,3,8])assert.throws(()=>H.assertInvestmentAllocations(rows(n),policy),/FINALISTS/);
    assert.throws(()=>H.assertInvestmentAllocations(rows(5),policy),/TOTAL_ALLOCATION/);
    const fake=database(),content={policy,investments:rows(5),cutoffMs:1000},plan={...content,planHash:C.hash(content)};
    await A.withSimulationScope({runId:RID,investmentPolicy:policy,clock:()=>1000,collection:fake.col,transaction:fake.runTransaction,batch:fake.batch},async()=>{
      await assert.rejects(()=>E.saveRequiredSimulationPlan({plan,admin:fake,accountId:RID}),/TOTAL_ALLOCATION/);
      await fake.col(A.COL.portfolioPlans).doc('required_investment').set(plan);
      await assert.rejects(()=>E.tickRequiredSimulation({admin:fake,accountId:RID}),/TOTAL_ALLOCATION/);
      assert.equal((await fake.col(A.COL.fills).get()).size,0);
    });
  });
  for(const settled of [false,true])await check('finalization_recovers_obsolete_truncated_intraday_answer_'+settled,async()=>{
    const x=await researchPipeline({prepared:true}),done=await x.drive();assert.equal(done.status,'complete');
    const ref=x.ref,qref=ref.collection('requests').doc('obsolete_intraday'),response={id:'resp_obsolete',status:'incomplete',incomplete_details:{reason:'max_output_tokens'},model:'gpt-6-astra',service_tier:'flex',usage:{input_tokens:1000,output_tokens:1000},output:[]};
    const cost=Sim.price(response.model,response.usage,'flex'),responseRef=await x.svc.saveJSON(ref,'old_response',response);
    // The provider identity and original request are retained even when unused.
    await qref.set({key:'obsolete_intraday',originalKey:'obsolete_intraday',responseId:response.id,responseRef,responseStatus:'incomplete',incompleteReason:'max_output_tokens',model:response.model,tier:'flex',reservation:cost,clockMs:done.shortlist.cutoffMs+3600000,stage:'event',startedAtMs:Date.now(),status:settled?'settled':'pending',actualNano:settled?cost:null});
    await ref.collection(A.COL.modelRequests).doc('obsolete_model').set({fn:'reviseHolding',responseId:response.id,status:'rejected',error:'incomplete'});
    x.responses.set(response.id,response);
    await ref.set({status:'incomplete',work:{stage:'finalize'},error:{code:'SIMULATION_AI_RESPONSE_FAILED',message:'AI response incomplete: max_output_tokens'},spentNano:done.spentNano+(settled?cost:0),reservedNano:settled?0:cost,pendingAiCount:settled?0:1},{merge:true});
    assert((await x.svc.overview({owner:'operator'})).runs[0].canRecheckAI);
    await x.svc.control({runId:x.runId,command:'retry'},'operator');const recovered=await x.drive();
    assert.equal(recovered.status,'complete',JSON.stringify(recovered.error));assert.equal(recovered.spentNano,done.spentNano+cost);assert.equal(recovered.reservedNano,0);assert.equal(recovered.pendingAiCount,0);assert.equal(x.calls.length,5);
    assert.equal((await qref.get()).data().finalizationDisposition,'not_required');assert.equal((await qref.get()).data().responseStatus,'incomplete');
    assert.equal(recovered.buys,done.buys);assert.equal(recovered.pnlMinor,done.pnlMinor);
  });
  await check('simulation_outcomes_fixed_comparison_costs_missing_bars_and_population',()=>{
    const I=require('./_investorSimulationInsights'),at=Date.UTC(2026,8,3,19,55),close=at+300000;
    const investment={symbol:'AAA',boughtShares:50,investedMinor:500025,entryPriceMicros:100000000,exitPriceMicros:105000000,entryAtMs:at-3600000,exitAtMs:at-1800000,heldMs:1800000,status:'CLOSED',pnlMinor:24950,returnBps:499};
    const run={runId:'r',batchId:'b',date:'2026-09-03',status:'complete',investments:[investment],pnlMinor:24950,spentNano:1000000000};
    const plan={planHash:'p',policy:{version:'v'},documentHashes:{AAA:'d'},investments:{AAA:{conviction:'LOW',allocationUsd:5000,sizingReason:'Risk',takeProfitBps:500,stopLossBps:500,evidenceIds:['fact']}}};
    const prices={symbols:{AAA:{bars:[{t:new Date(at).toISOString(),c:110,v:10000}]}}},fills=[{symbol:'AAA',side:'buy'},{symbol:'AAA',side:'sell',role:'TARGET'}];
    const a=I.analyze({run,plan,prices,fills,closeMs:close}),x=a.investments[0];
    assert.equal(x.heldToCloseMinor,49675);assert.equal(x.exitBenefitMinor,-24725);assert.equal(x.exitReason,'TARGET');assert.equal(x.documentHash,'d');assert.deepEqual(x.evidenceIds,['fact']);
    assert.equal(I.analyze({run,plan,prices,fills,closeMs:close+300000}).investments[0].heldToCloseMinor,null);
    assert.equal(I.analyze({run,plan,prices,fills:[...fills,fills[0]],closeMs:close}).investments[0].heldToCloseMinor,null);
    prices.symbols.AAA.bars[0].v=1;assert.equal(I.analyze({run,plan,prices,fills,closeMs:close}).investments[0].heldToCloseMinor,null);
    const summary=I.summarize([{...run,outcomeAnalysis:a},{status:'incomplete',pnlMinor:9999999,spentNano:2000000000},{status:'complete',pnlMinor:0,spentNano:0,investments:[]}]);
    assert.equal(summary.completed,2);assert.equal(summary.wins,1);assert.equal(summary.flat,1);assert.equal(summary.failed,1);assert.equal(summary.pnlMinor,24950);assert.equal(summary.aiCostNano,3000000000);assert.equal(summary.completedAiCostNano,1000000000);assert.equal(summary.tradeWins,1);assert.equal(summary.compared,1);
  });
  await check('simulation_overall_progress_is_monotonic_across_worker_resume',()=>{
    const I=require('./_investorSimulationInsights');
    assert.equal(I.progress({overallProgress:65},{stage:'load_research',done:0,total:67}),65);
    assert.equal(I.progress({managerDone:true,progress:50},{stage:'load_prices'}),89.5);
    assert.equal(I.progress({status:'incomplete',overallProgress:99}),99);
    assert.equal(I.progress({status:'complete'}),100);
    assert.equal(I.progress({},{stage:'screening_profiles',done:160,total:320}),7.5);
  });
  ...
  await check('required_investments_complete_real_pipeline_with_two_purchases_and_no_extra_ai',async()=>{
    ...
    const pointers=await x.svc.readJSON(x.ref,r.repositoryPointersRef);
    const broken=pointers.map(p=>p.unitId.startsWith('company_')?{...p,pointer:{cacheId:'missing_research',artifact:'missing'}}:p);
    const replayRun={...r,repositoryPointersRef:await x.svc.saveJSON(x.ref,'price_only_test',broken)};
    const stages=[],packets=await x.svc.repositoryPackets(replayRun,config,{status:'ready',units:[]},{onProgress:async w=>stages.push(w.stage)});
    assert(packets.packets.every(p=>p.data.length===0));assert(packets.packets.some(p=>p.bars.length));assert(!stages.includes('load_research'));
    const overview=await x.svc.overview({batchId:r.batchId,owner:'operator'});
    assert.equal(overview.outcomes.completed,1);assert.equal(overview.outcomes.positions.length,2);assert.equal(overview.runs[0].outcomeAnalysis.version,'simulation-outcomes.v2');
    assert.equal((await x.svc.getRun(x.runId)).outcomeAnalysis.investments.length,2);assert.equal(x.calls.length,5);
  ...
  await check('prepared_document_binds_exact_sources_retains_risks_and_rejects_unknown_future_or_changed_evidence',()=>{
    ...
    const packet={symbol:'AAA',cutoffMs:cutoff,dossierHash:'source',claims:[claim,risk],fundamentals:{revenueMinor:'12345'}};
    const source=H.sourcePacket(packet,{filings:{facts:[{factId:'f1',filedDate:'2026-09-01',valueScaled:'12345',scale:2}],lineage:[]}});
    const output={schemaVersion:'prepared-research-document.v1',symbol:'AAA',sections:Object.fromEntries(H.SECTIONS.map(k=>[k,{summary:'Evidence summary',evidenceIds:['claim:c1','fact:f1'],missing:[]}]))};
    const doc=H.bindDocument(output,source);assert.equal(doc.baseline.fundamentals.revenueMinor,'12345');assert.equal(doc.evidence.find(e=>e.id==='fact:f1').valueScaled,'12345');assert(doc.evidence.some(e=>e.claimId==='risk'),'mandatory adverse evidence cannot be omitted by Luna');H.assertDocument(doc);
    assert.throws(()=>H.bindDocument({...output,symbol:'BBB'},source),/SYMBOL_MISMATCH/);
    const bad=JSON.parse(JSON.stringify(output));bad.sections.business.evidenceIds=['claim:invented'];assert.throws(()=>H.bindDocument(bad,source),/UNKNOWN_EVIDENCE/);
    doc.evidence[0].text='altered';assert.throws(()=>H.assertDocument(doc),/DOCUMENT_CORRUPT/);
    assert.throws(()=>H.sourcePacket({...packet,claims:[{...claim,publishedAtMs:cutoff+1}]}),/FUTURE_EVIDENCE/);
  });
  await check('prepared_citation_schema_constrains_dense_catalog_and_recovers_only_exact_references',()=>{
    const H=require('./_investorResearchHandoff'),Ajv=require('ajv/dist/2020'),cutoff=Date.UTC(2026,8,3),packet={symbol:'AAA',cutoffMs:cutoff,fundamentals:{revenueMinor:'12345'},claims:[{claimId:'c1',documentVersionId:'d1',claimType:'FACT',text:'Exact supplied claim.'},{claimId:'shared',documentVersionId:'d2',claimType:'RISK_FACTOR',text:'Material risk.'}]};
    const source=H.sourcePacket(packet,{filings:{facts:Array.from({length:400},(_,i)=>({factId:i?'f'+i:'shared',valueScaled:String(i),filedDate:'2026-09-01'})),lineage:[]}}),before=JSON.stringify(source),wire=H.preparationWire(source);
    assert.equal(JSON.stringify(source),before,'wire construction must not mutate the paid request source');
    const validate=new Ajv({strict:false}).compile(wire.schema),output={schemaVersion:'prepared-research-document.v1',symbol:'AAA',sections:Object.fromEntries(H.SECTIONS.map(k=>[k,{summary:'Supported summary',evidenceIds:[wire.source.catalog[0].citationId],missing:[]}]))};
    assert(validate(output),JSON.stringify(validate.errors));let enumCount=0;(function walk(v){if(v&&typeof v==='object'){if(v.enum)enumCount+=v.enum.length;Object.values(v).forEach(walk);}})(wire.schema);assert(enumCount<1000);
    output.sections.business.evidenceIds=['invented'];assert(!validate(output));output.sections.business.evidenceIds=['source:999999'];assert(!validate(output));
    output.sections.business={summary:'Unsupported prose must be discarded',evidenceIds:['c1','shared','invented'],missing:[]};
    output.sections.financialHealth.evidenceIds=['baseline.fundamentals'];
    const doc=H.bindDocument(output,source,{recoverReferences:true});H.assertDocument(doc);
    assert(!JSON.stringify(doc).includes('Unsupported prose must be discarded'));assert.deepEqual(doc.sections.business.evidenceIds,['claim:c1']);assert.deepEqual(doc.sections.financialHealth.evidenceIds,['baseline:fundamentals']);
    assert.deepEqual(doc.referenceRecovery.discardedSections,['business']);assert(doc.referenceRecovery.unknownEvidenceIds.includes('shared'),'ambiguous raw IDs cannot be guessed');assert.equal(doc.evidence.filter(x=>x.kind==='financial_fact').length,400);assert(doc.evidence.some(x=>x.claimType==='RISK_FACTOR'));
    assert.equal(JSON.stringify(source),before);assert.throws(()=>H.bindDocument({...output,symbol:'BBB'},source,{recoverReferences:true}),/SYMBOL_MISMATCH/);
  });
  await check('prepared_saved_unknown_citations_resume_without_rebuying_failed_luna_or_earlier_steps',async()=>{
    const H=require('./_investorResearchHandoff'),bind=H.bindDocument,x=await researchPipeline({prepared:true,failure:'document_citations'});let failed;
    // Recreate the old binder rejecting a paid, otherwise schema-valid answer.
    H.bindDocument=(output,source)=>bind(output,source);
    try{failed=await x.drive();}finally{H.bindDocument=bind;}
    assert.equal(failed.status,'incomplete');assert.equal(failed.error.details.failed[0].error,'HANDOFF_UNKNOWN_EVIDENCE');assert.equal(x.calls.length,3);
    const request=(await x.ref.collection('requests').where('responseId','==',failed.error.details.failed[0].responseId).get()).docs[0],saved=JSON.stringify(request.data());
    const view=await x.svc.overview({owner:'operator'});assert(view.runs[0].canRecoverHandoff);assert(!view.runs[0].canRetryAIStep);
    await x.svc.control({runId:x.runId,command:'retry'},'operator');assert.equal((await x.svc.getRun(x.runId)).aiRequestRetries,undefined);assert.equal(x.calls.length,3);
    x.setValid();const done=await x.drive();assert.equal(done.status,'complete',JSON.stringify(done.error));assert.equal(x.calls.length,5,'only the second document and joint decision are newly purchased');
  ...
  await check('single_uncapped_measurement_finishes_all_steps_above_normal_ceiling_and_reports_actual_costs',async()=>{
    ...
    const requests=(await x.ref.collection('requests').get()).docs.map(d=>d.data());assert.equal(requests.reduce((n,q)=>n+q.actualNano,0),done.spentNano);assert.equal(Object.values(done.costByStage).reduce((n,q)=>n+q.spentNano,0),done.spentNano);
    await assert.rejects(()=>x.svc.createBatch({count:2,from:'2026-04-22',to:'2026-04-23',budgetMode:'measure_actual_cost'},'operator','invalid-uncapped-batch'),/exactly one/);
    const capped=await x.svc.createBatch({count:1,from:'2026-04-23',to:'2026-04-23'},'operator','normal-budget-after-measurement');assert.equal((await x.svc.getRun(capped.runIds[0])).ceilingNano,Sim.CEILING);
    await x.svc.execute(x.runId);assert.equal(x.calls.length,5);
  });
  await check('prepared_joint_buy_retains_verified_valuation_mandate_rank_and_source_gates',()=>{
    const H=require('./_investorResearchHandoff'),mandate=JSON.parse(JSON.stringify(P.EXAMPLE_MANDATE_PROPOSAL)),cutoff=Date.parse(mandate.asOf);
    const valuation={horizonTradingDays:mandate.forecast.horizonTradingDays,method:'comparable_multiples',methodRationale:'Forward scenario assumptions',forwardInputBasis:'assumption_only',guidanceClaimId:null,preferredValuationZone:null,
      scenarios:mandate.forecast.outcomeBuckets.map(b=>({id:b.id,probabilityPpm:b.probabilityPpm,realizationPpm:'1000000',terminalPriceMicros:b.terminalPriceMicros,assumptions:[{name:'metricPerShareMicros',value:b.terminalPriceMicros,unit:'micros'},{name:'multipleMilli',value:'1000',unit:'milli'}]}))};
    const memo={schemaVersion:'research-memo.v1',symbol:mandate.symbol,asOf:mandate.asOf,checklist:Object.fromEntries(['business','whatChanged','agreementDisagreement','risks','valuationFramework','disconfirmingEvidence','returnAndHorizon','versusAlternatives','mandateOrAbstain'].map(k=>[k,'Source-linked underwriting.'])),factualPremises:[],inferences:[],valuation,bearCase:'Loss scenario',thesisHealth:'INTACT',proposedDecision:'BUY',reasonCode:null,mandate};
    const packet={symbol:mandate.symbol,cutoffMs:cutoff,claims:mandate.sourceManifest.map(m=>({claimId:m.claimId,documentVersionId:m.documentVersionId})),dossierHash:'frozen'};
    const output={schemaVersion:'prepared-investment-decision.v1',research:[memo],allocation:{schemaVersion:'portfolio-synthesis.v1',planClass:'EXPANSION',decisions:[{symbol:mandate.symbol,decision:'BUY',capitalRank:mandate.allocation.capitalRank,reasonCode:null,fundingState:'FUNDED',reason:'Positive verified forecast'}],expansionMandates:[mandate],holdingAnalysis:[],comparisonNote:'Selected against cash'}};
    assert(H.validateJoint(output,[packet])[mandate.symbol].bindingHash);
    for(const [mutate,error] of [
      [x=>x.allocation.expansionMandates[0].action.entry.limitPriceMicros='1',/CONFLICTING_MANDATE/],
      [x=>x.allocation.decisions[0].capitalRank=4,/RANK_MISMATCH/],
      [x=>x.research[0].valuation.scenarios[0].terminalPriceMicros='1',/TERMINAL_PRICE_NOT_CALCULATOR_BOUND/],
      [x=>x.research[0].mandate.sourceManifest[0].documentVersionId='invented',/SOURCE_MISMATCH/]]) {
      const bad=JSON.parse(JSON.stringify(output));mutate(bad);assert.throws(()=>H.validateJoint(bad,[packet]),error);
    }
    assert.throws(()=>H.validateJoint(output,[packet],[],true),/EXPANSION_BLOCKED/);
  });
  await check('full_prepared_documents_joint_astra_decision_and_replay_reuses_paid_background_response',async()=>{
    const x=await researchPipeline({prepared:true,failure:'usage'}),r=await x.drive();assert.equal(r.status,'complete',JSON.stringify(r.error));
    assert.deepEqual(x.calls.map(b=>b.text.format.name),['historical_shortlist','universe_review_v1','prepared_research_document_v1','prepared_research_document_v1','prepared_investment_decision_v1']);
    ...
    assert.equal(r.costByStage.manager_document.requests,2);assert.equal(r.costByStage.manager_decision.requests,1);assert.equal(r.costByStage.manager_synthesis,undefined);
    const requests=(await x.ref.collection('requests').get()).docs.map(d=>d.data());assert.equal(r.spentNano,requests.reduce((n,q)=>n+q.actualNano,0));
    const detail=await x.svc.detail(x.runId,'operator');assert.equal(detail.preparedDocuments.length,2);
    await x.svc.execute(x.runId);assert.equal(x.calls.length,5);
  });
  await check('prepared_budget_resume_keeps_old_paid_memo_and_replaces_only_unfinished_research',async()=>{
    const x=await researchPipeline({failure:'research'}),r=await x.drive();assert.equal(r.status,'incomplete');
    const cp=await x.svc.readJSON(x.ref,r.managerCheckpointRef);assert.equal(cp.data.research.completed.length,1);
    const before=x.calls.length,spent=r.spentNano;
    await x.ref.set({error:{code:'SIMULATION_BUDGET_INSUFFICIENT'},budgetFailure:{code:'SIMULATION_BUDGET_INSUFFICIENT'}},{merge:true});
    await x.svc.control({runId:x.runId,command:'retry'},'operator');x.setValid();const done=await x.drive();
    assert.equal(done.status,'complete',JSON.stringify(done.error));assert(done.spentNano>spent);assert.equal(x.calls.length,before+2);
    assert.equal(x.calls.filter(b=>b.text.format.name==='prepared_research_document_v1').length,1);assert.equal(x.calls.filter(b=>b.text.format.name==='prepared_investment_decision_v1').length,1);
    const old=(await x.ref.collection(A.COL.modelRequests).where('fn','==','researchCompany').get()).docs.map(d=>d.data());assert(old.some(q=>q.status==='superseded'&&q.supersededStatus==='rejected'));assert(old.some(q=>q.status==='complete'));
  });
  await check('prepared_joint_schema_retry_preserves_both_luna_documents_and_only_rebuys_failed_decision',async()=>{
    const x=await researchPipeline({prepared:true,failure:'joint_schema'}),r=await x.drive();assert.equal(r.status,'incomplete');assert.equal(r.error.code,'SIMULATION_RESEARCH_INCOMPLETE');assert.equal(x.calls.length,5);
    assert((await x.svc.overview({owner:'operator'})).runs[0].canRetryAIStep);await x.svc.control({runId:x.runId,command:'retry'},'operator');x.setValid();
    const done=await x.drive();assert.equal(done.status,'complete',JSON.stringify(done.error));assert.equal(x.calls.length,6);assert.equal(x.calls.filter(b=>b.text.format.name==='prepared_research_document_v1').length,2);
  });
  ...
  await check('shared_total_budget_has_no_stage_holdbacks_and_actual_cost_uses_returned_tier',async()=>{
    ...
    await x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',stage:'manager_review',body:{...body,max_output_tokens:24000}});assert.equal(x.posts(),1);assert.equal(postedBody.max_output_tokens,31920);
  ...
  await check('large_output_allowance_fits_remaining_total_before_paid_submission',async()=>{
    let posted;const x=await metered(async(url,opts)=>{posted=JSON.parse(opts.body);return {ok:true,status:200,json:async()=>completed};});
    await x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body:{...body,max_output_tokens:60000}});
    assert.equal(x.posts(),1);assert.equal(x.counts(),1);assert.equal(posted.max_output_tokens,75344);const r=(await x.ref.get()).data();assert.equal(r.reservedNano,0);assert.equal(r.spentNano,30000000);const q=(await x.ref.collection('requests').get()).docs[0].data();assert(q.reservation<=Sim.CEILING);
  });
  await check('budget_reservation_is_atomic_across_concurrent_calls',async()=>{const x=await metered(async()=>({ok:true,status:200,json:async()=>({id:'resp_fixture',status:'in_progress'})}));await x.ref.set({spentNano:Sim.CEILING-95000000},{merge:true});await Promise.allSettled([1,2,3].map(i=>x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body:{...body,max_output_tokens:2048,input:'request '+i}})));const r=(await x.ref.get()).data();assert(r.spentNano+r.reservedNano<=Sim.CEILING);assert.equal(x.posts(),1);});
  ...
  await check('missing_usage_checks_saved_response_five_times_without_repurchase_or_false_zero',async()=>{
    let ready=false,gets=0;const x=await metered(async(u,o)=>{if(o.method==='GET')gets++;return {ok:true,status:200,json:async()=>ready?completed:{...completed,usage:null}};});
    for(let i=1;i<=5;i++)await assert.rejects(()=>x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body}),e=>e.code===(i<5?'SIMULATION_USAGE_PENDING':'SIMULATION_USAGE_UNKNOWN'));
    assert.equal(x.posts(),1);assert.equal(gets,4);const blocked=(await x.ref.get()).data();assert(blocked.reservedNano>0);assert.equal(blocked.spentNano,0);assert.equal(blocked.pendingAiCount,1);
    const q=(await x.ref.collection('requests').get()).docs[0];assert.equal(q.data().status,'awaiting_usage');assert.equal(q.data().responseStatus,'completed');assert.equal(q.data().usageChecks,5);
    ready=true;await x.meter.drain({recovery:true});await x.meter.drain();assert.equal(x.posts(),1);assert.equal((await x.ref.get()).data().spentNano,30000000);assert.equal((await x.ref.get()).data().reservedNano,0);
  });
  await check('failed_background_response_preserves_provider_error_and_recheck_never_posts',async()=>{
    const failure={id:'resp_fixture',status:'failed',usage:null,error:{code:'server_error',message:'Provider could not finish the background response'}};
    let gets=0;const x=await metered(async(u,o)=>{if(o.method==='GET')gets++;return {ok:true,status:200,json:async()=>failure};});
    await assert.rejects(()=>x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body}),e=>e.code==='SIMULATION_AI_RESPONSE_FAILED'&&e.message.includes('server_error'));
    const q=(await x.ref.collection('requests').get()).docs[0];assert.equal(q.data().providerError.message,failure.error.message);assert.equal(q.data().responseStatus,'failed');
    const br=x.fake.col('InvestorAI_SimulationBatches').doc(BID),configRef=await x.svc.saveJSON(br,'config',{});await br.set({batchId:BID,owner:'operator',configRef,runIds:[RID],status:'incomplete'});
    await x.ref.set({batchId:BID,status:'incomplete',leaseUntil:0,aiRecheckRequired:true,error:{code:'SIMULATION_AI_RESPONSE_FAILED',message:'failure'}},{merge:true});
    const before=(await x.ref.get()).data();await assert.rejects(()=>x.svc.control({runId:RID,command:'retry'},'intruder'));
    await x.svc.control({runId:RID,command:'retry'},'operator');await x.svc.execute(RID);const after=await x.svc.getRun(RID);
    assert.equal(x.posts(),1);assert.equal(gets,1);assert.equal(after.status,'incomplete');assert.equal(after.aiRecheckRequired,true);assert.equal(after.error.details.providerError.code,'server_error');assert.equal(after.reservedNano,before.reservedNano);assert.equal(after.spentNano,0);
    const overview=await x.svc.overview({owner:'operator',batchId:BID});assert.equal(overview.runs[0].canRecheckAI,true);
    await x.svc.schedule({dispatch:async()=>{throw Error('must not repeatedly relaunch a failed response');}});assert.equal(gets,1);
  });
  await check('unknown_submission_remains_unresolved_when_draining_other_responses',async()=>{
    const x=await metered(async()=>{throw Error('network closed after submission');});await assert.rejects(()=>x.meter.request({method:'POST',url:'https://api.openai.com/v1/responses',body}));
    assert.equal(await x.meter.drain(),1);assert.equal((await x.ref.get()).data().pendingAiCount,1);assert((await x.ref.get()).data().reservedNano>0);assert.equal(x.posts(),1);
  });
  ...
  await check('shared_preparation_budget_block_then_accepted_response_replay_to_session_close',async()=>{
    ...
    try {
      ...
      let submitted=0,priceRequests=0,prepared=false,blockInput=true,wall=Date.now();
      const svc=Sim.create({admin:fake,wallNow:()=>wall,publicFetch:sec.publicFetch,env:{OPENAI_API_KEY:'fixture'},fetchImpl:async(url,opts)=>{
        ...
        if(url.endsWith('/input_tokens'))return {ok:true,status:200,json:async()=>({input_tokens:blockInput?5000000:1000})};
      ...
      const blocked=await svc.execute(b.runIds[0]),blockedRun=await svc.getRun(b.runIds[0]);
      assert.equal(blockedRun.error.code,'SIMULATION_BUDGET_INSUFFICIENT');assert.equal(submitted,0);assert.equal(blockedRun.spentNano,0);
      assert(blockedRun.error.message.includes('No tokens were purchased'));
      // Separately exercise replay from an already accepted response fixture.
      // An unaffordable input cannot bypass the overall budget. The saved accepted answer is reused.
      const mr=(await ref.collection(A.COL.modelRequests).get()).docs.find(d=>d.data().fn==='reviewUniverse');assert(mr);
      const review={schemaVersion:'universe-review.v1',universeVersion:config.roster.universeVersion,universeHash:config.roster.universeHash,eligibleCount:config.roster.eligibleCount,coverage:config.roster.symbols.map(symbol=>({symbol,reviewDirective:'NONE',provisionalDisposition:'IGNORE',reason:'No opportunity in this fixture',changedSincePrior:false,reasonCode:null})),holdingAnalysis:[],researchRequests:[],managerNote:'Previously accepted fixture review'};
      await mr.ref.set({status:'complete',output:review,returnedModel:'gpt-6-astra',costMinor:'0',tokens:{},responseId:'accepted_fixture'},{merge:true});
      const managerCp=await svc.readJSON(ref,blockedRun.managerCheckpointRef);managerCp.stage='review';delete managerCp.data.review;delete managerCp.data.noBuyReasons;managerCp.data.expansionBlocked=false;
      await ref.set({status:'queued',error:null,budgetFailure:null,managerCheckpointRef:await svc.saveJSON(ref,'funded_fixture_checkpoint',managerCp)},{merge:true});
      ...
      assert(stages.filter(s=>s.stage==='replay').some(s=>s.done>0&&s.done<s.total));assert.equal(run.spentNano,0);
      fake.runTransaction=transaction;
      assert.equal(run.status,'complete',JSON.stringify({result,error:run.error}));assert.equal(run.returnBps,0);assert.equal(run.progress,100);assert.equal(submitted,0);assert.equal(priceRequests,repository.companies+repository.dates+1);
      ...
      blockInput=false;const prior=priceRequests,b2=await svc.createBatch({count:1,from:'2026-09-03',to:'2026-09-03'},'operator','reuse');
