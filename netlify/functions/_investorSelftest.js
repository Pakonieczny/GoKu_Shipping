...
async function simulatorAdversarial({only=null}={}) {
  ...
  await check('learning_parallel_collection_handles_296_records_without_minute_roundtrips',async()=>{
    ...
    const v=await svc.view('operator');assert.equal(v.current.recordCount,296);assert.equal(v.current.statistics.failed,296);assert.equal(v.current.eligibleCount,0);assert(v.current.collectionCompletedAtMs);assert.equal(posts,0);assert(v.current.reviewIndex>0,'worker continues local diagnostic groups without a scheduler delay');
  ...
  await check('learning_resume_recovers_paid_verbose_answer_and_compacts_only_unpurchased_pages',async()=>{
    ...
    const row=i=>({runId:'sim_'+String(i).padStart(24,'0'),date:'2026-04-01',status:'incomplete',eligible:false,cohort:'legacy',shortlistInput:{huge:'x'.repeat(200000)}});
    ...
    await svc.control('operator',a.analysisId,'resume');await svc.execute(a.analysisId);let saved=(await ref.get()).data();assert.equal(saved.datasetCollection,'pages_v2');assert.equal(saved.pageCount,3);assert.equal(saved.statistics.total,25);
    await svc.execute(a.analysisId);view=await svc.view('operator',{evidencePage:0});assert.equal(view.evidence.cases[0].finding.length,4000,'paid prose retained in full');assert.equal(view.current.reviewIndex,1);assert.equal(view.current.spentNano,5619000000);assert.equal(paid,0);assert.equal((await ref.collection('pages').get()).size,25,'original pages remain untouched');
    await svc.execute(a.analysisId);assert.equal(paid,0,'uncompleted-only groups require no paid historical search');
  });
  ...
  await check('learning_evidence_is_compact_and_price_sampling_is_explicit',async()=>{
    ...
    assert(L.evidenceGroups(Array.from({length:30},(_,i)=>({runId:String(i),status:i%2?'complete':'incomplete'}))).every(g=>g.length<=12&&g.every(r=>r.status===g[0].status)));
  });
