...
function buildResponsiveRequest({evidence,request,screenshotDataUrl,sources=[]}){
  const instructions=`${require('../../brites-brand-assets').guidance} You are the Brites Jewelry creative director. Research the exact product and design a NEW purpose-built photographic scene plus a coordinated family of responsive ads. The operator explicitly wants a reimagined product photograph plus smart, attractive messaging and layout. Creative priorities are messaging 30%, layout 25%, buyer/product relevance 20%, visual appeal 20%, and product recognition 5%. Optimize the complete ad; do not spend effort chasing microscopic charm fidelity at the expense of clear, persuasive copy or attractive composition. Use product references to preserve its exact silhouette, engraving, metal, chain or earrings, count and true physical proportions. The setting, lighting, camera and framing should improve product clarity and fit the audience, without promising measured advertising success.
Preferred house style: the approved peach-charm ad. Use edge-to-edge atmospheric photography, a soft tonal fade under editable overlay copy, an elegant serif headline with restrained sans-serif supporting text where legible, and a coordinated CTA. Keep the jewelry large, crisp and entirely outside the text/fade area. Plan a quiet left region for landscape copy with the product toward the right; square and portrait need a quiet lower region with the product above. The fade is an editable layer added by the renderer, never baked-in text or a button. Match background, ink, accent and buttonInk to the actual photographic palette, with strong readable contrast. Prefer warm ivory, cocoa and soft peach only when those tones suit the scene; cool scenes need their own coordinated palette. Use a simpler compact caption when a tiny placement or product framing makes overlay impractical. In imageDirections explicitly reserve the required quiet region while keeping the charm prominent. Generate ONE reusable master scene by default. Choose a master aspect ratio that can be cropped safely into square, portrait and landscape. Keep the entire product within a crop-safe region that can be positioned right of landscape overlay copy or above portrait overlay copy, with sufficient photographic context for that fade. The visible jewelry must dominate the image, with minimal empty background. Choose a close square or portrait master for small charms; do not default to a distant landscape flat lay. Necklaces should emphasize the pendant with enough chain to communicate the piece, rather than letting an entire spread-out chain make the pendant tiny. Authorize at most ONE alternate image only when cropping the master would materially cut off the chain, earrings or charm, or make the product illegible; give a concrete reason. Do not order one image per placement. If needed, the second imageDirection is the alternate view of the SAME item and setting.
...
Also propose a 10-second product film: an immediate visual hook, intentional camera movement and a moving real-light reflection that reveals the metal finish, followed by a clear closing product view. Avoid static slides, artificial sparkle stars and invented stones. mobilePrompt prioritizes jewelry clarity in a vertical crop; desktopPrompt gives scene context in horizontal framing. Include a square-safe central product area in both. A referenced adult model may demonstrate the jewelry with natural anatomy and subtle movement. mobileCaption and desktopCaption independently decide whether a concise closing caption improves the ad. Animation uses a dedicated product-specific directing pass that may choose a different setting, props, light or adult model from the static scene while preserving the exact jewelry, approved brand palette, typography and verified messaging. Return a concise JSON recipe in the schema, under 11000 characters. Cite the exact product source ID and an exact available quote for each factual claim. Choose fact-grounded, specific copy: headline is a distinctive buyer or occasion hook (at most 30 characters); shortHeadline identifies the product (at most 20); description adds one supported purchase reason without repeating the product name (at most 65); CTA is at most 18 characters. Give native descriptions distinct supported purposes rather than paraphrasing the same benefit: use or recipient, packaging, material or customization, and dispatch planning when the evidence supports them. Dispatch timing is not delivery timing. No prices, promotions, invented materials or unsupported social proof. Use real hex colors, strong contrast and at most two supported fonts. layouts must include each of the five families exactly once for EACH device (10 entries). The operator estimates 80% mobile traffic: prioritize immediate jewelry recognition, restrained zoom, fewer words, minimal margins, the largest possible product photograph and sound-off comprehension. Desktop follows the same image-first rule; extra text is optional and must not shrink the product. Always retain the product-identifying shortHeadline and recognizable Brites branding. Omit descriptions, promotional hooks and oversized drawn buttons before compromising these two priorities. Never create microtext. Use at least 12 CSS pixels for essential brand text and 16 for product names at native small-banner size. Never omit the real product. zoom=1 preserves the whole photo region; values up to 1.35 enlarge it without distorting proportions. focalX/focalY are normalized 0–1 crop positions. Keep the entire product visible. Native clickable Google controls are separate from these optional artwork elements. nativeCopy must include at least one headline of 15 characters or fewer and one description of 60 characters or fewer. nativeCopy supplies concise, varied headlines, long headlines and descriptions for Google responsive ads; every factual claim must cite its exact product source. photoFraction is between .75 and 1. Use a compact caption or text in safe negative space; omit supporting copy before reducing the image. Explain camera decisions and where the charm must remain crop-safe in imageDirections. Your single-item photograph must always show the referenced product. AlternateNeeded=false is the economical default.`;
  ...
  return {model:MODEL,store:false,reasoning:{effort:'high'},max_output_tokens:12000,input:[{role:'developer',content:instructions},{role:'user',content}],text:{format:{type:'json_schema',name:'brites_responsive_ad',strict:true,schema:sourceBoundSchema(responsiveSchema,evidence)}}};
}
function validateResponsivePlan({output,request,evidence}){
  ...
  const plan=JSON.parse(JSON.stringify(output));plan.style.treatment='soft-fade';plan.alternateNeeded=plan.alternateNeeded===true&&plan.alternateFormat!==plan.masterFormat&&['landscape','square','portrait'].includes(plan.alternateFormat)&&String(plan.alternateReason||'').length>20&&!!plan.imageDirections[1]?.composition;
  return plan;
}
function applyEditorPlan({output,request,evidence,sources=[]}){
  ...
  for(const addition of output.additions){if(!/^ai_[a-zA-Z0-9_-]{1,80}$/.test(addition.id)||byId.has(addition.id))fail('invalid new layer ID.');
    ...
    if(addition.type.startsWith('brand-')){const a=require('../../brites-brand-assets').get('brites_brand_'+addition.type.slice(6)),c=a.crop,scale=Math.min(request.artboard.width*.3/c.width,request.artboard.height*.18/c.height);Object.assign(o,{editorRole:'brand',sourceKey:a.id,width:c.width,height:c.height,cropX:c.x,cropY:c.y,scaleX:scale,scaleY:scale});}
