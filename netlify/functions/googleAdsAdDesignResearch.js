...
Generate ONE reusable master scene by default. Choose a master aspect ratio that can be cropped safely into square, portrait and landscape. Keep the entire product within a crop-safe central region and leave context around it. The visible jewelry must dominate the image, with minimal empty background. Choose a close square or portrait master for small charms; do not default to a distant landscape flat lay. Necklaces should emphasize the pendant with enough chain to communicate the piece, rather than letting an entire spread-out chain make the pendant tiny. Authorize at most ONE alternate image only when cropping the master would materially cut off the chain, earrings or charm, or make the product illegible; give a concrete reason. Do not order one image per placement. If needed, the second imageDirection is the alternate view of the SAME item and setting.
...
function validateResponsivePlan({output,request,evidence}){
  ...
  const plan=JSON.parse(JSON.stringify(output));plan.alternateNeeded=plan.alternateNeeded===true&&plan.alternateFormat!==plan.masterFormat&&['landscape','square','portrait'].includes(plan.alternateFormat)&&String(plan.alternateReason||'').length>20&&!!plan.imageDirections[1]?.composition;
  return plan;
}
