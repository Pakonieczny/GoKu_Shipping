...
function applyEditorPlan({output,request,evidence,sources=[]}){
  ...
  for(const claim of output.factClaims){const source=available.get(claim.sourceId);if(!source||!['product:'+request.productId,'landing'].includes(claim.sourceId))fail('the claim “'+str(claim.claim,150)+'” cites a different or unavailable product source.');const corpus=norm(JSON.stringify(source.data));if(!str(claim.claim)||!str(claim.quote)||!corpus.includes(norm(claim.quote))||!corpus.includes(norm(claim.claim)))fail('the claim “'+str(claim.claim,150)+'” with quote “'+str(claim.quote,200)+'” is not found in '+claim.sourceId+'.');}
