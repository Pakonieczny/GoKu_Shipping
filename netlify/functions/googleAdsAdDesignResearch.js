...
FRESH RESEARCH PACKAGE: ${JSON.stringify(requestEvidence)}`;
  ...
  function validateResult({output,evidence,channel,group}={}){
    ...
    const factClaims=(multi?(output.factClaims||[]):(output.factClaims||[]).slice(0,12)).map(f=>{const source=sources.get(String(f.sourceId)),quote=str(f.quote,450),claim=str(f.claim,250);if(!source||!quote||!claim)throw new Error('A factual claim is missing its source quote.');if(multi){const pid=String(f.productId||'');if(pid?(!evidence.sourceBindings.productIds.includes(pid)||f.sourceId!=='product:'+pid):f.sourceId!=='landing')throw new Error('A factual claim was transferred to a different product or an unverified uploaded photo.');}const corpus=norm(JSON.stringify(source.data));if(!corpus.includes(norm(quote))||!corpus.includes(norm(claim)))throw new Error('A factual claim is not supported by its exact source.');return {claim,sourceId:f.sourceId,quote,...(multi?{productId:String(f.productId||'')}:{})};});
