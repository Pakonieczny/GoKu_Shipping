...
const productSceneGuidance = 'Choose a fresh product-specific scene from the selected charm motif, metal, style and intended mood. Coordinate the photographic background, lighting, subtle supporting props, copy-area fade and CTA as one palette. Do not inherit a previous product’s peach, ivory or other setting merely because its layout was approved. State the chosen palette and the connection between setting/props and this product in each image direction. For a crocodile, a restrained sage, muted river-stone or botanical setting can complement the gold; these are creative possibilities, not mandatory green water or a reason to recolor the jewelry. Every new scene MUST include one or two well-placed, recognizable supporting props that tell a story tied to the charm meaning and design. Explain the specific prop choice and placement in each direction. For a crocodile, use a softly lit riverbank vignette with a delicate reed or small botanical leaf and a smooth river pebble, not the same tabletop tinted green. Design a new physical environment, surface arrangement and depth, not a colour wash over a previous backdrop. Keep props secondary, outside the copy zone, and never imply extra included merchandise. Use soft pastel, low-saturation backgrounds and gentle diffused light throughout. Let the jewelry or a restrained scene accent carry colour; headlines and supporting text must be dark near-neutral charcoal, not colourful green or other saturated ink. CTA colours must be muted and readable. Continue real photographic texture behind the messaging with a long gentle translucent fade, still visibly transparent at 75 percent of image height, rather than a solid footer. Honor explicit operator art direction. Keep each format in the same product-specific visual family, adapting prop placement and framing to its shape.';
...
const responsiveSchema=object({motion:object({concept:text,mobilePrompt:text,desktopPrompt:text,mobileCaption:{type:'boolean'},desktopCaption:{type:'boolean'}}),productId:text,groupRef:text,rationale:text,
  scenePlans:{type:'array',minItems:5,maxItems:6,items:object({key:{type:'string',enum:['landscape','square','portrait','tall','banner','slim']},reason:text,direction:object({concept:text,composition:text,lighting:text,background:text,preserveProduct:strings,avoid:strings,sourceIds:strings})})},
...
function validateResponsivePlan({output,request,evidence}){
  ...
  if(output.scenePlans!==undefined){
    ...
    if(!Array.isArray(ids)||ids.length<5||ids.length>6||new Set(ids).size!==ids.length||['landscape','square','portrait','tall','banner'].some(k=>!ids.includes(k)))throw Error('Provide one scene for each of the five compatible aspect-ratio families.');
...
module.exports={productSceneGuidance,buildSubjectFocusRequest,validateSubjectFocus,createAdDesignResearch,parseResponse,compactEvidence,MODEL,schema,buildEditorRequest,buildResponsiveRequest,validateResponsivePlan,applyEditorPlan,EDITOR_FONTS,EDITOR_PROPERTIES};
