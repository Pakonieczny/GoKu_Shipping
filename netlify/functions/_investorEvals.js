...
const Simulator = (() => {
  ...
  const retryableResearchError = error => error==='schema_invalid'||/^HANDOFF_/.test(error||'')||['SIMULATION_INVESTMENT_PLAN_INVALID','SIMULATION_TOTAL_ALLOCATION_INVALID','SIMULATION_ALLOCATION_INVALID','SIMULATION_HORIZON_INVALID'].includes(error);
  ...
  function simulationRequestBody(body,stage) {
    ...
    if(stage==='manager_decision'&&out.model==='gpt-6-astra')out.input.push({role:'system',content:'HOLDING STRATEGY: Prefer evidenced multi-hour trades within the chosen 1–3-session deadline, not minute-scale scalping. Set stopLossBps and takeProfitBps together using only supplied pre-cutoff price variability, adverse scenarios, catalyst timing and costs. Explain in outlook why ordinary opening volatility should not invalidate the thesis, what loss invalidates it, and why the profit target suits the intended duration. Each company baseline.marketObservation.technicals block supplies pre-cutoff measurements (atr14Bps, avgDailyRange20Bps, gapAbsMean20Bps, gapAbsP90_60Bps, closeVsMa50Bps, closeVsMa200Bps, relativeToMarket20dBps/60dBps, avgDollarVolume20Usd); the SPY entry in historical_market.observations carries the same measurements for the whole market. Use them: for a one-session hold, a stop tighter than about 1.0x atr14Bps or the typical overnight gap is likely to trigger on ordinary noise; for two or three sessions, allow roughly 1.5x-2x atr14Bps; place the target at least 1.5x the stop distance unless a dated catalyst justifies otherwise, and reduce allocation rather than tighten the stop when the resulting dollar risk is too large. If technicals are unavailable, say so; do not invent ATR, probabilities or a universal tight percentage. Reduce allocation when a defensible wider stop implies too much dollar risk. Do not widen stops merely to force longer holds or suppress a genuine protective exit. A session allowance is a maximum, not a promise to hold. Round-trip friction is about 25 bps (10 bps spread each way plus $0.005/share), so an expectedReturnBps that does not clearly exceed that hurdle is not worth buying beyond the policy minimum. Entry occurs at the observed bar open. The target is active from that first bar. The protective stop is NOT evaluated during the first 30 minutes after entry; in that window only an emergency stop at twice the stop distance applies, so ordinary opening volatility cannot end the trade, and after it the stop is checked on every five-minute bar. Size the stop for the hold, not the open. An opening trigger precedes later extrema; otherwise if both thresholds occur in the same five-minute bar, stop-first is a conservative unresolved ordering, not a known price path. No prices after the historical cutoff may inform this decision.'});
  ...
  function shortlistProfiles(packets,roster,cutoffMs) {
    const D=require('./_investorDossier'),T=require('./_investorTechnicals'),by=new Map(packets.map(p=>[p.symbol,p]));
    const spy=by.get('SPY'),marketSeries=C.completedDaily(spy?.daily?.series||require('./_investorHistory').seriesFromDailyDoc(spy?.daily||{}),cutoffMs);
    return roster.symbols.map(symbol=>{
      const p=by.get(symbol),visible=(p?.data||[]).filter(x=>x.knownAtMs<=cutoffMs);
      const version=decode(visible.filter(x=>x.collection===A.COL.dossierVersions&&C.availableAt(decode(x.data))<=cutoffMs).sort((a,b)=>a.knownAtMs-b.knownAtMs).at(-1)?.data);
      const member=roster.members?.find(x=>x.symbol===symbol)||{},series=C.completedDaily(p?.daily?.series||require('./_investorHistory').seriesFromDailyDoc(p?.daily||{}),cutoffMs),last=series.at(-1),returns=D.returnsBps(series,{asOfMs:cutoffMs});
      const prior=series.slice(-21,-1),volume=prior.length?prior.reduce((n,b)=>n+(Number(b.v)||0),0)/prior.length:0;
      const docs=visible.filter(x=>x.collection===A.COL.documents&&x.knownAtMs>=cutoffMs-30*86400000).sort((a,b)=>b.knownAtMs-a.knownAtMs).slice(0,2);
      return {symbol,name:version?.identity?.name||member.company||symbol,sector:version?.identity?.sector||member.sector||null,
        researchAsOfMs:version?.asOfMs||null,priceDate:last?.date||null,price:last?.c||null,returnsBps:returns.ok?returns.bps:null,
        volumeRatio:volume?Math.round(Number(last.v)/volume*100)/100:null,
        revenueGrowthBps:version?.fundamentals?.revenueGrowthBps??null,fcfMarginBps:version?.fundamentals?.fcfMarginBps??null,netDebtEbitdaMilli:version?.fundamentals?.netDebtEbitdaMilli??null,
        trailingMultipleMilli:version?D.trailingMultipleMilli(version,returns):null,
        recentFilings:docs.map(x=>({availableAtMs:x.knownAtMs,title:String(x.data.title||x.data.form||'Filing').slice(0,100)})),
        technicals:T.screeningRow(T.summarize(series,{market:marketSeries})),
        missing:!version?['company_research']:(version.dataQuality?.missing||[]).slice(0,4)};
    });
  }
  ...
  function shortlistedRoster(roster,shortlist) {
    const symbols=shortlist.selected.map(x=>x.symbol).sort(),chosen=new Set(symbols);
    return {...roster,sourceUniverseHash:roster.universeHash,universeHash:hash({source:roster.universeHash,shortlist:shortlist.inputHash,symbols}),eligibleCount:symbols.length,symbols,
      members:(roster.members||[]).filter(x=>chosen.has(x.symbol)),excluded:[...(roster.excluded||[]),...roster.symbols.filter(s=>!chosen.has(s)).map(symbol=>({symbol,included:false,reason:'not_shortlisted_for_session'}))]};
  }
  ...
  function create({admin=A,fetchImpl=globalThis.fetch,wallNow=Date.now,env=process.env,publicFetch=null}={}) {
    ...
    async function createBatch(config,owner,key) {
      ...
      const b={batchId,owner,count,dates,runIds,budgetMode,config:{strategyVersionId:strategy?.versionId||'baseline',from:config.from,to:config.to,count,companyRange:config.companyRange,maxHoldingSessions:3,budgetMode,initialCashMinor:'10000000',feedDelayMinutes:15,spreadBps:10,feePerShareMicros:5000},
        ...
        limitations:['Current eligible universe: survivorship-limited historical selection.','Missing research is reconstructed from SEC filings and financial statements. Broader historical news and confirmed earnings calendars may be unavailable.','SEC aggregates are retrieved today and filtered by filing availability; later provider corrections may remain. Original filing sources and retrieval timestamps are retained.','First-time preparation precedes the five-minute replay target.','Historical recognition by pretrained models is possible.','Resource-limited reasoning; truncated or skipped required reviews are incomplete.','One to three sessions per holding within a common three-session observation window. Released cash is not reinvested. Price returns exclude dividends and cash interest. Suspected share-basis changes and unexecutable timed exits make the result unavailable. Overlapping windows are correlated.']};
      const savedConfig={investmentPolicy,policy,roster,aiPlan,budgetMode,budgetVersion:BUDGET_VERSION,sourceSnapshotDate:new Date(wallNow()).toISOString().slice(0,10),control:{riskMandate:control.riskMandate || null,universeRemovals:control.universeRemovals || []},rates:P.MODEL_RATES};
    ...
    async function ensureRepository(batch,config) {
      const priceDates=config.investmentPolicy?.maxHoldingSessions?[...new Set(batch.dates.flatMap(d=>require('./_investorSimulationHorizon').sessions(d).map(s=>s.date)))].sort():batch.dates;
      const repositoryId='repository_'+hash({version:REPOSITORY_VERSION,universe:config.roster.universeHash,from:batch.config.from,to:batch.config.to,dates:priceDates,snapshot:config.sourceSnapshotDate}).slice(0,40);
      const ref=scenarioCol.doc(repositoryId),prior=await ref.get();
      if(!prior.exists) {
        const units=[...repositorySymbols(config).map(symbol=>({unitId:'company_'+symbol,kind:'company',symbol,researchRequired:!!repositoryRow(config,symbol)})),...priceDates.map(date=>({unitId:'prices_'+date,kind:'prices',date}))];
        ...
        await ref.set({repositoryId,version:REPOSITORY_VERSION,priceValidationVersion:PRICE_VALIDATION_VERSION,total:units.length,companies:repositorySymbols(config).length,dates:priceDates.length,createdAtMs:wallNow()},{merge:true});
      }
      ...
      if(prior.exists&&prior.data()?.priceValidationVersion!==PRICE_VALIDATION_VERSION) {
        for(const date of priceDates)await rootTransaction(async tx=>{
    ...
    async function repositoryPackets(run,config,repository,{onProgress=async()=>{},shouldPause=async()=>false,screeningOnly=false,screeningCheckpoint=null,onScreeningCheckpoint=async()=>{}}={}) {
      ...
      const schedule=config.investmentPolicy?.maxHoldingSessions?require('./_investorSimulationHorizon').sessions(run.date):null;
      const prices=await readJSON(scenarioCol.doc(priceUnit.pointer.cacheId),priceUnit.pointer.artifact),endMs=(schedule?.at(-1).closeMs||M.sessionCloseMs(new Date(run.date+'T12:00:00Z')))+1200000,cutoff=M.nyWallClockToUtcMs(run.date,P.CUTOFFS_ET.evidenceFreezeMin);
      if(schedule)for(const session of schedule.slice(1)) {
        const unit=units.find(u=>u.unitId==='prices_'+session.date);if(!unit?.pointer)throw fail('SIMULATION_STATE_MISSING','Missing saved prices for '+session.date);
        const next=await readJSON(scenarioCol.doc(unit.pointer.cacheId),unit.pointer.artifact);
        if(hash(prices.provenance)!==hash(next.provenance))throw fail('HISTORICAL_PRICE_BASIS_CHANGED');
        for(const symbol of allSymbols) {
          const prior=prices.symbols[symbol],row=next.symbols[symbol];
          if(!prior||!row||row.error)throw fail('HISTORICAL_BARS_MISSING','Missing verified session data for '+symbol+' on '+session.date);
          prices.symbols[symbol]={...prior,bars:[...prior.bars,...row.bars],coverage:{...prior.coverage,expected:prior.coverage.expected+row.coverage.expected,missing:prior.coverage.missing+row.coverage.missing}};
        }
      }
      ...
      if(run.managerDone&&config.investmentPolicy&&run.evidenceReleasedThroughMs!=null&&!run.evidenceReleaseCursor) {
        ...
        return {packets,meta:{cutoffMs:cutoff,endMs,...(schedule?{sessions:schedule}:{}),symbols:allSymbols,evidenceCoverage:run.evidenceCoverage,priceCoverage:run.priceCoverage}};
      }
      ...
      for(let index=cp?.next||0;index<symbols.length;index++){
        ...
        const data=company.data.filter(x=>x.knownAtMs<=(schedule?cutoff:endMs)),series=company.daily.filter(b=>b.date<run.date).slice(-400);
    ...
    async function prepareShortlist(run,config,packets,{request,onProgress=async()=>{},shouldPause=async()=>false,save}={}) {
      ...
      if(config.roster.symbols.length<=count)selected=config.roster.symbols.map(symbol=>({symbol,reason:'Universe already fits the 50-company limit; retained for manager review.'}));
        ...
        const screening={cutoffMs,inputHash,columns:['symbol','name','sector','researchAsOfMs','priceDate','price','returnsBps','volumeRatio','revenueGrowthBps','fcfMarginBps','netDebtEbitdaMilli','trailingMultipleMilli','recentFilings','technicals','missing'],
          profiles:input.profiles.map(p=>[p.symbol,p.name,p.sector,p.researchAsOfMs,p.priceDate,p.price,p.returnsBps,p.volumeRatio,p.revenueGrowthBps,p.fcfMarginBps,p.netDebtEbitdaMilli,p.trailingMultipleMilli,p.recentFilings,p.technicals||null,p.missing]),
          technicalsBasis:'technicals: pre-cutoff daily-bar context in basis points of the last close. atr14Bps = 14-session average true range; gapAbsMean20Bps = mean absolute overnight gap; closeVsMa50Bps/closeVsMa200Bps = distance from moving averages; return20dBps; relativeToMarket20dBps/60dBps = return minus SPY over the same sessions; avgDollarVolume20Usd = 20-session average dollar volume.',
          limitations:'No new web research. Financial values may be missing. Recent filings are filing metadata only, not confirmed news catalysts. Technicals are descriptive, not forecasts.'};
        const body={model:plan.shortlistModel,reasoning:{effort:plan.shortlistReasoning},max_output_tokens:8000,background:true,store:true,
    ...
    async function execute(runId,{deadlineMs=wallNow()+11*60000}={}) {
      ...
      try {
        ...
        const replaySteps=meta.sessions?require('./_investorSimulationHorizon').replaySteps(meta.sessions):null;
        ...
        await A.withSimulationScope(scope,async()=>{
          ...
          let managerRoster=config.roster;
          while(wallNow()<deadlineMs) {
            ...
            if(!run.managerDone) {
              ...
              const out=await manager.runManagerMeeting({claim:{runId:'meeting_'+runId,payload:{accountId:runId,tradingDate:run.date},checkpoint:cp},control:ctrl,
                deps:{admin:A,gateway:require('./_investorOpenai').createGateway({admin:A,env}),now:()=>run.clockMs,universe:{...require('./_investorUniverse'),freezeEligibleSnapshot:()=>managerRoster},checkpoint,progress:async work=>{if(await paused())throw fail('SIMULATION_PREPARATION_YIELD');await report(work);},shouldYield:paused},budget:()=>Math.max(0,deadlineMs-wallNow()),minStageMs:5000});
            ...
            // The initial AI plan governs the complete observation window.
            ...
            await report({stage:'replay',label:'Replaying prices and checking trade instructions',done:replaySteps?Math.max(0,replaySteps.indexOf(run.clockMs)):Math.max(0,Math.round((run.clockMs-M.nyWallClockToUtcMs(run.date,570))/300000)),total:replaySteps?replaySteps.length-1:Math.round((meta.endMs-M.nyWallClockToUtcMs(run.date,570))/300000),unit:'market steps',current:null});
            ...
            const marketTimeMs=Math.min(run.clockMs-15*60000,meta.sessions?.find(s=>run.clockMs>=s.openMs&&run.clockMs<=s.closeMs+1200000)?.closeMs||M.sessionCloseMs(new Date(meta.cutoffMs)));
            ...
            await save({phase:'Replaying the market',marketTimeMs,...(meta.sessions?{sessionNumber:meta.sessions.findIndex(s=>run.clockMs<=s.closeMs+1200000)+1}:{}),investments,investedMinor:investments.reduce((n,x)=>n+x.investedMinor,0),curvePreview:[...(run.curvePreview||[]).filter(p=>p.atMs!==point.atMs),{atMs:point.atMs,returnBps:point.returnBps}].slice(-85),peakNavMinor:Math.max(run.peakNavMinor||10000000,nav),maxDrawdownBps:Math.max(run.maxDrawdownBps||0,10000*(Math.max(run.peakNavMinor||10000000,nav)-nav)/Math.max(run.peakNavMinor||10000000,nav)),progress:Math.min(100,Math.max(0,replaySteps?100*Math.max(0,replaySteps.indexOf(run.clockMs))/(replaySteps.length-1):100*(run.clockMs-M.nyWallClockToUtcMs(run.date,570))/(meta.endMs-M.nyWallClockToUtcMs(run.date,570)))),
    ...
    async function outcomeAnalysis(run) {
      ...
      if(plan.sessions)for(const session of plan.sessions.slice(1)) {
        const ptr=pointers.find(u=>u.unitId==='prices_'+session.date)?.pointer;if(!ptr)throw fail('SIMULATION_STATE_MISSING');
        const next=await readJSON(scenarioCol.doc(ptr.cacheId),ptr.artifact);
        for(const [symbol,value] of Object.entries(prices.symbols))value.bars=[...value.bars,...(next.symbols[symbol]?.bars||[])];
      }
