...
const TASKS = Object.freeze({
  ...
  dip_simulation:      { targetFunction: "investorDipSim-background",    category: "event",     heavy: false, engine: "manager" },
  bar_repository:      { targetFunction: "investorBars-background",      category: "event",     heavy: false, engine: "manager" },
...
function createJobs(A) {
  ...
  /** Tasks owned by one engine, for engine-scoped queue reads. */
  function tasksForEngine(engine) { return Object.keys(TASKS).filter((t) => TASKS[t].engine === engine); }
  ...
  return {
    taskFor, payloadHash,
    enqueueOnce, dueJobs, orderForDispatch, dispatchPlan, tasksForEngine, retireQueuedJobs,
