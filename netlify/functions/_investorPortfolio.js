...
async function readActiveMandates(accountId, { admin = null } = {}) {
  const D = admin || A;
  try { return rows(await D.col(D.COL.activeMandates).where("accountId", "==", accountId).get()); } catch { return []; }
}
...
async function snapshot({ accountId, asOfMs = Date.now(), admin = null, sectorOf = null } = {}) {
  ...
  const out = {
    ...
    positions: pos, workingOrders: open, activeMandates: mandates.map((m) => ({ symbol: m.symbol, status: m.status || null, desiredVersionId: m.desiredVersionId || null,
      ...
      action: m.action || null, allocation: m.allocation || null,
