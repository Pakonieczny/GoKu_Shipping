...
const MANAGER_VERSION = "manager.v1";
...
function freezeDecisionCutoff({ runStartedAtMs, tradingDate, inclusionRule = "published_at_else_immutable_first_seen", nowMs = Date.now() } = {}) {
...
function buildRiskMaintenancePlan({ holdingAnalysis = [], holdings = [], priorMandateBySymbol = {}, hardDeadlineMs = null, nowMs = Date.now(), tradingDate = null } = {}) {
  ...
  for (const symbol of heldSymbols) {
    ...
    if (a.mandate) {
      ...
      if (bad.length && a.researchDirective !== "FULL_REUNDERWRITE") { violations.push({ symbol, code: "ANTI_GOALPOST", details: bad }); actionRequired.push({ symbol, reason: `ANTI_GOALPOST:${bad.join(",")}` }); continue; }
      ...
      actionableMandates.push({ ...a.mandate, decision: a.decision, planClass: "RISK_MAINTENANCE", revisionResult: a.revisionResult });
...
function composeFinalDecisionRows({ roster, workset, effective, maintenance = null, research = null, synthesis = null, overrides = {}, claimGate = {} } = {}) {
  ...
  const holdingBy = new Map((effective && effective.holdingAnalysis || []).map((a) => [a.symbol, a]));
...
async function runManagerMeeting({ claim, deps: partial = {}, budget = () => 10 * 60 * 1000, minStageMs = 90 * 1000, control = null } = {}) {
  const deps = { ...defaultDeps(partial.admin || null), ...partial };
  const now = deps.now;
  ...
  while (stage !== "complete") {
    ...
    if (stage === "freeze") {
      ...
      const portfolioBySymbol = Object.fromEntries(workset.rows.map((r) => [r.symbol, { held: r.held, pending: r.pending, activeMandate: portfolio.activeMandates.some((m) => m.symbol === r.symbol) }]));
      ...
      try { coverageInput = assertExactCoverageInput(cards, snapshotRoster); }
        ...
        st.failure = { code: e.code, message: e.message };
        await record({ status: "failed_closed", universeVersion: snapshotRoster.universeVersion, universeHash: snapshotRoster.universeHash, eligibleCount: snapshotRoster.eligibleCount, noBuyReasons: st.noBuyReasons, failure: st.failure });
        return { done: true, failed: true, reason: e.code, checkpoint: { stage, data: st }, summary: { managerRunId, status: "failed_closed", noBuyReasons: st.noBuyReasons } };
      }
      ...
      const contextManifestHash = sha({ roster: { v: snapshotRoster.universeVersion, h: snapshotRoster.universeHash, n: snapshotRoster.eligibleCount }, cards: cards.cards.map((c) => c.dossierHash), holdings: holdingPackets.packets.map((p) => p.dossierHash), portfolio: portfolio.contentHash, cutoff: cutoff.cutoffMs, policy: policy.policyHash });
      ...
      await record({ status: "running", universeVersion: st.roster.universeVersion, universeHash: st.roster.universeHash, eligibleCount: st.roster.eligibleCount, contextManifestHash, cutoffMs: cutoff.cutoffMs, policyHash: policy.policyHash, cardsCount: cards.count, heldCount: workset.managedPositionSymbols.length });
      stage = "review";
      continue;
    }
    if (stage === "review") {
      const { cards, holdingPackets, portfolio } = await rebuildContext(st, deps, accountId);
      const r = await deps.gateway.reviewUniverse({ cards: cards.cards, universeManifest: st.roster, holdings: holdingPackets.packets, portfolio: portfolio, policy: { policyHash: policy.policyHash, riskPolicyHash: policy.riskPolicyHash, riskMandate: policy.riskMandate }, contextManifestHash: st.contextManifestHash, waitMs: Math.max(30000, Math.min(budget() - minStageMs, 8 * 60 * 1000)) });
    ...
    if (stage === "coverage") {
      ...
      await record({ status: "running", coverage: { ok: v.ok, completedCount: v.completedCount, eligibleCount: v.eligibleCount, missing: v.missing.length, duplicates: v.duplicates.length, unknown: v.unknown.length, repaired: st.repair ? st.repair.repaired : 0 } });
      stage = "maintenance";
      continue;
    }
    if (stage === "maintenance") {
      ...
      await record({ status: "running", maintenance: { actionable: st.maintenance.actionable.length, actionRequired: st.maintenance.actionRequired.length, staged: st.maintenance.staged.status, deadlineMissed: plan.deadlineMissed } });
      stage = "research";
      continue;
    }
    if (stage === "research") {
      ...
      const holdingDeadlineMs = deps.market ? deps.market.nyWallClockToUtcMs(tradingDate, POLICY.CUTOFFS_ET.expansionSoftDeadlineMin) : null;
      ...
      const worker = async (request) => {
        ...
        const packet = await R.buildPacket({ symbol: request.symbol, cutoff: st.cutoff, directive: request.reviewDirective, admin: deps.admin, deps });
        if (!packet.ok) return { ok: false, symbol: request.symbol, error: packet.reason };
        const portfolio = await deps.portfolio.getSnapshot({ accountId, asOfMs: st.cutoff.cutoffMs, admin: deps.admin });
        const bound = deps.tools ? deps.tools.allowlisted(deps.toolBindings || deps.tools.productionBindings({ accountId }), policy.toolPolicy, { symbol: request.symbol, cutoffMs: st.cutoff.cutoffMs }) : { tools: null, log: [] };
        ...
        return { ok: persisted.persisted === true, symbol: request.symbol, memoId: persisted.memoId || null, proposedDecision: persisted.proposedDecision || null, reasonCode: persisted.reasonCode || null,
          mandate: persisted.mandate || null, memo: persisted.memo || null, factualPremises: persisted.factualPremises || [], error: persisted.persisted ? null : persisted.reason, toolCalls: bound.log.length, costMinor: r.costMinor || "0" };
      };
      const out = await pool.run({ requests, concurrency: policy.maxConcurrentResearchJobs, worker, deadlineMs: holdingDeadlineMs,
        budgetRemaining: () => budget() > minStageMs, onResult: async (row) => { st.research = st.research || { completed: [], failed: [], deferred: [] }; if (row.ok) st.research.completed.push({ symbol: row.request.symbol, memoId: row.result.memoId, proposedDecision: row.result.proposedDecision, reasonCode: row.result.reasonCode, mandate: row.result.mandate, memo: row.result.memo, factualPremises: row.result.factualPremises, request: row.request }); } });
      st.research = { completed: out.completed.map((r) => ({ symbol: r.request.symbol, request: r.request, memoId: r.result.memoId, proposedDecision: r.result.proposedDecision, reasonCode: r.result.reasonCode, mandate: r.result.mandate, memo: r.result.memo, factualPremises: r.result.factualPremises })),
    ...
    if (stage === "synthesis") {
      ...
      if (st.expansionBlocked || !candidates.length) {
      ...
      const feasible = deps.portfolioRisk && typeof deps.portfolioRisk.buildFeasibleAlternatives === "function"
        ? safeCall(() => deps.portfolioRisk.buildFeasibleAlternatives({ researchResults: candidates.map((c) => ({ symbol: c.symbol, memo: c.memo, mandate: c.mandate })), holdings: holdingPackets.packets, portfolio, policy, marks: {}, advBySymbol: {}, sectorOf: sectorLookup(deps), clusterOf: null })) : null;
      ...
      const r = await deps.gateway.finalizePortfolio({ coverage: st.effective.coverage, researchResults: candidates.map((c) => ({ symbol: c.symbol, memo: c.memo })), holdings: holdingPackets.packets, portfolio, feasibleAlternatives: feasible, policy: { policyHash: policy.policyHash, riskPolicyHash: policy.riskPolicyHash, riskMandate: policy.riskMandate }, contextManifestHash: sha({ c: st.contextManifestHash, memos: candidates.map((c) => c.memoId), attempt: st.synthesisAttempt || 0 }), waitMs: Math.max(30000, Math.min(budget() - minStageMs, 8 * 60 * 1000)) });
      ...
      st.synthesis = { ok: true, decisions: synth.decisions, expansionMandates: synth.expansionMandates.filter((m) => !claimBlocked.includes(m.symbol)), comparisonNote: synth.comparisonNote, claimGate, claimBlocked, requestId: r.requestId, attempt: (st.synthesisAttempt || 0) + 1 };
      stage = "activation";
      continue;
    }
    if (stage === "activation") {
      ...
      await record({ status: "running", activation: st.activation });
      stage = "persist";
      continue;
    }
    if (stage === "persist") {
      const committed = st.activation && st.activation.status === "COMMITTED";
      const synthesisForRows = st.synthesis && st.synthesis.ok ? { decisions: st.synthesis.decisions, expansionMandates: committed ? st.synthesis.expansionMandates : [] } : null;
      let decisions = composeFinalDecisionRows({ roster: st.roster, workset: st.workset, effective: st.effective, maintenance: st.maintenance, research: { completed: (st.research && st.research.completed || []).map((c) => ({ request: c.request, result: c })) }, synthesis: synthesisForRows, overrides: st.overrides || {}, claimGate: st.synthesis && st.synthesis.claimGate || {} });
...
async function rebuildContext(st, deps, accountId, { cardsNeeded = true } = {}) {
  const portfolio = await deps.portfolio.snapshot({ accountId, asOfMs: st.cutoff.cutoffMs, admin: deps.admin });
  const portfolioBySymbol = Object.fromEntries(st.workset.rows.map((r) => [r.symbol, { held: r.held, pending: r.pending, activeMandate: portfolio.activeMandates.some((m) => m.symbol === r.symbol) }]));
  const cards = cardsNeeded ? await deps.dossier.compactCards({ symbols: st.roster.symbols, cutoff: st.cutoff, admin: deps.admin, portfolioBySymbol, deps }) : { cards: [], missing: [] };
  const holdingPackets = await deps.dossier.expandedHoldingDeltas({ symbols: st.workset.managedPositionSymbols, cutoff: st.cutoff, admin: deps.admin, portfolioBySymbol, deps });
  return { cards, holdingPackets, portfolio };
}
...
async function runEventRevision({ claim, deps: partial = {}, control = null } = {}) {
  ...
  const cutoffMs = Number(payload.cutoff) || deps.now();
  const policy = POLICY.loadActiveSync(control || {});
  const R = deps.research;
  const state = await deps.portfolio.symbolState({ accountId, symbol, admin: deps.admin });
  const prior = await R.latest(symbol, { admin: deps.admin });
  const packet = await R.buildPacket({ symbol, cutoff: cutoffMs, prior, directive: "DELTA_REVISION", admin: deps.admin, deps });
  if (!packet.ok) return { ok: false, symbol, state: state.kind, reason: packet.reason, decision: "ABSTAIN", reasonCode: "DATA_INCOMPLETE" };
  const portfolio = await deps.portfolio.getSnapshot({ accountId, asOfMs: cutoffMs, admin: deps.admin });
  const bound = deps.tools ? deps.tools.allowlisted(deps.toolBindings || deps.tools.productionBindings({ accountId }), policy.toolPolicy, { symbol, cutoffMs }) : { tools: null, log: [] };
  ...
  let assessment;
  const baseline = state.appliedMandate || (prior ? prior.memo : null) || {};
  if (state.kind === "UNFILLED_ENTRY" || state.kind === "PARTIALLY_FILLED_ENTRY") {
    const r = await deps.gateway.reviseEntry({ baseline: { symbol, ...baseline }, delta: packet.delta, state: { kind: state.kind, ownedQuantityUnits: state.ownedQuantityUnits, remainingEntryUnits: state.remainingEntryUnits, entries: state.entries }, portfolio });
    ...
    const r = await deps.gateway.reviseHolding({ baseline: { symbol, ...baseline }, delta: packet.delta, position: state.position, mandate: state.appliedMandate });
  ...
  if (assessment.ok && assessment.researchDirective === "FULL_REUNDERWRITE") {
    const full = await R.buildPacket({ symbol, cutoff: cutoffMs, prior, directive: "FULL_REUNDERWRITE", admin: deps.admin, deps });
    ...
    const scenarios = deps.portfolioRisk && typeof deps.portfolioRisk.runScenarios === "function" ? safeCall(() => deps.portfolioRisk.runScenarios({ portfolio: state.portfolio, candidates: [], policy, marks: {}, advBySymbol: {}, sectorOf: sectorLookup(deps), clusterOf: null })) : null;
