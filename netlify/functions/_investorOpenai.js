...
function createGateway({ admin = null, fetchImpl = null, env = process.env, now = Date.now } = {}) {
  ...
  async function decidePreparedPortfolio({documents=[],packets=[],completedResearch=[],holdings=[],portfolio,marks,policy,marketState,expansionBlocked=false,contextManifestHash,waitMs=DEFAULT_WAIT_MS}={}) {
    ...
    if(investmentPolicy) {
      ...
      const mandate=cashAllowed?`Up to ${investmentPolicy.maxCompanies} finalists are supplied. Decide BUY or PASS for EACH one (decision + decisionReason), with integer allocationUsd $5,000–$95,000 on BUY rows and exactly 0 on PASS rows, combined at most $95,000 including entry fees. Concentration is allowed: when one finalist is clearly the strongest, put most or all of the $95,000 into it rather than spreading it thin. Holding cash is a legitimate recorded decision, but lean toward buying: BUY the best-ranked finalist whenever its session-1 expected return net of about 25 bps round-trip friction is positive. Add a second or third BUY only when that finalist's own net session-1 score is positive and within 50 bps or 25% of the primary's, and it is not the same bet in the same sector. PASS every finalist only when none has a positive net score, and say so in comparisonNote. Complete the full assessment, horizon comparison, stop and target for PASS rows too so the decision can be audited.`:`Invest in EACH of the ${investmentPolicy.minCompanies}–${investmentPolicy.maxCompanies} supplied finalists with integer allocationUsd $5,000–$95,000 each, combined at most $95,000 including entry fees. Fund only the finalists with a positive session-1 score net of about 25 bps round-trip friction; when the policy range allows fewer companies than were supplied, omit the weakest finalists down to the minimum count, and give any forced position with a nonpositive score the $5,000 minimum.`;
