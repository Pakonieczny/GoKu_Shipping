...
async function recordClaims({ symbol, documentId, documentVersionId, sourceId, claims = [], extractedBy = null,
  publishedAtMs = null, firstSeenAtMs = Date.now(), admin = null }) {
  const DB = admin || A;
  ...
  for (const c of claims) {
    ...
    const row = {
      ...
      extractedBy: extractedBy || null, supersedes: null, supersededBy: null,
    ...
    if (row.claimType === "GUIDANCE" && row.metric && row.periodLabel) {
      ...
      if (prior) {
        ...
        await DB.col(DB.COL.claims).doc(prior.claimId).set({ supersededBy: claimId, supersededAtMs: Date.now() }, { merge: true });
...
async function claimsForCompany(symbol, { asOfMs = Date.now(), claimTypes = null, limit = 400, admin = null } = {}) {
  ...
  snap.forEach((d) => {
    ...
    const knownAt = Number(c.publishedAtMs ?? c.firstSeenAtMs);
...
function latestGuidanceFrom(claims, { asOfMs = Date.now() } = {}) {
  ...
  for (const c of claims || []) {
    ...
    if (!cur || knownAt > Number(cur.publishedAtMs ?? cur.firstSeenAtMs)) by.set(key, c);
  }
...
function nextEarningsFrom(claims, { asOfMs = Date.now() } = {}) {
  ...
  const candidates = (claims || []).filter((c) => c.claimType === "EARNINGS_DATE" && c.date && c.date >= today
    && Number.isFinite(Number(c.publishedAtMs ?? c.firstSeenAtMs)) && Number(c.publishedAtMs ?? c.firstSeenAtMs) <= asOfMs)
