...
function returnsBps(series, { asOfMs }) {
  const rows = require("./_investorDecisionContext").completedDaily(series, asOfMs);
...
function cardFromVersion(version, { cutoffMs, price = null, sectorPrice = null, marketPrice = null, changes = [],
  ...
  const card = {
    ...
    guidance: g ? { periodLabel: g.periodLabel, metric: g.metric, lowMicros: g.lowValue == null ? null : require("./_investorDecisionContext").moneyMicros(g.lowValue, g.unit),
      highMicros: g.highValue == null ? null : require("./_investorDecisionContext").moneyMicros(g.highValue, g.unit), unit: g.unit, issuedAt: g.issuedAt,
    ...
    changes: (changes || []).slice(0, MAX_CHANGES_ON_CARD).map((c) => ({ eventId: c.eventId || c.deltaId, type: c.form || c.eventClass || c.type || null,
      title: c.title || null, summary: c.summary || null, documentVersionId: c.documentVersionId || null, publishedAtMs: c.publishedAtMs || null, firstSeenAtMs: c.firstSeenAtMs || null,
  ...
  if (g && g.metric === "revenue" && fu.revenueTtmMinor && price && price.ok && fu.sharesOutstanding) {
    ...
    try {
      const lo = require("./_investorDecisionContext").moneyMicros(g.lowValue, g.unit), hi = require("./_investorDecisionContext").moneyMicros(g.highValue, g.unit);
      if (lo == null || hi == null) throw new Error("unsupported guidance unit");
      const mid = (BigInt(lo) + BigInt(hi)) / 2n;                       // whole units
      const mcapMinor = BigInt(price.closeMicros) * BigInt(fu.sharesOutstanding) / 10000n; // micros×shares → minor
      if (mid > 0n) card.valuation.forwardMultipleMilli = M.toCanonical(mcapMinor * 1000n * 10000n / mid);
...
async function versionAsOf(symbol, { cutoffMs, admin = null, pointer = null } = {}) {
  const p = pointer || await current(symbol, { admin });
  let id = p && p.currentVersionId, seen = new Set();
  while (id && !seen.has(id)) {
    seen.add(id);
    const v = await readVersion(id, { admin });
    if (!v) return null;
    if (require("./_investorDecisionContext").availableAt(v) <= cutoffMs) return v;
    id = v.previousVersionId;
  }
  return null;
}
...
async function persistVersion(version, { admin = null, sourceAtMs = null, nowMs = Date.now() } = {}) {
  ...
  const result = await D.runTransaction(async (tx) => {
    ...
    let doc = { ...next, knownAtMs: nowMs, versionId, previousVersionId: pointer.currentVersionId || null, ...D.envelope({ created_by: "dossier.persistVersion" }) };
...
async function pendingChanges(symbol, { cutoffMs = Date.now(), admin = null, limit = 50 } = {}) {
  ...
  const rows = [];
  snap.forEach((d) => { const x = d.data(); if (Math.max(Number(x.inclusionAtMs)||0, Number(x.firstSeenAtMs ?? x.createdAtMs)||0) <= Number(cutoffMs)) rows.push(x); });
...
async function markDeltasReviewed({ symbol, deltaIds = [], managerRunId, materiality = "reviewed", admin = null }) {
  const D = db(admin);
  const pointer = D.col(D.COL.dossiers).doc(String(symbol).toUpperCase());
...
async function expandedHoldingDeltas({ symbols = [], cutoff, admin = null, deps = {}, portfolioBySymbol = {}, memoBySymbol = {} } = {}) {
  ...
  for (const symbol of symbols) {
    ...
    const version = await versionAsOf(sym, { cutoffMs, admin, pointer });
    if (!version) {
      missing.push({ symbol: sym, reason: "no_dossier_known_at_cutoff" });
      const history = await require("./_investorResearch").history(sym, { admin, cutoffMs, openedAtMs:Date.parse(portfolioBySymbol[sym] && portfolioBySymbol[sym].position && portfolioBySymbol[sym].position.openedAt || "") || null }).catch(() => ({ original:null, revisions:[] }));
      packets.push({ symbol:sym, card:require("./_investorDecisionContext").unavailableCard(sym,cutoffMs,"no_dossier_known_at_cutoff"),
        priorMemo:history.latest || null, thesisHistory:history, pendingChanges:[], dossierHash:null,
        position:portfolioBySymbol[sym] && portfolioBySymbol[sym].position || null, activeMandate:portfolioBySymbol[sym] && portfolioBySymbol[sym].mandate || null });
      continue;
    }
    const memo = memoBySymbol[sym] || await (deps.research || require("./_investorResearch")).latest(sym, { admin, cutoffMs }).catch(() => null);
    ...
    packets.push({ symbol: sym, card, delta, pendingChanges: changes, claims:await (deps.evidence || require("./_investorEvidence")).claimsForCompany(sym,{asOfMs:cutoffMs,admin}), priorMemo: memo ? { memoId: memo.memoId || memo.researchMemoId || null,
      researchVersion: memo.researchVersion || null, dossierVersionId: memo.dossierVersionId || null, asOfMs: memo.asOfMs || null, memo: memo.memo || memo, label: "PRIOR_INTERPRETATION_NOT_EVIDENCE" } : null,
      thesisHistory: await require("./_investorResearch").history(sym, { admin, cutoffMs, openedAtMs:Date.parse(portfolioBySymbol[sym] && portfolioBySymbol[sym].position && portfolioBySymbol[sym].position.openedAt || "") || null }).catch(() => ({ original: null, revisions: [] })),
      position: portfolioBySymbol[sym] && portfolioBySymbol[sym].position || null, activeMandate: portfolioBySymbol[sym] && portfolioBySymbol[sym].mandate || null,
...
module.exports = {
  ...
  current, readVersion, versionAsOf, persistVersion, recordRoutineDelta, pendingChanges, markDeltasReviewed,
