...
const CASH_POLICY='last-resort.v1';
const fail=(code,message=code)=>{throw Object.assign(Error(message),{code});};
function policyFor(range,strategy=null,{shared=false,riskMandate=null,cashPolicy=CASH_POLICY}={}) {
  ...
  if(shared&&cashPolicy!==null&&cashPolicy!==CASH_POLICY)fail('SIMULATION_INVESTMENT_POLICY_INVALID');
  ...
  return {...(strategy?{strategy}:{}),...(shared?{coreVersion:CORE_VERSION,...(cashPolicy?{cashPolicy}:{}),riskMandate:riskMandate||require('./_investorPolicy').RISK_MANDATE,spreadBps:"10",feePerShareMicros:"5000",barMinutes:5,feedDelayMinutes:15,stopGraceMinutes:30}:{}),version:shared?CORE_VERSION:strategy?'required-investment.v4':VERSION,companyRange:range,minUsd:5000,maxUsd:95000,minCompanies:RANGES[range].min,maxCompanies:RANGES[range].max,maxTotalUsd:95000,maxHoldingSessions:3,entry:'FIRST_AVAILABLE_SESSION_PRICE',horizonMarginBps:'25',downsideWeightBps:'5000'};
}
...
function sharedInstructions(policy,fn) {
 ...
 const preferInvestment=policy.cashPolicy===CASH_POLICY;
 ...
 if(fn==='reviewUniverse')return common+`Choose ${Math.max(1,policy.minCompanies)}–${policy.maxCompanies} distinct new finalists from the supplied shortlist, fewer only if fewer eligible names are supplied. Existing holdings and pending entries are excluded from new selection. Request RESEARCH_NOW for each finalist. Keep other rows NONE and reasons under 15 words. A finalist is not a forced purchase.`+(preferInvestment?' The mandate favors investment: select the strongest actionable opportunities, using source completeness and ability to support a one-to-three-session thesis as well as relative rank. Prefer a supported alternative over a higher-ranked name with a known essential evidence gap. Do not select only names already known to be uninvestable. Ordinary uncertainty is not an eligibility failure.':'');
 const entryRules=preferInvestment?`INVESTMENT MANDATE ${CASH_POLICY}: Put capital to work in the strongest supported opportunities. An all-PASS / zero-new-investment decision is the absolute last resort, not a neutral default. BUY the best eligible finalist whenever its honestly estimated return net of execution costs over the selected holding horizon is positive and its thesis has identifiable dated source support. Seek the requested company count where supported; do not reject an investable best finalist merely because another finalist is unsuitable. Modest conviction, ordinary uncertainty, incomplete nonessential metrics, lack of a perfect catalyst, or a preference to wait for a better setup should reduce allocation (toward the $5,000 BASE minimum), not automatically cause PASS. A negative uncertainty/downside-adjusted horizon score is not itself a cash-entry gate: that score chooses duration, not whether a positive-net-return thesis may be funded. No fixed extra return hurdle or requirement for certainty.
 Before returning all PASS, explicitly compare a small position in the strongest finalist with zero investment. All PASS is permitted only if EVERY finalist has a concrete disqualifier: essential evidence is absent or contradictory so no honest thesis/positive net-return estimate can be supported; expected net return is nonpositive over the chosen permitted horizon; severe evidenced downside cannot be made acceptable with reduced size and the existing protection/risk limits; or a specific portfolio/execution constraint prevents entry. In each PASS decisionReason identify the decisive source fact or exact missing prerequisite. In comparisonNote explain why even a small position in the strongest finalist is ruled out, and distinguish inadequate data from an adverse investment outlook. Do not call the whole market uninvestable after researching only these finalists. Never invent evidence or inflate return estimates to meet this mandate. Risk limits, source honesty and execution eligibility still take precedence over the preference to invest.`:'BUY requires a positive expected return net of costs and identifiable source support; material missing evidence requires PASS.';
 return common+`Make ONE combined investment decision for all supplied finalists. Return BUY or PASS for every finalist. ${entryRules} Return BASE allocationUsd from 5000 to 95000 for BUY, zero for PASS, combined at most 95000. A single high-conviction finalist may take the whole 95000. The shared executor applies allocation/stop/target multipliers exactly once and reduces executable quantity for current cash and the supplied risk mandate; never loosen stops to fit a size. Compare the actual supplied portfolio, not assumed empty cash. No averaging down or duplicate owned/pending symbols.
...
module.exports={RANGES,VERSION,CORE_VERSION,CASH_POLICY,screeningProfile,shortlistSchema,selectShortlist,sharedInstructions,policyFor,sessions,replaySteps,score,choose,validate};
