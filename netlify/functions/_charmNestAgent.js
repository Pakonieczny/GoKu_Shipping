...
const PACKING_INSTRUCTIONS = `Grade EVERY numbered charm by its CUT SILHOUETTE, including its hoop, for a laser-cut jewelry nesting solver. Interior engraving is not a usable cavity. Return one profile per index: adaptability (0 rigid/difficult, 100 fits many gaps), interlock (0 no useful recesses/protrusions, 100 strongly complementary), edgeAffinity (0 poor edge fit, 100 useful straight/compact edge fit), priority (0 save as filler, 100 anchor early), edgeRole (long-edge, short-edge, corner, interior or either), family, compatible mate families, up to three promising absolute rotations, and a brief geometric reason. Reuse supplied cached profiles unless visibly inconsistent with the silhouette. Shapes are drawn normalized to a cell: use their real dimensions to judge pair compatibility. Select up to two particularly compatible partners per shape, scoring actual recess/protrusion complementarity and relative dimensions 0–100, with a short reason. Explicitly score an attractive but incompatible pair low if needed. Identify which slender long shapes can line the long perimeter efficiently, which shorter straight shapes suit short edges, and which shapes actually suit corners. A thin bounding box alone is insufficient: assess cut-contour straightness, relative dimensions, protruding hoops and the inward-facing recesses available to neighbors. Keep versatile small compact shapes as late gap fillers; pair protrusions with suitably sized open concavities. Treat disc hoops as protrusions to tuck inward beside a complementary shape. Never force all pieces onto the perimeter. Include long bars, discs with offset hoops, and concave/branched shapes in the analysis. Never infer compatibility from the artwork subject or name. Use supplied integer indices only. Dates and whole-order membership take precedence: never change membership, size, clearance, outline, or the fill ceiling. These grades will directly control the next charm and preferred neighbors in EVERY guided construction; geometry rejects collisions and validates the actual fit. Variants may explore other angles, but must retain your shape priorities and pair preferences. Return suggestions for up to 24 anchors as an additional ordering hint. Do not claim an achieved density, optimality or verification.`;
// Claude structured outputs reject maxItems. Bound arrays and scores in
// normalizePackingPlan after generation; keep the wire schema in its supported subset.
const PACKING_SCHEMA = {"type":"object","additionalProperties":false,"properties":{"profiles":{"type":"array","items":{"type":"object","additionalProperties":false,"properties":{"index":{"type":"integer"},"adaptability":{"type":"number"},"interlock":{"type":"number"},"edgeAffinity":{"type":"number"},"priority":{"type":"number"},"edgeRole":{"type":"string","enum":["long-edge","short-edge","corner","interior","either"]},"family":{"type":"string","enum":["round","compact","elongated","concave","branched","angular"]},"mates":{"type":"array","items":{"type":"string","enum":["round","compact","elongated","concave","branched","angular"]}},"angles":{"type":"array","items":{"type":"number"}},"note":{"type":"string"}},"required":["index","adaptability","interlock","edgeAffinity","priority","edgeRole","family","mates","angles","note"]}},"pairs":{"type":"array","items":{"type":"object","additionalProperties":false,"properties":{"a":{"type":"integer"},"b":{"type":"integer"},"score":{"type":"number"},"reason":{"type":"string"}},"required":["a","b","score","reason"]}},"suggestions":{"type":"array","items":{"type":"object","additionalProperties":false,"properties":{"index":{"type":"integer"},"angles":{"type":"array","items":{"type":"number"}}},"required":["index","angles"]}},"summary":{"type":"string"}},"required":["profiles","pairs","suggestions","summary"]};
...
function buildRequest(mode, body) {
  ...
  if (mode === "packing") {
    ...
    const pieces = (body.pieces || []).slice(0, 120).map((p,index) => ({ index, widthPt: num(p.widthPt), heightPt: num(p.heightPt), areaPt2: num(p.areaPt2), cachedProfile: p.cachedProfile || null }));
    ...
    return { system: PACKING_INSTRUCTIONS, schema: PACKING_SCHEMA, content, effort: "medium", pieceCount: pieces.length };
  }
  if (mode === "place") {
...
async function run(mode, body) {
  ...
  if (mode === "labelRead") {
    ...
    return { skipped: `${mode === "name" ? "naming" : mode === "packing" ? "shape analysis" : /^engrave/.test(mode) ? "engraving classifier" : "review"} unavailable (${e.status ? e.status + " " : ""}${String(detail).slice(0, 200)})` };
  }
}
