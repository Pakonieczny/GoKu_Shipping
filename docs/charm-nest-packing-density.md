...
### September 21 follow-up: 10° default, checkpoint memory, and deadline retention

Paul requested returning to 10°. Settings version 21 migrates existing workspaces to 36 base orientations; explicit 2°/5° choices remain available. The 0.35 edge weight and geometry, FIFO, whole-order and fill constraints remain in force.

The browser out-of-memory save warning exposed a real serialization defect: live worker `best` messages included `rec` and `grids`, with references to all prepared rotation masks. The worker now strips those internal fields before structured cloning both live and final results. Workspace capture independently removes them from older best records, preserves shared typed-array identity within a checkpoint, and coalesces pending saves instead of building a queue of full snapshots. Automatic concurrency is at most two searches for 10° and one for denser settings, with material sheets queued through export/verification. Finished search workers and excess workers from earlier settings are terminated to release their heaps.

A reproducible deadline defect also discarded an earlier legal layout: a trial interrupted after the first piece of an undated multi-piece order could replace the incumbent, then final whole-order cleanup removed that piece. In the regression, an earlier one-piece layout became an empty saved sheet. Incomplete orders are now removed before trial comparison/publication. Final browser selection additionally compares completed worker results against the best layout already received, retaining that layout and its worker parameters when it is better.
...
### Shape grades, compatible neighbors and perimeter roles

At Paul's request, AI packing guidance is now a pre-placement step. Every distinct input geometry receives adaptability, interlock, edge-affinity and anchor-priority grades, a shape family, compatible families, proposed rotations and an edge role (long edge, short edge, corner, interior or either). The model sees cut silhouettes and physical dimensions, and is asked to account for straight contours, slender bars, inward-facing recesses, hoops, relative size and small gap fillers. It also scores specific complementary pairs. These are predictions, not verified optimal neighbors.

Queues are analyzed in batches of 80 distinct shapes, without truncating after 120 inputs. Duplicate instances inherit their geometry's profile and pair links. Explicit pair scores govern same-batch pairs; model-provided family compatibility also connects shapes across batches. Saved batch job IDs/results resume after refresh; the workspace retains a bounded geometry-keyed profile cache. An unchanged queue reuses its response. Missing or malformed grades hold the sheet for retry instead of quietly launching an unguided search. Settings can explicitly select geometry-only operation.

The standard geometric solver follows these grades on every guided construction: it dynamically chooses the next unpinned candidate from anchor priority, flexibility, interlock and compatibility with already placed pieces, subject to chronological constructions and whole-order membership. Pair and edge-role preferences enter coarse candidate generation and exact-mask placement scoring, including transactional extension and repair. Detailed contact checks consider the four strongest available partner preferences to bound runtime; all candidates still pass geometric collision and stock-boundary checks. The existing count/offcut/contact comparator remains authoritative for preserving the best completed legal layout, so model advice cannot replace it with a worse result.
