...
### 2026-09-21 — manufacturing roles and assembled previews

`CharmNestGeom.pathRole/isCutLine` is the shared source of cut classification for grouping, back masks, sheet tint and thumbnails. Explicit CUT layers (including coloured strokes) supply material geometry. ENGRAVE and HATCH layers are front artwork at every RGB value and cannot remove back material or become a body/hoop. Unlabelled legacy achromatic strokes remain supported; red/blue alone never declare an opening. Explicit compound cut paths and welded hoop apertures remain open, with the existing engraving clearance. Expanded unlabelled perimeter ink uses the former narrow-trace simplification automatically; named CUT compound geometry is never simplified this way.

Reported source vectors reproduced both defects: SEA TURTLE2's red shell on ENGRAVE was subtracted as a hole; MIDDLE_5903's HATCH fill was selected instead of its dark-red CUT body, and then welded into a perforated silhouette. The regression fixtures retain only source path geometry and manufacturing layers. Both now fit the supplied text on their bodies, preserve actual hoop apertures, and pass ink containment and mirrored back-export checks.

Operator front, back, catalog and server-generated thumbnails show physical cuts in black. Original artwork colours and export paths remain intact. Catalog previews use current assembled vectors instead of historical red/detached-hoop PNGs, with two visible-preview jobs at a time and an 80-thumbnail cache; browsing does not add nesting masks or source documents to the saved workspace.

On recovery, an old HATCH/ENGRAVE-derived outline is rebuilt from the source CUT geometry and welded again. Changed geometry invalidates its sheet/checkpoint and engraving approval for re-nesting/review. Unapproved engraving drafts are refitted under current material rules; unchanged approved work is retained.

Checks: material-roles, material-preview, recovered-hoops, bridge-units, engraving-updates, text-reflow, engrave-worker, engrave-view, engrave-stream, production-export, production-export-ui, vector-fidelity and workspace. Rendered actual source examples were visually inspected: date inside turtle shell; FYB inside palm; black assembled outlines; open hoop holes. This is local verification; publication must be checked separately.
