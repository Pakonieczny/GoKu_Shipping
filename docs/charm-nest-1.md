...
- GF 14/20 and SS never receive an urgency, mixed-order, or manual partial-sheet exception. A completed small queue remains a saved working sheet. Full means the configured fill ceiling is reached, or a completed capacity search overflows a whole order that would itself fit an empty sheet. An interrupted or merely time-limited search cannot release a partial sheet. The default ceiling remains 74%; this is not a promise of 100% physical coverage.
...
**Full end-to-end browser release gate remains open.** The existing Design Station bridge accepts production and loopback origins, not the PR preview origin; the hello handshake therefore times out. Its allowlist has not been widened. Browser security also rejected uploading the synthetic test artwork, reporting declined permission; the upload was not retried through another route. The stored sandbox snapshot and existing master library are available and can support the remaining rehearsal without uploading new artwork once the preview is authorized.

Proposed narrowly scoped test access (requires approval before changing the bridge trust boundary): accept a bridge message only when both sender and Design Station are on `https://deploy-preview-12.goldenspike.app`, the station is in sandbox mode, and all existing source/nonce checks pass. No wildcard preview domains and no additional production origin. Then rehearse pull → pool → nest → approvals → labels → sandbox commit, refresh during processing, two consecutive sets, new-order intake and solid-only nesting. Do not describe the release gate as passed until these interactions actually complete. The user's instruction is to push to main only when done.
