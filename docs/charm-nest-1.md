...
4. **Nest (default engine: Claude)** — Claude Opus 5 plans every placement itself. Each round it receives the rendered sheet (light grey, numbered, every cut outline traced in red), what is placed, the remaining charms with crops and sizes, the largest open pockets the geometry measured, and the tool's feedback on its previous moves. It returns moves (charm, angle, centre in inches, why). The measuring tool executes each move exactly; if it collides, it slides the piece up to the snap distance (default 0.25 in) to the nearest legal spot and says so; otherwise it fails with the reason (crosses the stock edge / overlaps by N in²). Rounds continue until all charms are placed, Claude judges the sheet full, or the round cap (default 10) is hit. No random trials. Every move and Claude's reason for it is logged live in the card's Agent panel. Settings → Engine switches to the geometric solver below.
4b. **Nest (solver option)** — `charm-nest-solver.js` in a Worker: largest-first, contact-scored, gravity tie-break, random restarts within the time budget. Positive clearance dilates each mask by half of it; **negative clearance (the default, −0.5 pt) erodes instead: adjacent outlines may touch and their strokes overlap by up to |clearance|, bodies never more.** Sheet inset pre-filled as wall; mirroring never applied. Coarse pass (0.5 px/pt, exhaustive, summed-area-table pre-tests) picks candidates; fine pass (2 px/pt, bit-parallel) makes the exact decision. Each committed placement is posted to the page and drawn immediately.
...
6b. **AI inspection of the sheet** — Claude gets the rendered sheet and the placement list and reports real problems (a lone ring placed on its own, an incomplete charm, mirrored text). A merge or exclude finding is applied automatically and the sheet re-nests once; anything else is shown on the card and in the report.
