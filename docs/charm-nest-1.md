...
| `netlify/functions/charmNestName.js` | Claude Opus 5, low effort, structured output → layer names (and metal suggestions for unassigned files) |
...
2. **Group** — outlines = closed, stroked, near-black paths above the minimum size (page-sized frames ignored). Every other segment goes to the outline it overlaps most; ties to the smaller outline; small closed black paths fully inside an outline merge as holes/details. Loose elements are reported, not silently dropped.
3. **Silhouettes** — the outline alone is stroked at 6 px/pt and flood-filled from the border; unreached = solid. Member ink outside the outline is unioned in. A leaking outline is flagged `open path` and excluded. Geometry hash → duplicates become quantities.
4. **Nest** — `charm-nest-solver.js` in a Worker: largest-first, contact-scored, gravity tie-break, random restarts within the time budget. Masks dilated by half the clearance; sheet inset pre-filled as wall; mirroring never applied. Coarse pass (0.5 px/pt, exhaustive, summed-area-table pre-tests) picks candidates; fine pass (2 px/pt, bit-parallel) makes the exact decision. Each committed placement is posted to the page and drawn immediately.
...
6. **Verify** — twice, independently of the solver: geometry re-rasterised at 6 px/pt (overlap, bounds, minimum gaps) and the written file re-rendered layer by layer with pdf.js (overlap, bounds, empty layers). A flagged sheet is `Partial`, still downloadable, and says why.
...
Stock size per metal (defaults to the 7.125 × 6.03 in reference), machine kerf (clearance default 0.5 pt — tighten toward 0.33 pt if the last piece will not fit), whether small charms may nest inside jump rings (currently no), legacy `.ai` files (rejected with instructions), grain/orientation constraints (free rotation assumed; angle step is a setting).
