...
| `netlify/functions/_investorApiSchemas.js` | Pinned Ajv 2020 (2020-12 mode) schema compilation and format validation, run at cold start; fail closed. **[v2]** Invalid schemas, unsupported OpenAI Strict Structured Outputs constructs, a missing `additionalProperties:false`, bad formats or unsafe numeric fields fail the deploy rather than reaching a request. The strict model subset is generated from the same canonical schema source, never hand-maintained alongside it |
...
**Do not create a separate "AI committee" module. `_investorManager.js` is the single investment authority.** (v1 constraint, restored — it was lost when this section's file table was cut.)
...
| `package.json` | **[v2] Leave alone** | It is `etsy-app`, shared with unrelated subsystems, and its build script targets `cherry-viewer`. This plan adds no test framework, no Playwright and no axe dependency, and no npm scripts. **Ajv is the exception**: `_investorApiSchemas.js` compiles every schema through pinned Ajv 2020 at build and cold start, so `ajv` and `ajv-formats` must be added to `dependencies` — it is a runtime dependency of the functions, not test tooling. Netlify installs the existing dependencies at deploy; the investor system needs nothing further from it. Adding a lockfile remains worthwhile for reproducible deploys, but it is not part of this plan. |
