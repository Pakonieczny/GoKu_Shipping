...
const Master = window.Master = (() => {
  ...
  function render() {
    ...
    const rows = [...B.master.entries.values()].filter(e => !q || e.sku.includes(q)).sort((a, b) => a.sku.localeCompare(b.sku));
    v.querySelector("#mCount").textContent = `${B.master.entries.size} SKU(s) indexed`;
    const grid = v.querySelector("#mGrid");
    grid.innerHTML = rows.slice(0, 600).map(e => `<div class="skuTile${e.blocked ? " blocked" : ""}" data-sku="${esc(e.sku)}">${e.thumbUrl ? `<img crossorigin="anonymous" src="${e.thumbUrl}" loading="lazy" alt="">` : `<div style="aspect-ratio:1;background:#fff;border-radius:6px"></div>`}<div class="sku" title="${esc(e.sku)}">${esc(e.sku)}</div><div class="meta">${(e.widthPt * MM).toFixed(1)} × ${(e.heightPt * MM).toFixed(1)} mm · ${e.holes} hole(s)${e.sizes ? ` · sizes ${Object.keys(e.sizes).join("/")}` : ""}</div><div class="meta">up ${e.upAngle == null ? "as drawn" : Math.round(e.upAngle) + "°"} · ${esc(e.labelSource || "text")}${e.hashSource === "server" ? " · server" : ""}</div>${e.blocked ? `<div class="bad">${esc(e.blocked)}</div>` : ""}${e.sizeMoved ? `<div class="bad">size moved &gt;5% on re-index</div>` : ""}<div class="row"><label style="display:flex;gap:4px;align-items:center;font-size:11px"><input type="checkbox" data-eng="${esc(e.sku)}" ${e.engravable !== false ? "checked" : ""}>engravable</label><input type="number" data-up="${esc(e.sku)}" value="${e.upAngle == null ? "" : Math.round(e.upAngle)}" placeholder="up°" style="width:52px;border:1px solid var(--line);border-radius:6px;padding:2px 4px;font-size:11px">${e.blocked ? `<button class="btn ghost xs" data-unblock="${esc(e.sku)}">unblock</button>` : ""}${e.aiUrl ? `<a class="btn ghost xs" href="${e.aiUrl}" target="_blank" rel="noopener">.ai</a>` : ""}</div></div>`).join("") + (rows.length > 600 ? `<div class="libEmpty">${rows.length - 600} more — narrow the search</div>` : "");
  ...
  return { entryFor, fetchEntry, load, indexFile, render, patch, keepOutOf, skuRegex, stripPng };
})();
