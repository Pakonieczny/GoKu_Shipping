...
const Engrave = window.Engrave = (() => {
  ...
  function emptyWhy(jobs, nWords, nDone) {
    ...
    if (!r && !jobs.length && window.Recall && Recall.on()) return line("Nothing in this set was engraved.", `<button class="btn ghost sm" data-go="hist">Another set…</button>`);
    ...
    if (tab === "done") {
      ...
      bk.innerHTML = (decided.length
        ? `<div class="section" style="margin-top:2px">Decided · ${decided.length}</div><div class="rvList" id="egDone">` + decided.map(j2 => {
            ...
            return `<div class="doneRow hoverItem" data-rid="${esc(j2.row.order.receiptId)}" data-key="${esc(j2.key)}"><b class="mono">${esc(j2.row.order.receiptId)}</b><span class="sku mono">${esc(j2.row.spec.designSku || "")}</span><span class="w">${w}</span><span class="ost ${j2.state === "skipped" ? "warn" : "ok"}" title="${j2.state === "written" ? "the back file is saved with the sheet" : j2.state === "skipped" ? "cut plain, nothing on the back" : "approved — the back file is written when the sheet is"}">${j2.state === "written" ? "written" : j2.state === "skipped" ? "no engraving" : "approved"}</span><span class="by">${esc(who)}${j2.approvedAt ? " · " + fmtT(j2.approvedAt) : ""}</span><button class="btn ghost xs" data-a="reopen" title="send it back to the words step — the back file already written is superseded">Reopen</button></div>`;
          }).join("") + `</div>`
