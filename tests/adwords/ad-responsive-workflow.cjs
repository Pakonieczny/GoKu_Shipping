...
async function setup(){const e=await ctx.setupFixture(),bytes=await sharp({create:{width:1024,height:1024,channels:3,background:'#d1b284'}}).jpeg().toBuffer();e.D.fullSourceBytes=async()=>bytes;
 ...
 e.D.reviewImages=async()=>{e.calls.quality++;return {pass:true,productFaithful:true,mobileReadable:true,score:98,issues:[],estimatedUsd:.1,costEstimated:false}};
...
(async()=>{
 ...
 const repair=await setup();repair.D.reviewImages=async()=>{repair.calls.quality++;return {pass:repair.calls.quality>1,productFaithful:repair.calls.quality>1,mobileReadable:true,score:repair.calls.quality>1?98:70,issues:repair.calls.quality===1?['Chain links changed']:[],estimatedUsd:.1,costEstimated:false}};const corrected=await repair.svc.editorAIRun({workspaceId:repair.id,jobId:repair.jobId});ok(corrected.ok&&repair.calls.images===2&&repair.calls.quality===2,'one targeted repair restores fidelity without rerunning research');await repair.svc.editorAIRun({workspaceId:repair.id,jobId:repair.jobId});ok(repair.calls.images===2&&repair.calls.responses===1,'corrected scene receipts prevent duplicate charges');
