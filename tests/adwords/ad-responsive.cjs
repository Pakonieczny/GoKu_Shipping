...
if(require.main===module)(async()=>{
 ...
 const smallBoard={key:'display_300x50',width:300,height:50},previewCandidate={artboard:smallBoard,device:'mobile',document:responsive.document(plan,photo,smallBoard,'mobile'),sources:[photo],responsive:{layoutVersion:responsive.layoutVersion,plan,images:[photo],variants:[]}};
 const dense=await e.renderAIProofs(previewCandidate,{preview:true}),denseMeta=await sharp(Buffer.from(dense[0].dataBase64,'base64')).metadata();ok(denseMeta.width===1200&&denseMeta.height===200&&denseMeta.format==='png','small banners get fresh four-times-resolution lossless previews');ok(dense[0].width===300&&dense[0].height===50,'preview density does not change the actual Google ad dimensions');
