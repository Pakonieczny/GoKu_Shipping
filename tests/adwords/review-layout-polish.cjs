...
for(const key of ['display_468x60','display_728x90','display_930x180','display_970x90','display_980x120']){const board=R.boards.find(b=>b.key===key),d=R.document({...plan,style:{...plan.style,treatment:'soft-fade'}},image,board,'desktop'),logo=d.objects.find(o=>o.id==='brand_icon'||o.editorId==='brand_icon'||o.sourceKey==='brites_brand_icon');assert(logo);assert(board.width-logo.left-logo.width*logo.scaleX>=Math.min(board.width*.035,board.height*.30)-.01,key+' logo inset');}
