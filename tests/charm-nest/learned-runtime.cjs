...
const html=fs.readFileSync('charm-nest-1.html','utf8');const from=html.indexOf('function onWorkerMessage('),to=html.indexOf('async function startServerNest(',from);
const c=vm.createContext({assert,console});vm.runInContext(`
const finishes=[],messages=[],S={settings:{maxFill:.74}};
const activeCharms=sh=>sh.charms, agent=()=>({}),agentUpdate=()=>{},log=()=>{},renderProgress=()=>{},renderSat=()=>{},drawPreview=()=>{},computeSaturation=()=>{},finishNest=(sh,r)=>finishes.push(r),fmt={pct:x=>x+'%'};
