...
(async () => {
  /* ── interpretation (§15 · every option pattern, alias, no-design, "Rose Quartz", staff note) ── */
  {
    ...
    // "Use this charm" on a SKU no master file holds: the listing's alias stands in for it, and only for it
    sp = O.interpretLine(order, mk({ sku: 'BR-GONE-09', listingId: '9999', variations: [] }), ctx); assert.strictEqual(sp.designSku, 'BR-ALS-02', 'an alias stands in for an unknown SKU'); assert.strictEqual(sp.skuSource, 'alias'); assert.deepStrictEqual(sp.problems, []);
  ...
  let master;
  {
    ...
    /* ── fit (§15 · a hole, a thin ring, a diagonal band; largest size; heart refused) ── */
    {
      ...
      const bigger = G.layoutLines(fit.lines, font, fit.size + 0.1, undefined, fit.angle, fit.centre); assert(!G.verifyInk(bigger.cmds, mask).ok, '0.1 pt more must fail');
