...
const ctx={window:dom.window,document,console:{error:(...x)=>errors.push(x)},B:{engrave:{items:jobs,fonts:{ok:true,Regular:{},workerFonts:{Regular:new ArrayBuffer(8)}}}},S:{settings:{},mode:'engrave'},PT:72/25.4,MM:25.4/72,SOURCE_LABEL:{},
  Pool:{charmOf:()=>charm,sheetOf:()=>({fileBase:'sheet'}),update:async()=>{},sizeEntry:()=>({thumbUrl:'/cached-thumb.png'})},
  Master:{entryFor:()=>({}),thumbOf:()=>'/cached-thumb.png'},G:{glyphCoverage:()=>({ok:true})},Gate:{modern:()=>false},
...
(async()=>{
  ...
  const row0=document.querySelector('[data-open="order0"]');await until(()=>row0.querySelector('img'));const thumb=row0.querySelector('img');
  ...
  assert.equal(row0.querySelector('img'),thumb,'decoded thumbnail is retained');
  ...
  assert.equal(document.querySelector('[data-open="order0"] img'),thumb,'returning to the tab reuses the loaded thumbnail');
