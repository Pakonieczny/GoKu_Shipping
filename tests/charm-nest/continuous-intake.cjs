...
(async()=>{
  ...
  const c={document,window:{IntersectionObserver:IO},IntersectionObserver:IO,O,OV:{sort:'arrival',limit:48},rowsOf:()=>rows,visibleRows:()=>rows,viewMode:()=> 'cards',el:(tag,cls)=>{const n=document.createElement(tag);n.className=cls;return n;},stateWords:()=>['info','processing'],dueOf:()=>({txt:'—'}),placeOf:()=>null,imageFor:()=>null,wordsOf:()=>'',Master:{entryFor:()=>null},Pool:{charmOf:()=>null},requestAnimationFrame:fn=>fn(),esc:String,labelOf:String,OrderWin:{open(){}},Review:{problemText:String}};
