...
(async()=>{
  assert.equal(O.intakePlan({count:79}).phase,'fill');assert.equal(O.intakePlan({count:80}).phase,'final');assert.equal(O.intakePlan({count:33}).phase,'fill');
  assert.equal(O.intakePlan({count:10,area:90,capacity:100}).phase,'final','capacity pressure can precede 80 pieces');
  ...
  assert.equal(O.intakePlan({count:20,budgetS:180}).budgetMs,12000);assert.equal(O.intakePlan({count:80,budgetS:180}).budgetMs,180000);
  ...
  const c={document,window:{IntersectionObserver:IO},IntersectionObserver:IO,O,OV:{sort:'arrival',limit:48},rowsOf:()=>rows,visibleRows:()=>rows,viewMode:()=> 'cards',el:(tag,cls)=>{const n=document.createElement(tag);n.className=cls;return n;},stateWords:()=>['info','processing'],dueOf:()=>({txt:'—'}),placeOf:()=>null,imageFor:()=>null,wordsOf:()=>'',paintImages(){},watchImages(){},esc:String,labelOf:String,OrderWin:{open(){}},Review:{problemText:String}};
  vm.createContext(c);vm.runInContext(bridge.slice(bridge.indexOf('  let listObserver='),bridge.indexOf('  /* The tab used to')),c);c.renderBody();
  ...
  observe([{isIntersecting:true}]);assert.equal(document.querySelectorAll('[data-key]').length,130);assert.equal(document.querySelector('.ordMore'),null);
...
// Incremental nesting must not move a finished sheet; at 80 it frees automatic
...
(async()=>{
 function setup(oldCount,newCount,{sealed=false}={}){
   ...
   const c={window:{},O,S:{settings:{maxFill:.74,optimizeAt:85,finalOptimizeCount:80,budgetS:180},sheets:{gold:{pages,active:0}}},allSheets:()=>pages,pagesOf:()=>pages,activePage:()=>pages[c.S.sheets.gold.active],activeCharms:p=>p.charms,Orders:{rows:()=>[row]},Sets:{ofRun:()=>[]},Gate:{state:()=>({forceFill:{}}),modern:()=>true,nestable:()=>true},Pool:{addAll:async()=>{const p=pages[c.S.sheets.gold.active];for(let i=oldCount;i<oldCount+newCount;i++){const charm=mk(i);p.charms.push(charm);row.poolIds.push(charm.poolId);}row.state='pooled';},update:async()=>{}},usableArea:()=>10000,inflatedArea:x=>x.areaPt2,agent(){},startNest:p=>{starts++;p.status='complete';p.persistedDone=true;p.dirty=false;},sleep:async()=>{throw Error('unexpected unfinished worker');},RunCtl:{onSheetDone(){throw Error('unexpected hold');}}};
 ...
 const final=setup(75,5);await final.live.add(final.run);const fixed=final.pg.charms[0];fixed.pinned={cxPt:1,cyPt:2,angle:0};const auto=final.pg.charms[1];auto.pinned={cxPt:2,cyPt:2,angle:0};auto.arrivalPin=true;
 assert.equal(final.live.prepareSheet(final.pg).phase,'final');assert(fixed.pinned);assert.equal(auto.pinned,null);assert.equal(final.pg.placements.length,75,'previous layout seeds the next search');
 console.log('Live nesting OK: finalized sheets fixed, arrivals append to draft, full search at 80, operator pins preserved');
})().catch(e=>{console.error(e);process.exitCode=1;});
