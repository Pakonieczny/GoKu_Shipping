...
(async()=>{
 ...
 sh.draft=false;sh.setId='set-test';await w.RoseStock.record(sh);assert(sh.roseCutAt);assert.equal(sh.roseHistory.length,1);assert(!sh.el.querySelector('.roseHistory button'),'a cut sheet offers no more buttons');assert.equal(sh.el.querySelectorAll('.roseLineTimeline time').length,1);assert(sh.el.querySelector('[data-solid="size"]').disabled);assert(allowance.disabled);assert(!sh.el.textContent.includes('Using sheet'));assert(!sh.el.querySelector('.roseStockHead'));ctx.calls=[];w.RoseStock.paint(ctx,sh,2,'lines');assert.equal(ctx.lineWidth,2,'historical cut line is twice its previous display width');
