...
(async()=>{
  ...
  assert.deepEqual([...sh.el.querySelectorAll('.roseActions button')].map(b=>b.textContent),['Update contour','Record completed cut']);
  ...
  assert(!dense.el.querySelector('[data-rose="plan"]'),'a full sheet offers no Cut Sheet');
  assert.match(dense.el.querySelector('.roseNote').textContent,/fills the sheet, so it needs no new green line/);
  assert.equal(open.el.querySelector('[data-rose="plan"]').textContent,'Cut Sheet','a sheet with room left can be cut');
