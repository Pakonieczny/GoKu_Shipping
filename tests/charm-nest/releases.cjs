...
(async()=>{
 ...
 const write=CN.persistSheet; CN.persistSheet=async()=>{throw new Error('test upload interrupted')};
 ...
 CN.persistSheet=write;await Promise.all([Gate.assemble(B.run),Gate.assemble(B.run)]);
