...
(async()=>{
 ...
 const stops=[],r={runId:'r',sheets:{},status:'nesting'};let saved=0;const rc={B:{run:r},CN:{renderCard(){}},sheetName:sh=>sh.metal,save:async()=>{saved++;},stop:(why,fix)=>{stops.push({why,fix});r.status='stopped';}};vm.createContext(rc);vm.runInContext(bridge.slice(bridge.indexOf('  function onSheetDone(sh, err)'),bridge.indexOf('  function waitForReview(r)')),rc);
