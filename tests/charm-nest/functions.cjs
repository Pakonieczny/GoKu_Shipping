...
(async () => {
  ...
  assert(packingRequest.system.includes('never change membership') && packingRequest.schema.properties.suggestions.type==='array');
  anthro.callClaudeRaw=async request=>{assert(!/"(?:maxItems|maximum|minimum|maxLength|minLength)":/.test(JSON.stringify(request.outputFormat.schema)),"actual provider request uses supported schema keywords");return ({stop_reason:'end_turn',content:[{type:'text',text:JSON.stringify({summary:'Place the long silhouette early.',profiles:[{index:0,adaptability:20,interlock:80,edgeAffinity:95,priority:90,edgeRole:'long-edge',family:'elongated',mates:['concave'],angles:[-90],note:'straight boundary'}],pairs:[{a:0,b:1,score:92,reason:'matching recess'},{a:0,b:55,score:90}],suggestions:[{index:1,angles:[-30,390]},{index:1,angles:[0]},{index:999,angles:[0]},{index:0,angles:['bad',45]}]})}]});};
