...
assert.equal(card.querySelectorAll('.laserSeal').length,0,'unfinished sheets and sets never display an earned badge');
assert.equal(card.querySelector('.libCard').nextElementSibling.className,'productionRow','QR and compact statistics follow the sheet');
assert.match(card.querySelector('[data-sheet-status]').textContent,/0 \/ 1Backs saved/);
assert(!/In preparation|Awaiting approval|checks complete|Layout checked/.test(card.textContent),'no repeated status panels');
...
assert.equal(card.closest('[data-laser-area]').dataset.laserArea,'ready');assert.equal(card.querySelector('[data-sheet-status]').firstElementChild.className,'laserSeal earned','badge precedes counter');assert.equal(card.querySelectorAll('.laserSeal').length,2,'only qualifying sheet and set earn badges');assert.equal(card.querySelector('.sheetQR img'),qr,'updating totals preserves loaded QR');
jobs.set('j',{copies:['c1'],state:'words'});L.changed();frames.shift()();
assert.equal(card.closest('[data-laser-area]').dataset.laserArea,'pending','reopening revokes whole set seal immediately');assert.match(card.textContent,/1 awaiting approval/);
assert.equal(card.querySelectorAll('.laserSeal').length,0,'reopening removes both badges');
jobs.clear();L.changed();frames.shift()();
(async()=>{await L.poll(true);frames.shift()();assert.equal(card.closest('[data-laser-area]').dataset.laserArea,'pending','deleted cloud sheet cannot retain readiness');console.log('Laser Library UI OK: compact QR-side statistics, earned-only badges, immediate approvals/reopens, stable images and section movement');})().catch(e=>{console.error(e);process.exitCode=1;});
