...
async function functions(st, name, req, res, base) {
  ...
  // production artwork is read through the same-origin asset function, in ranged parts
  if (name === 'charmNestAsset') {
    const assets = require(path.join(root, 'netlify/functions/_charmNestAsset.js'));
    const headers = new Headers(); for (const [k, v] of Object.entries(req.headers)) headers.set(k, Array.isArray(v) ? v.join(',') : String(v));
    const out = await assets.serve(new Request(new URL(req.url, base), { method: req.method, headers }), st.bucket);
    res.writeHead(out.status, Object.fromEntries(out.headers.entries()));
    return res.end(out.body ? Buffer.from(await out.arrayBuffer()) : undefined);
  }
...
function fakeAdmin(st, base) {
  ...
  const apply = (target, src) => { for (const [k, v] of Object.entries(src)) { if (v && v.__inc != null) target[k] = (target[k] || 0) + v.__inc; else if (v && v.__del) delete target[k]; else if (v === SERVER_TS) { const at = Date.now(); target[k] = { toMillis: () => at }; } else target[k] = v; } return target; };   // fixed when written, as Firestore does: a clock read on every get made each record look changed
  const docRef = (coll, id) => ({ id, path: coll + '/' + id, collection: sub => collection(coll + '/' + id + '/' + sub), async get() { const d = st.docs.get(coll + '/' + id); return { exists: !!d, id, data: () => (d ? { ...d } : undefined), get: k => d && d[k] }; }, async set(data, o) { const cur = (o && o.merge && st.docs.get(coll + '/' + id)) || {}; st.docs.set(coll + '/' + id, apply({ ...cur }, data)); }, async update(data) { const cur = st.docs.get(coll + '/' + id); if (!cur) throw new Error('NOT_FOUND: ' + coll + '/' + id); st.docs.set(coll + '/' + id, apply({ ...cur }, data)); }, async delete() { st.docs.delete(coll + '/' + id); } });
  /* (orderBy(FieldPath.documentId()) orders by the document's id, and startAfter starts past the value given) */
  const query = (coll, filters = [], order = null, lim = 0, after = null) => ({ where: (f, op, v) => query(coll, filters.concat([[f, op, v]]), order, lim, after), orderBy: (f, dir) => query(coll, filters, [f, dir || 'asc'], lim, after), limit: n => query(coll, filters, order, n, after), startAfter: v => query(coll, filters, order, lim, v), select() { return this; }, count() { const q = this; return { get: async () => { const r = await q.get(); return { data: () => ({ count: r.size }) }; } }; }, async get() { let rows = [...st.docs.entries()].filter(([k]) => k.startsWith(coll + '/') && !k.slice(coll.length + 1).includes('/')).map(([k, v]) => ({ id: k.slice(coll.length + 1), data: () => ({ ...v }), ref: docRef(coll, k.slice(coll.length + 1)), get: f => v[f] })); for (const [f, op, v] of filters) rows = rows.filter(r => { const x = r.data()[f]; return op === '==' ? x === v : op === '>=' ? x >= v : op === '<=' ? x <= v : op === '>' ? x > v : op === '<' ? x < v : op === '!=' ? x !== v : op === 'in' ? v.includes(x) : op === 'array-contains' ? Array.isArray(x) && x.includes(v) : op === 'array-contains-any' ? Array.isArray(x) && x.some(e => v.includes(e)) : true; }); const key = r => { const x = order[0] === '__name__' ? r.id : r.data()[order[0]]; return x && x.toMillis ? x.toMillis() : x; }; if (order) rows.sort((a, b) => { const xv = key(a), yv = key(b); const c = xv > yv ? 1 : xv < yv ? -1 : 0; return order[1] === 'desc' ? -c : c; }); if (order && after != null) rows = rows.filter(r => (order[1] === 'desc' ? key(r) < after : key(r) > after)); if (lim) rows = rows.slice(0, lim); return { size: rows.length, docs: rows, empty: !rows.length, forEach: fn => rows.forEach(fn) }; }, async add(data) { const id = 'auto' + Math.random().toString(36).slice(2, 8); await docRef(coll, id).set(data); return docRef(coll, id); }, doc: id => docRef(coll, id || 'auto' + Math.random().toString(36).slice(2, 10)) });
  const collection = c => query(c);
  const db = { collection, batch() { const ops = []; return { set: (ref, data, o) => ops.push(() => ref.set(data, o)), update: (ref, data) => ops.push(() => ref.update(data)), delete: ref => ops.push(() => ref.delete()), async commit() { for (const o of ops) await o(); } }; }, async getAll(...refs) { /* a trailing { fieldMask } is read options, not a document */ if (refs.length && typeof refs[refs.length - 1].get !== 'function') refs.pop(); return Promise.all(refs.map(r => r.get())); }, async runTransaction(fn) { const t = { get: ref => ref.get(), getAll: (...refs) => db.getAll(...refs), set: (ref, data, o) => ref.set(data, o), update: (ref, data) => ref.update(data), delete: ref => ref.delete() }; return fn(t); } };
  const bucket = { name: 'test-bucket', /* one page of names in order, as the real listing (autoPaginate:false) returns it */ async getFiles(q = {}) { const names = [...st.blobs.keys()].filter(k => k.startsWith(q.prefix || '')).sort().filter(k => !q.pageToken || k > q.pageToken), page = names.slice(0, q.maxResults || 1000); return [page.map(n => this.file(n)), names.length > page.length ? Object.assign({}, q, { pageToken: page[page.length - 1] }) : null]; }, async getMetadata() { return [{ location: 'TEST', storageClass: 'STANDARD', cors: [{ origin: st.corsOrigins || ['*'], method: ['GET', 'PUT', 'HEAD'], responseHeader: ['Content-Type'], maxAgeSeconds: 3600 }] }]; }, file(p) { return { name: p, async save(buf, o) { st.blobs.set(p, { buf: Buffer.from(buf), generation: (st.generation = (st.generation || 0) + 1), meta: { contentType: o && o.contentType, metadata: (o && o.metadata && o.metadata.metadata) || {} } }); }, async exists() { return [st.blobs.has(p)]; }, async download() { const b = st.blobs.get(p); if (!b) throw new Error('no blob ' + p); return [b.buf]; }, async delete() { st.blobs.delete(p); }, async getMetadata() { const b = st.blobs.get(p); if (!b) throw Object.assign(new Error('no blob'), { code: 404 }); return [{ contentType: b.meta.contentType, size: b.buf.length, generation: b.generation || 1, metadata: Object.assign({ firebaseStorageDownloadTokens: 't' }, b.meta.metadata) }]; }, createReadStream({ start = 0, end } = {}) { const b = st.blobs.get(p); if (!b) throw Object.assign(new Error('no blob'), { code: 404 }); return require('stream').Readable.from([b.buf.subarray(start, end == null ? b.buf.length : end + 1)]); }, async setMetadata(m) { const b = st.blobs.get(p) || { buf: Buffer.alloc(0), meta: { metadata: {} } }; if (m.contentType) b.meta.contentType = m.contentType; Object.assign(b.meta.metadata, m.metadata || {}); st.blobs.set(p, b); }, async getSignedUrl() { return [`${base}/__put/${encodeURIComponent(p)}`]; } }; } };
  return { firestore: Object.assign(() => db, { FieldValue, Timestamp, FieldPath: { documentId: () => '__name__' } }), storage: () => ({ bucket: () => bucket }) };
}
...
async function start(opts = {}) {
  ...
  const handler = (base) => async (req, res) => {
    try {
      ...
      if (p.startsWith('/__put/')) { const key = decodeURIComponent(p.slice(7)); const buf = await readBody(req); st.blobs.set(key, { buf, generation: (st.generation = (st.generation || 0) + 1), meta: { contentType: req.headers['content-type'], metadata: { firebaseStorageDownloadTokens: 't' } } }); return json(res, 200, { ok: true }); }
  ...
  st.bucket = admin.storage().bucket();
  ...
  anthro.callClaudeRaw = async (o) => { const sys = (o.system && o.system[0] && o.system[0].text) || ''; const mode = /Grade EVERY numbered charm/.test(sys) ? 'packing' : /engraver and decide/.test(sys) ? 'engraveIntent' : /rendered BACK/.test(sys) ? 'engraveReview' : /strip directly BELOW/.test(sys) ? 'labelRead' : /quality reviewer/.test(sys) ? 'grouping' : /final inspector/.test(sys) ? 'layout' : 'name'; const fn = st.agentResults[mode]; const text = JSON.stringify(typeof fn === 'function' ? fn(o) : (fn || defaultAgent(mode, o))); return { stop_reason: 'end_turn', usage: { input_tokens: 1, output_tokens: 1 }, content: [{ type: 'text', text }] }; };
...
function defaultAgent(mode, o) {
  ...
  // shape grades for the solver: one profile per numbered shape on the contact sheet
  if (mode === 'packing') { const txt = String(o.messages[0].content[0].text); let pieces = []; try { pieces = JSON.parse(txt.slice(txt.indexOf('Numbered shapes:') + 16)); } catch (_) {} return { profiles: pieces.map(p => ({ index: p.index, adaptability: 60, interlock: 40, edgeAffinity: 50, priority: 50, edgeRole: 'either', family: 'compact', mates: ['compact'], angles: [0, 90], note: 'test grade' })), pairs: [], suggestions: [], summary: 'test grades' }; }
