...
function fakeAdmin(st, base) {
  ...
  const query = (coll, filters = [], order = null, lim = 0) => ({ where: (f, op, v) => query(coll, filters.concat([[f, op, v]]), order, lim), orderBy: (f, dir) => query(coll, filters, [f, dir || 'asc'], lim), limit: n => query(coll, filters, order, n), select() { return this; }, count() { const q = this; return { get: async () => { const r = await q.get(); return { data: () => ({ count: r.size }) }; } }; }, async get() { let rows = [...st.docs.entries()].filter(([k]) => k.startsWith(coll + '/') && !k.slice(coll.length + 1).includes('/')).map(([k, v]) => ({ id: k.slice(coll.length + 1), data: () => ({ ...v }), ref: docRef(coll, k.slice(coll.length + 1)), get: f => v[f] })); for (const [f, op, v] of filters) rows = rows.filter(r => { const x = r.data()[f]; return op === '==' ? x === v : op === '>=' ? x >= v : op === '<=' ? x <= v : op === '>' ? x > v : op === '<' ? x < v : op === '!=' ? x !== v : op === 'in' ? v.includes(x) : op === 'array-contains' ? Array.isArray(x) && x.includes(v) : op === 'array-contains-any' ? Array.isArray(x) && x.some(e => v.includes(e)) : true; }); if (order) rows.sort((a, b) => { const x = a.data()[order[0]], y = b.data()[order[0]]; const xv = x && x.toMillis ? x.toMillis() : x, yv = y && y.toMillis ? y.toMillis() : y; const c = xv > yv ? 1 : xv < yv ? -1 : 0; return order[1] === 'desc' ? -c : c; }); if (lim) rows = rows.slice(0, lim); return { size: rows.length, docs: rows, empty: !rows.length, forEach: fn => rows.forEach(fn) }; }, async add(data) { const id = 'auto' + Math.random().toString(36).slice(2, 8); await docRef(coll, id).set(data); return docRef(coll, id); }, doc: id => docRef(coll, id || 'auto' + Math.random().toString(36).slice(2, 10)) });
