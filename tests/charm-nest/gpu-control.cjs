...
(async()=>{
 ...
 const result=await S.solve(job,{gpu:slow,onGPU:p=>progress.push(p)});
 ...
 console.log('GPU control OK: unavailable-device worker fallback, measured slow-device fallback, result verification and comparison cancellation');
